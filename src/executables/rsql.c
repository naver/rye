...
int (*rsql_text_utf8_to_console) (const char *, const int, char **, int *) = NULL;

int (*rsql_text_console_to_utf8) (const char *, const int, char **, int *) = NULL;
...
static int rsql_execute_statements (const RSQL_ARGUMENT * rsql_arg, char *stream, int stream_length, int line_no);

static SESSION_CMD get_session_cmd (char **argument, char *cmd_line);
static int rsql_do_session_cmd (SESSION_CMD cmd, char *argumen, RSQL_ARGUMENT * rsql_arg);
...
static SESSION_CMD get_query_or_rsql_cmd (char **argument, RSQL_QUERY * query, int *line_number, RSQL_INPUT * input);
...
initialize_rsql_column_width_info_list ()
{
  ...
  rsql_column_width_info_list = malloc (sizeof (RSQL_COLUMN_WIDTH_INFO) * COLUMN_WIDTH_INFO_LIST_INIT_SIZE);
  if (rsql_column_width_info_list == NULL)
    {
  ...
  while (!rsql_input_end (input))
    {
      ...
      if (cmd == S_CMD_EXIT)
        ...
        {
          (void) rsql_execute_statements (rsql_arg, query.query, query.length, line_number);
      ...
      else
        {
          ...
          if (input->type == RSQL_INPUT_PROMPT || !rsql_prompt_only_command (cmd))
            {
              if (rsql_do_session_cmd (cmd, argument, rsql_arg) == DO_CMD_EXIT)
                {
                  exit_code = EXIT_FAILURE;
                  break;
                }
...
rsql_input_final (RSQL_INPUT * input)
{
  ...
  if (input->fname != NULL && input->fp != NULL)
    {
      assert (input->type == RSQL_INPUT_EDIT_CONTENTS || input->type == RSQL_INPUT_COMMAND);
      ...
      if (input->string_buffer == NULL)
        {
          ...
          er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, error, 1, LINE_BUFFER_SIZE);
...
rsql_read_line (RSQL_INPUT * input)
{
  ...
  switch (input->type)
    ...
    case RSQL_INPUT_COMMAND:
      ...
      line = fgets ((char *) input->string_buffer, input->string_buffer_size, input->fp);
      break;
    case RSQL_INPUT_PROMPT:
      assert (input->string_buffer_size >= 0);
      assert (input->fp == NULL);
      ...
      if (rsql_text_console_to_utf8 != NULL)
        {
          ...
          if (utf8_line_buf == NULL)
            {
              er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_OUT_OF_VIRTUAL_MEMORY, 1, utf8_line_buf_size);
          ...
          if ((*rsql_text_console_to_utf8) (line, line_length, &utf8_line_read, &utf8_line_buf_size) != NO_ERROR)
            {
              GOTO_EXIT_ON_ERROR;
            }
 ...
 */
static SESSION_CMD
get_query_or_rsql_cmd (char **argument, RSQL_QUERY * query, int *line_number, RSQL_INPUT * input)
{
  char *line_read = NULL;
  int line_length;
  ...
  while ((line_read = rsql_read_line (input)) != NULL)
    {
      ...
      if (RSQL_SESSION_COMMAND_PREFIX (line_read[0]) && is_in_block == false)
        ...
        {
          error = rsql_query_append_string (query, line_read, line_length, read_whole_line);
          if (error != NO_ERROR)
            {
 ...
 */
static int
rsql_do_session_cmd (SESSION_CMD cmd, char *argument, RSQL_ARGUMENT * rsql_arg)
{
  ...
  switch (cmd)
    ...
    case S_CMD_AUTOCOMMIT:
      ...
      fprintf (rsql_Output_fp, "AUTOCOMMIT IS %s\n",
               (rsql_arg->auto_commit && prm_get_bool_value (PRM_ID_RSQL_AUTO_COMMIT) ? "ON" : "OFF"));
      break;
    ...
    case S_CMD_CHECKPOINT:
      if (au_is_dba_group_member (au_get_user ()))
        ...
        {
          fprintf (rsql_Output_fp, "Checkpointing is only allowed for" " the rsql started with --user=DBA\n");
        }
      break;
    ...
    case S_CMD_KILLTRAN:
      if (au_is_dba_group_member (au_get_user ()))
        ...
        {
          fprintf (rsql_Output_fp, "Killing transaction is only allowed" " for the rsql started with --user=DBA\n");
        }
      break;
    ...
    case S_CMD_RESTART:
      ...
      if (db_restart_ex (UTIL_RSQL_NAME, rsql_arg->db_name,
                         rsql_arg->user_name, rsql_arg->passwd, db_get_client_type ()) != NO_ERROR)
        {
      ...
      if (*argument == '\0')
        {
          fprintf (rsql_Output_fp, "\n\t%s\n\n",
                   ...
                   (cmd == S_CMD_PRINT_CMD) ? rsql_Print_cmd : rsql_Pager_cmd);
        }
      else
        {
          strncpy ((cmd == S_CMD_SHELL_CMD) ? rsql_Shell_cmd :
                   (cmd == S_CMD_EDIT_CMD) ? rsql_Editor_cmd :
                   (cmd == S_CMD_PRINT_CMD) ? rsql_Print_cmd : rsql_Pager_cmd, argument, PATH_MAX - 1);
        }
      break;
    ...
    case S_CMD_INFO:
      rsql_help_info ((argument[0] == '\0') ? NULL : argument,
                      rsql_arg->auto_commit && prm_get_bool_value (PRM_ID_RSQL_AUTO_COMMIT));
      break;
            ...
            for (i = 0; i < rsql_column_width_info_list_size; i++)
              {
                ...
                fprintf (rsql_Output_fp,
                         "COLUMN-WIDTH %s : %d\n",
                         rsql_column_width_info_list[i].name, rsql_column_width_info_list[i].width);
              }
          }
        else
          {
            ...
            if (val_str == NULL)
            ...
            else
              {
                ...
                if (rsql_set_column_width_info (column_name, width) != RSQL_SUCCESS)
                  {
                    return DO_CMD_FAILURE;
                  }
              }
        ...
        if (*argument != '\0')
          {
            ...
            if (result != 0 || string_width < 0)
              {
                fprintf (rsql_Error_fp, "ERROR: Invalid string-width(%s).\n", argument);
              }
          ...
          {
            fprintf (rsql_Output_fp, "STRING-WIDTH : %d\n", rsql_arg->string_width);
          }
      }
      break;
        ...
        if (*argument != '\0')
          {
            ...
            if (result != 0 || groupid < GLOBAL_GROUPID)
              {
#if 0                           /* TODO - */
                fprintf (rsql_Error_fp, "ERROR: Invalid groupid(%s).\n", argument);
#endif
              }
    ...
    case S_CMD_HISTO:
      if (histo_is_supported ())
              ...
              if (histo_start (false) == NO_ERROR)
              ...
              else
                {
                  if (er_errid () == ER_AU_DBA_ONLY)
                    {
                      fprintf (rsql_Output_fp, "Histogram is allowed only for DBA\n");
                    }
                  else
                    {
        ...
        {
          fprintf (rsql_Output_fp,
                   "Histogram is possible when the rsql started with " "`communication_histogram=yes'\n");
        }
      break;
      ...
      if (histo_is_supported ())
      ...
      else
        {
          fprintf (rsql_Output_fp, "Histogram on execution statistics "
                   "is only allowed for the rsql started " "with `communication_histogram=yes'\n");
        }
      break;
      ...
      if (rsql_Is_interactive)
        {
          if (argument[0] != '\0')
              ...
              if (i > 0)
                {
                  ...
                  if (hist != NULL)
                    {
                      if (rsql_edit_contents_append (hist->line, true) != RSQL_SUCCESS)
                        {
                          return DO_CMD_FAILURE;
                        }
                    }
                    ...
                    {
                      fprintf (rsql_Error_fp, "ERROR: Invalid history number(%s).\n", argument);
                    }
                }
              else
                {
            ...
            {
              fprintf (rsql_Error_fp, "ERROR: HISTORYRead {history_number}\n");
            }
        }
      break;
 ...
 */
static int
rsql_execute_statements (const RSQL_ARGUMENT * rsql_arg, char *stream, int stream_length, int line_no)
{
      ...
      if (db_compile_statement (session) != NO_ERROR)
        {
          ...
          if (rsql_arg->continue_on_error && (db_error_code () != ER_IT_EMPTY_STATEMENT))
            {
      ...
      if (db_error < 0)
        {
          ...
          if (rsql_arg->auto_commit
              && prm_get_bool_value (PRM_ID_RSQL_AUTO_COMMIT) && stmt_type != RYE_STMT_ROLLBACK_WORK)
            {
              do_abort_transaction = true;
            }
...
//          const char *msg_p;
...
#if 0
            msg_p = ((rsql_Row_count > 1) ? rsql_get_message (RSQL_ROWS) : rsql_get_message (RSQL_ROW));
#endif
            n = snprintf (stmt_msg, LINE_BUFFER_SIZE, rsql_get_message (RSQL_ROWS), rsql_Row_count, "selected");
            n = MIN (MAX (n, 0), avail_size - 1);
            avail_size -= n;
            break;
          }
        ...
        case RYE_STMT_GET_OPT_LVL:
          if (result != NULL)
            {
              rsql_results (rsql_arg, result, db_get_query_type_ptr (result), line_no, stmt_type);
            }
          break;
        ...
        case RYE_STMT_INSERT:
          {
            ...
            msg_p = ((db_error > 1) ? rsql_get_message (RSQL_ROWS) : rsql_get_message (RSQL_ROW));
            n = snprintf (stmt_msg, LINE_BUFFER_SIZE, msg_p, db_error, "affected");
            n = MIN (MAX (n, 0), avail_size - 1);
            avail_size -= n;
            break;
          }
      ...
      if (rsql_arg->time_on)
        {
          ...
          n = sprintf (time, " (%ld.%06ld sec) ", elapsed_time.tv_sec, elapsed_time.tv_usec);
          n = MIN (MAX (n, 0), avail_size - 1);
          avail_size -= n;
      ...
      if (rsql_arg->auto_commit
          ...
          && stmt_type != RYE_STMT_COMMIT_WORK && stmt_type != RYE_STMT_ROLLBACK_WORK)
        {
#if 1                           /* TODO - #955 set PERSIST */
          if (db_Connect_status != DB_CONNECTION_STATUS_NOT_CONNECTED)
            {
              ...
              if (db_error < 0)
                ...
                {
                  strncat (stmt_msg, rsql_get_message (RSQL_STAT_COMMITTED_TEXT), avail_size - 1);
                }
#if 1                           /* TODO - #955 set PERSIST */
            }
#endif
  ...
  snprintf (rsql_Scratch_text, SCRATCH_TEXT_LEN,
            rsql_get_message (RSQL_EXECUTE_END_MSG_FORMAT), num_stmts - rsql_Num_failures);
...
rsql_print_database (void)
{
  ...
  if (db_name == NULL || PRM_NODE_INFO_GET_IP (&connected_node) == INADDR_NONE)
    {
  ...
  else
    {
      ...
      prm_node_info_to_str (converted_host_name, sizeof (converted_host_name), &connected_node);
...
rsql_set_sys_param (const char *arg_str)
{
  ...
  if (strncmp (arg_str, "cost", 4) == 0 && sscanf (arg_str, "cost %127s %127s", plantype, val) == 2)
    ...
    {
      if (strncmp (arg_str, "persist", 7) == 0 && sscanf (arg_str, "persist %4095s", ans) == 1)
        {
          persist = true;
        }
      else
      ...
      snprintf (ans, len - 1, "%s%s", (error != NO_ERROR ? "error: set " : ""), names);
    }
  ans[len - 1] = '\0';
...
rsql_get_sys_param (const char *arg_str)
{
  ...
  if (strncmp (arg_str, "cost", 4) == 0 && sscanf (arg_str, "cost %127s", plantype) == 1)
  ...
  else
    {
      ...
      snprintf (ans, len - 1, "%s%s", (error != NO_ERROR ? "error: get " : ""), names);
    }
  ans[len - 1] = '\0';
...
rsql_exit_session (int error)
{
  ...
  if (rsql_Is_interactive && !prm_commit_on_shutdown && db_commit_is_needed () && !feof (rsql_Input_fp))
    {
      FILE *tf;
      ...
      for (; fgets (line_buf, LINE_BUFFER_SIZE, rsql_Input_fp) != NULL;)
        {
          ...
          fprintf (tf, rsql_get_message (RSQL_TRANS_TERMINATE_PROMPT_RETRY_TEXT));
          fflush (tf);
        }
...
rsql_exit_cleanup ()
{
  ...
  if (rsql_Error_fp != NULL && rsql_Error_fp != oldout && rsql_Error_fp != stdout && rsql_Error_fp != stderr)
    {
...
rsql (const char *argv0, RSQL_ARGUMENT * rsql_arg)
{
  ...
  if (rsql_arg->user_name != NULL && strcasecmp (rsql_arg->user_name, "DBA") == 0)
    {
      strncpy (rsql_Prompt, rsql_get_message (RSQL_ADMIN_PROMPT), sizeof (rsql_Prompt));
    }
  else
    {
      strncpy (rsql_Prompt, rsql_get_message (RSQL_PROMPT), sizeof (rsql_Prompt));
    }
  ...
  if (rsql_arg->db_name != NULL)
    {
      err_code = db_restart_ex (argv0, rsql_arg->db_name, rsql_arg->user_name, rsql_arg->passwd, client_type);
      if (err_code != NO_ERROR)
        {
          if (!rsql_Is_interactive || rsql_arg->passwd != NULL || err_code != ER_AU_INVALID_PASSWORD)
            {
          ...
          rsql_arg->passwd = getpass (rsql_get_message (RSQL_PASSWD_PROMPT_TEXT));
...
{
  return (msgcat_message (MSGCAT_CATALOG_RSQL, MSGCAT_RSQL_SET_RSQL, message_index));
}
  ...
  if (rsql_column_width_info_list_index >= rsql_column_width_info_list_size)
    {
      temp_list = realloc (rsql_column_width_info_list,
                           sizeof (RSQL_COLUMN_WIDTH_INFO) * (rsql_column_width_info_list_size * 2));
      if (temp_list == NULL)
        {
          rsql_Error_code = RSQL_ERR_NO_MORE_MEMORY;
      ...
      for (i = rsql_column_width_info_list_index; i < rsql_column_width_info_list_size; i++)
        {
          rsql_column_width_info_list[i].name = NULL;
          rsql_column_width_info_list[i].width = 0;
        }
