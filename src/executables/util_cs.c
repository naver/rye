...
static int
find_connect_host_index (const char *dbname, const PRM_NODE_LIST * node_list, HA_STATE expect_server_state)
{
  int i;
...
static int
find_connect_server (char *db_name, PRM_NODE_INFO * db_host, const char *database_name, BACKUPDB_CONNECT_ORDER c_order)
{
  char *ptr;
  int idx = -1;
  ...
  if (idx >= 0)
    ...
    {
      PRINT_AND_LOG_ERR_MSG (msgcat_message (MSGCAT_CATALOG_UTILS, MSGCAT_UTIL_SET_BACKUPDB, BACKUPDB_NOT_FOUND_HOST));
      return -1;
    }
...
backupdb (UTIL_FUNCTION_ARG * arg)
{
  ...
  force_overwrite = utility_get_option_bool_value (arg_map, BACKUP_FORCE_OVERWRITE_S);
  make_slave = utility_get_option_bool_value (arg_map, BACKUP_MAKE_SLAVE_S);
  connect_mode = utility_get_option_string_value (arg_map, BACKUP_CONNECT_MODE_S, 0);
      ...
      if (stat (backup_path, &st_buf) != 0 || !S_ISDIR (st_buf.st_mode))
        {
          PRINT_AND_LOG_ERR_MSG (msgcat_message (MSGCAT_CATALOG_UTILS,
                                                 MSGCAT_UTIL_SET_BACKUPDB, BACKUPDB_INVALID_PATH));
          goto error_exit;
        }
    }
  ...
  if (find_connect_server (db_name, &db_host_info, database_name, c_order) < 0)
    {
      goto error_exit;
    }
  ...
  if (db_restart (arg->command_name, TRUE, db_fullname) == NO_ERROR)
    {
      ...
      if (bk_run_backup (db_name, &db_host_info, backup_path,
                         ...
                         remove_log_archives, force_overwrite, make_slave, server_state) == NO_ERROR)
        {
...
print_backup_usage:
  fprintf (stderr, msgcat_message (MSGCAT_CATALOG_UTILS,
                                   MSGCAT_UTIL_SET_BACKUPDB, BACKUPDB_MSG_USAGE), basename (arg->argv0));
...
addvoldb (UTIL_FUNCTION_ARG * arg)
{
  ...
  if (strcasecmp (volext_string_purpose, "data") == 0)
  ...
  else
    {
      PRINT_AND_LOG_ERR_MSG (msgcat_message (MSGCAT_CATALOG_UTILS,
                                             ...
                                             ADDVOLDB_MSG_BAD_PURPOSE), volext_string_purpose);
  ...
  if (sa_mode && ext_info.max_writesize_in_sec > 0)
    {
      ext_info.max_writesize_in_sec = 0;
      fprintf (stderr, msgcat_message (MSGCAT_CATALOG_UTILS,
                                       MSGCAT_UTIL_SET_ADDVOLDB, ADDVOLDB_INVALID_MAX_WRITESIZE_IN_SEC));
    }
  ...
  if (check_database_name (database_name) || check_volume_name (ext_info.fullname))
    {
      goto error_exit;
    }
...
print_addvol_usage:
  fprintf (stderr, msgcat_message (MSGCAT_CATALOG_UTILS,
                                   MSGCAT_UTIL_SET_ADDVOLDB, ADDVOLDB_MSG_USAGE), basename (arg->argv0));
  util_log_write_errid (MSGCAT_UTIL_GENERIC_INVALID_ARGUMENT);
error_exit:
  return EXIT_FAILURE;
}
...
spacedb (UTIL_FUNCTION_ARG * arg)
{
  ...
  if (output_file == NULL)
  ...
  else
    {
      ...
      if (outfp == NULL)
        {
          PRINT_AND_LOG_ERR_MSG (msgcat_message (MSGCAT_CATALOG_UTILS,
                                                 MSGCAT_UTIL_SET_SPACEDB, SPACEDB_MSG_BAD_OUTPUT), output_file);
          goto error_exit;
        }
    }
  ...
  if (summarize && purpose)
    {
      ...
      size_title_format =
        (size_unit_type == SPACEDB_SIZE_UNIT_PAGE) ?
        SPACEDB_OUTPUT_SUMMARIZED_PURPOSE_TITLE_PAGE : SPACEDB_OUTPUT_SUMMARIZED_PURPOSE_TITLE_SIZE;
  ...
  fprintf (outfp, msgcat_message (MSGCAT_CATALOG_UTILS,
                                  MSGCAT_UTIL_SET_SPACEDB, title_format), database_name, io_size_str, log_size_str);
              ...
              else
                {
                  fprintf (outfp,
                           msgcat_message (MSGCAT_CATALOG_UTILS,
                                           MSGCAT_UTIL_SET_SPACEDB,
                                           output_format),
                           i, VOL_PURPOSE_STRING (vol_purpose), num_total_str, num_free_str, vol_label);
                }
            }
        }
      else
          ...
          if (purpose)
            {
              ...
              fprintf (outfp, msgcat_message (MSGCAT_CATALOG_UTILS,
                       ...
                       num_free_str, num_data_used_str, num_index_used_str, num_temp_used_str, vol_label);
            }
          else
            {
              fprintf (outfp, msgcat_message (MSGCAT_CATALOG_UTILS,
  ...
  if (nvols > 1 && !summarize)
    {
      ...
      if (purpose)
      ...
      else
        {
          fprintf (outfp, msgcat_message (MSGCAT_CATALOG_UTILS,
                                          MSGCAT_UTIL_SET_SPACEDB,
                                          output_format), nvols, " ", num_total_str, num_free_str, " ");
        }
    }
  ...
  if (!summarize)
    {
      fprintf (outfp, msgcat_message (MSGCAT_CATALOG_UTILS,
                                      MSGCAT_UTIL_SET_SPACEDB,
                                      SPACEDB_OUTPUT_TITLE_TMP_VOL), database_name, io_size_str);
  ...
  for (i = 0; i < nvols; i++)
    {
      if (disk_get_purpose_and_space_info ((temp_volid + i), &vol_purpose, &space_info) != NULL_VOLID)
        {
      ...
      for (i = 0; i < SPACEDB_NUM_VOL_PURPOSE; i++)
        {
          ...
          spacedb_get_size_str (num_used_str,
                                db_summarize_ntotal_pages[i] - db_summarize_nfree_pages[i], size_unit_type);
          ...
          if (purpose)
            {
              ...
              fprintf (outfp,
                       ...
                       num_free_str, num_data_used_str, num_index_used_str, num_temp_used_str, db_summarize_nvols[i]);
            }
          else
            {
              fprintf (outfp,
      ...
      if (purpose)
        {
          ...
          fprintf (outfp,
                   ...
                   num_free_str, num_data_used_str, num_index_used_str, num_temp_used_str, total_volume_count);
        }
      else
        {
          fprintf (outfp,
                   msgcat_message (MSGCAT_CATALOG_UTILS,
                                   MSGCAT_UTIL_SET_SPACEDB,
                                   output_format),
                   "TOTAL", num_total_str, num_used_str, num_free_str, total_volume_count);
        }
    }
...
print_space_usage:
  fprintf (stderr, msgcat_message (MSGCAT_CATALOG_UTILS,
                                   MSGCAT_UTIL_SET_SPACEDB, SPACEDB_MSG_USAGE), basename (arg->argv0));
  util_log_write_errid (MSGCAT_UTIL_GENERIC_INVALID_ARGUMENT);
error_exit:
  ...
  if (output_file == NULL)
  ...
  else
    {
      ...
      if (outfp == NULL)
        {
          PRINT_AND_LOG_ERR_MSG (msgcat_message (MSGCAT_CATALOG_UTILS,
                                                 MSGCAT_UTIL_SET_LOCKDB, LOCKDB_MSG_BAD_OUTPUT), output_file);
          goto error_exit;
        }
    }
...
print_lock_usage:
  fprintf (stderr, msgcat_message (MSGCAT_CATALOG_UTILS,
                                   MSGCAT_UTIL_SET_LOCKDB, LOCKDB_MSG_USAGE), basename (arg->argv0));
  util_log_write_errid (MSGCAT_UTIL_GENERIC_INVALID_ARGUMENT);
...
#else /* CS_MODE */
  fprintf (stderr, msgcat_message (MSGCAT_CATALOG_UTILS,
                                   MSGCAT_UTIL_SET_LOCKDB, LOCKDB_MSG_NOT_IN_STANDALONE), basename (arg->argv0));
  return EXIT_FAILURE;
#endif /* !CS_MODE */
}
...
doesmatch_transaction (const ONE_TRAN_INFO * tran, int *tran_index_list,
                       int index_list_size,
                       const char *username, const char *hostname, const char *progname, const char *sql_id)
{
  int i;
  ...
  if (num_valid > 0)
  ...
  else
    {
      fprintf (stdout, msgcat_message (MSGCAT_CATALOG_UTILS,
                                       MSGCAT_UTIL_SET_TRANLIST, TRANLIST_MSG_NONE_TABLE_ENTRIES));
    }
          ...
          if (isvalid_transaction (&info->tran[i]) && !XASL_ID_IS_NULL (&info->tran[i].query_exec_info.xasl_id))
            {
              fprintf (stdout, msgcat_message (MSGCAT_CATALOG_UTILS,
                                               ...
                                               TRANLIST_MSG_SQL_ID), info->tran[i].query_exec_info.sql_id);
              fprintf (stdout, msgcat_message (MSGCAT_CATALOG_UTILS,
                                               MSGCAT_UTIL_SET_TRANLIST,
                                               TRANLIST_MSG_TRAN_INDEX), info->tran[i].tran_index);
...
kill_transactions (TRANS_INFO * info, int *tran_index_list, int list_size,
                   const char *username, const char *hostname, const char *progname, const char *sql_id, bool verify)
{
  int i, ok;
          ...
          for (i = 0; i < info->num_trans; i++)
            {
              if (doesmatch_transaction (&info->tran[i], tran_index_list,
                                         list_size, username, hostname, progname, sql_id))
                {
                  print_tran_entry (&info->tran[i], dump_level);
                }
            }
              ...
              if (doesmatch_transaction (&info->tran[i], tran_index_list,
                                         list_size, username, hostname, progname, sql_id))
                {
                  fprintf (stdout, msgcat_message (MSGCAT_CATALOG_UTILS,
                                                   ...
                                                   KILLTRAN_MSG_KILLING), info->tran[i].tran_index);
                      ...
                      if (nfailures == 0)
                        {
                          fprintf (stdout,
                                   msgcat_message (MSGCAT_CATALOG_UTILS,
                                                   MSGCAT_UTIL_SET_KILLTRAN, KILLTRAN_MSG_KILL_FAILED));
                          fprintf (stdout, msgcat_message (MSGCAT_CATALOG_UTILS, MSGCAT_UTIL_SET_TRANLIST, header));
                          fprintf (stdout, msgcat_message (MSGCAT_CATALOG_UTILS, MSGCAT_UTIL_SET_TRANLIST, underscore));
                        }
                      ...
                      else      /* probably it is the case of timeout */
                        {
                          PRINT_AND_LOG_ERR_MSG (msgcat_message
                                                 (MSGCAT_CATALOG_UTILS,
                                                  MSGCAT_UTIL_SET_KILLTRAN, KILLTRAN_MSG_KILL_TIMEOUT));
                        }
                      nfailures++;
                    }
                }
  ...
  if (dump_level == TRANDUMP_FULL_INFO)
  ...
  else
    {
      fprintf (stdout, msgcat_message (MSGCAT_CATALOG_UTILS,
               ...
               tran_info->tran_index,
               tran_get_tranlist_state_name (tran_info->state),
               tran_info->db_user, tran_info->host_name, tran_info->process_id, tran_info->program_name);
    }
...
tranlist (UTIL_FUNCTION_ARG * arg)
{
  ...
  error = db_restart_ex (arg->command_name, database_name, username, password, BOOT_CLIENT_READ_ONLY_ADMIN_UTILITY);
  if (error != NO_ERROR)
    {
      char msg_buf[64];
      ...
      if (error == ER_AU_INVALID_PASSWORD && password == NULL)
        {
          ...
          snprintf (msg_buf, 64, msgcat_message (MSGCAT_CATALOG_UTILS,
                                                 MSGCAT_UTIL_SET_TRANLIST, TRANLIST_MSG_USER_PASSWORD), username);
          ...
          error = db_restart_ex (arg->command_name, database_name,
                                 username, password, BOOT_CLIENT_READ_ONLY_ADMIN_UTILITY);
        }
  ...
  if (!au_is_dba_group_member (au_get_user ()))
    {
      PRINT_AND_LOG_ERR_MSG (msgcat_message (MSGCAT_CATALOG_UTILS,
                                             MSGCAT_UTIL_SET_TRANLIST, TRANLIST_MSG_NOT_DBA_USER), username);
      db_shutdown ();
      goto error_exit;
    }
...
print_tranlist_usage:
  fprintf (stderr, msgcat_message (MSGCAT_CATALOG_UTILS,
                                   MSGCAT_UTIL_SET_TRANLIST, TRANLIST_MSG_USAGE), basename (arg->argv0));
  util_log_write_errid (MSGCAT_UTIL_GENERIC_INVALID_ARGUMENT);
...
killtran (UTIL_FUNCTION_ARG * arg)
{
  ...
  dump_client_info_flag = utility_get_option_bool_value (arg_map, KILLTRAN_DISPLAY_CLIENT_INFO_S);
      ...
      if (error == ER_AU_INVALID_PASSWORD && (dba_password == NULL || strlen (dba_password) == 0))
        {
          ...
          passbuf = getpass (msgcat_message (MSGCAT_CATALOG_UTILS,
                                             MSGCAT_UTIL_SET_KILLTRAN, KILLTRAN_MSG_DBA_PASSWORD));
  ...
  if (dump_trantab_flag || dump_client_info_flag || include_query_exec_info
  ...
  else
    {
      if (kill_tran_index != NULL && strlen (kill_tran_index) > 0)
        {
          ...
          if (result != 0 || value <= 0)
            {
              PRINT_AND_LOG_ERR_MSG (msgcat_message (MSGCAT_CATALOG_UTILS,
                                                     MSGCAT_UTIL_SET_KILLTRAN, KILLTRAN_MSG_INVALID_TRANINDEX), ptr);
      ...
      if (kill_transactions (info, tran_index_list,
                             list_size, kill_user, kill_host, kill_progname, kill_sql_id, !force) <= 0)
        {
          if (info)
            {
              logtb_free_trans_info (info);
...
print_killtran_usage:
  fprintf (stderr, msgcat_message (MSGCAT_CATALOG_UTILS,
                                   MSGCAT_UTIL_SET_KILLTRAN, KILLTRAN_MSG_USAGE), basename (arg->argv0));
  util_log_write_errid (MSGCAT_UTIL_GENERIC_INVALID_ARGUMENT);
  ...
  if (output_file == NULL)
  ...
  else
    {
      ...
      if (outfp == NULL)
        {
          PRINT_AND_LOG_ERR_MSG (msgcat_message (MSGCAT_CATALOG_UTILS,
                                                 MSGCAT_UTIL_SET_PLANDUMP, PLANDUMP_MSG_BAD_OUTPUT), output_file);
          goto error_exit;
        }
    }
...
print_plandump_usage:
  fprintf (stderr, msgcat_message (MSGCAT_CATALOG_UTILS,
                                   MSGCAT_UTIL_SET_PLANDUMP, PLANDUMP_MSG_USAGE), basename (arg->argv0));
  util_log_write_errid (MSGCAT_UTIL_GENERIC_INVALID_ARGUMENT);
...
#else /* CS_MODE */
  fprintf (stderr, msgcat_message (MSGCAT_CATALOG_UTILS,
                                   MSGCAT_UTIL_SET_PLANDUMP, PLANDUMP_MSG_NOT_IN_STANDALONE), basename (arg->argv0));
  return EXIT_FAILURE;
#endif /* !CS_MODE */
}
...
paramdump (UTIL_FUNCTION_ARG * arg)
{
  ...
  if (output_file == NULL)
  ...
  else
    {
      ...
      if (outfp == NULL)
        {
          PRINT_AND_LOG_ERR_MSG (msgcat_message (MSGCAT_CATALOG_UTILS,
                                                 MSGCAT_UTIL_SET_PARAMDUMP, PARAMDUMP_MSG_BAD_OUTPUT), output_file);
          goto error_exit;
        }
    }
  ...
  fprintf (outfp, msgcat_message (MSGCAT_CATALOG_UTILS,
                                  MSGCAT_UTIL_SET_PARAMDUMP, PARAMDUMP_MSG_SERVER_PARAMETER), database_name);
  ...
  fprintf (outfp, msgcat_message (MSGCAT_CATALOG_UTILS, MSGCAT_UTIL_SET_PARAMDUMP, PARAMDUMP_MSG_PERSIST_PARAMETER));
...
print_dumpparam_usage:
  fprintf (stderr, msgcat_message (MSGCAT_CATALOG_UTILS,
                                   MSGCAT_UTIL_SET_PARAMDUMP, PARAMDUMP_MSG_USAGE), basename (arg->argv0));
  util_log_write_errid (MSGCAT_UTIL_GENERIC_INVALID_ARGUMENT);
...
statdump (UTIL_FUNCTION_ARG * arg)
{
  ...
  output_type = utility_get_option_string_value (arg_map, STATDUMP_OUTPUT_TYPE_S, 0);
  ...
  if (output_file == NULL)
  ...
  else
    {
      ...
      if (outfp == NULL)
        {
          PRINT_AND_LOG_ERR_MSG (msgcat_message (MSGCAT_CATALOG_UTILS,
                                                 MSGCAT_UTIL_SET_STATDUMP, STATDUMP_MSG_BAD_OUTPUT), output_file);
          goto error_exit;
        }
    }
  ...
  /* error message log file */
  snprintf (er_msg_file, sizeof (er_msg_file) - 1, "%s_%s.err", local_db_name, arg->command_name);
  er_init (er_msg_file, ER_EXIT_DEFAULT);
  ...
  for (i = 0; i < node_list.num_nodes; i++)
    {
      ...
      repl_stats_size = sizeof (MONITOR_STATS) * repl_monitor[i]->meta->num_stats;
  ...
  do
    {
      if (dump_type == MNT_DUMP_TYPE_CSV_DATA && is_header_printed == false)
        {
          monitor_dump_stats (outfp, server_monitor, NULL, NULL, cumulative, MNT_DUMP_TYPE_CSV_HEADER, substr);
          ...
          for (i = 0; i < node_list.num_nodes; i++)
            {
              ...
              monitor_dump_stats (outfp, repl_monitor[i], NULL, NULL, cumulative, MNT_DUMP_TYPE_CSV_HEADER, substr);
            }
          is_header_printed = true;
        }

      /* server statdump */
      if (monitor_copy_global_stats (server_monitor, server_cur_stats) != NO_ERROR)
        {
          PRINT_AND_LOG_ERR_MSG ("%s\n", db_error_string (3));
          continue;
        }
      mnt_calc_hit_ratio (server_cur_stats, server_monitor->meta->num_stats);
      monitor_dump_stats (outfp, server_monitor, server_cur_stats, server_old_stats, cumulative, dump_type, substr);
      ...
      for (i = 0; i < node_list.num_nodes; i++)
        {
          ...
          if (monitor_copy_global_stats (repl_monitor[i], repl_cur_stats[i]) != NO_ERROR)
            {
              PRINT_AND_LOG_ERR_MSG ("%s\n", db_error_string (3));
              continue;
            }
      ...
      for (i = 0; i < node_list.num_nodes; i++)
        {
          ...
          monitor_dump_stats (outfp, repl_monitor[i],
                              repl_cur_stats[i], repl_old_stats[i], cumulative, dump_type, substr);
          memcpy (repl_old_stats[i], repl_cur_stats[i], sizeof (MONITOR_STATS) * MNT_SIZE_OF_REPL_EXEC_STATS);
...
print_statdump_usage:
  fprintf (stderr, msgcat_message (MSGCAT_CATALOG_UTILS,
                                   MSGCAT_UTIL_SET_STATDUMP, STATDUMP_MSG_USAGE), basename (arg->argv0));
  util_log_write_errid (MSGCAT_UTIL_GENERIC_INVALID_ARGUMENT);
