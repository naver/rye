...
static int check_server_capabilities (int server_cap, int client_type, int opt_cap);
...
net_histo_setup_names (void)
{
  ...
  net_Req_buffer[NET_SERVER_BO_FIND_NBEST_ENTRIES].name = "NET_SERVER_BO_FIND_NBEST_ENTRIES";
  net_Req_buffer[NET_SERVER_BO_GET_SERVER_STATE].name = "NET_SERVER_BO_GET_SERVER_STATE";
  net_Req_buffer[NET_SERVER_BO_NOTIFY_HA_APPLY_STATE].name = "NET_SERVER_BO_NOTIFY_HA_APPLY_STATE";
  ...
  net_Req_buffer[NET_SERVER_BTREE_ADD_INDEX].name = "NET_SERVER_BTREE_ADD_INDEX";
  net_Req_buffer[NET_SERVER_BTREE_DEL_INDEX].name = "NET_SERVER_BTREE_DEL_INDEX";
  net_Req_buffer[NET_SERVER_BTREE_LOAD_DATA].name = "NET_SERVER_BTREE_LOAD_DATA";
  ...
  net_Req_buffer[NET_SERVER_CT_CAN_ACCEPT_NEW_REPR].name = "NET_SERVER_CT_CAN_ACCEPT_NEW_REPR";
  ...
  net_Req_buffer[NET_SERVER_CSS_DUMP_SERVER_STAT].name = "NET_SERVER_CSS_DUMP_SERVER_STAT";
  ...
  net_Req_buffer[NET_SERVER_PRM_GET_FORCE_PARAMETERS].name = "NET_SERVER_PRM_GET_FORCE_PARAMETERS";
  ...
  net_Req_buffer[NET_SERVER_REPL_LOG_GET_EOF_LSA].name = "NET_SERVER_REPL_LOG_GET_EOF_LSA";
  ...
  net_Req_buffer[NET_SERVER_MIGRATOR_GET_LOG_PAGES].name = "NET_SERVER_MIGRATOR_GET_LOG_PAGES";
  net_Req_buffer[NET_SERVER_UPDATE_GROUP_ID].name = "NET_SERVER_UPDATE_GROUP_ID";
  net_Req_buffer[NET_SERVER_BLOCK_GLOBAL_DML].name = "NET_SERVER_BLOCK_GLOBAL_DML";

  net_Req_buffer[NET_SERVER_BK_PREPARE_BACKUP].name = "NET_SERVER_BK_PREPARE_BACKUP";
  net_Req_buffer[NET_SERVER_BK_BACKUP_VOLUME].name = "NET_SERVER_BK_BACKUP_VOLUME";
  net_Req_buffer[NET_SERVER_BK_BACKUP_LOG_VOLUME].name = "NET_SERVER_BK_BACKUP_LOG_VOLUME";
}
      ...
      if (net_Req_buffer[i].request_count)
        {
          ...
          fprintf (stream, "%-29s %6d X %10d+%10d b, %10.6f s\n",
                   ...
                   net_Req_buffer[i].total_size_sent, net_Req_buffer[i].total_size_received, server_time);
...
int
net_client_request_send_msg (unsigned int *eid, int request, int num_buffers, ...)
{
  int rc;
  va_list args;
...
int
net_client_request_send_msg_v (unsigned int *eid, int request, int num_buffers, va_list args)
{
  if (net_Server_name[0] == '\0')
    {
  ...
  *eid = css_send_request_to_server_v (net_Server_host, request, num_buffers, args);
  if (*eid == 0)
    {
...
net_client_data_send_msg_v (unsigned int eid, int num_buffers, va_list args)
{
  ...
  css_error = css_send_data_to_server_v (net_Server_host, CSS_RID_FROM_EID (eid), num_buffers, args);
...
int
net_client_request_recv_msg (CSS_NET_PACKET ** recv_packet, unsigned int eid, int timeout, int num_buffers, ...)
{
  int rc;
  va_list args;
  ...
  rc = net_client_request_recv_msg_v (recv_packet, eid, timeout, num_buffers, args);
...
net_client_request_recv_msg_v (CSS_NET_PACKET ** recv_packet,
                               unsigned int eid, UNUSED_ARG int timeout, int num_buffers, va_list args)
{
  int css_error;
  CSS_MAP_ENTRY *entry;
  int rid;
  ...
  css_error = css_recv_data_from_server_v (recv_packet, entry->conn, rid, -1, num_buffers, args);
 ...
 */
int
net_client_request (int request, CSS_NET_PACKET ** recv_packet, int num_send_buffers, int num_recv_buffers, ...)
{
  unsigned int eid;
  ...
  error = net_client_request_send_msg_v (&eid, request, num_send_buffers, args);
  va_end (args);
  ...
  if (error == NO_ERROR)
    {
      error = net_client_request_recv_msg_v (recv_packet, eid, -1, num_recv_buffers, tmp_args);
    }
...
net_client_get_log_header (LOGWR_CONTEXT * ctx_ptr,
                           char *argbuf, int argsize, char *replybuf, int replysize, char **logpg_area_buf)
{
  unsigned int eid;
  char *ptr;
  int error = NO_ERROR;
  ...
  error = net_client_request_recv_msg (&recv_packet, eid, -1, 1, replybuf, replysize);
  if (error != NO_ERROR)
    {
      return (error);
    }
    ...
    case GET_NEXT_LOG_PAGES:
      {
        ...
        if (length <= 0)
          ...
          {
            logpg_area = css_net_packet_get_buffer (recv_packet, 1, length, true);
            if (logpg_area == NULL)
              {
                    ...
                    if (unzip_logpg_area == NULL)
                    ...
                    else
                      {
                        if (!log_unzip (unzip_logpg_area, length, logpg_area))
                          ...
                          {
                            assert (unzip_logpg_area->data_length <= LOG_PAGESIZE);

                            free_and_init (logpg_area);
                            logpg_area = malloc (unzip_logpg_area->data_length);
                            if (logpg_area == NULL)
                              {
                                assert (false); /* TODO - */
                                error = ER_NET_SERVER_DATA_RECEIVE;
                              ...
                              {
                                memcpy (logpg_area, unzip_logpg_area->log_data, (int) unzip_logpg_area->data_length);
                                     ...
                                     char *argbuf, int argsize, char *replybuf, int replysize)
{
  ...
  error = net_client_request_send_msg (&eid, NET_SERVER_MIGRATOR_GET_LOG_PAGES, 1, argbuf, argsize);
  ...
  error = net_client_request_recv_msg (&recv_packet, eid, -1, 2,
                                       replybuf, replysize, ctx_ptr->logpg_area, ctx_ptr->logpg_area_size);
  if (error != NO_ERROR)
    {
      return error;
    }
      ...
      if (is_zipped)
        {
          if (!log_unzip (ctx_ptr->unzip_area, length, ctx_ptr->logpg_area))
            ...
            {
              memcpy (ctx_ptr->logpg_area, ctx_ptr->unzip_area->log_data, (int) ctx_ptr->unzip_area->data_length);
...
net_client_logwr_send_end_msg (unsigned int eid, int error)
{
  ...
  return (net_client_data_send_msg (eid, 1, request, OR_ALIGNED_BUF_SIZE (a_request)));
}
...
net_client_request_recv_stream (int request,
                                ...
                                char *replybuf, int replybuf_size, char *databuf, int datasize, FILE * outfp)
{
  unsigned int eid;
  int size;
  int error;
  ...
  error = net_client_request_send_msg (&eid, request, 2, send_argbuffer, send_argsize, databuf, datasize);
  ...
  error = net_client_request_recv_msg (NULL, eid, -1, 1, recv_replybuf, recv_replybuf_size);
  if (error != NO_ERROR)
    {
      goto end;
    }
  ...
  while (file_size > 0 && error == NO_ERROR)
    {
      ...
      error = net_client_request_recv_msg (&recv_packet, eid, -1, 1, reply_streamdata, reply_streamdata_size);
...
int
net_client_ping_server (int client_val, int *server_val, UNUSED_ARG int timeout)
{
  ...
  error = net_client_request (NET_SERVER_PING, NULL, 1, 1, request, OR_INT_SIZE, reply_buf, OR_INT_SIZE);
...
net_client_ping_server_with_handshake (int client_type,
                                       bool check_capabilities, int opt_cap, RYE_VERSION * server_version)
{
  const char *server_host;
  char *ptr;
  int error = NO_ERROR;
  OR_ALIGNED_BUF (OR_VERSION_SIZE + (OR_INT_SIZE * 3) + MAXHOSTNAMELEN) a_request;
  char *request = OR_ALIGNED_BUF_START (a_request);
  ...
  request_size = (OR_VERSION_SIZE) + (OR_INT_SIZE * 3) + or_packed_string_length (boot_Host_name, &strlen2);
  ...
  error = net_client_request (NET_SERVER_PING_WITH_HANDSHAKE, NULL, 1, 1, request, request_size, reply, reply_size);
  if (error != NO_ERROR)
    {
  ...
  if (check_capabilities == true &&
      check_server_capabilities (server_capabilities, client_type, opt_cap) != server_capabilities)
    {
