...
static int check_client_capabilities (THREAD_ENTRY * thread_p, int client_cap, const char *client_host);
static int er_log_slow_query (THREAD_ENTRY * thread_p,
                              EXECUTION_INFO * info, int time, MONITOR_STATS * diff_stats, char *queryinfo_string);
static void event_log_slow_query (THREAD_ENTRY * thread_p, EXECUTION_INFO * info, int time, MONITOR_STATS * diff_stats);
static void event_log_many_ioreads (THREAD_ENTRY * thread_p,
                                    EXECUTION_INFO * info, int time, MONITOR_STATS * diff_stats);
static void event_log_temp_expand_pages (THREAD_ENTRY * thread_p,
                                         EXECUTION_INFO * info, INT64 expand_pages, UINT64 expand_clock);
static void check_reset_on_commit (THREAD_ENTRY * thread_p);

static FILE *dump_func_open_tmpfile (THREAD_ENTRY * thread_p, unsigned int rid);
static void dump_func_send_result (THREAD_ENTRY * thread_p, unsigned int rid, int send_chunk_size, FILE * tmpfp);
...
return_error_to_client (THREAD_ENTRY * thread_p, unsigned int eid)
{
  ...
  if (area != NULL)
    {
      css_send_error_packet (conn, CSS_RID_FROM_EID (eid), (char *) area, length);
    }
...
server_capabilities (void)
{
  ...
  if (db_is_allowed_modification () == false || server_state == HA_STATE_TO_BE_SLAVE)
    {
 ...
 */
static int
check_client_capabilities (THREAD_ENTRY * thread_p, int client_cap, const char *client_host)
{
  int server_cap;
 ...
 */
void
server_ping (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
...
{
  OR_ALIGNED_BUF (OR_VERSION_SIZE + (OR_INT_SIZE * 2) + MAXHOSTNAMELEN) a_reply;
  char *reply = OR_ALIGNED_BUF_START (a_reply);
   ...
   */
  if (rel_check_net_compatible (&client_version, &server_version) == REL_NOT_COMPATIBLE)
    {
      ...
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_NET_DIFFERENT_RELEASE, 2, buf1, buf2);
      return_error_to_client (thread_p, rid);
      return CSS_UNPLANNED_SHUTDOWN;
    }

  if (check_client_capabilities (thread_p, client_capabilities, client_host) != client_capabilities)
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_NET_SERVER_HAND_SHAKE, 1, client_host);
      return_error_to_client (thread_p, rid);
      return CSS_UNPLANNED_SHUTDOWN;
  ...
  reply_size = OR_VERSION_SIZE + (OR_INT_SIZE * 2) + or_packed_string_length (boot_Host_name, &strlen1);
 ...
 */
void
slocator_fetch (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  OID oid;
  LOCK lock;
  OID class_oid;
  ...
  copy_area = NULL;
  success = xlocator_fetch (thread_p, &oid, lock, &class_oid, prefetch, &copy_area);
  ...
  if (copy_area == NULL)
    {
      css_send_reply_to_client (thread_p->conn_entry, rid, 1, reply, OR_ALIGNED_BUF_SIZE (a_reply));
    }
  else
    {
      css_send_reply_to_client (thread_p->conn_entry, rid, 3,
                                reply, OR_ALIGNED_BUF_SIZE (a_reply), desc_ptr, desc_size, content_ptr, content_size);
 ...
 */
void
slocator_get_class (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  OID class_oid, oid;
  LOCK lock;
  int prefetching;
  ...
  copy_area = NULL;
  success = xlocator_get_class (thread_p, &class_oid, &oid, lock, prefetching, &copy_area);
  if (success != NO_ERROR)
    {
      return_error_to_client (thread_p, rid);
    }
 ...
 */
void
slocator_fetch_all (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  HFID hfid;
  LOCK lock;
  ...
  int success;
  char *ptr;
  OR_ALIGNED_BUF (NET_COPY_AREA_SENDRECV_SIZE + (OR_INT_SIZE * 2) + (OR_BIGINT_ALIGNED_SIZE * 2) + OR_OID_SIZE) a_reply;
  char *reply = OR_ALIGNED_BUF_START (a_reply);
  char *desc_ptr = NULL;
  int desc_size;
  char *content_ptr;
  ...
  copy_area = NULL;
  success = xlocator_fetch_all (thread_p, &hfid, &lock, &class_oid, &nobjects, &nfetched, &last_oid, &copy_area);
 ...
 */
void
slocator_repl_force (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  int success;
  ...
  if (copy_area)
    {
      ...
      if (content_size > 0)
        {
          new_content_ptr = css_net_packet_get_buffer (recv_packet, 2, content_size, false);
 ...
 */
void
slocator_force (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  int success;
  ...
  if (copy_area)
    {
      if (num_objs > 0)
        {
          packed_desc = css_net_packet_get_buffer (recv_packet, 1, packed_desc_size, false);
          if (packed_desc == NULL)
            {
              error = ER_NET_SERVER_DATA_RECEIVE;
            }
      ...
      if (error)
      ...
      else
        {
          ...
          css_send_reply_to_client (thread_p->conn_entry, rid, 3,
                                    reply, OR_ALIGNED_BUF_SIZE (a_reply), packed_desc, packed_desc_size, NULL, 0);
        }
    }
 ...
 */
void
slocator_fetch_lockset (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  int success;
  LC_COPYAREA *copy_area;
  LC_LOCKSET *lockset;
  ...
  do
    {
      ...
      css_send_reply_to_client (thread_p->conn_entry, rid, 4,
                                reply, OR_ALIGNED_BUF_SIZE (a_reply),
                                packed, send_size, desc_ptr, desc_size, content_ptr, content_size);
 ...
 */
void
slocator_find_class_oid (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
 ...
 */
void
slocator_reserve_classnames (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  ...
  if (malloc_area != NULL)
    {
      ...
      reserved = xlocator_reserve_class_names (thread_p, num_classes, classnames, class_oids);
    }
 ...
 */
void
slocator_delete_class_name (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  const char *classname;
 ...
 */
void
slocator_rename_class_name (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
 ...
 */
void
slocator_assign_oid (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  HFID hfid;
  int expected_length;
  ...
  success = (xlocator_assign_oid (thread_p, &hfid, &perm_oid, expected_length,
                                  &class_oid, classname) == NO_ERROR) ? NO_ERROR : ER_FAILED;
  if (success != NO_ERROR)
    {
      return_error_to_client (thread_p, rid);
    }
 ...
 */
void
sqst_server_get_statistics (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  OID classoid;
  ...
  css_send_reply_to_client (thread_p->conn_entry, rid, 2, reply, OR_ALIGNED_BUF_SIZE (a_reply), buffer, buffer_length);
  if (buffer != NULL)
    {
 ...
 */
void
slog_checkpoint (THREAD_ENTRY * thread_p, unsigned int rid, UNUSED_ARG char *request, UNUSED_ARG int reqlen)
{
 ...
 */
void
slogtb_set_interrupt (THREAD_ENTRY * thread_p, UNUSED_ARG unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  int set;
...
slogtb_set_suppress_repl_on_transaction (THREAD_ENTRY * thread_p,
                                         unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  int set;
  OR_ALIGNED_BUF (OR_INT_SIZE) a_reply;
  char *reply = OR_ALIGNED_BUF_START (a_reply);
 ...
 */
void
slogtb_reset_wait_msecs (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  int wait_msecs;
  OR_ALIGNED_BUF (OR_INT_SIZE) a_reply;
  char *reply = OR_ALIGNED_BUF_START (a_reply);
 ...
 */
void
slogpb_dump_stat (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  FILE *outfp;
  int buffer_size;
 ...
 */
void
sacl_reload (THREAD_ENTRY * thread_p, unsigned int rid, UNUSED_ARG char *request, UNUSED_ARG int reqlen)
{
  int error;
  OR_ALIGNED_BUF (OR_INT_SIZE) a_reply;
  char *reply = OR_ALIGNED_BUF_START (a_reply);
 ...
 */
void
sacl_dump (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  FILE *outfp;
  int buffer_size;
 ...
 */
void
slock_dump (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  FILE *outfp;
  int buffer_size;
 ...
 */
void
sheap_create (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  int error;
  HFID hfid;
  char *ptr;
 ...
 */
void
sheap_destroy (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  int error;
  HFID hfid;
  OR_ALIGNED_BUF (OR_INT_SIZE) a_reply;
 ...
 */
void
stran_server_commit (THREAD_ENTRY * thread_p, unsigned int rid, UNUSED_ARG char *request, UNUSED_ARG int reqlen)
{
  TRAN_STATE state;
  OR_ALIGNED_BUF (OR_INT_SIZE + OR_INT_SIZE) a_reply;
  char *reply = OR_ALIGNED_BUF_START (a_reply);
 ...
 */
void
stran_server_abort (THREAD_ENTRY * thread_p, unsigned int rid, UNUSED_ARG char *request, UNUSED_ARG int reqlen)
{
  TRAN_STATE state;
  OR_ALIGNED_BUF (OR_INT_SIZE + OR_INT_SIZE) a_reply;
  char *reply = OR_ALIGNED_BUF_START (a_reply);
...
check_reset_on_commit (THREAD_ENTRY * thread_p)
{
  ...
  server_state = svr_shm_get_server_state ();
  if (has_updated && server_state == HA_STATE_TO_BE_SLAVE && BOOT_NORMAL_CLIENT_TYPE (client_type))
    {
      reset_on_commit = true;
      er_log_debug (ARG_FILE_LINE, "check_reset_on_commit(): "
                    "(has_updated && to-be-standby && normal client) " "DB_CONNECTION_STATUS_RESET\n");
    }
  else if (server_state == HA_STATE_SLAVE)
    {
      ...
      if (BOOT_RSQL_CLIENT_TYPE (client_type))
      ...
      else if (client_type == BOOT_CLIENT_READ_WRITE_BROKER)
        {
          reset_on_commit = true;
          er_log_debug (ARG_FILE_LINE, "check_reset_on_commit(): "
                        "(standby && read-write broker) " "DB_CONNECTION_STATUS_RESET\n");
        }
      else if (BOOT_NORMAL_CLIENT_TYPE (client_type) && thread_p->conn_entry->conn_reset_on_commit == true)
        {
          /* db modification error occurred */
          reset_on_commit = true;

          er_log_debug (ARG_FILE_LINE, "check_reset_on_commit(): "
                        "(standby && conn->reset_on_commit && normal client) " "DB_CONNECTION_STATUS_RESET\n");
          ...
          if (thread_p->conn_entry->ignore_repl_delay == false)
            {
              ...
              er_log_debug (ARG_FILE_LINE, "check_reset_on_commit(): "
                            ...
                            "&& default and broker client) " "DB_CONNECTION_STATUS_RESET\n");
            }
        }
    }
  else if (server_state == HA_STATE_MASTER && client_type == BOOT_CLIENT_SLAVE_ONLY_BROKER)
    {
      reset_on_commit = true;
      er_log_debug (ARG_FILE_LINE, "check_reset_on_commit(): "
                    "(active && slave only broker) " "DB_CONNECTION_STATUS_RESET\n");
    }
 ...
 */
void
stran_server_has_updated (THREAD_ENTRY * thread_p, unsigned int rid, UNUSED_ARG char *request, UNUSED_ARG int reqlen)
{
  int has_updated;
  OR_ALIGNED_BUF (OR_INT_SIZE) a_reply;
  char *reply = OR_ALIGNED_BUF_START (a_reply);
 ...
 */
void
stran_server_start_topop (THREAD_ENTRY * thread_p, unsigned int rid, UNUSED_ARG char *request, UNUSED_ARG int reqlen)
{
  int success;
  LOG_LSA topop_lsa;
 ...
 */
void
stran_server_end_topop (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  TRAN_STATE state;
  LOG_LSA topop_lsa;
  int xresult;
 ...
 */
void
stran_server_savepoint (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  int success;
 ...
 */
void
stran_server_partial_abort (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  TRAN_STATE state;
  const char *savept_name;
  LOG_LSA savept_lsa;
...
stran_server_is_active_and_has_updated (THREAD_ENTRY * thread_p,
                                        unsigned int rid, UNUSED_ARG char *request, UNUSED_ARG int reqlen)
{
 ...
 */
void
sboot_initialize_server (THREAD_ENTRY * thread_p, unsigned int rid, UNUSED_ARG char *request, UNUSED_ARG int reqlen)
{
  css_send_abort_to_client (thread_p->conn_entry, rid);
}
 ...
 */
void
sboot_register_client (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  int xint;
  ...
  tran_index = xboot_register_client (thread_p, &client_credential, client_lock_wait, &tran_state, &server_credential);
  ...
  css_send_reply_to_client (thread_p->conn_entry, rid, 2, reply, OR_ALIGNED_BUF_SIZE (a_reply), area, area_size);
 ...
 */
void
sboot_notify_unregister_client (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  int tran_index;
 ...
 */
void
sboot_add_volume_extension (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  OR_ALIGNED_BUF (OR_INT_SIZE) a_reply;
  char *reply = OR_ALIGNED_BUF_START (a_reply);
  const char *tmp_str;
 ...
 */
void
sboot_find_last_temp (THREAD_ENTRY * thread_p, unsigned int rid, UNUSED_ARG char *request, UNUSED_ARG int reqlen)
{
  int nvols;
  OR_ALIGNED_BUF (OR_INT_SIZE) a_reply;
  char *reply = OR_ALIGNED_BUF_START (a_reply);
 ...
 */
void
sboot_get_server_state (THREAD_ENTRY * thread_p, unsigned int rid, UNUSED_ARG char *request, UNUSED_ARG int reqlen)
{
  HA_STATE server_state;
  OR_ALIGNED_BUF (OR_INT_SIZE) a_reply;
  char *reply = OR_ALIGNED_BUF_START (a_reply);
 ...
 */
void
sboot_notify_ha_apply_state (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  int i, status;
 ...
 */
void
sqst_update_statistics (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  ...
  error =
    xstats_update_statistics (thread_p, &classoid,
                              ...
                              (with_fullscan ? STATS_WITH_FULLSCAN : STATS_WITH_SAMPLING));
  if (error != NO_ERROR)
    {
      return_error_to_client (thread_p, rid);
    }
 ...
 */
void
sqst_update_all_statistics (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  int error, update_stats, with_fullscan;
  char *ptr;
  OR_ALIGNED_BUF (OR_INT_SIZE) a_reply;
 ...
 */
void
sbtree_add_index (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  BTID btid;
  ...
  return_btid = xbtree_add_index (thread_p, &btid, num_atts, att_type, &class_oid, attr_id);
  if (return_btid == NULL)
    {
      return_error_to_client (thread_p, rid);
 ...
 */
void
sbtree_load_data (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  BTID btid;
  OID class_oid;
  HFID hfid;
 ...
 */
void
sbtree_delete_index (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  BTID btid;
  int success;
  OR_ALIGNED_BUF (OR_INT_SIZE) a_reply;
 ...
 */
void
sbtree_find_unique (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  OID class_oid;
  BTID btid;
  OID oid;
 ...
 */
void
sdisk_get_purpose_and_space_info (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  int int_volid;
  VOLID volid;
 ...
 */
void
sdk_vlabel (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  ...
  if (xdisk_get_fullname (thread_p, database_name, (VOLID) int_volid, vol_fullname) == NULL)
    {
      return_error_to_client (thread_p, rid);
      area_length = 0;
      area = NULL;
  ...
  css_send_reply_to_client (thread_p->conn_entry, rid, 2, reply, OR_ALIGNED_BUF_SIZE (a_reply), area, area_length);
  if (area)
    {
      free_and_init (area);
    }
 ...
 */
void
sqfile_get_list_file_page (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  QUERY_ID query_id;
  int volid, pageid;
  char *ptr;
  ...
  css_send_reply_to_client (thread_p->conn_entry, rid, 2,
                            reply, OR_ALIGNED_BUF_SIZE (a_reply), aligned_page_buf, page_size);
  return;
 ...
 */
void
sqmgr_prepare_query (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  XASL_ID xasl_id, *p;
  char *ptr;
  ...
  if (stream.xasl_stream_size > 0)
    {
      ptr = css_net_packet_get_buffer (recv_packet, 1, stream.xasl_stream_size, false);
      if (ptr == NULL)
        {
          er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_NET_SERVER_DATA_RECEIVE, 0);
          css_send_abort_to_client (thread_p->conn_entry, rid);
      ...
      if (get_xasl_header && !XASL_ID_IS_NULL (p))
        {
          ...
          if (reply_buffer == NULL)
            {
              reply_buffer_size = 0;
              er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_OUT_OF_VIRTUAL_MEMORY, 1, reply_buffer_size);
  ...
  css_send_reply_to_client (thread_p->conn_entry, rid, 2,
                            reply, OR_ALIGNED_BUF_SIZE (a_reply), reply_buffer, reply_buffer_size);
 ...
 */
void
sqmgr_execute_query (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  XASL_ID xasl_id;
  ...
  OR_ALIGNED_BUF (OR_INT_SIZE * 4 + OR_PTR_ALIGNED_SIZE + OR_INT_SIZE) a_reply;
  int query_timeout;
  ...
  assert (css_find_conn_by_tran_index (logtb_get_current_tran_index (thread_p)) != NULL);
  ...
  old_expand_pages = mnt_get_stats_with_time (thread_p, MNT_STATS_DISK_TEMP_EXPAND, &old_expand_clock);
  ...
  if (shard_key_size > 0)
    {
      shard_key_data = css_net_packet_get_buffer (recv_packet, 2, shard_key_size, false);
  ...
  list_id = xqmgr_execute_query (thread_p, &xasl_id, &query_id,
                                 ...
                                 query_timeout, shard_groupid, shard_key, &xasl_cache_entry_p);
  ...
  if (xasl_cache_entry_p)
    {
      ...
      if (trace_slow_msec >= 0 || trace_ioread > 0)
        {
          ...
          response_time = (end.tv_sec - start.tv_sec) * 1000 + (end.tv_usec - start.tv_usec) / 1000;
          ...
          if (response_time >= trace_slow_msec)
            {
              queryinfo_string_length = er_log_slow_query (thread_p, &info,
                                                           response_time, diff_stats, queryinfo_string);
              event_log_slow_query (thread_p, &info, response_time, diff_stats);
            }

          if (trace_ioread > 0 && (diff_stats[MNT_STATS_DATA_PAGE_IOREADS].value >= trace_ioread))
            {
      ...
      new_expand_pages = mnt_get_stats_with_time (thread_p, MNT_STATS_DISK_TEMP_EXPAND, &new_expand_clock);

      if (new_expand_pages - old_expand_pages > 0)
        {
          event_log_temp_expand_pages (thread_p, &info,
                                       new_expand_pages - old_expand_pages, new_expand_clock - old_expand_clock);
        }
  ...
  css_send_reply_to_client (thread_p->conn_entry, rid, 4,
                            ...
                            page_ptr_copied, page_size, queryinfo_string, queryinfo_string_length);
...
er_log_slow_query (THREAD_ENTRY * thread_p, EXECUTION_INFO * info,
                   int time, MONITOR_STATS * diff_stats, char *queryinfo_string)
{
  ...
  if (prm_get_bool_value (PRM_ID_SQL_TRACE_EXECUTION_PLAN) == true)
    {
      monitor_dump_stats_to_buffer (NULL, stat_buf, STATDUMP_BUF_SIZE, diff_stats, MNT_DUMP_TYPE_NORMAL, NULL);
    }
  else
    {
 ...
 */
static void
event_log_slow_query (THREAD_ENTRY * thread_p, EXECUTION_INFO * info, int time, MONITOR_STATS * diff_stats)
{
  FILE *log_fp;
  int indent = 2;
  LOG_TDES *tdes;
  ...
  fprintf (log_fp,
           ...
           mnt_clock_to_time (diff_stats[MNT_STATS_DATA_PAGE_FETCHES].acc_time),
           diff_stats[MNT_STATS_DATA_PAGE_IOREADS].value,
           mnt_clock_to_time (diff_stats[MNT_STATS_DATA_PAGE_IOREADS].acc_time),
           diff_stats[MNT_STATS_DATA_PAGE_IOWRITES].value,
           mnt_clock_to_time (diff_stats[MNT_STATS_DATA_PAGE_IOWRITES].acc_time));
  ...
  fprintf (log_fp, "%*cwait: csect=%ld, lock=%ld, latch=%ld\n\n", indent, ' ',
           ...
           mnt_clock_to_time (diff_stats[MNT_STATS_SQL_TRACE_LOCK_WAITS].acc_time),
           mnt_clock_to_time (diff_stats[MNT_STATS_SQL_TRACE_LATCH_WAITS].acc_time));
 ...
 */
static void
event_log_many_ioreads (THREAD_ENTRY * thread_p, EXECUTION_INFO * info, int time, MONITOR_STATS * diff_stats)
{
  FILE *log_fp;
  int indent = 2;
  LOG_TDES *tdes;
 ...
 */
static void
event_log_temp_expand_pages (THREAD_ENTRY * thread_p, EXECUTION_INFO * info, INT64 expand_pages, UINT64 expand_clock)
{
  FILE *log_fp;
  int indent = 2;
  LOG_TDES *tdes;
  ...
  fprintf (log_fp, "%*ctime: %ld\n", indent, ' ', mnt_clock_to_time (expand_clock));
  fprintf (log_fp, "%*cpages: %lld\n\n", indent, ' ', (long long int) expand_pages);
 ...
 */
void
sqmgr_end_query (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  QUERY_ID query_id;
  int success = NO_ERROR;
  OR_ALIGNED_BUF (OR_INT_SIZE) a_reply;
 ...
 */
void
sqmgr_drop_query_plan (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  XASL_ID xasl_id;
  int status;
  char *ptr, *reply;
 ...
 */
void
sqmgr_drop_all_query_plans (THREAD_ENTRY * thread_p, unsigned int rid, UNUSED_ARG char *request, UNUSED_ARG int reqlen)
{
  int status;
  char *reply;
  OR_ALIGNED_BUF (OR_INT_SIZE) a_reply;
 ...
 */
void
sqmgr_dump_query_plans (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  FILE *outfp;
  int buffer_size;
 ...
 */
void
sqmgr_get_query_info (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  QUERY_ID query_id;
  int count;
  int length;
  ...
  css_send_reply_to_client (thread_p->conn_entry, rid, 2, reply, OR_ALIGNED_BUF_SIZE (a_reply), buf, length);
  free_and_init (buf);
}
 ...
 */
void
sct_can_accept_new_repr (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  OID classoid;
  HFID hfid;
  ...
  success = xcatalog_is_acceptable_new_representation (thread_p, &classoid, &hfid, &can_accept);
  if (success != NO_ERROR)
    {
      return_error_to_client (thread_p, rid);
    }
 ...
 */
int
xs_receive_data_from_client (THREAD_ENTRY * thread_p, char **area, int *datasize, int timeout)
{
  unsigned int eid;
  ...
  css_error = css_recv_data_packet_from_client (&recv_packet, thread_p->conn_entry, CSS_RID_FROM_EID (eid), timeout, 0);
 ...
 */
void
slocator_find_lockhint_class_oids (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  int num_classes;
  ...
  malloc_size = ((sizeof (char *) + sizeof (LOCK) + sizeof (int) + sizeof (OID)) * num_classes);
  ...
  if (malloc_area != NULL)
    {
      ...
      guessed_class_oids = (OID *) ((char *) many_locks + (sizeof (int) * num_classes));
      ...
      allfind = xlocator_find_lockhint_class_oids (thread_p, num_classes,
                                                   many_classnames,
                                                   many_locks,
                                                   guessed_class_oids, quit_on_errors, &found_lockhint, &copy_area);
    }
 ...
 */
void
slocator_fetch_lockhint_classes (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  int success;
  LC_COPYAREA *copy_area;
  ...
  while (copy_area && ((lockhint->num_classes > lockhint->num_classes_processed)));
 ...
 */
void
sthread_kill_tran_index (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  int success;
  int kill_tran_index;
  int kill_pid;
  ...
  success = (xthread_kill_tran_index (thread_p, kill_tran_index, kill_user,
                                      kill_host, kill_pid) == NO_ERROR) ? NO_ERROR : ER_FAILED;
  if (success != NO_ERROR)
    {
      return_error_to_client (thread_p, rid);
    }
 ...
 */
void
sthread_dump_cs_stat (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  FILE *outfp;
  int buffer_size;
 ...
 */
void
sserver_stats_dump (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  FILE *outfp;
  int buffer_size;
 ...
 */
void
slogtb_get_pack_tran_table (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  char *buffer, *ptr;
  int size;
  OR_ALIGNED_BUF (OR_INT_SIZE + OR_INT_SIZE) a_reply;
  ...
  if (error != NO_ERROR)
  ...
  else
    {
      ...
      ptr = or_pack_int (ptr, error);
      css_send_reply_to_client (thread_p->conn_entry, rid, 2, reply, OR_ALIGNED_BUF_SIZE (a_reply), buffer, size);
      free_and_init (buffer);
    }
}
 ...
 */
void
slogtb_dump_trantable (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  FILE *outfp;
  int buffer_size;
...
xlog_send_log_pages_to_client (THREAD_ENTRY * thread_p,
                               ...
                               INT64 eof_pageid, INT64 first_pageid, int num_page, int file_status)
{
  ...
  rc = css_send_reply_to_client (thread_p->conn_entry, rid, 2,
                                 reply, OR_ALIGNED_BUF_SIZE (a_reply), logpg_area, area_size);
  if (rc)
    {
      return ER_FAILED;
    }
  ...
  er_log_debug (ARG_FILE_LINE,
                ...
                first_pageid, num_page, LOG_HA_FILESTAT_NAME (file_status), HA_STATE_NAME (server_state));
 ...
 */
int
xlog_send_log_pages_to_migrator (THREAD_ENTRY * thread_p, char *logpg_area, int area_size)
{
  OR_ALIGNED_BUF (OR_INT_SIZE) a_reply;
  char *reply = OR_ALIGNED_BUF_START (a_reply);
  unsigned int rid, rc;
  ...
  er_log_debug (ARG_FILE_LINE, "xlog_send_log_pages_to_migrator, area_size(%d)\n", area_size);
 ...
 */
int
xlog_get_page_request_with_reply (THREAD_ENTRY * thread_p, LOG_PAGEID * fpageid_ptr)
{
  char *reply = NULL;
  int reply_size;
  ...
  error = xs_receive_data_from_client (thread_p, &reply, &reply_size,
                                       prm_get_bigint_value (PRM_ID_HA_COPY_LOG_TIMEOUT));
  if (error != NO_ERROR)
    {
      if (reply)
        {
 ...
 */
void
sheap_get_class_num_objs_and_pages (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  HFID hfid;
 ...
 */
void
sqp_get_server_info (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  int success = NO_ERROR;
  OR_ALIGNED_BUF (OR_INT_SIZE + OR_INT_SIZE) a_reply;
  char *reply = OR_ALIGNED_BUF_START (a_reply);
      ...
      if (info_bits & (1 << i))
        {
          switch ((1 << i))
            ...
            case SI_SYS_DATETIME:
              success = ((db_sys_datetime (&value[i]) == NO_ERROR) ? NO_ERROR : ER_FAILED);
              break;
 ...
 */
void
sprm_server_change_parameters (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  OR_ALIGNED_BUF (OR_INT_SIZE) a_reply;
  char *reply = OR_ALIGNED_BUF_START (a_reply);
 ...
 */
void
sprm_server_obtain_parameters (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  ...
  if (reply_data == NULL)
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_OUT_OF_VIRTUAL_MEMORY, 1, reply_data_size);
  ...
  css_send_reply_to_client (thread_p->conn_entry, rid, 2,
                            reply, OR_ALIGNED_BUF_SIZE (a_reply), reply_data, reply_data_size);
  if (reply_data != NULL)
    {
 ...
 */
void
sprm_server_dump_parameters (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  FILE *outfp;
  int buffer_size;
 ...
 */
void
srepl_set_info (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  int success = NO_ERROR;
  OR_ALIGNED_BUF (OR_INT_SIZE) a_reply;
  char *reply = OR_ALIGNED_BUF_START (a_reply);
 ...
 */
void
srepl_log_get_eof_lsa (THREAD_ENTRY * thread_p, unsigned int rid, UNUSED_ARG char *request, UNUSED_ARG int reqlen)
{
 ...
 */
void
slogwr_get_log_pages (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  OR_ALIGNED_BUF (OR_INT_SIZE * 2) a_reply;
  char *reply = OR_ALIGNED_BUF_START (a_reply);
  char *ptr;
  ...
  error = xlogwr_get_log_pages (thread_p, first_pageid, ((compressed_protocol == 1) ? true : false));
  ...
  ptr = or_pack_int (ptr, error);
  (void) css_send_reply_to_client (thread_p->conn_entry, rid, 1, reply, OR_ALIGNED_BUF_SIZE (a_reply));
 ...
 */
void
ssession_find_or_create_session (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  ...
  if (key.id == DB_EMPTY_SESSION
      || memcmp (server_session_key, xboot_get_server_session_key (),
                 SERVER_SESSION_KEY_SIZE) != 0 || xsession_check_session (thread_p, &key) != NO_ERROR)
    {
      /* not an error yet */
      er_clear ();
      /* create new session */
 ...
 */
void
ssession_end_session (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  int err = NO_ERROR;
  SESSION_KEY key;
  OR_ALIGNED_BUF (OR_INT_SIZE) a_reply;
 ...
 */
void
sboot_get_locales_info (THREAD_ENTRY * thread_p, unsigned int rid, UNUSED_ARG char *request, UNUSED_ARG int reqlen)
{
  int err = NO_ERROR;
  ...
  for (i = 0; i < LANG_MAX_COLLATIONS; i++)
    {
      ...
      assert (lc != NULL);
      if (i > LANG_COLL_UTF8_EN_CI && lc->coll.coll_id == LANG_COLL_UTF8_EN_CI)
        {
          continue;
        }
      found_coll++;
  ...
  css_send_reply_to_client (thread_p->conn_entry, rid, 2, reply, OR_ALIGNED_BUF_SIZE (a_reply), data_reply, size);
 ...
 */
void
slock_change_class_xlock_to_ulock (THREAD_ENTRY * thread_p, unsigned int rid, char *request, int reqlen)
{
  int error = NO_ERROR;
  OR_ALIGNED_BUF (OR_INT_SIZE) a_reply;
  char *reply;
 ...
 */
void
smigrator_get_log_pages (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  char *ptr;
  LOG_PAGEID first_pageid;
  int compressed_protocol;
  ...
  error = xmigrator_get_log_pages (thread_p, first_pageid, ((compressed_protocol == 1) ? true : false));
 ...
 */
void
slogtb_update_group_id (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  ...
  error = xlogtb_update_group_id (thread_p, migrator_id, group_id, target, on_off);
 ...
 */
void
slogtb_block_global_dml (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  int start_or_end;
  OR_ALIGNED_BUF (OR_INT_SIZE) a_reply;
  char *reply;
 ...
 */
void
sbk_prepare_backup (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  ...
  if (session == NULL)
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_OUT_OF_VIRTUAL_MEMORY, 1, sizeof (BK_BACKUP_SESSION));
      return_error_to_client (thread_p, rid);
      area_size = 0;
      goto end;
    }

  error = xbk_prepare_backup (thread_p, num_threads, do_compress, sleep_msecs, make_slave, session);
 ...
 */
void
sbk_backup_volume (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  int buf_size;
  int error;
 ...
 */
void
sbk_backup_log_volume (THREAD_ENTRY * thread_p, unsigned int rid, char *request, UNUSED_ARG int reqlen)
{
  ...
  error = xbk_backup_log_volume (thread_p, rid, buf_size, delete_unneeded_logarchives);
...
static void
dump_func_send_result (THREAD_ENTRY * thread_p, unsigned int rid, int send_chunk_size, FILE * tmpfp)
{
  OR_ALIGNED_BUF (OR_INT_SIZE) a_reply;
  char *reply = OR_ALIGNED_BUF_START (a_reply);
  char *buffer = NULL;
  ...
  while (file_size > 0)
    {
      ...
      css_send_reply_to_client (thread_p->conn_entry, rid, 1, buffer, send_size);
    }
