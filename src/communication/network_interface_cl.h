...
extern int locator_fetch (OID * oidp, LOCK lock, OID * class_oid, int prefetch, LC_COPYAREA ** fetch_copyarea);
#if defined (ENABLE_UNUSED_FUNCTION)
extern int locator_get_class (OID * class_oid, const OID * oid,
                              LOCK lock, int prefetching, LC_COPYAREA ** fetch_copyarea);
#endif
extern int locator_fetch_all (const HFID * hfid, LOCK * lock,
                              OID * class_oidp, INT64 * nobjects,
                              INT64 * nfetched, OID * last_oidp, LC_COPYAREA ** fetch_copyarea);
...
extern LC_FIND_CLASSNAME locator_reserve_class_names (const int num_classes,
                                                      const char **class_names, OID * class_oids);
...
extern int locator_assign_oid (const HFID * hfid, OID * perm_oid,
                               int expected_length, OID * class_oid, const char *class_name);
#if defined (ENABLE_UNUSED_FUNCTION)
...
extern char *disk_get_fullname (const char *database_name, VOLID volid, char *vol_fullname);
...
#if defined (ENABLE_UNUSED_FUNCTION)
extern int boot_check_db_consistency (int check_flag, OID * oids, int num_oids);
#endif
...
extern int boot_notify_ha_apply_state (const PRM_NODE_INFO * node_info, HA_APPLY_STATE state);
extern char *stats_get_statistics_from_server (OID * classoid, unsigned int timestamp, int *length_ptr);
extern int stats_update_statistics (OID * classoid, int update_stats, int with_fullscan);
extern int stats_update_all_statistics (int update_stats, int with_fullscan);

extern int btree_add_index (BTID * btid, int num_atts, DB_TYPE * att_type, OID * class_oid, int attr_id);
extern int btree_load_data (BTID * btid, OID * class_oid, HFID * hfid);
extern int btree_delete_index (BTID * btid);
extern BTREE_SEARCH btree_find_unique (OID * class_oid, BTID * btid, DB_IDXKEY * key, OID * oid);
...
extern QFILE_LIST_ID *qmgr_execute_query (const XASL_ID * xasl_id,
                                          ...
                                          int query_timeout,
                                          int shard_groupid,
                                          DB_VALUE * shard_key, QUERY_EXECUTE_STATUS_FLAG * qe_status_flag);
...
extern int catalog_is_acceptable_new_representation (OID * class_id, HFID * hfid, int *can_accept);
...
extern int heap_get_class_num_objects_pages (HFID * hfid, int approximation, DB_BIGINT * nobjs, int *npages);
...
extern int net_client_request_send_msg (unsigned int *eid, int request, int num_buffers, ...);
extern int net_client_request_send_msg_v (unsigned int *eid, int request, int num_buffers, va_list args);
extern int net_client_data_send_msg (unsigned int eid, int num_buffers, ...);
extern int net_client_data_send_msg_v (unsigned int eid, int num_buffers, va_list args);

extern int net_client_request_recv_msg (CSS_NET_PACKET ** recv_packet,
                                        unsigned int eid, int timeout, int num_buffers, ...);
extern int net_client_request_recv_msg_v (CSS_NET_PACKET ** recv_packet,
                                          unsigned int eid, int timeout, int num_buffers, va_list args);
...
extern int net_client_get_log_header (LOGWR_CONTEXT * ctx_ptr, char *argbuf,
                                      int argsize, char *replybuf, int replysize, char **logpg_area_buf);
extern int net_client_request_with_rbl_context (RBL_SYNC_CONTEXT * ctx_ptr,
                                                char *argbuf, int argsize, char *replybuf, int replysize);
...
extern int net_client_request_recv_stream (int request, char *argbuf,
                                           ...
                                           int replybuf_size, char *databuf, int datasize, FILE * outfp);
...
extern int net_client_ping_server_with_handshake (int client_type,
                                                  bool check_capabilities, int opt_cap, RYE_VERSION * svr_version);

extern int net_client_init (const char *dbname, const PRM_NODE_INFO * node_info);
...
extern int logtb_update_group_id (int migrator_id, int group_id, int target, int on_off);
extern int logtb_block_globl_dml (int start_or_end);
extern int bk_prepare_backup (int num_threads, int do_compress,
                              int sleep_msecs, int make_slave, BK_BACKUP_SESSION * session);
extern int bk_backup_volume (BK_BACKUP_SESSION * session);
extern int bk_backup_log_volume (BK_BACKUP_SESSION * session, int delete_unneeded_logarchives);
