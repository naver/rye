...
static int scan_init_scan_pred (SCAN_PRED * scan_pred_p,
                                REGU_VARIABLE_LIST regu_list, PRED_EXPR * pred_expr, PR_EVAL_FNC pr_eval_fnc);
...
static ROP_TYPE compare_val_op (const DB_IDXKEY * key1,
                                const ROP_TYPE op1,
                                const DB_IDXKEY * key2, const ROP_TYPE op2, const int num_index_term);
...
static int scan_init_multi_range_optimization (THREAD_ENTRY * thread_p,
                                               MULTI_RANGE_OPT * multi_range_opt, bool use_range_opt, int max_size);
static int scan_dump_key_into_tuple (THREAD_ENTRY * thread_p,
                                     INDX_SCAN_ID * iscan_id,
                                     const DB_IDXKEY * key, OID * oid, QFILE_TUPLE_RECORD * tplrec);
static SCAN_CODE call_get_next_index_oidset (THREAD_ENTRY * thread_p, SCAN_ID * scan_id, INDX_SCAN_ID * isidp);
static int scan_key_compare (const DB_IDXKEY * key1, const DB_IDXKEY * key2, const int num_index_term);
...
scan_init_filter_info (FILTER_INFO * filter_info_p, SCAN_PRED * scan_pred,
                       ...
                       VAL_DESCR * val_descr, OID * class_oid, OR_CLASSREP * classrepr, int indx_id)
{
...
scan_init_indx_coverage (THREAD_ENTRY * thread_p,
                         int coverage_enabled,
                         OUTPTR_LIST * output_val_list,
                         REGU_VARIABLE_LIST regu_val_list, VAL_DESCR * vd, QUERY_ID query_id, INDX_COV * indx_cov)
{
  int err = NO_ERROR;
  ...
  indx_cov->type_list = (QFILE_TUPLE_VALUE_TYPE_LIST *) malloc (sizeof (QFILE_TUPLE_VALUE_TYPE_LIST));
  ...
  if (num_membuf_pages > 0)
    {
      indx_cov->max_tuples = (num_membuf_pages * IO_PAGESIZE) / BTREE_MAX_KEYLEN;
  ...
  indx_cov->tplrec = (QFILE_TUPLE_RECORD *) malloc (sizeof (QFILE_TUPLE_RECORD));
  ...
  indx_cov->lsid = (QFILE_LIST_SCAN_ID *) malloc (sizeof (QFILE_LIST_SCAN_ID));
...
scan_init_index_key_limit (THREAD_ENTRY * thread_p, INDX_SCAN_ID * isidp, KEY_INFO * key_infop, VAL_DESCR * vd)
{
  ...
  if (key_infop->key_limit_l != NULL)
    {
      if (fetch_peek_dbval (thread_p, key_infop->key_limit_l, vd, NULL, NULL, &dbvalp) != NO_ERROR)
        {
          goto exit_on_error;
        }
  ...
  if (key_infop->key_limit_u != NULL)
    {
      if (fetch_peek_dbval (thread_p, key_infop->key_limit_u, vd, NULL, NULL, &dbvalp) != NO_ERROR)
        {
          goto exit_on_error;
        }
      orig_type = DB_VALUE_DOMAIN_TYPE (dbvalp);
...
scan_alloc_oid_buf (void)
{
  ...
  if (oid_buf_p == NULL)
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_OUT_OF_VIRTUAL_MEMORY, 1, oid_buf_size);
    }
 ...
 */
static int
scan_key_compare (const DB_IDXKEY * key1, const DB_IDXKEY * key2, const int num_index_term)
{
  int rc = DB_UNK;
  ...
  while (key_cnt > 1 && n < key_cnt - 1)
    {
      if (DB_IDXKEY_IS_NULL (&curp->lower_key))
        ...
        {
          c = btree_compare_key (NULL, NULL, &curp->lower_key, &nextp->lower_key, NULL);
        }
      ...
      while (next_n < key_cnt)
        {
          ...
          if (is_mergeable == true)
            {
              cmp_1 = compare_val_op (&curp->upper_key, cur_op2, &nextp->lower_key, next_op1, curp->num_index_term);
          ...
          if (is_mergeable == true)
            {
              cmp_2 = compare_val_op (&curp->lower_key, cur_op1, &nextp->upper_key, next_op2, curp->num_index_term);
          ...
          if (is_mergeable == true)
            {
              ...
              cmp_3 = compare_val_op (&curp->lower_key, cur_op1, &nextp->lower_key, next_op1, curp->num_index_term);
              if (cmp_3 == ROP_NA)
                {
                  is_mergeable = false;
                }
          ...
          if (is_mergeable == true)
            {
              ...
              cmp_4 = compare_val_op (&curp->upper_key, cur_op2, &nextp->upper_key, next_op2, curp->num_index_term);
              if (cmp_4 == ROP_NA)
                {
                  is_mergeable = false;
                }
  ...
  if (pk_next_regu_list != NULL)
    {
      ...
      for (i = 0, operand = pk_next_regu_list; operand != NULL; i++, operand = operand->next)
        {
          assert (operand->value.type == TYPE_POS_VALUE);

          ret = fetch_peek_dbval (thread_p, &(operand->value), vd, NULL, NULL, &val);
          if (ret != NO_ERROR)
            {
              goto err_exit;
            }
  ...
  for (i = 0; operand != NULL; operand = operand->next, i++)
    {
      ...
      ret = fetch_copy_dbval (thread_p, &(operand->value), vd, NULL, NULL, &(retval->vals[i]));
      if (ret != NO_ERROR)
        {
          goto err_exit;
        }
      ...
      if (tp_valid_indextype (val_type) == false)
        {
          ...
          er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ret, 2, pr_type_name (val_type), pr_type_name (col_type));
      ...
      if (col_type != val_type || col_type == DB_TYPE_NUMERIC)
        {
          ...
          if (TP_IS_NUMERIC_TYPE (val_type) || TP_IS_DATE_OR_TIME_TYPE (val_type))
            {
              /* Coerce the value to index domain  */
#if 1                           /* TODO - at current, do not care ret value */
              (void) tp_value_coerce_strict (val, &(retval->vals[i]), idx_dom);
#endif
            }
        }
      else                      /* (col_type == val_type && col_type != DB_TYPE_NUMERIC) */
        {
          ...
          if (TP_IS_CHAR_TYPE (col_type) && TP_IS_CHAR_TYPE (val_type))
            {
              LANG_RT_COMMON_COLL (TP_DOMAIN_COLLATION (idx_dom), DB_GET_STRING_COLLATION (val), common_coll);
              if (common_coll == -1)
                {
  ...
  if (key_ranges->key1)
    {
      ...
      if (key_ranges->key1->type == TYPE_FUNC && key_ranges->key1->value.funcp->ftype == F_IDXKEY)
        {
  ...
  if (key_ranges->key2)
    {
      ...
      if (key_ranges->key2->type == TYPE_FUNC && key_ranges->key2->value.funcp->ftype == F_IDXKEY)
        {
      ...
      assert ((key_ranges->key1 != NULL && key_val_range->num_index_term > 0) || key_val_range->num_index_term == 0);
  ...
  if (iscan_id->pk_next_regu_list != NULL)
    {
      ...
      part_key_desc = indexp->asc_desc[key_val_range->num_index_term - 1] ? true : false;
    }
#endif
  ...
  if (key_ranges->key1)
    {
      ...
      ret = scan_dbvals_to_idxkey (thread_p, lower_key,
                                   ...
                                   ((iscan_id->pk_next_regu_list != NULL
                                     && !part_key_desc) ? iscan_id->pk_next_regu_list : NULL), vd, key_minmax);
  ...
  else
    {
      ...
      assert (key_val_range->range == INF_LE || key_val_range->range == INF_LT);

      ret = btree_coerce_idxkey (lower_key, indexp, key_val_range->num_index_term, key_minmax);
    }
  ...
  if (key_ranges->key2)
  ...
  else
    {
      ...
      if (key_val_range->range == EQ_NA)
        ...
        {
          assert (key_val_range->range == GE_INF || key_val_range->range == GT_INF);

          ret = btree_coerce_idxkey (upper_key, indexp, key_val_range->num_index_term, key_minmax);
        }
    }
    ...
    case R_RANGELIST:
      {
        ...
        if (key_val_range->range >= GE_LE && key_val_range->range <= GT_LT)
          {
            /* to fix multi-column index NULL problem */
            if (DB_IDXKEY_IS_NULL (lower_key) || DB_IDXKEY_IS_NULL (upper_key))
              {
                key_val_range->range = NA_NA;
            ...
            else
              {
                ...
                c = scan_key_compare (lower_key, upper_key, key_val_range->num_index_term);
...
scan_get_index_oidset (THREAD_ENTRY * thread_p, SCAN_ID * s_id)
{
      ...
      for (i = 0; i < key_cnt; i++)
        {
          ...
          ret = scan_regu_key_to_index_key (thread_p, &key_ranges[i], &key_vals[i], iscan_id, s_id->vd);
      ...
      for (i = 0; i < key_cnt; i++)
        {
          ...
          /* the last partial-key domain is desc */
          part_key_desc = indexp->asc_desc[key_vals[i].num_index_term - 1] ? true : false;
  ...
  scan_init_filter_info (&key_filter, &iscan_id->key_pred,
                         ...
                         &(BTS->btid_int.cls_oid), BTS->btid_int.classrepr, BTS->btid_int.indx_id);
      ...
      while (iscan_id->curr_keyno < key_cnt)
        {
          ...
          if (range == NA_NA)
            {
              ...
              if (iscan_id->curr_keyno < key_cnt
                  ...
                  && iscan_id->key_limit_lower == -1 && indx_infop->key_info.key_limit_reset)
                {
          ...
          n = btree_range_search (thread_p,
                                  &indx_infop->indx_id.i.btid, &key_vals[iscan_id->curr_keyno], &key_filter, iscan_id);
          if (n < 0)
            {
#if 1                           /* TODO - */
 ...
 */
static void
scan_init_scan_id (SCAN_ID * scan_id, int fixed, QPROC_FETCH_TYPE fetch_type, VAL_LIST * val_list, VAL_DESCR * vd)
{
                     ...
                     int num_attrs_rest, ATTR_ID * attrids_rest, HEAP_CACHE_ATTRINFO * cache_rest)
{
  ...
  /* scan predicates */
  if (scan_init_scan_pred (&hsidp->scan_pred, regu_list_pred, pr, ((pr) ? eval_fnc (thread_p, pr) : NULL)) != NO_ERROR)
    {
      return ER_FAILED;
    }
...
scan_open_index_scan (THREAD_ENTRY * thread_p, SCAN_ID * scan_id,
                      ...
                      ATTR_ID * attrids_pred,
                      HEAP_CACHE_ATTRINFO * cache_pred,
                      int num_attrs_rest, ATTR_ID * attrids_rest, HEAP_CACHE_ATTRINFO * cache_rest, QUERY_ID query_id)
{
  int ret = NO_ERROR;
  INDX_SCAN_ID *isidp;
  BTID *btid;
  ...
  BTS->btid_int.indx_id = heap_classrepr_find_index_id (BTS->btid_int.classrepr, BTS->btid_int.sys_btid);
  ...
  if (scan_init_scan_pred (&isidp->key_pred, regu_list_key, pr_key,
                           ((pr_key) ? eval_fnc (thread_p, pr_key) : NULL)) != NO_ERROR)
    {
      goto exit_on_error;
    }
  ...
  /* scan predicates */
  if (scan_init_scan_pred (&isidp->scan_pred, regu_list_pred, pr, ((pr) ? eval_fnc (thread_p, pr) : NULL)) != NO_ERROR)
    {
      goto exit_on_error;
    }
  ...
  if (isidp->key_cnt > 0)
    {
      isidp->key_vals = (KEY_VAL_RANGE *) malloc (isidp->key_cnt * sizeof (KEY_VAL_RANGE));
  ...
  if (scan_init_indx_coverage (thread_p, coverage_enabled, output_val_list,
                               regu_val_list, vd, query_id, &(isidp->indx_cov)) != NO_ERROR)
    {
      goto exit_on_error;
    }
  ...
  /* initialize multiple range search optimization structure */
  {
    bool use_multi_range_opt =
      (BTS->btid_int.classrepr->indexes[BTS->btid_int.indx_id].n_atts > 1
       ...
       && isidp->key_limit_upper > 0
       && isidp->key_limit_upper < DB_INT32_MAX && isidp->key_limit_lower == -1) ? true : false;

    if (scan_init_multi_range_optimization (thread_p,
                                            &(isidp->multi_range_opt),
                                            use_multi_range_opt, (int) isidp->key_limit_upper) != NO_ERROR)
      {
        goto exit_on_error;
      }
...
scan_open_list_scan (THREAD_ENTRY * thread_p, SCAN_ID * scan_id,
                     /* fields of SCAN_ID */
                     QPROC_FETCH_TYPE fetch_type, VAL_LIST * val_list, VAL_DESCR * vd,
                     /* fields of LLIST_SCAN_ID */
                     QFILE_LIST_ID * list_id,
                     REGU_VARIABLE_LIST regu_list_pred, PRED_EXPR * pr, REGU_VARIABLE_LIST regu_list_rest)
{
  ...
  /* scan predicates */
  if (scan_init_scan_pred (&llsidp->scan_pred, regu_list_pred, pr, ((pr) ? eval_fnc (thread_p, pr) : NULL)) != NO_ERROR)
    {
      return ER_FAILED;
    }
...
scan_start_scan (THREAD_ENTRY * thread_p, SCAN_ID * scan_id)
{
  ...
  switch (scan_id->type)
    ...
    case S_HEAP_SCAN:
      ...
      ret = heap_scancache_start (thread_p, &hsidp->scan_cache, &hsidp->hfid, &hsidp->cls_oid, scan_id->fixed);
      if (ret != NO_ERROR)
        {
          goto exit_on_error;
        }
      ...
      if (hsidp->caches_inited != true)
        {
          ...
          ret = heap_attrinfo_start (thread_p, &hsidp->cls_oid,
                                     ...
                                     hsidp->pred_attrs.attr_ids, hsidp->pred_attrs.attr_cache);
          if (ret != NO_ERROR)
            {
              goto exit_on_error;
            }
          ...
          ret = heap_attrinfo_start (thread_p, &hsidp->cls_oid,
                                     ...
                                     hsidp->rest_attrs.attr_ids, hsidp->rest_attrs.attr_cache);
          if (ret != NO_ERROR)
            {
    ...
    case S_INDX_SCAN:
      ...
      ret = heap_scancache_start (thread_p, &isidp->scan_cache,
                                  &isidp->hfid, &(isidp->bt_scan.btid_int.cls_oid), scan_id->fixed);
      if (ret != NO_ERROR)
        {
          goto exit_on_error;
        }
      ...
      if (isidp->caches_inited != true)
        {
          if (isidp->key_pred.regu_list != NULL)
            {
              ...
              ret =
                heap_attrinfo_start (thread_p,
                                     ...
                                     isidp->key_attrs.attr_ids, isidp->key_attrs.attr_cache);
              if (ret != NO_ERROR)
                {
                  goto exit_on_error;
                }
          ...
          ret =
            heap_attrinfo_start (thread_p, &(isidp->bt_scan.btid_int.cls_oid),
                                 isidp->pred_attrs.num_attrs, isidp->pred_attrs.attr_ids, isidp->pred_attrs.attr_cache);
          if (ret != NO_ERROR)
            {
              if (isidp->key_pred.regu_list != NULL)
                {
          ...
          ret =
            heap_attrinfo_start (thread_p, &(isidp->bt_scan.btid_int.cls_oid),
                                 isidp->rest_attrs.num_attrs, isidp->rest_attrs.attr_ids, isidp->rest_attrs.attr_cache);
          if (ret != NO_ERROR)
            {
              if (isidp->key_pred.regu_list != NULL)
                {
      ...
      if (s_id->s.isid.indx_info)
        {
          if (scan_init_index_key_limit (thread_p, &s_id->s.isid,
                                         &s_id->s.isid.indx_info->key_info, s_id->vd) != NO_ERROR)
            {
              status = S_ERROR;
              break;
            }
  ...
  while (1)
    {
      ...
      oids_cnt = isidp->multi_range_opt.use ? isidp->multi_range_opt.cnt : isidp->oid_list.oid_cnt;
  ...
  if (on_trace)
    {
      ...
      scan_id->stats.num_fetches += mnt_get_stats (thread_p, MNT_STATS_DATA_PAGE_FETCHES) - old_fetches;
      scan_id->stats.num_ioreads += mnt_get_stats (thread_p, MNT_STATS_DATA_PAGE_IOREADS) - old_ioreads;
    }
...
scan_next_heap_scan (THREAD_ENTRY * thread_p, SCAN_ID * scan_id)
{
  ...
  scan_init_filter_info (&data_filter, &hsidp->scan_pred,
                         &hsidp->pred_attrs, scan_id->val_list, scan_id->vd, &hsidp->cls_oid, NULL, -1);
  ...
  while (1)
    {
      ...
      sp_scan =
        heap_next (thread_p, &hsidp->hfid, &hsidp->cls_oid,
                   &hsidp->curr_oid, &recdes, &hsidp->scan_cache, scan_id->fixed);
      ...
      ev_res = eval_data_filter (thread_p, &hsidp->curr_oid, &recdes, &data_filter);
      if (ev_res == V_ERROR)
        {
          return S_ERROR;
        }
      ...
      if (hsidp->rest_regu_list)
        {
          ...
          if (heap_attrinfo_read_dbvalues (thread_p, &hsidp->curr_oid,
                                           &recdes, hsidp->rest_attrs.attr_cache) != NO_ERROR)
            {
              return S_ERROR;
            }
          ...
          if (scan_id->val_list)
            {
              if (fetch_val_list (thread_p, hsidp->rest_regu_list,
                                  scan_id->vd, &hsidp->curr_oid, NULL, NULL, PEEK) != NO_ERROR)
                {
                  return S_ERROR;
                }
            }
...
scan_next_index_scan (THREAD_ENTRY * thread_p, SCAN_ID * scan_id)
{
  ...
  if (isidp->multi_range_opt.use && isidp->multi_range_opt.sort_att_idx == NULL)
    {
#if 1                           /* TODO - trace */
      assert (false);
#endif
  ...
  scan_init_filter_info (&data_filter, &isidp->scan_pred,
                         ...
                         scan_id->vd, &(isidp->bt_scan.btid_int.cls_oid), NULL, -1);
          ...
          if (isidp->curr_oidno < oids_cnt - 1)
          ...
          else
            {
              ...
              if (BTREE_END_OF_SCAN (&isidp->bt_scan)
                  && isidp->indx_info->range_type != R_RANGELIST && isidp->indx_info->range_type != R_KEYLIST)
                {
                  return S_END;
                }
              else
          ...
          if (thread_is_on_trace (thread_p))
            {
              ...
              ADD_TIMEVAL (scan_id->stats.elapsed_lookup, lookup_start, lookup_end);
            }
          ...
          if (isidp->multi_range_opt.use)
            {
              ...
              curr_item = isidp->multi_range_opt.top_n_items[isidp->curr_oidno];
          ...
          if (scan_id->val_list)
            {
              ...
              if (fetch_val_list (thread_p, isidp->indx_cov.regu_val_list,
                                  scan_id->vd, NULL, tplrec.tpl, tpl_type_list, PEEK) != NO_ERROR)
                {
                  return S_ERROR;
                }
            }
...
scan_next_index_lookup_heap (THREAD_ENTRY * thread_p, SCAN_ID * scan_id,
                             INDX_SCAN_ID * isidp, FILTER_INFO * data_filter, TRAN_ISOLATION isolation)
{
  SCAN_CODE sp_scan;
  DB_LOGICAL ev_res;
  RECDES recdes = RECDES_INITIALIZER;
  ...
  sp_scan = heap_get (thread_p, isidp->curr_oidp, &recdes, &isidp->scan_cache, scan_id->fixed);
  ...
  if (sp_scan != S_SUCCESS)
    {
      ...
      if (QPROC_OK_IF_DELETED (sp_scan, isolation))
        {
          (void) heap_get_indexname_of_btid (thread_p, &(isidp->bt_scan.btid_int.cls_oid), btid, &indx_name_p);

          class_name_p = heap_get_class_name (thread_p, &(isidp->bt_scan.btid_int.cls_oid));
          ...
          er_set (ER_NOTIFICATION_SEVERITY, ARG_FILE_LINE,
                  ...
                  isidp->curr_oidp->slotid, btid->vfid.volid, btid->vfid.fileid, btid->root_pageid);
  ...
  /* evaluate the predicates to see if the object qualifies */
  ev_res = eval_data_filter (thread_p, isidp->curr_oidp, &recdes, data_filter);
  if (ev_res == V_ERROR)
    {
      return S_ERROR;
    }
  ...
  if (use_range_opt)
    {
      ...
      multi_range_opt->top_n_items = (RANGE_OPT_ITEM **) malloc (max_size * sizeof (RANGE_OPT_ITEM *));
      if (multi_range_opt->top_n_items == NULL)
        {
          err = ER_OUT_OF_VIRTUAL_MEMORY;
          goto exit_on_error;
        }
      multi_range_opt->buffer = (RANGE_OPT_ITEM **) malloc (max_size * sizeof (RANGE_OPT_ITEM *));
      ...
      mnt_stats_counter (thread_p, MNT_STATS_BTREE_MULTIRANGE_OPTIMIZATION, 1);
    }
                          ...
                          const DB_IDXKEY * key, OID * oid, QFILE_TUPLE_RECORD * tplrec)
{
  ...
  if (iscan_id == NULL || iscan_id->indx_cov.val_descr == NULL
      || iscan_id->indx_cov.output_val_list == NULL || iscan_id->rest_attrs.attr_cache == NULL)
    {
      return ER_FAILED;
    }
  ...
  error = btree_attrinfo_read_dbvalues (thread_p, key,
                                        ...
                                        iscan_id->bt_scan.btid_int.indx_id, iscan_id->rest_attrs.attr_cache);
  if (error != NO_ERROR)
    {
      return error;
    }

  error = fetch_val_list (thread_p, iscan_id->rest_regu_list, iscan_id->indx_cov.val_descr, oid, NULL, NULL, PEEK);
  if (error != NO_ERROR)
    {
      return error;
    }
  ...
  error = qdata_copy_valptr_list_to_tuple (thread_p,
                                           &(iscan_id->indx_cov.lsid->list_id),
                                           iscan_id->indx_cov.output_val_list, iscan_id->indx_cov.val_descr, tplrec);
  if (error != NO_ERROR)
    {
      return error;
    }
...
scan_print_stats_json (SCAN_ID * scan_id, json_t * stats)
{
  ...
  scan = json_pack ("{s:i, s:I, s:I}",
                    ...
                    "fetch", scan_id->stats.num_fetches, "ioread", scan_id->stats.num_ioreads);
  ...
  if (scan_id->type == S_HEAP_SCAN || scan_id->type == S_LIST_SCAN)
  ...
  else if (scan_id->type == S_INDX_SCAN)
    {
      fprintf (fp, ", readkeys: %d, filteredkeys: %d, rows: %d",
               scan_id->stats.read_keys, scan_id->stats.qualified_keys, scan_id->stats.key_qualified_rows);
      ...
      if (scan_id->stats.covered_index == false)
        {
          fprintf (fp, " (lookup time: %d, rows: %d)",
                   TO_MSEC (scan_id->stats.elapsed_lookup), scan_id->stats.data_qualified_rows);
        }
    }
  else
    {
