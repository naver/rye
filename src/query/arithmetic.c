...
static int qdata_add_int_to_dbval (DB_VALUE * int_val_p, DB_VALUE * dbval_p, DB_VALUE * result_p);
...
static int qdata_add_sequence_to_dbval (DB_VALUE * seq_val_p,
                                        DB_VALUE * dbval_p, DB_VALUE * result_p, TP_DOMAIN * domain_p);
#endif
...
db_floor_dbval (DB_VALUE * result, DB_VALUE * value)
{
  ...
  switch (res_type)
    ...
    case DB_TYPE_NUMERIC:
      {
        ...
        if (s)
        ...
        else
          {
            ...
            er_status = DB_MAKE_NUMERIC (result, DB_PULL_NUMERIC (value), p, 0);
          }
    ...
    default:
      ...
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, er_status, 2, pr_type_name (res_type), pr_type_name (DB_TYPE_DOUBLE));
      break;
    }
        ...
        if (s)
          {
            ...
            if (increment)
            ...
            else
              {
                ...
                er_status = DB_MAKE_NUMERIC (result, db_locate_numeric (value), p, s);
              }
          }
        else
          {
            /* the given numeric value has a scale of 0 */
            er_status = DB_MAKE_NUMERIC (result, db_locate_numeric (value), p, 0);
          }
...
db_sign_dbval (DB_VALUE * result, DB_VALUE * value)
{
  ...
  if (!TP_IS_NUMERIC_TYPE (res_type))
    {
      value = db_value_cast_arg (value, &tmp_value, DB_TYPE_DOUBLE, &error_status);
    }
    ...
    case DB_TYPE_NUMERIC:
      {
        ...
        er_status = DB_MAKE_NUMERIC (result, num, DB_VALUE_PRECISION (value), DB_VALUE_SCALE (value));
        break;
      }
    default:
      er_status = ER_TP_CANT_COERCE;
...
db_exp_dbval (DB_VALUE * result, DB_VALUE * value)
{
  ...
  switch (type)
    ...
    default:
      assert (false);
      error_status = ER_TP_CANT_COERCE;
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, error_status, 2, pr_type_name (type), pr_type_name (DB_TYPE_DOUBLE));
      goto exit_on_error;
      break;
    }
...
db_mod_int (DB_VALUE * result, DB_VALUE * value1, DB_VALUE * value2)
{
  ...
  switch (type2)
    ...
    default:
      er_status = ER_TP_CANT_COERCE;
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, er_status, 2, pr_type_name (type2), pr_type_name (DB_TYPE_DOUBLE));
      goto exit;
    }
      ...
      if (d2 == 0)
      ...
      else
        {
          ...
          (void) numeric_internal_double_to_num (dtmp,
                                                 MAX (DB_VALUE_SCALE (value1), DB_VALUE_SCALE (value2)), num, &p, &s);
          er_status = DB_MAKE_NUMERIC (result, num, p, s);
        }
      break;
    default:
  ...
  if (!FINITE (num_scale_up))
    {
      ...
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, error_status, 1, tp_Double_domain.type->name);
    }
  else
    {
      if (num_scale_up > 0)
...
db_round_dbval (DB_VALUE * result, DB_VALUE * value1, DB_VALUE * value2)
{
      ...
      if (tp_value_coerce (value1, &cast_value, domain) != DOMAIN_COMPATIBLE)
        {
          er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE,
                  ER_TP_CANT_COERCE, 2, pr_type_name (DB_VALUE_DOMAIN_TYPE (value1)), pr_type_name (DB_TYPE_DOUBLE));
          return ER_TP_CANT_COERCE;
        }
  ...
  if (type2 != DB_TYPE_INTEGER)
    {
      ...
      if (tp_value_coerce (value2, &cast_format, domain) != DOMAIN_COMPATIBLE)
        {
          er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE,
                  ER_TP_CANT_COERCE, 2, pr_type_name (DB_VALUE_DOMAIN_TYPE (value2)), pr_type_name (DB_TYPE_INTEGER));
          return ER_TP_CANT_COERCE;
        }
  ...
  switch (type1)
    ...
    default:
      ...
      er_status = ER_TP_CANT_COERCE;
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, er_status, 2, pr_type_name (type1), pr_type_name (DB_TYPE_DOUBLE));
      break;
    }
...
db_log_dbval (DB_VALUE * result, DB_VALUE * value1, DB_VALUE * value2)
{
  ...
  if (!TP_IS_NUMERIC_TYPE (type1))
    {
      value1 = db_value_cast_arg (value1, &tmp_value1, DB_TYPE_DOUBLE, &error_status);
    }

  type2 = DB_VALUE_DOMAIN_TYPE (value2);
  if (!TP_IS_NUMERIC_TYPE (type2))
    {
      value2 = db_value_cast_arg (value2, &tmp_value2, DB_TYPE_DOUBLE, &error_status);
    }
  ...
  switch (type1)
    ...
    default:
      assert (false);
      error_status = ER_TP_CANT_COERCE;
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, error_status, 2, pr_type_name (type1), pr_type_name (DB_TYPE_DOUBLE));
      goto exit_on_error;
      break;
    }
...
db_trunc_dbval (DB_VALUE * result, DB_VALUE * value1, DB_VALUE * value2)
{
  ...
  type2 = DB_VALUE_DOMAIN_TYPE (value2);
  if (type2 != DB_TYPE_INTEGER
      && type2 != DB_TYPE_BIGINT && type2 != DB_TYPE_NUMERIC && type2 != DB_TYPE_DOUBLE && !QSTR_IS_CHAR (type2))
    {
      er_status = ER_TP_CANT_COERCE;
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, er_status, 2, pr_type_name (type2), pr_type_name (DB_TYPE_DOUBLE));
  ...
  if (pr_is_string_type (type2) && strcasecmp (DB_PULL_STRING (value2), "default") == 0)
    {
...
db_bit_count_dbval (DB_VALUE * result, DB_VALUE * value)
{
  ...
  switch (type)
    ...
    case DB_TYPE_NUMERIC:
      numeric_coerce_num_to_double ((DB_C_NUMERIC) db_locate_numeric (value), DB_VALUE_SCALE (value), &d);
      if (d < 0)
        {
          bi = (DB_BIGINT) (d - 0.5f);
        }
...
db_typeof_dbval (DB_VALUE * result, DB_VALUE * value)
{
  ...
  switch (type)
    ...
    case DB_TYPE_NUMERIC:
      ...
      if (buf == NULL)
        {
          er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_OUT_OF_VIRTUAL_MEMORY, 1, 128);
      ...
      if (type == DB_TYPE_NUMERIC)
        {
          snprintf (buf, 128, "%s (%u, %u)", type_name,
                    value->domain.numeric_info.precision, value->domain.numeric_info.scale);
        }
      else
        {
      ...
      if (DB_GET_INTEGER (&cmp_result) < 0)
      ...
      else
        {
          ...
          if (DB_GET_INTEGER (&cmp_result) < 1)
            {
              numeric_coerce_num_to_double (DB_GET_NUMERIC (value4), DB_VALUE_SCALE (value4), &res);
              res += 1.0;
            }
          else
            {
...
#define RETURN_ERROR_WITH_TWO_ARGS(err, arg1, arg2) \
  ...
  if (DB_VALUE_TYPE (value4) == DB_TYPE_DOUBLE)
  ...
  else
    {
      ...
      if (tp_value_coerce (value4, &tmpval, &tp_Double_domain) != DOMAIN_COMPATIBLE)
        {
          er_status = ER_TP_CANT_COERCE;
          RETURN_ERROR_WITH_TWO_ARGS (er_status, pr_type_name (DB_VALUE_TYPE (value4)), pr_type_name (DB_TYPE_DOUBLE));
        }
          ...
          if (cast_status == DOMAIN_COMPATIBLE)
          ...
          else
            {
              ...
              if (cast_status == DOMAIN_COMPATIBLE)
                ...
                {
                  RETURN_ERROR_WITH_TWO_ARGS (ER_TP_CANT_COERCE, pr_type_name (type), pr_type_name (DB_TYPE_TIME));
                }
            }
        }
      ...
      if (cast_type != DB_VALUE_DOMAIN_TYPE (value2))
        {
          ...
          if (cast_status != DOMAIN_COMPATIBLE)
            {
              RETURN_ERROR_WITH_TWO_ARGS (ER_TP_CANT_COERCE, pr_type_name (type), pr_type_name (cast_type));
            }
  ...
  switch (type)
      ...
      if (DB_VALUE_DOMAIN_TYPE (value2) != DB_TYPE_DATE)
        {
          RETURN_ERROR_WITH_TWO_ARGS (ER_TP_CANT_COERCE,
                                      pr_type_name (DB_VALUE_DOMAIN_TYPE (value2)), pr_type_name (DB_TYPE_DATE));
        }
      if (DB_VALUE_DOMAIN_TYPE (value3) != DB_TYPE_DATE)
        {
          RETURN_ERROR_WITH_TWO_ARGS (ER_TP_CANT_COERCE,
                                      pr_type_name (DB_VALUE_DOMAIN_TYPE (value3)), pr_type_name (DB_TYPE_DATE));
        }
#endif
    ...
    case DB_TYPE_DATETIME:
      ...
      if (DB_VALUE_DOMAIN_TYPE (value2) != DB_TYPE_DATETIME)
        {
          RETURN_ERROR_WITH_TWO_ARGS (ER_TP_CANT_COERCE,
                                      pr_type_name (DB_VALUE_DOMAIN_TYPE (value2)), pr_type_name (DB_TYPE_DATETIME));
        }
      if (DB_VALUE_DOMAIN_TYPE (value3) != DB_TYPE_DATETIME)
        {
          RETURN_ERROR_WITH_TWO_ARGS (ER_TP_CANT_COERCE,
                                      pr_type_name (DB_VALUE_DOMAIN_TYPE (value3)), pr_type_name (DB_TYPE_DATETIME));
        }
#endif
    ...
    case DB_TYPE_TIME:
      ...
      if (DB_VALUE_DOMAIN_TYPE (value2) != DB_TYPE_TIME)
        {
          RETURN_ERROR_WITH_TWO_ARGS (ER_TP_CANT_COERCE,
                                      pr_type_name (DB_VALUE_DOMAIN_TYPE (value2)), pr_type_name (DB_TYPE_TIME));
        }
      if (DB_VALUE_DOMAIN_TYPE (value3) != DB_TYPE_TIME)
        {
          RETURN_ERROR_WITH_TWO_ARGS (ER_TP_CANT_COERCE,
                                      pr_type_name (DB_VALUE_DOMAIN_TYPE (value3)), pr_type_name (DB_TYPE_TIME));
        }
#endif
    ...
    case DB_TYPE_DOUBLE:
      if (get_number_dbval_as_double (&d1, value1) != NO_ERROR)
        {
          RETURN_ERROR_WITH_TWO_ARGS (ER_TP_CANT_COERCE,
                                      pr_type_name (DB_VALUE_DOMAIN_TYPE (value1)), pr_type_name (DB_TYPE_DOUBLE));
        }
      if (get_number_dbval_as_double (&d2, value2) != NO_ERROR)
        {
          RETURN_ERROR_WITH_TWO_ARGS (ER_TP_CANT_COERCE,
                                      pr_type_name (DB_VALUE_DOMAIN_TYPE (value2)), pr_type_name (DB_TYPE_DOUBLE));
        }
      if (get_number_dbval_as_double (&d3, value3) != NO_ERROR)
        {
          RETURN_ERROR_WITH_TWO_ARGS (ER_TP_CANT_COERCE,
                                      pr_type_name (DB_VALUE_DOMAIN_TYPE (value3)), pr_type_name (DB_TYPE_DOUBLE));
        }
      break;
    ...
    case DB_TYPE_NUMERIC:
       ...
       */
      cast_domain =
        tp_domain_construct (DB_TYPE_NUMERIC, NULL, DB_MAX_NUMERIC_PRECISION, DB_MAX_NUMERIC_PRECISION / 2, NULL);
      if (cast_domain == NULL)
        {
          ...
          if (cast_status != DOMAIN_COMPATIBLE)
            {
              RETURN_ERROR_WITH_TWO_ARGS (ER_TP_CANT_COERCE_OVERFLOW,
                                          pr_type_name (DB_VALUE_TYPE
                                                        (value1)), pr_type_name (TP_DOMAIN_TYPE (cast_domain)));
            }
          ...
          if (cast_status != DOMAIN_COMPATIBLE)
            {
              RETURN_ERROR_WITH_TWO_ARGS (ER_TP_CANT_COERCE_OVERFLOW,
                                          pr_type_name (DB_VALUE_TYPE
                                                        (value2)), pr_type_name (TP_DOMAIN_TYPE (cast_domain)));
            }
          ...
          if (cast_status != DOMAIN_COMPATIBLE)
            {
              RETURN_ERROR_WITH_TWO_ARGS (ER_TP_CANT_COERCE_OVERFLOW,
                                          pr_type_name (DB_VALUE_TYPE
                                                        (value3)), pr_type_name (TP_DOMAIN_TYPE (cast_domain)));
            }
      ...
      cast_domain = tp_domain_construct (DB_TYPE_NUMERIC, NULL, DB_INTEGER_PRECISION, 0, NULL);
      if (cast_domain == NULL)
        {
          RETURN_ERROR (er_errid ());
        }
 ...
 */
int
qdata_add_dbval (DB_VALUE * dbval1_p, DB_VALUE * dbval2_p, DB_VALUE * result_p)
{
  int error_status = NO_ERROR;
  ...
  if (TP_IS_SET_TYPE (type1))
    ...
    {
      dbval1_p = db_value_cast_arg (dbval1_p, &tmp_dbval1, DB_TYPE_DOUBLE, &error_status);
    }
  ...
  if (TP_IS_SET_TYPE (type2))
    ...
    {
      dbval2_p = db_value_cast_arg (dbval2_p, &tmp_dbval2, DB_TYPE_DOUBLE, &error_status);
    }
  ...
  switch (type1)
    ...
    case DB_TYPE_NUMERIC:
      error_status = qdata_add_numeric_to_dbval (dbval1_p, dbval2_p, result_p);
      break;
    ...
    case DB_TYPE_SEQUENCE:
      ...
      if (!TP_IS_SET_TYPE (type2))
        {
          error_status = ER_TP_CANT_COERCE;
          er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, error_status, 2, pr_type_name (type2), pr_type_name (type1));
          goto exit_on_error;
        }
      ...
      error_status = qdata_add_sequence_to_dbval (dbval1_p, dbval2_p, result_p, domain_p);
#else
      error_status = ER_TP_CANT_COERCE;
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, error_status, 2, pr_type_name (type1), pr_type_name (DB_TYPE_DOUBLE));
#endif
...
exit_on_error:
  ...
  if (er_errid () == NO_ERROR)
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_TP_CANT_COERCE, 2, pr_type_name (type1), pr_type_name (type2));
    }
...
static int
qdata_add_int_to_dbval (DB_VALUE * int_val_p, DB_VALUE * dbval_p, DB_VALUE * result_p)
{
  int i;
  DB_TYPE type;
...
static int
qdata_add_bigint_to_dbval (DB_VALUE * bigint_val_p, DB_VALUE * dbval_p, DB_VALUE * result_p)
{
  DB_BIGINT bi;
  DB_TYPE type;
  ...
  switch (type)
    ...
    case DB_TYPE_NUMERIC:
      if (numeric_db_value_add (numeric_val_p, dbval_p, result_p) != NO_ERROR)
        {
          er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_QPROC_OVERFLOW_HAPPENED, 0);
          return ER_QPROC_OVERFLOW_HAPPENED;
        }
      break;
...
qdata_coerce_dbval_to_numeric (DB_VALUE * dbval_p, DB_VALUE * result_p)
{
  ...
  if (type == DB_TYPE_NUMERIC)
    {
      db_value_domain_init (result_p, DB_TYPE_NUMERIC, DB_VALUE_PRECISION (dbval_p), DB_VALUE_SCALE (dbval_p));
    }
  else
    {
...
qdata_add_numeric (DB_VALUE * numeric_val_p, DB_VALUE * dbval_p, DB_VALUE * result_p)
{
  ...
  if (qdata_coerce_dbval_to_numeric (dbval_p, &dbval_tmp) != NO_ERROR
      || numeric_db_value_add (&dbval_tmp, numeric_val_p, result_p) != NO_ERROR)
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_QPROC_OVERFLOW_HAPPENED, 0);
      return ER_QPROC_OVERFLOW_HAPPENED;
    }
 ...
 */
int
qdata_subtract_dbval (DB_VALUE * dbval1_p, DB_VALUE * dbval2_p, DB_VALUE * result_p)
{
  int error_status = NO_ERROR;
  DB_VALUE tmp_dbval1, tmp_dbval2;
  DB_TYPE type1, type2;
  ...
  switch (type1)
    {
    case DB_TYPE_BIGINT:
      error_status = qdata_subtract_bigint_to_dbval (dbval1_p, dbval2_p, result_p);
      break;

    case DB_TYPE_INTEGER:
      error_status = qdata_subtract_int_to_dbval (dbval1_p, dbval2_p, result_p);
      break;

    case DB_TYPE_DOUBLE:
      error_status = qdata_subtract_double_to_dbval (dbval1_p, dbval2_p, result_p);
      break;

    case DB_TYPE_NUMERIC:
      error_status = qdata_subtract_numeric_to_dbval (dbval1_p, dbval2_p, result_p);
      break;
    ...
    case DB_TYPE_SEQUENCE:
      ...
      error_status = qdata_subtract_sequence_to_dbval (dbval1_p, dbval2_p, result_p, domain_p);
#else
      error_status = ER_TP_CANT_COERCE;
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, error_status, 2, pr_type_name (type1), pr_type_name (DB_TYPE_DOUBLE));
#endif
...
qdata_subtract_numeric_to_dbval (DB_VALUE * numeric_val_p, DB_VALUE * dbval_p, DB_VALUE * result_p)
{
  ...
  switch (type)
    ...
    case DB_TYPE_BIGINT:
      if (qdata_coerce_dbval_to_numeric (dbval_p, &dbval_tmp) != NO_ERROR
          || numeric_db_value_sub (numeric_val_p, &dbval_tmp, result_p) != NO_ERROR)
        {
          er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_QPROC_OVERFLOW_SUBTRACTION, 0);
          return ER_FAILED;
        }
 ...
 */
int
qdata_multiply_dbval (DB_VALUE * dbval1_p, DB_VALUE * dbval2_p, DB_VALUE * result_p)
{
  int error_status = NO_ERROR;
  DB_VALUE tmp_dbval1, tmp_dbval2;
  DB_TYPE type1, type2;
  ...
  switch (type1)
    {
    case DB_TYPE_INTEGER:
      error_status = qdata_multiply_int_to_dbval (dbval1_p, dbval2_p, result_p);
      break;

    case DB_TYPE_BIGINT:
      error_status = qdata_multiply_bigint_to_dbval (dbval1_p, dbval2_p, result_p);
      break;

    case DB_TYPE_DOUBLE:
      error_status = qdata_multiply_double_to_dbval (dbval1_p, dbval2_p, result_p);
      break;

    case DB_TYPE_NUMERIC:
      error_status = qdata_multiply_numeric_to_dbval (dbval1_p, dbval2_p, result_p);
      break;
...
qdata_multiply_numeric (DB_VALUE * numeric_val_p, DB_VALUE * dbval, DB_VALUE * result_p)
{
  ...
  if (qdata_coerce_dbval_to_numeric (dbval, &dbval_tmp) != NO_ERROR
      || numeric_db_value_mul (numeric_val_p, &dbval_tmp, result_p) != NO_ERROR)
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_QPROC_OVERFLOW_MULTIPLICATION, 0);
      return ER_FAILED;
    }
 ...
 */
int
qdata_divide_dbval (DB_VALUE * dbval1_p, DB_VALUE * dbval2_p, DB_VALUE * result_p)
{
  int error_status = NO_ERROR;
  DB_VALUE tmp_dbval1, tmp_dbval2;
  DB_TYPE type1, type2;
  ...
  switch (type1)
    ...
    case DB_TYPE_BIGINT:
      error_status = qdata_divide_bigint_to_dbval (dbval1_p, dbval2_p, result_p);
      break;

    case DB_TYPE_DOUBLE:
      error_status = qdata_divide_double_to_dbval (dbval1_p, dbval2_p, result_p);
      break;

    case DB_TYPE_NUMERIC:
      error_status = qdata_divide_numeric_to_dbval (dbval1_p, dbval2_p, result_p);
      break;
...
qdata_divide_numeric_to_dbval (DB_VALUE * numeric_val_p, DB_VALUE * dbval_p, DB_VALUE * result_p)
{
  ...
  switch (type2)
    ...
    case DB_TYPE_BIGINT:
      if (qdata_coerce_dbval_to_numeric (dbval_p, &dbval_tmp) != NO_ERROR
          || numeric_db_value_div (numeric_val_p, &dbval_tmp, result_p) != NO_ERROR)
        {
          er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_QPROC_OVERFLOW_DIVISION, 0);
          return ER_FAILED;
        }
    ...
    case DB_TYPE_DOUBLE:
      return
        qdata_divide_double (qdata_coerce_numeric_to_double (numeric_val_p), DB_GET_DOUBLE (dbval_p), result_p, true);
...
qdata_bit_not_dbval (DB_VALUE * dbval_p, DB_VALUE * result_p)
{
  ...
  if (!(type == DB_TYPE_INTEGER || type == DB_TYPE_BIGINT))
    {
      dbval_p = db_value_cast_arg (dbval_p, &tmp_dbval, DB_TYPE_BIGINT, &error_status);
    }
  ...
  switch (type)
    ...
    default:
      assert (false);
      error_status = ER_TP_CANT_COERCE;
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, error_status, 2, pr_type_name (type), pr_type_name (DB_TYPE_BIGINT));
      goto exit_on_error;
      break;
    }
 ...
 */
int
qdata_bit_and_dbval (DB_VALUE * dbval1_p, DB_VALUE * dbval2_p, DB_VALUE * result_p)
{
  int error_status = NO_ERROR;
  DB_VALUE tmp_dbval1, tmp_dbval2;
  ...
  if (!(dbval_type == DB_TYPE_INTEGER || dbval_type == DB_TYPE_BIGINT))
    {
      dbval1_p = db_value_cast_arg (dbval1_p, &tmp_dbval1, DB_TYPE_BIGINT, &error_status);
    }

  dbval_type = DB_VALUE_DOMAIN_TYPE (dbval2_p);
  if (!(dbval_type == DB_TYPE_INTEGER || dbval_type == DB_TYPE_BIGINT))
    {
      dbval2_p = db_value_cast_arg (dbval2_p, &tmp_dbval2, DB_TYPE_BIGINT, &error_status);
    }
 ...
 */
int
qdata_bit_or_dbval (DB_VALUE * dbval1_p, DB_VALUE * dbval2_p, DB_VALUE * result_p)
{
  int error_status = NO_ERROR;
  DB_VALUE tmp_dbval1, tmp_dbval2;
  DB_TYPE dbval_type, type[2];
 ...
 */
int
qdata_bit_xor_dbval (DB_VALUE * dbval1_p, DB_VALUE * dbval2_p, DB_VALUE * result_p)
{
  int error_status = NO_ERROR;
  DB_VALUE tmp_dbval1, tmp_dbval2;
  DB_TYPE dbval_type, type[2];
...
qdata_unary_minus_dbval (DB_VALUE * result_p, DB_VALUE * dbval_p)
{
  ...
  if (!TP_IS_NUMERIC_TYPE (res_type))
    {
      dbval_p = db_value_cast_arg (dbval_p, &tmp_dbval, DB_TYPE_DOUBLE, &error_status);
    }
  ...
  switch (res_type)
    ...
    case DB_TYPE_VARCHAR:
      error_status = tp_value_str_cast_to_number (dbval_p, &cast_value, &res_type);
      if (error_status != NO_ERROR)
        {
          goto exit_on_error;
        }
    ...
    case DB_TYPE_NUMERIC:
      error_status = DB_MAKE_NUMERIC (result_p,
                                      DB_GET_NUMERIC (dbval_p), DB_VALUE_PRECISION (dbval_p), DB_VALUE_SCALE (dbval_p));
      if (error_status != NO_ERROR)
        {
          goto exit_on_error;
        }
 ...
 */
int
qdata_extract_dbval (const MISC_OPERAND extr_operand, DB_VALUE * dbval_p, DB_VALUE * result_p)
{
  DB_TYPE dbval_type;
  DB_DATE date;
  DB_TIME time;
  ...
  switch (dbval_type)
    ...
    case DB_TYPE_DATETIME:
      ...
      db_datetime_decode (datetime, &extvar[MONTH], &extvar[DAY],
                          &extvar[YEAR], &extvar[HOUR], &extvar[MINUTE], &extvar[SECOND], &extvar[MILLISECOND]);
            ...
            if (db_string_to_datetime_ex (str_date, str_date_len, &datetime_s) == NO_ERROR)
              {
                db_datetime_decode (&datetime_s, &extvar[MONTH],
                                    ...
                                    &extvar[HOUR], &extvar[MINUTE], &extvar[SECOND], &extvar[MILLISECOND]);
                break;
              }
            /* no date/time can be extracted from string, error */
            er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_TP_CANT_COERCE, 2,
                    pr_type_name (DB_TYPE_VARCHAR), pr_type_name (DB_TYPE_DATETIME));
            return ER_FAILED;
 ...
 */
int
qdata_strcat_dbval (DB_VALUE * dbval1_p, DB_VALUE * dbval2_p, DB_VALUE * result_p)
{
  int error_status = NO_ERROR;
  DB_VALUE tmp_dbval1, tmp_dbval2;
