...
static int date_to_char (const DB_VALUE * src_value,
                         const DB_VALUE * format_str, const DB_VALUE * date_lang, DB_VALUE * result_str);
static int number_to_char (const DB_VALUE * src_value,
                           const DB_VALUE * format_str, const DB_VALUE * number_lang, DB_VALUE * result_str);
...
static int get_next_format (char *sp, DB_TYPE str_type, int *format_length, char **next_pos);
...
static int add_and_normalize_date_time (int *years,
                                        ...
                                        int *milliseconds,
                                        DB_BIGINT y,
                                        DB_BIGINT m, DB_BIGINT d, DB_BIGINT h, DB_BIGINT mi, DB_BIGINT s, DB_BIGINT ms);
...
static int db_check_or_create_null_term_string (const DB_VALUE * str_val,
                                                ...
                                                bool ignore_trail_spaces, char **str_out, bool * do_alloc);
...
static int get_string_date_token_id (const STRING_DATE_TOKEN token_type,
                                     const INTL_LANG intl_lang_id, const char *cs, int *token_id, int *token_size);
...
db_time_format (const DB_VALUE * time_value, const DB_VALUE * format, const DB_VALUE * date_lang, DB_VALUE * result)
{
  ...
  if (!(res_type == DB_TYPE_DATETIME || res_type == DB_TYPE_TIME))
    {
      time_value = db_value_cast_arg (time_value, &tmp_time_value, DB_TYPE_TIME, &error_status);
    }
  format = db_value_cast_arg (format, &tmp_format, DB_TYPE_VARCHAR, &error_status);
  date_lang = db_value_cast_arg (date_lang, &tmp_date_lang, DB_TYPE_INTEGER, &error_status);
  ...
  if (lld == NULL)
    {
      ...
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, error_status, 2,
              lang_get_lang_name_from_id (date_lang_id), lang_charset_name (INTL_CODESET_UTF8));
      goto exit_on_error;
    }
  ...
  /* %p       AM or PM */
  STRNCPY (format_specifiers['p'], (h > 11) ? lld->am_pm[PM_NAME] : lld->am_pm[AM_NAME], 64);

  /* %r       Time, 12-hour (hh:mm:ss followed by AM or PM) */
  sprintf (format_specifiers['r'], "%02d:%02d:%02d %s",
           (h % 12 == 0) ? 12 : (h % 12), mi, s, (h > 11) ? lld->am_pm[PM_NAME] : lld->am_pm[AM_NAME]);
...
db_to_char (const DB_VALUE * src_value,
            const DB_VALUE * format_or_length, const DB_VALUE * lang_str, DB_VALUE * result_str)
{
  int error_status = NO_ERROR;
  DB_TYPE type;
  ...
  if (type == DB_TYPE_NULL || is_number (src_value))
    {
      return number_to_char (src_value, format_or_length, lang_str, result_str);
    }
...
db_to_date (const DB_VALUE * src_str, const DB_VALUE * format_str, const DB_VALUE * date_lang, DB_VALUE * result_date)
{
  ...
  /* arg cast type **********************************************************
   */
  src_str = db_value_cast_arg (src_str, &tmp_src_str, DB_TYPE_VARCHAR, &error_status);
  date_lang = db_value_cast_arg (date_lang, &tmp_date_lang, DB_TYPE_INTEGER, &error_status);
  ...
  error_status =
    db_check_or_create_null_term_string (src_str, stack_buf_str,
                                         sizeof (stack_buf_str), true, true, &initial_buf_str, &do_free_buf_str);
  ...
  if (no_user_format)
    {
      ...
      default_format_str = lang_date_format_parse (date_lang_id, DB_TYPE_DATE);
      ...
      DB_MAKE_VARCHAR (&default_format, strlen (default_format_str),
                       (char *) default_format_str, strlen (default_format_str), LANG_GET_DEFAULT_COLLATION);
  ...
  /* arg cast type **********************************************************
   */
  format_str = db_value_cast_arg (format_str, &tmp_format_str, DB_TYPE_VARCHAR, &error_status);
  ...
  error_status =
    db_check_or_create_null_term_string (format_str, stack_buf_format,
                                         ...
                                         true, true, &initial_buf_format, &do_free_buf_format);
  if (error_status != NO_ERROR)
    {
      goto exit_on_error;
    }
  ...
  while (cs < last_src)
    {
      ...
      cur_format = get_next_format (cur_format_str_ptr, DB_TYPE_DATE, &cur_format_size, &next_format_str_ptr);
      switch (cur_format)
        {
        case DT_YYYY:
          if (yearcount != 0)
        ...
        case DT_MONTH:
          ...
          error_status = get_string_date_token_id (SDT_MONTH, date_lang_id, cs, &month, &token_size);
          if (error_status != NO_ERROR)
            {
              goto exit_on_error;
            }
        ...
        case DT_MON:
          ...
          error_status = get_string_date_token_id (SDT_MONTH_SHORT, date_lang_id, cs, &month, &token_size);
          if (error_status != NO_ERROR)
            {
              goto exit_on_error;
            }
        ...
        case DT_DAY:
          ...
          error_status = get_string_date_token_id (SDT_DAY, date_lang_id, cs, &day_of_the_week, &token_size);
          if (error_status != NO_ERROR)
            {
              goto exit_on_error;
            }
        ...
        case DT_DY:
          ...
          error_status = get_string_date_token_id (SDT_DAY_SHORT, date_lang_id, cs, &day_of_the_week, &token_size);
          if (error_status != NO_ERROR)
            {
              goto exit_on_error;
            }
 ...
 */
int
db_to_time (const DB_VALUE * src_str, const DB_VALUE * format_str, const DB_VALUE * date_lang, DB_VALUE * result_time)
{
  int error_status = NO_ERROR;
  DB_VALUE tmp_src_str, tmp_format_str, tmp_date_lang;
  char *cur_format_str_ptr, *next_format_str_ptr;
  ...
  if (no_user_format)
    {
      ...
      /* error parsing Rye default format, try the locale format, if any */
      default_format_str = lang_date_format_parse (date_lang_id, DB_TYPE_TIME);
      if (default_format_str == NULL)
        {
  ...
  while (cs < last_src)
    {
      ...
      cur_format = get_next_format (cur_format_str_ptr, DB_TYPE_TIME, &cur_format_size, &next_format_str_ptr);
      switch (cur_format)
        {
        case DT_AM:
        case DT_A_M:
        ...
        case DT_P_M:
          ...
          error_status = get_string_date_token_id (SDT_AM_PM, date_lang_id, cs, &am_pm_id, &token_size);
          if (error_status != NO_ERROR)
            {
              goto exit_on_error;
            }
  ...
  if (no_user_format)
    {
      ...
      default_format_str = lang_date_format_parse (date_lang_id, DB_TYPE_DATETIME);
      if (default_format_str == NULL)
        {
  ...
  while (cs < last_src)
    {
      ...
      cur_format = get_next_format (cur_format_str_ptr, DB_TYPE_DATETIME, &cur_format_size, &next_format_str_ptr);
      switch (cur_format)
        {
        case DT_YYYY:
          if (yearcount != 0)
              ...
              const DB_VALUE * number_lang, DB_VALUE * result_num)
{
   ...
   */
  src_str = db_value_cast_arg (src_str, &tmp_src_str, DB_TYPE_VARCHAR, &error_status);
  number_lang = db_value_cast_arg (number_lang, &tmp_number_lang, DB_TYPE_INTEGER, &error_status);
  ...
  error_status =
    db_check_or_create_null_term_string (src_str, stack_buf_str,
                                         sizeof (stack_buf_str), true, false, &initial_buf_str, &do_free_buf_str);
  if (error_status != NO_ERROR)
    {
      goto exit_on_error;
    }
  ...
  if (!has_user_format)
  ...
  else                          /* format_str != NULL */
    {
      ...
      error_status =
        db_check_or_create_null_term_string (format_str, stack_buf_format,
                                             sizeof (stack_buf_format),
                                             true, false, &initial_buf_format, &do_free_buf_format);
      if (error_status != NO_ERROR)
        {
          goto exit_on_error;
        }
  ...
  while (cs < last_cs)
    {
      cur_format = get_number_token (number_lang_id, format_str_ptr, &token_length, last_format, &next_fsp);
      switch (cur_format)
        {
        case N_FORMAT:
          if (count_format != 0)
          ...
          error_status = make_number (cs, last_cs,
                                      format_str_ptr, &token_length, result_num, precision, scale, number_lang_id);
          if (error_status == NO_ERROR)
            {
              count_format++;
              cs += token_length;
              ...
              const DB_VALUE * format_str, const DB_VALUE * date_lang, DB_VALUE * result_str)
{
  ...
  if (src_type != DB_TYPE_DATE && src_type != DB_TYPE_TIME && src_type != DB_TYPE_DATETIME)
    {
      error_status = ER_TP_CANT_COERCE;
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, error_status, 2,
  ...
  if (no_user_format)
    {
      ...
      DB_MAKE_VARCHAR (result_str, result_len, result_buf, result_len, collation_id);
    }
  else
    {
      ...
      switch (src_type)
        ...
        case DB_TYPE_TIME:
          ...
          break;
        case DB_TYPE_DATETIME:
          db_datetime_decode (DB_GET_DATETIME (src_value), &month, &day, &year, &hour, &minute, &second, &millisecond);
          break;
        default:
          break;
        }
      ...
      while (i < result_size)
        {
          ...
          cur_format = get_next_format (cur_format_str_ptr, src_type, &cur_format_size, &next_format_str_ptr);
          switch (cur_format)
            {
            case DT_CC:
              ...
              if (cur_format == DT_MONTH)
                {
                  error_status =
                    print_string_date_token (SDT_MONTH, date_lang_id,
                                             month - 1, token_case_mode, &result_buf[i], &token_size);
                }
              ...
              if (cur_format == DT_DAY)
                {
                  error_status =
                    print_string_date_token (SDT_DAY, date_lang_id,
                                             tmp_int, token_case_mode, &result_buf[i], &token_size);
                }
            ...
            case DT_PM:
              {
                ...
                error_status =
                  print_string_date_token (SDT_AM_PM, date_lang_id, am_pm_id, 0, &result_buf[i], &am_pm_len);
                ...
                const DB_VALUE * format_str, const DB_VALUE * number_lang, DB_VALUE * result_str)
{
  ...
  if (format_str == NULL || !has_user_format)
    {
      ...
      DB_MAKE_VARCHAR (result_str, (ssize_t) strlen (cs), cs, strlen (cs), collation_id);
      result_str->need_clear = true;
      return error_status;
    }
  else
    {
      ...
      if (is_char_string (format_str))
      ...
      else
        {
          error_status = ER_TP_CANT_COERCE;
          er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, error_status, 2,
                  pr_type_name (DB_VALUE_DOMAIN_TYPE (format_str)), pr_type_name (DB_TYPE_VARCHAR));
          free_and_init (cs);
          return error_status;
        }
      ...
      while (format_str_ptr != last_format)
        {
          ...
          switch (cur_format)
            {
            case N_FORMAT:
              if (make_number_to_char (number_lang_id, cs, format_str_ptr, &token_length, &res_ptr) != NO_ERROR)
                {
                  error_status = ER_QSTR_INVALID_FORMAT;
                  er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, error_status, 0);
                  free_and_init (cs);
  ...
  DB_MAKE_VARCHAR (result_str, (ssize_t) strlen (res_string), res_string, strlen (res_string), collation_id);
  result_str->need_clear = true;
  free_and_init (cs);
 ...
 */
static int
make_number_to_char (const INTL_LANG lang, char *num_string, char *format_str, int *length, char **result_str)
{
  int flag_sign = 1;
          ...
          if (init_format[i] == fraction_symbol)
            {
              break;
                   ...
                   init_format[i] != 's' && init_format[i] != 'c' && init_format[i] != digit_grouping_symbol)
            {
              return ER_FAILED;
            }
          else
...
make_number (char *src, char *last_src, char *token,
             int *token_length, DB_VALUE * r, const int precision, const int scale, const INTL_LANG number_lang_id)
{
  int error_status = NO_ERROR;
  int state = 1;
  int i, j, k;
  ...
  *token_length = 0;
  ...
  if (strlen (token) >= 4 && strncasecmp (token, "eeee", 4) == 0 &&
      char_tolower (*src) == 'e' && (*(src + 1) == '+' || *(src + 1) == '-'))
    {
      ...
      if (error_status != NO_ERROR || numeric_coerce_string_to_num (result_str, strlen (result_str), r) != NO_ERROR)
        {
  ...
  switch (c)
    ...
    case 's':
      ...
      if ((char_tolower (fsp[*length + 1]) == 'c' ||
           char_tolower (fsp[*length + 1]) == 's') && fsp[*length + 2] == digit_grouping_symbol)
        {
          return N_INVALID;
        }
    ...
    case '0':
      while (fsp[*length] == '9' || fsp[*length] == '0' ||
             ...
             fsp[*length] == fraction_symbol || fsp[*length] == digit_grouping_symbol)
        {
          *length += 1;
        }
 ...
 */
static int
get_next_format (char *sp, DB_TYPE str_type, int *format_length, char **next_pos)
{
  /* sp : start position          */
  *format_length = 0;
...
db_format (const DB_VALUE * value, const DB_VALUE * decimals, const DB_VALUE * number_lang, DB_VALUE * result)
{
  ...
  if (!(arg2_type == DB_TYPE_INTEGER || arg2_type == DB_TYPE_BIGINT))
    {
      decimals = db_value_cast_arg (decimals, &tmp_decimals, DB_TYPE_BIGINT, &error_status);
    }
  number_lang = db_value_cast_arg (number_lang, &tmp_number_lang, DB_TYPE_INTEGER, &error_status);
    ...
    case DB_TYPE_VARCHAR:
      {
        ...
        error_status = db_string_trim (BOTH, &trim_charset, value, &trimmed_val);
        if (error_status != NO_ERROR)
          {
            goto exit_on_error;
          }
  ...
  error_status = number_to_char (num_dbval_p, &format_val, number_lang, &formatted_val);
  if (error_status == NO_ERROR)
    {
       ...
       */
      db_make_null (&trim_charset);
      error_status = db_string_trim (LEADING, &trim_charset, &formatted_val, result);
...
add_and_normalize_date_time (int *year, int *month,
                             ...
                             DB_BIGINT d, DB_BIGINT h, DB_BIGINT mi, DB_BIGINT s, DB_BIGINT ms)
{
...
db_date_add_sub_interval_days (DB_VALUE * result, const DB_VALUE * date, const DB_VALUE * db_days, bool is_add)
{
  ...
  db_days = db_value_cast_arg (db_days, &tmp_db_days, DB_TYPE_INTEGER, &error_status);
    ...
    case DB_TYPE_VARCHAR:
      {
        ...
        if (db_date_parse_datetime_parts (date_s, str_len, &db_datetime, &has_explicit_time, NULL))
          {
...
db_date_add_sub_interval_expr (DB_VALUE * result, const DB_VALUE * date,
                               const DB_VALUE * expr, const int unit, bool is_add)
{
  int sign = 0;
  ...
  switch (unit)
    ...
    case PT_SECOND_MILLISECOND:
      if (expr_s)
        {
          narg = sscanf (expr_s, "%lld%c%63s", (long long *) &seconds, &delim, millisec_s);
      ...
      if (expr_s)
        {
          narg =
            sscanf (expr_s, "%lld%c%lld%c%lld%c%63s", (long long *) &hours,
                    &delim, (long long *) &minutes, &delim, (long long *) &seconds, &delim, millisec_s);
          millisec = db_str_to_millisec (millisec_s);
      ...
      if (expr_s)
        {
          narg = sscanf (expr_s, "%lld%c%lld%c%lld", (long long *) &hours,
                         &delim, (long long *) &minutes, &delim, (long long *) &seconds);
      ...
      if (expr_s)
        {
          narg =
            sscanf (expr_s, "%lld%c%lld%c%lld%c%lld", (long long *) &days,
                    &delim, (long long *) &hours, &delim, (long long *) &minutes, &delim, (long long *) &seconds);
      ...
      if (sign ^ is_add)
        {
          ret = sub_and_normalize_date_time (&y, &m, &d, &h, &mi, &s, &ms,
                                             years, months, days, hours, minutes, seconds, millisec);
        }
      else
        {
...
db_date_format (const DB_VALUE * date_value, const DB_VALUE * format, const DB_VALUE * date_lang, DB_VALUE * result)
{
  ...
  if (!(res_type == DB_TYPE_DATETIME || res_type == DB_TYPE_DATE))
    {
      date_value = db_value_cast_arg (date_value, &tmp_date_value, DB_TYPE_DATETIME, &error_status);
    }
  format = db_value_cast_arg (format, &tmp_format, DB_TYPE_VARCHAR, &error_status);
  date_lang = db_value_cast_arg (date_lang, &tmp_date_lang, DB_TYPE_INTEGER, &error_status);
          ...
          if (j > 0 && format_s[j - 1] == '%')
            {
              ...
              switch (format_s[j])
                ...
                case 'a':
                  /* %a Abbreviated weekday name (Sun..Sat) */
                  error_status = get_string_date_token_id (SDT_DAY_SHORT, date_lang_id, sstr + i, &dow, &token_size);
                  if (error_status != NO_ERROR)
                    {
                      goto exit_on_error;
                    }
                ...
                case 'b':
                  ...
                  error_status = get_string_date_token_id (SDT_MONTH_SHORT, date_lang_id, sstr + i, &m, &token_size);
                  if (error_status != NO_ERROR)
                    {
                      goto exit_on_error;
                    }
                ...
                case 'M':
                  /* %M Month name (January..December) */
                  error_status = get_string_date_token_id (SDT_MONTH, date_lang_id, sstr + i, &m, &token_size);
                  if (error_status != NO_ERROR)
                    {
                      goto exit_on_error;
                    }
                ...
                case 'p':
                  /* %p AM or PM */
                  error_status = get_string_date_token_id (SDT_AM_PM, date_lang_id, sstr + i, &am_pm_id, &token_size);
                  if (error_status != NO_ERROR)
                    {
                      goto exit_on_error;
                    }
                ...
                case 'W':
                  /* %W Weekday name (Sunday..Saturday) */
                  error_status = get_string_date_token_id (SDT_DAY, date_lang_id, sstr + i, &dow, &token_size);
                  if (error_status != NO_ERROR)
                    {
                      goto exit_on_error;
                    }
          ...
          if (w == 0 || w == 1)
          ...
          else
            {
              ...
              if (db_add_weeks_and_days_to_date (&d, &m, &y,
                                                 dow2 >= 1
                                                 && dow2 <= 4 ? w - 2 : w - 1, dow == 0 ? 6 : dow - 1) == ER_FAILED)
                {
                  goto exit_on_error;
                }
            }
  ...
  *do_alloc = false;
  ...
  if (val_size < 0
      || ((val_size < DB_VALUE_PRECISION (str_val)
           || DB_VALUE_PRECISION (str_val) == TP_FLOATING_PRECISION_VALUE) && val_buf[val_size] == '\0'))
    {
  ...
  if (ignore_trail_spaces && val_size > 0)
    {
      ...
      while (val_buf < val_buf_end_non_space &&
             ((*val_buf_end_non_space) == ' ' ||
              (*val_buf_end_non_space) == '\t' || (*val_buf_end_non_space) == '\r' || (*val_buf_end_non_space) == '\n'))
        {
          val_buf_end_non_space--;
        }
  ...
  if (lld == NULL)
    {
      error_status = ER_LANG_CODESET_NOT_AVAILABLE;
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, error_status, 2,
              lang_get_lang_name_from_id (intl_lang_id), lang_charset_name (INTL_CODESET_UTF8));
      return error_status;
    }
  ...
  for (i = 0; i < search_size; i++)
    {
      ...
      cmp =
        intl_case_match_tok (intl_lang_id,
                             ...
                             (unsigned char *) cs, strlen (p[token_index]), cs_size, token_size);
...
print_string_date_token (const STRING_DATE_TOKEN token_type,
                         const INTL_LANG intl_lang_id, int token_id, int case_mode, char *buffer, int *token_size)
{
  const char *p;
  int error_status = NO_ERROR;
  int token_len;
  ...
  if (token_len < print_len)
    {
      (void) qstr_pad_string ((unsigned char *) buffer + *token_size, print_len - token_len);
...
db_get_date_format (const DB_VALUE * format_str, DATETIME_FORMAT * format)
{
  ...
  *format = get_next_format (fmt_str_ptr, DB_TYPE_DATETIME, &format_size, &next_fmt_str_ptr);

  if (*format == DT_INVALID || (next_fmt_str_ptr != NULL && *next_fmt_str_ptr != 0))
    {
      error_status = ER_QSTR_INVALID_FORMAT;
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, error_status, 0);
      goto exit_on_error;
