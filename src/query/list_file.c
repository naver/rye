...
static PAGE_PTR qfile_allocate_new_page (THREAD_ENTRY * thread_p,
                                         QFILE_LIST_ID * list_id_p, PAGE_PTR page_p, bool is_ovf_page);
...
static int qfile_save_single_bound_item_tuple (QFILE_TUPLE_DESCRIPTOR *
                                               tuple_descr_p, char *tuple_p, char *page_p, int tuple_length);
...
static QFILE_LIST_ID *qfile_union_list (THREAD_ENTRY * thread_p,
                                        QFILE_LIST_ID * list_id1, QFILE_LIST_ID * list_id2, int flag);
...
static int qfile_get_tuple_from_current_list (THREAD_ENTRY * thread_p,
                                              QFILE_LIST_SCAN_ID * scan_id_p, QFILE_TUPLE_RECORD * tuple_record_p);
...
static SCAN_CODE qfile_retrieve_tuple (THREAD_ENTRY * thread_p,
                                       QFILE_LIST_SCAN_ID * scan_id_p, QFILE_TUPLE_RECORD * tuple_record_p, int peek);
...
qfile_compare_tuple_values (QFILE_TUPLE tuple1, QFILE_TUPLE tuple2, TP_DOMAIN * domain_p, int *compare_result)
{
  ...
  if (length1 == 0)
  ...
  else
    {
      ...
      rc = (*(pr_type_p->data_readval)) (&buf, &dbval1, domain_p, -1, is_copy);
      if (rc != NO_ERROR)
        {
          return ER_FAILED;
        }
  ...
  else
    {
      if (length1 == 0)
      ...
      else
        {
          ...
          rc = (*(pr_type_p->data_readval)) (&buf, &dbval2, domain_p, -1, is_copy);
          if (rc != NO_ERROR)
            {
              return ER_FAILED;
            }
  ...
  if (length1 == 0 && length2 == 0)
    ...
    {
      *compare_result = (*(pr_type_p->cmpval)) (&dbval1, &dbval2, 0, 1, domain_p->collation_id);
    }
...
qfile_store_xasl (THREAD_ENTRY * thread_p, XASL_STREAM * stream)
{
  ...
  num_pages = file_get_numpages (thread_p, &xasl_id->temp_vfid, NULL, NULL, NULL);
      ...
      if (page_index >= QFILE_PAGE_EXTENDS || page_index >= n)
        {
          if (nth_page >= num_pages)
            {
              if (file_alloc_pages_as_noncontiguous (thread_p,
                                                     &xasl_id->temp_vfid,
                                                     vpid_array, &nth_page,
                                                     QFILE_PAGE_EXTENDS, NULL, NULL, NULL, NULL) == NULL)
                {
                  goto error;
                }
          ...
          n = file_find_nthpages (thread_p, &xasl_id->temp_vfid, vpid_array, nth_page, QFILE_PAGE_EXTENDS);
          if (n < 0)
            {
              goto error;
            }
...
qfile_load_xasl_node_header (THREAD_ENTRY * thread_p, const XASL_ID * xasl_id_p, XASL_NODE_HEADER * xasl_header_p)
{
  ...
  xasl_page_p =
    pgbuf_fix (thread_p, &xasl_id_p->first_vpid, OLD_PAGE, PGBUF_LATCH_READ, PGBUF_UNCONDITIONAL_LATCH, PAGE_XASL);
  if (xasl_page_p == NULL)
    {
      return;
    }
...
qfile_load_xasl (THREAD_ENTRY * thread_p, const XASL_ID * xasl_id_p, char **xasl_p, int *size_p)
{
  ...
  cur_page_p = pgbuf_fix (thread_p, &xasl_id_p->first_vpid, OLD_PAGE,
                          PGBUF_LATCH_READ, PGBUF_UNCONDITIONAL_LATCH, PAGE_XASL);
  if (cur_page_p == NULL)
    {
      return 0;
    }
...
qfile_open_list (THREAD_ENTRY * thread_p,
                 QFILE_TUPLE_VALUE_TYPE_LIST * type_list_p, SORT_LIST * sort_list_p, QUERY_ID query_id, int flag)
{
  QFILE_LIST_ID *list_id_p;
  int len, i;
  ...
  if (temp_file_p->membuf && list_id_p->last_vpid.volid == NULL_VOLID)
  ...
  else
    {
      ...
      last_page_ptr = pgbuf_fix (thread_p, &list_id_p->last_vpid, OLD_PAGE,
                                 PGBUF_LATCH_WRITE, PGBUF_UNCONDITIONAL_LATCH, PAGE_QRESULT);
...
qfile_allocate_new_ovf_page (THREAD_ENTRY * thread_p,
                             ...
                             PAGE_PTR prev_page_p, int tuple_length, int offset, int *tuple_page_size_p)
{
  PAGE_PTR new_page_p;
  VPID new_vpid;
...
qfile_allocate_new_page_if_need (THREAD_ENTRY * thread_p,
                                 QFILE_LIST_ID * list_id_p, PAGE_PTR * page_p, int tuple_length, bool is_ovf_page)
{
  PAGE_PTR new_page_p;
...
qfile_add_tuple_to_list (THREAD_ENTRY * thread_p, QFILE_LIST_ID * list_id_p, QFILE_TUPLE tuple)
{
  ...
  for (offset = tuple_page_size, tuple_p = (char *) tuple + offset;
       offset < tuple_length; offset += tuple_page_size, tuple_p += tuple_page_size)
    {
      new_page_p = qfile_allocate_new_ovf_page (thread_p, list_id_p,
                                                cur_page_p, prev_page_p, tuple_length, offset, &tuple_page_size);
      if (new_page_p == NULL)
        {
...
qfile_save_tuple (QFILE_TUPLE_DESCRIPTOR * tuple_descr_p,
                  QFILE_TUPLE_TYPE tuple_type, char *page_p, int *tuple_length_p)
{
  char *tuple_p;
...
qfile_add_overflow_tuple_to_list (THREAD_ENTRY * thread_p,
                                  QFILE_LIST_ID * list_id_p, PAGE_PTR ovf_tuple_page_p, QFILE_LIST_ID * input_list_id_p)
{
...
xqfile_get_list_file_page (THREAD_ENTRY * thread_p, QUERY_ID query_id,
                           VOLID vol_id, PAGEID page_id, char *page_buf_p, int *page_size_p)
{
...
qfile_advance_single (THREAD_ENTRY * thread_p,
                      ...
                      UNUSED_ARG QFILE_TUPLE_RECORD * last_tuple_p, UNUSED_ARG QFILE_TUPLE_VALUE_TYPE_LIST * types)
{
  if (next_scan_p == NULL)
    {
      return S_END;
...
qfile_advance_group (THREAD_ENTRY * thread_p,
                     ...
                     QFILE_TUPLE_RECORD * last_tuple_p, QFILE_TUPLE_VALUE_TYPE_LIST * types)
{
  SCAN_CODE status;
  int error_code, cmp;
 ...
 */
static int
qfile_add_one_tuple (THREAD_ENTRY * thread_p, QFILE_LIST_ID * dest_list_p, QFILE_TUPLE lhs, UNUSED_ARG QFILE_TUPLE rhs)
{
      ...
      if (!have_lhs)
        {
          if (qfile_advance (thread_p, advance_func, &lhs, &last_lhs,
                             lhs_scan_p, last_lhs_scan_p, lhs_file_p, &have_lhs) != NO_ERROR)
            {
              goto error;
            }
        }
      ...
      if (!have_rhs)
        {
          if (qfile_advance (thread_p, advance_func, &rhs, &last_rhs,
                             rhs_scan_p, last_rhs_scan_p, rhs_file_p, &have_rhs) != NO_ERROR)
            {
              goto error;
            }
        }
...
qfile_union_list (THREAD_ENTRY * thread_p, QFILE_LIST_ID * list_id1_p, QFILE_LIST_ID * list_id2_p, UNUSED_ARG int flag)
{
...
qfile_make_sort_key (THREAD_ENTRY * thread_p, SORTKEY_INFO * key_info_p,
                     RECDES * key_record_p, QFILE_LIST_SCAN_ID * input_scan_p, QFILE_TUPLE_RECORD * tuple_record_p)
{
  int i, nkeys, length;
      ...
      if (o0 && o1)
        {
          ...
          assert (TP_DOMAIN_TYPE (key_info_p->key[i].col_dom) != DB_TYPE_VARIABLE);

          order = (*key_info_p->key[i].sort_f) (d0, d1, key_info_p->key[i].col_dom, 0, 1);
                           ...
                           void *extra_arg, int limit, bool do_close)
{
  ...
  sort_result =
    sort_listfile (thread_p, NULL_VOLID, estimated_pages, get_func, &info,
                   put_func, &info, cmp_func, &info.key_info, dup_option, limit);
                 ...
                 SORT_LIST * sort_list_p, QUERY_OPTIONS option, bool do_close)
{
  ...
  return qfile_sort_list_with_func (thread_p, list_id_p, sort_list_p, option,
                                    ls_flag, NULL, NULL, NULL, NULL, NO_SORT_LIMIT, do_close);
}
...
qfile_get_tuple (THREAD_ENTRY * thread_p, PAGE_PTR first_page_p,
                 QFILE_TUPLE tuple, QFILE_TUPLE_RECORD * tuple_record_p, QFILE_LIST_ID * list_id_p)
{
  VPID ovfl_vpid;
  char *tuple_p;
  int offset;
...
qfile_scan_list (THREAD_ENTRY * thread_p, QFILE_LIST_SCAN_ID * scan_id_p,
                 SCAN_CODE (*scan_func) (THREAD_ENTRY * thread_p,
                                         QFILE_LIST_SCAN_ID *), QFILE_TUPLE_RECORD * tuple_record_p, int peek)
{
  SCAN_CODE qp_scan;
...
qfile_close_scan (THREAD_ENTRY * thread_p, QFILE_LIST_SCAN_ID * scan_id_p)
{
  ...
  if ((scan_id_p->position == S_ON
       || (scan_id_p->position == S_AFTER && scan_id_p->keep_page_on_finish)) && scan_id_p->curr_pgptr)
    {
      qmgr_free_old_page_and_init (thread_p, scan_id_p->curr_pgptr, scan_id_p->list_id.tfile_vfid);
    }
