...
static QPROC_TPLDESCR_STATUS qexec_generate_tuple_descriptor (THREAD_ENTRY *
                                                              thread_p,
                                                              QFILE_LIST_ID *
                                                              list_id, VALPTR_LIST * outptr_list, VAL_DESCR * vd);
...
static int qexec_clear_access_spec_list (XASL_NODE * xasl_p,
                                         THREAD_ENTRY * thread_p, ACCESS_SPEC_TYPE * list, int final);
...
static int qexec_orderby_distinct_by_sorting (THREAD_ENTRY * thread_p,
                                              XASL_NODE * xasl, QUERY_OPTIONS option, XASL_STATE * xasl_state);
...
static GROUPBY_STATE *qexec_initialize_groupby_state (GROUPBY_STATE * gbstate,
                                                      ...
                                                      * type_list, QFILE_TUPLE_RECORD * tplrec);
...
static int qexec_open_scan (THREAD_ENTRY * thread_p,
                            ...
                            SCAN_ID * s_id, QUERY_ID query_id, bool scan_immediately_stop);
...
static int qexec_execute_update (THREAD_ENTRY * thread_p, XASL_NODE * xasl, XASL_STATE * xasl_state);
static int qexec_execute_delete (THREAD_ENTRY * thread_p, XASL_NODE * xasl, XASL_STATE * xasl_state);
static int qexec_execute_insert (THREAD_ENTRY * thread_p, XASL_NODE * xasl, XASL_STATE * xasl_state);
...
static int qexec_end_mainblock_iterations (THREAD_ENTRY * thread_p,
                                           XASL_NODE * xasl, XASL_STATE * xasl_state, QFILE_TUPLE_RECORD * tplrec);
static void qexec_clear_mainblock_iterations (THREAD_ENTRY * thread_p, XASL_NODE * xasl);

static int qexec_check_modification (THREAD_ENTRY * thread_p, XASL_NODE * xasl, XASL_STATE * xasl_state);
...
static int qexec_remove_duplicates_for_replace (THREAD_ENTRY * thread_p,
                                                ...
                                                HEAP_CACHE_ATTRINFO *
                                                index_attr_info,
                                                const HEAP_IDX_ELEMENTS_INFO * idx_info, int *removed_count);
static int qexec_oid_of_duplicate_key_update (THREAD_ENTRY * thread_p,
                                              ...
                                              HEAP_CACHE_ATTRINFO *
                                              index_attr_info,
                                              const HEAP_IDX_ELEMENTS_INFO * idx_info, OID * unique_oid);
static int qexec_execute_duplicate_key_update (THREAD_ENTRY * thread_p,
                                               ...
                                               attr_info,
                                               HEAP_CACHE_ATTRINFO *
                                               index_attr_info, HEAP_IDX_ELEMENTS_INFO * idx_info, int *force_count);
...
static int qexec_lock_table_update_delete (THREAD_ENTRY * thread_p,
                                           XASL_NODE * aptr_list,
                                           UPDDEL_CLASS_INFO * upd_cls, bool is_shard_table, bool is_catalog_table);
static int qexec_lock_table_select_for_update (THREAD_ENTRY * thread_p,
                                               XASL_NODE * scan_list, int *num_shard_table, bool * for_update);
static int qexec_init_internal_class (THREAD_ENTRY * thread_p, UPDDEL_CLASS_INFO_INTERNAL * class);
static void qexec_clear_internal_class (THREAD_ENTRY * thread_p, UPDDEL_CLASS_INFO_INTERNAL * class);
static int qexec_upddel_setup_current_class (THREAD_ENTRY * thread_p,
                                             UPDDEL_CLASS_INFO * class_, UPDDEL_CLASS_INFO_INTERNAL * class_info);
...
static TOPN_STATUS qexec_add_tuple_to_topn (THREAD_ENTRY * thread_p,
                                            TOPN_TUPLES * sort_stop, QFILE_TUPLE_DESCRIPTOR * tpldescr);
static int qexec_topn_tuples_to_list_id (THREAD_ENTRY * thread_p,
                                         XASL_NODE * xasl, XASL_STATE * xasl_state, bool is_final);
...
static int qexec_get_orderbynum_upper_bound (THREAD_ENTRY * tread_p,
                                             PRED_EXPR * pred, VAL_DESCR * vd, DB_VALUE * ubound);
...
static int qexec_lock_table_and_shard_key (THREAD_ENTRY * thread_p,
                                           XASL_NODE * xasl,
                                           int shard_groupid, DB_VALUE * shard_key, bool req_from_migrator);
static int qexec_insert_gid_skey_info (THREAD_ENTRY * thread_p, int shard_groupid, DB_VALUE * shard_key);


static int qexec_modify_catalog_table (THREAD_ENTRY * thread_p,
                                       DB_IDXKEY * pk, void *ct_data, CATCLS_TABLE * table, CT_MODIFY_MODE modify_mode);
/* for qexec_modify_catalog_table () */
static CT_CACHE_INFO *qexec_get_info_of_catalog_table (THREAD_ENTRY * thread_p, CATCLS_TABLE * table);
static int qexec_get_attrids_from_catalog_table (THREAD_ENTRY * thread_p,
                                                 int *att_ids, OID * class_oid, CATCLS_TABLE * ct_table);
static int qexec_insert_with_values (THREAD_ENTRY * thread_p, OID * class_oid,
                                     HFID * hfid, ATTR_ID * attr_ids, int force_page_allocation, DB_IDXKEY * values);
static int qexec_update_with_values (THREAD_ENTRY * thread_p, OID * class_oid,
                                     HFID * hfid, ATTR_ID * attr_ids, OID * oid, DB_IDXKEY * values);
static int qexec_cast_ct_index_stats_to_idxkey (DB_IDXKEY * val, void *ct_table);
...
qexec_generate_tuple_descriptor (THREAD_ENTRY * thread_p,
                                 QFILE_LIST_ID * list_id, VALPTR_LIST * outptr_list, VAL_DESCR * vd)
{
  ...
  status = qdata_generate_tuple_desc_for_valptr_list (thread_p, list_id, outptr_list, vd, &(list_id->tpl_descr));
...
qexec_upddel_add_unique_oid_to_ehid (THREAD_ENTRY * thread_p, XASL_NODE * xasl, XASL_STATE * xasl_state)
{
  ...
  if (xasl == NULL || xasl->type != BUILDLIST_PROC || EHID_IS_NULL (&(xasl->proc.buildlist.upd_del_ehid)))
    {
      return NO_ERROR;
    }
  ...
  if (reg_var_list != NULL)
    {
      ret = fetch_peek_dbval (thread_p, &reg_var_list->value, &xasl_state->vd, NULL, NULL, &dbval);
      if (ret != NO_ERROR)
        {
      ...
      if (!DB_IS_NULL (dbval))
        {
          ...
          switch (eh_search)
            ...
            case EH_KEY_FOUND:
              er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_QPROC_MULTI_UPD_DEL, 0);
              /* fall through */
                         ...
                         XASL_STATE * xasl_state, QFILE_TUPLE_RECORD * tplrec)
{
  ...
  if (xasl->type == BUILDLIST_PROC)
      ...
      switch (tpldescr_status)
        ...
        case QPROC_TPLDESCR_RETRY_BIG_REC:
          ...
          if (qdata_copy_valptr_list_to_tuple (thread_p, xasl->list_id,
                                               xasl->outptr_list, &xasl_state->vd, tplrec) != NO_ERROR)
            {
              QEXEC_GOTO_EXIT_ON_ERROR;
            }
    ...
    {
      if (qdata_evaluate_aggregate_list (thread_p, xasl->proc.buildvalue.agg_list, &xasl_state->vd) != NO_ERROR)
        {
          QEXEC_GOTO_EXIT_ON_ERROR;
        }
    }
 ...
 */
static int
qexec_clear_regu_var (XASL_NODE * xasl_p, REGU_VARIABLE * regu_var, int loose_end)
{
  int pg_cnt;
  ...
  switch (regu_var->type)
    ...
    case TYPE_INARITH:
    case TYPE_OUTARITH:
      pg_cnt += qexec_clear_arith_list (xasl_p, regu_var->value.arithptr, loose_end);
      break;
    case TYPE_FUNC:
      if (loose_end == false)
        {
      ...
      pg_cnt += qexec_clear_regu_list (xasl_p, regu_var->value.funcp->operand, loose_end);
      break;
    case TYPE_DBVAL:
      if (loose_end == true)
        {
 ...
 */
static int
qexec_clear_regu_list (XASL_NODE * xasl_p, REGU_VARIABLE_LIST list, int loose_end)
{
        ...
        case T_COMP_EVAL_TERM:
          {
            ...
            pg_cnt += qexec_clear_regu_var (xasl_p, et_comp->comp_lhs, loose_end);
            pg_cnt += qexec_clear_regu_var (xasl_p, et_comp->comp_rhs, loose_end);
          }
          break;
        case T_ALSM_EVAL_TERM:
          {
            ...
            pg_cnt += qexec_clear_regu_var (xasl_p, et_alsm->elemset, loose_end);
          }
          break;
        case T_LIKE_EVAL_TERM:
          {
            ...
            pg_cnt += qexec_clear_regu_var (xasl_p, et_like->pattern, loose_end);
            pg_cnt += qexec_clear_regu_var (xasl_p, et_like->esc_char, loose_end);
          }
          break;
        case T_RLIKE_EVAL_TERM:
          {
            ...
            pg_cnt += qexec_clear_regu_var (xasl_p, et_rlike->pattern, loose_end);
            pg_cnt += qexec_clear_regu_var (xasl_p, et_rlike->case_sensitive, loose_end);
 ...
 */
static int
qexec_clear_access_spec_list (XASL_NODE * xasl_p, THREAD_ENTRY * thread_p, ACCESS_SPEC_TYPE * list, int loose_end)
{
  ACCESS_SPEC_TYPE *p;
  HEAP_SCAN_ID *hsidp;
  INDX_SCAN_ID *isidp;
  ...
  for (p = list; p; p = p->next)
    {
      ...
      switch (p->s_id.type)
        {
        case S_HEAP_SCAN:
          pg_cnt += qexec_clear_regu_list (xasl_p, p->s_id.s.hsid.scan_pred.regu_list, loose_end);
          pg_cnt += qexec_clear_regu_list (xasl_p, p->s_id.s.hsid.rest_regu_list, loose_end);
          hsidp = &p->s_id.s.hsid;
        ...
        case S_INDX_SCAN:
          pg_cnt += qexec_clear_regu_list (xasl_p, p->s_id.s.isid.key_pred.regu_list, loose_end);
          pg_cnt += qexec_clear_regu_list (xasl_p, p->s_id.s.isid.scan_pred.regu_list, loose_end);
          pg_cnt += qexec_clear_regu_list (xasl_p, p->s_id.s.isid.rest_regu_list, loose_end);

          pg_cnt += qexec_clear_regu_list (xasl_p, p->s_id.s.isid.pk_next_regu_list, loose_end);

          if (p->s_id.s.isid.indx_cov.regu_val_list != NULL)
            {
              pg_cnt += qexec_clear_regu_list (xasl_p, p->s_id.s.isid.indx_cov.regu_val_list, loose_end);
            }

          if (p->s_id.s.isid.indx_cov.output_val_list != NULL)
            {
              pg_cnt += qexec_clear_regu_list (xasl_p, p->s_id.s.isid.indx_cov.output_val_list->valptrp, loose_end);
            }
        ...
        case S_LIST_SCAN:
          pg_cnt += qexec_clear_regu_list (xasl_p, p->s_id.s.llsid.scan_pred.regu_list, loose_end);
          pg_cnt += qexec_clear_regu_list (xasl_p, p->s_id.s.llsid.rest_regu_list, loose_end);
          break;
      ...
      switch (p->type)
        {
        case TARGET_CLASS:
          pg_cnt += qexec_clear_regu_list (xasl_p, p->s.cls_node.cls_regu_list_key, loose_end);
          pg_cnt += qexec_clear_regu_list (xasl_p, p->s.cls_node.cls_regu_list_pred, loose_end);
          pg_cnt += qexec_clear_regu_list (xasl_p, p->s.cls_node.cls_regu_list_rest, loose_end);

          pg_cnt += qexec_clear_regu_list (xasl_p, p->s.cls_node.cls_regu_list_pk_next, loose_end);
              ...
              if (indx_info)
                {
                  ...
                  for (i = 0; i < N; i++)
                    {
                      pg_cnt += qexec_clear_regu_var (xasl_p, indx_info->key_info.key_ranges[i].key1, loose_end);
                      pg_cnt += qexec_clear_regu_var (xasl_p, indx_info->key_info.key_ranges[i].key2, loose_end);
                    }
                  pg_cnt += qexec_clear_regu_var (xasl_p, indx_info->key_info.key_limit_l, loose_end);
                  pg_cnt += qexec_clear_regu_var (xasl_p, indx_info->key_info.key_limit_u, loose_end);
                }
            }
          break;

        case TARGET_LIST:
          pg_cnt += qexec_clear_regu_list (xasl_p, p->s.list_node.list_regu_list_pred, loose_end);
          pg_cnt += qexec_clear_regu_list (xasl_p, p->s.list_node.list_regu_list_rest, loose_end);
          break;
 ...
 */
static int
qexec_clear_agg_list (XASL_NODE * xasl_p, AGGREGATE_TYPE * list, int loose_end)
{
  AGGREGATE_TYPE *p;
  int pg_cnt;
  ...
  for (p = list; p; p = p->next)
    {
      ...
      pg_cnt += qexec_clear_regu_var (xasl_p, &p->group_concat_sep, loose_end);
...
qexec_clear_xasl (THREAD_ENTRY * thread_p, XASL_NODE * xasl, bool loose_end)
{
  ...
  if (xasl->outptr_list)
    {
      pg_cnt += qexec_clear_regu_list (xasl, xasl->outptr_list->valptrp, loose_end);
    }
  pg_cnt += qexec_clear_access_spec_list (xasl, thread_p, xasl->spec_list, loose_end);
    ...
    case BUILDLIST_PROC:
      {
        ...
        if (buildlist->eptr_list)
          {
            pg_cnt += qexec_clear_xasl (thread_p, buildlist->eptr_list, loose_end);
          }
        ...
        if (buildlist->g_outptr_list)
          {
            pg_cnt += qexec_clear_regu_list (xasl, buildlist->g_outptr_list->valptrp, loose_end);
          }
        pg_cnt += qexec_clear_regu_list (xasl, buildlist->g_regu_list, loose_end);
        if (loose_end == false && buildlist->g_val_list)
          {
            qexec_clear_db_val_list (buildlist->g_val_list->valp);
          }
        pg_cnt += qexec_clear_agg_list (xasl, buildlist->g_agg_list, loose_end);
        pg_cnt += qexec_clear_pred (xasl, buildlist->g_having_pred, loose_end);
        pg_cnt += qexec_clear_pred (xasl, buildlist->g_grbynum_pred, loose_end);
    ...
    case BUILDVALUE_PROC:
      {
        ...
        pg_cnt += qexec_clear_agg_list (xasl, buildvalue->agg_list, loose_end);
        pg_cnt += qexec_clear_arith_list (xasl, buildvalue->outarith_list, loose_end);
          ...
          for (i = 0; i < xasl->proc.insert.odku->no_assigns; i++)
            {
              ...
              if (assignment->regu_var != NULL)
                {
                  pg_cnt += qexec_clear_regu_var (xasl, assignment->regu_var, loose_end);
                }
            }
        }
          ...
          for (i = 0; i < xasl->proc.insert.no_val_lists; i++)
            {
              ...
              for (regu_list = valptr_list->valptrp; regu_list != NULL; regu_list = regu_list->next)
                {
                  pg_cnt += qexec_clear_regu_var (xasl, &regu_list->value, loose_end);
                }
            }
        }
      break;
                  ...
                  if (ordby_info && ordby_info->ordbynum_pos_cnt > 0)
                    {
                      ...
                      tplrec.size = 0;
                      tplrec.tpl = NULL;
                      error = qfile_get_tuple (thread_p, page, page + key->s.original.offset, &tplrec, list_idp);
                      if (error == NO_ERROR)
                        {
                          data = tplrec.tpl;
                          /* update orderby_num() in the tuple */
...
qexec_fill_sort_limit (THREAD_ENTRY * thread_p, XASL_NODE * xasl, XASL_STATE * xasl_state, int *limit_ptr)
{
  ...
  *limit_ptr = NO_SORT_LIMIT;
  ...
  if (fetch_peek_dbval (thread_p, xasl->orderby_limit, &xasl_state->vd, NULL, NULL, &dbvalp) != NO_ERROR)
    {
      return ER_FAILED;
    }
      ...
      if (status != DOMAIN_COMPATIBLE)
        {
          er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_TP_CANT_COERCE, 2,
                  pr_type_name (orig_type), pr_type_name (TP_DOMAIN_TYPE (domainp)));
          return ER_FAILED;
        }
  ...
  if (thread_is_on_trace (thread_p))
    {
      ...
      if (xasl->orderby_stats.orderby_filesort)
        {
          xasl->orderby_stats.orderby_pages = mnt_get_stats (thread_p, MNT_STATS_SORT_DATA_PAGES) - old_sort_pages;
          xasl->orderby_stats.orderby_ioreads = mnt_get_stats (thread_p, MNT_STATS_SORT_IO_PAGES) - old_sort_ioreads;
        }
    }
  ...
  if (option != Q_DISTINCT)
  ...
  else
    {
      ...
      for (k = 0, order_ptr = orderby_list; k < list_id->type_list.type_cnt; k++, order_ptr = order_ptr->next)
        {
          ...
          if (order_list == NULL && xasl->spec_list
              && xasl->spec_list->indexptr && xasl->spec_list->indexptr->use_desc_index)
            {
      ...
      for (orderby_ptr = order_list, order_ptr = orderby_list;
           orderby_ptr != NULL; orderby_ptr = orderby_ptr->next, order_ptr = order_ptr->next)
        {
  ...
  if (ordbynum_val == NULL && orderby_list
  ...
  else
    {
      ...
      list_id = qfile_sort_list_with_func (thread_p, list_id, orderby_list,
                                           option, ls_flag, NULL, put_fn, NULL, &ordby_info, limit, true);
      if (list_id == NULL)
        {
          error = ER_FAILED;
          QEXEC_GOTO_EXIT_ON_ERROR;
...
qexec_gby_agg_tuple (THREAD_ENTRY * thread_p, GROUPBY_STATE * gbstate, QFILE_TUPLE tpl, int peek)
{
  ...
  if (fetch_val_list (thread_p, gbstate->g_regu_list,
                      &gbstate->xasl_state->vd, NULL, tpl, tpl_type_list, peek) != NO_ERROR)
    {
      QEXEC_GOTO_EXIT_ON_ERROR;
    }
  ...
  for (i = 0; i < gbstate->g_dim_levels; i++)
    {
      ...
      if (qdata_evaluate_aggregate_list (thread_p, gbstate->g_dim[i].d_agg_list, &gbstate->xasl_state->vd) != NO_ERROR)
        {
          QEXEC_GOTO_EXIT_ON_ERROR;
        }
    }
...
qexec_groupby (THREAD_ENTRY * thread_p, XASL_NODE * xasl, XASL_STATE * xasl_state, QFILE_TUPLE_RECORD * tplrec)
{
  ...
  if (qexec_initialize_groupby_state (&gbstate, buildlist->groupby_list,
                                      ...
                                      buildlist->g_with_rollup, xasl, xasl_state, &list_id->type_list, tplrec) == NULL)
    {
      QEXEC_GOTO_EXIT_ON_ERROR;
    }
   ...
   */
  {
    ...
    output_list_id = qfile_open_list (thread_p, &output_type_list,
                                      buildlist->after_groupby_list, xasl_state->query_id, ls_flag);
    ...
    if (thread_is_on_trace (thread_p))
      {
        ...
        if (xasl->groupby_stats.groupby_sort == true)
          {
            xasl->groupby_stats.groupby_pages = mnt_get_stats (thread_p, MNT_STATS_SORT_DATA_PAGES) - old_sort_pages;
            xasl->groupby_stats.groupby_ioreads = mnt_get_stats (thread_p, MNT_STATS_SORT_IO_PAGES) - old_sort_ioreads;
          }
      }
...
qexec_open_scan (THREAD_ENTRY * thread_p, ACCESS_SPEC_TYPE * curr_spec,
                 ...
                 int fixed, SCAN_ID * s_id, QUERY_ID query_id, bool scan_immediately_stop)
{
  SCAN_TYPE scan_type;
  INDX_INFO *indx_info;
      ...
      if (scan_type == S_HEAP_SCAN)
      ...
      else
        {
          if (scan_open_index_scan (thread_p, s_id,
                                    ...
                                    curr_spec->where_pred,
                                    curr_spec->s.cls_node.cls_regu_list_rest,
                                    curr_spec->s.cls_node.cls_regu_list_pk_next,
                                    ...
                                    curr_spec->s.cls_node.cache_rest, query_id) != NO_ERROR)
            {
              goto exit_on_error;
            }
          /* monitor */
  ...
  for (last_xptr = xasl; last_xptr->scan_ptr; last_xptr = last_xptr->scan_ptr)
    {
      if (!last_xptr->next_scan_block_on
          || (last_xptr->curr_spec
              && last_xptr->curr_spec->s_id.status == S_STARTED && !last_xptr->curr_spec->s_id.qualified_block))
        {
          break;
        }
    }
...
qexec_execute_scan (THREAD_ENTRY * thread_p, XASL_NODE * xasl,
                    XASL_STATE * xasl_state, QFILE_TUPLE_RECORD * ignore, XASL_SCAN_FNC_PTR next_scan_fnc)
{
  XASL_NODE *xptr;
  SCAN_CODE sc_scan;
  SCAN_CODE xs_scan;
...
qexec_intprt_fnc (THREAD_ENTRY * thread_p, XASL_NODE * xasl,
                  XASL_STATE * xasl_state, QFILE_TUPLE_RECORD * tplrec, XASL_SCAN_FNC_PTR next_scan_fnc)
{
  XASL_NODE *xptr;
  SCAN_CODE xs_scan;
  SCAN_CODE xb_scan;
      ...
      if (buildvalue->agg_list != NULL)
        {
          ...
          error = qexec_evaluate_aggregates_optimize (thread_p, buildvalue->agg_list, xasl->spec_list, &is_scan_needed);
          if (error != NO_ERROR)
            {
              is_scan_needed = true;
            }
      ...
      while ((ls_scan = scan_next_scan (thread_p, &xasl->curr_spec->s_id)) == S_SUCCESS)
        {
          if (count_star_with_iscan_opt)
            {
              xasl->proc.buildvalue.agg_list->accumulator.curr_cnt += (&xasl->curr_spec->s_id)->s.isid.oid_list.oid_cnt;
                  ...
                  if (!xasl->scan_ptr)
                    ...
                    {           /* handle the scan procedure */
                      ...
                      while ((xs_scan = (*next_scan_fnc) (thread_p,
                                                          xasl->scan_ptr,
                                                          xasl_state, tplrec, next_scan_fnc + 1)) == S_SUCCESS)
                        {
  ...
  if (list_id->type_list.domp == NULL)
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE,
              ER_OUT_OF_VIRTUAL_MEMORY, 1, list_id->type_list.type_cnt * sizeof (TP_DOMAIN *));
...
qexec_init_upddel_ehash_files (THREAD_ENTRY * thread_p, XASL_NODE * buildlist)
{
  ...
  if (xehash_create (thread_p, ehid, DB_TYPE_OBJECT, -1, NULL, 0, true) == NULL)
    {
      return ER_FAILED;
    }
 ...
 */
static int
qexec_execute_update (THREAD_ENTRY * thread_p, XASL_NODE * xasl, XASL_STATE * xasl_state)
{
  ...
  if (qexec_open_scan (thread_p, specp, xasl->val_list, &xasl_state->vd, true,
                       specp->fixed_scan, &specp->s_id, xasl_state->query_id, false) != NO_ERROR)
    {
      QEXEC_GOTO_EXIT_ON_ERROR;
    }
      ...
      while ((ls_scan = scan_next_scan (thread_p, s_id)) == S_SUCCESS)
        {
          ...
          if (internal_class.class_oid == NULL)
            {
              ...
              error = qexec_upddel_setup_current_class (thread_p, upd_cls, &internal_class);
              if (error != NO_ERROR)
                {
                  QEXEC_GOTO_EXIT_ON_ERROR;
                }
              ...
              if (heap_attrinfo_start (thread_p, &(upd_cls->class_oid),
                                       -1, NULL, &internal_class.attr_info) != NO_ERROR)
                {
                  QEXEC_GOTO_EXIT_ON_ERROR;
                }
          ...
          /* perform assignments */
          for (assign_idx = 0;
               assign_idx < update->no_assigns && vallist != NULL; assign_idx++, vallist = vallist->next)
            {
              HEAP_CACHE_ATTRINFO *attr_info;
          ...
          error = locator_attribute_info_force (thread_p,
                                                ...
                                                upd_cls->att_id,
                                                upd_cls->no_attrs,
                                                LC_FLUSH_UPDATE, &internal_class.scan_cache, &force_count);
...
#define MIN_NUM_ROWS_FOR_MULTI_DELETE   20
  ...
  if (qexec_open_scan (thread_p, specp, xasl->val_list,
                       &xasl_state->vd, true, specp->fixed_scan, &specp->s_id, xasl_state->query_id, false) != NO_ERROR)
    {
      QEXEC_GOTO_EXIT_ON_ERROR;
    }
      ...
      while ((ls_scan = scan_next_scan (thread_p, s_id)) == S_SUCCESS)
        {
          ...
          if (internal_class.class_oid == NULL)
            {
              /* find class HFID */
              error = qexec_upddel_setup_current_class (thread_p, del_cls, &internal_class);
              if (error != NO_ERROR)
                {
                  QEXEC_GOTO_EXIT_ON_ERROR;
                }
          ...
          if (locator_attribute_info_force (thread_p,
                                            ...
                                            &internal_class.scan_cache, &force_count) != NO_ERROR)
            {
              QEXEC_GOTO_EXIT_ON_ERROR;
            }
                                     ...
                                     const HEAP_IDX_ELEMENTS_INFO * idx_info, int *removed_count)
{
  ...
  *removed_count = 0;
  ...
  copyarea = locator_allocate_copy_area_by_attr_info (thread_p, attr_info, NULL, &new_recdes, oid->groupid, -1);
  if (copyarea == NULL)
    {
      goto error_exit;
    }
  ...
  if (idx_info->has_single_col)
    {
      error = heap_attrinfo_read_dbvalues (thread_p, &oid_Null_oid, &new_recdes, index_attr_info);
      if (error != NO_ERROR)
        {
          goto error_exit;
        }
  ...
  for (i = 0; i < idx_info->num_btids; ++i)
    {
      ...
      error = heap_attrvalue_get_key (thread_p, i, index_attr_info, &oid_Null_oid, &new_recdes, &btid, &key);
      if (error != NO_ERROR)
        {
          goto error_exit;
        }
      ...
      if (r == BTREE_KEY_FOUND)
        {
          ...
          error = locator_attribute_info_force (thread_p, &pruned_hfid,
                                                ...
                                                LC_FLUSH_DELETE, local_scan_cache, &force_count);
          if (error != NO_ERROR)
            {
              goto error_exit;
            }
...
qexec_oid_of_duplicate_key_update (THREAD_ENTRY * thread_p,
                                   ...
                                   HEAP_CACHE_ATTRINFO * attr_info,
                                   HEAP_CACHE_ATTRINFO * index_attr_info,
                                   const HEAP_IDX_ELEMENTS_INFO * idx_info, OID * unique_oid_p)
{
  LC_COPYAREA *copyarea = NULL;
  ...
  copyarea = locator_allocate_copy_area_by_attr_info (thread_p, attr_info, NULL, &recdes, oid->groupid, -1);
  if (copyarea == NULL)
    {
      goto error_exit;
    }
  ...
  for (i = 0; i < idx_info->num_btids && !found_duplicate; ++i)
    {
      ...
      assert (DB_IDXKEY_IS_NULL (&key));
      error_code = heap_attrvalue_get_key (thread_p, i, index_attr_info, &oid_Null_oid, &recdes, &btid, &key);
      if (error_code != NO_ERROR)
        {
          goto error_exit;
        }
...
qexec_execute_duplicate_key_update (THREAD_ENTRY * thread_p, ODKU_INFO * odku,
                                    ...
                                    HEAP_CACHE_ATTRINFO * attr_info,
                                    HEAP_CACHE_ATTRINFO * index_attr_info,
                                    HEAP_IDX_ELEMENTS_INFO * idx_info, int *force_count)
{
  int satisfies_constraints;
  int assign_idx;
  ...
  error = qexec_oid_of_duplicate_key_update (thread_p,
                                             scan_cache, oid, attr_info, index_attr_info, idx_info, &unique_oid);
  if (error != NO_ERROR)
    {
      goto exit_on_error;
    }
  ...
  scan_code = heap_get (thread_p, &unique_oid, &rec_descriptor, local_scan_cache, PEEK);
  ...
  for (assign_idx = 0; assign_idx < odku->no_assigns && error == NO_ERROR; assign_idx++)
    {
      ...
      error = fetch_peek_dbval (thread_p, assign->regu_var, vd, NULL, NULL, &val);
      if (error != NO_ERROR)
        {
          goto exit_on_error;
        }
  ...
  error = locator_attribute_info_force (thread_p, hfid, &unique_oid,
                                        ...
                                        odku->no_assigns, LC_FLUSH_UPDATE, local_scan_cache, force_count);
  if (error != NO_ERROR)
    {
      goto exit_on_error;
    }
 ...
 */
static int
qexec_execute_insert (THREAD_ENTRY * thread_p, XASL_NODE * xasl, XASL_STATE * xasl_state)
{
  ...
  if (aptr && qexec_execute_mainblock (thread_p, aptr, xasl_state) != NO_ERROR)
    {
      qexec_failure_line (__LINE__, xasl_state);
      return ER_FAILED;
    }
  ...
  if (insert->has_uniques && (insert->do_replace || odku_assignments != NULL))
    {
      if (heap_attrinfo_start_with_index (thread_p, &class_oid, &index_attr_info, &idx_info) < 0)
        {
          QEXEC_GOTO_EXIT_ON_ERROR;
        }
  ...
  for (k = 0; k < no_default_expr; k++)
    {
                    ...
                    case 0:
                      error =
                        (*(pr_type->data_readval)) (&buf, insert->vals[k],
                                                    attr->domain, attr->current_default_value.val_length, copy);
                      if (error != NO_ERROR)
                        {
                          QEXEC_GOTO_EXIT_ON_ERROR;
                        }
      ...
      status = tp_value_coerce (insert->vals[k], insert->vals[k], attr->domain);
      if (status != DOMAIN_COMPATIBLE)
        {
  ...
  if (specp)
    {
      ...
      if (locator_start_force_scan_cache (thread_p, &scan_cache, &insert->class_hfid, 0, &class_oid) != NO_ERROR)
        {
          QEXEC_GOTO_EXIT_ON_ERROR;
        }
          ...
          while ((ls_scan = scan_next_scan (thread_p, s_id)) == S_SUCCESS)
            {
              ...
              if (insert->do_replace && insert->has_uniques)
                {
                  ...
                  error = qexec_remove_duplicates_for_replace (thread_p,
                                                               &scan_cache,
                                                               &oid,
                                                               &attr_info, &index_attr_info, &idx_info, &removed_count);
                  if (error != NO_ERROR)
                    {
                      QEXEC_GOTO_EXIT_ON_ERROR;
                    }
              ...
              if (odku_assignments && insert->has_uniques)
                {
                  ...
                  error =
                    qexec_execute_duplicate_key_update (thread_p,
                                                        ...
                                                        &oid, &xasl_state->vd,
                                                        &scan_cache,
                                                        &attr_info, &index_attr_info, &idx_info, &force_count);
                  if (error != NO_ERROR)
                    {
                      QEXEC_GOTO_EXIT_ON_ERROR;
                    }
              ...
              if (locator_attribute_info_force (thread_p, &insert->class_hfid,
                                                &oid, &attr_info,
                                                NULL, 0, LC_FLUSH_INSERT, &scan_cache, &force_count) != NO_ERROR)
                {
                  QEXEC_GOTO_EXIT_ON_ERROR;
                }
  ...
  else
    {
      ...
      if (locator_start_force_scan_cache (thread_p, &scan_cache,
                                          &insert->class_hfid, insert->force_page_allocation, &class_oid) != NO_ERROR)
        {
          QEXEC_GOTO_EXIT_ON_ERROR;
        }
      scan_cache_inited = true;
      ...
      for (i = 0; i < insert->no_val_lists; i++)
        {
          for (regu_list = insert->valptr_lists[i]->valptrp, vallist =
            ...
            {
              if (fetch_peek_dbval (thread_p, &regu_list->value, &xasl_state->vd, NULL, NULL, &valp) != NO_ERROR)
                {
                  QEXEC_GOTO_EXIT_ON_ERROR;
                }
          ...
          if (insert->do_replace && insert->has_uniques)
            {
              ...
              error =
                qexec_remove_duplicates_for_replace (thread_p, &scan_cache,
                                                     &oid, &attr_info, &index_attr_info, &idx_info, &removed_count);
              if (error != NO_ERROR)
                {
                  QEXEC_GOTO_EXIT_ON_ERROR;
                }
...
exit_on_error:
  ...
  if (XASL_IS_FLAGED (xasl, XASL_LINK_TO_REGU_VARIABLE) && xasl->list_id != NULL)
    {
      qfile_clear_list_id (xasl->list_id);
    }
...
qexec_init_instnum_val (XASL_NODE * xasl, THREAD_ENTRY * thread_p, XASL_STATE * xasl_state)
{
  ...
  if (xasl->scan_ptr == NULL &&
      ...
      xasl->spec_list->access == INDEX && xasl->spec_list->indexptr && xasl->spec_list->indexptr->key_info.key_limit_l)
    {
      key_limit_l = xasl->spec_list->indexptr->key_info.key_limit_l;
      if (fetch_peek_dbval (thread_p, key_limit_l, &xasl_state->vd, NULL, NULL, &dbvalp) != NO_ERROR)
        {
          goto exit_on_error;
        }
      ...
      if (orig_type != DB_TYPE_BIGINT)
        {
          if (DB_IS_NULL (dbvalp) || tp_value_coerce (dbvalp, dbvalp, domainp) != DOMAIN_COMPATIBLE)
            {
              er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_TP_CANT_COERCE, 2,
                      pr_type_name (orig_type), pr_type_name (TP_DOMAIN_TYPE (domainp)));
              goto exit_on_error;
                                      ...
                                      XASL_NODE * xasl, XASL_STATE * xasl_state, QFILE_TUPLE_RECORD * tplrec)
{
  ...
  if (buildvalue->agg_list
      && qdata_finalize_aggregate_list (thread_p, buildvalue->agg_list, &xasl_state->vd, false) != NO_ERROR)
    {
      QEXEC_GOTO_EXIT_ON_ERROR;
    }
  ...
  if (buildvalue->having_pred != NULL)
    {
      ...
      if (ev_res != V_TRUE && qdata_set_valptr_list_unbound (thread_p, xasl->outptr_list, &xasl_state->vd) != NO_ERROR)
        {
          QEXEC_GOTO_EXIT_ON_ERROR;
        }
    }
  ...
  switch (xasl->type)
    ...
    case INTERSECTION_PROC:
      ...
      t_list_id = qfile_combine_two_list (thread_p,
                                          xasl->proc.union_.left->list_id, xasl->proc.union_.right->list_id, ls_flag);
  ...
  if (on_trace)
    {
      ...
      xasl->xasl_stats.fetches += mnt_get_stats (thread_p, MNT_STATS_DATA_PAGE_FETCHES) - old_fetches;
      xasl->xasl_stats.ioreads += mnt_get_stats (thread_p, MNT_STATS_DATA_PAGE_IOREADS) - old_ioreads;
    }
 ...
 */
static int
qexec_check_modification (THREAD_ENTRY * thread_p, UNUSED_ARG XASL_NODE * xasl, XASL_STATE * xasl_state)
{
  int error = NO_ERROR;
  int tran_index;
  ...
  if (error != NO_ERROR)
    {
      ...
      query_p = qmgr_get_query_entry (thread_p, xasl_state->query_id, tran_index);
      assert (query_p != NULL);
          ...
          for (xptr2 = xptr->aptr_list; xptr2; xptr2 = xptr2->next)
            {
              ...
              if (xptr2->status == XASL_CLEARED || xptr2->status == XASL_INITIALIZED)
                {
                  if (qexec_execute_mainblock (thread_p, xptr2, xasl_state) != NO_ERROR)
                    {
                      if (tplrec.tpl)
                        {
                          free_and_init (tplrec.tpl);
      ...
      if (xasl->spec_list)
        {
              ...
              for (specp = xptr->spec_list; specp; specp = specp->next)
                {
                  ...
                  if (qexec_open_scan (thread_p, specp,
                                       ...
                                       readonly_scan2,
                                       specp->fixed_scan,
                                       &specp->s_id, xasl_state->query_id, scan_immediately_stop) != NO_ERROR)
                    {
          ...
          func_vector = (XASL_SCAN_FNC_PTR) malloc (level * sizeof (XSAL_SCAN_FUNC));
          if (func_vector == NULL)
            {
              qexec_clear_mainblock_iterations (thread_p, xasl);
              QEXEC_GOTO_EXIT_ON_ERROR;
 ...
 */
int
qexec_update_index_stats (THREAD_ENTRY * thread_p, CIRP_CT_INDEX_STATS * index_stats)
{
  DB_IDXKEY key;
  int error = NO_ERROR;
  ...
  error = qexec_modify_catalog_table (thread_p, &key, index_stats, &table_IndexStats, CT_MODIFY_MODE_UPSERT);
 ...
 */
static int
qexec_insert_gid_skey_info (THREAD_ENTRY * thread_p, int shard_groupid, DB_VALUE * shard_key)
{
  DB_IDXKEY key;
  ...
  error = qexec_modify_catalog_table (thread_p, &key, &ct_gid_skey, &table_ShardGidSkey, CT_MODIFY_MODE_INSERT);
 ...
 */
int
qexec_upsert_applier_info (THREAD_ENTRY * thread_p, DB_IDXKEY * pk, CIRP_CT_LOG_APPLIER * applier)
{
  int error = NO_ERROR;

  error = qexec_modify_catalog_table (thread_p, pk, applier, &table_LogApplier, CT_MODIFY_MODE_UPSERT);
 ...
 */
int
qexec_update_applier_info (THREAD_ENTRY * thread_p, DB_IDXKEY * pk, CIRP_CT_LOG_APPLIER * applier)
{
  int error = NO_ERROR;

  error = qexec_modify_catalog_table (thread_p, pk, applier, &table_LogApplier, CT_MODIFY_MODE_UPDATE);
 ...
 */
int
qexec_upsert_analyzer_info (THREAD_ENTRY * thread_p, DB_IDXKEY * pk, CIRP_CT_LOG_ANALYZER * analyzer)
{
  int error = NO_ERROR;

  error = qexec_modify_catalog_table (thread_p, pk, analyzer, &table_LogAnalyzer, CT_MODIFY_MODE_UPSERT);
 ...
 */
int
qexec_update_analyzer_info (THREAD_ENTRY * thread_p, DB_IDXKEY * pk, CIRP_CT_LOG_ANALYZER * analyzer)
{
  int error = NO_ERROR;

  error = qexec_modify_catalog_table (thread_p, pk, analyzer, &table_LogAnalyzer, CT_MODIFY_MODE_UPDATE);
...
qexec_modify_catalog_table (THREAD_ENTRY * thread_p, DB_IDXKEY * pk,
                            void *ct_data, CATCLS_TABLE * table, CT_MODIFY_MODE modify_mode)
{
#define OP_INSERT 1
#define OP_UPDATE 2
  ...
  found_status = xbtree_find_unique (thread_p, &ct_cache->class_oid, &ct_cache->btid, pk, &oid);
  switch (found_status)
    {
  ...
  if (op_mode == OP_INSERT)
    {
      ...
      error = qexec_insert_with_values (thread_p, &ct_cache->class_oid,
                                        &ct_cache->hfid, ct_cache->atts, force_page_allocation, &values);
    }
  else
    {
      assert (op_mode == OP_UPDATE);
      error = qexec_update_with_values (thread_p, &ct_cache->class_oid, &ct_cache->hfid, ct_cache->atts, &oid, &values);
    }
  if (error != NO_ERROR)
    {
      GOTO_EXIT_ON_ERROR;
...
qexec_get_info_of_catalog_table (THREAD_ENTRY * thread_p, CATCLS_TABLE * table)
{
  ...
  if (strncasecmp (table->name, CT_LOG_ANALYZER_NAME, strlen (CT_LOG_ANALYZER_NAME)) == 0)
    {
      ct_cache_info = &Analyzer_info;
      tmp_cache.cast_func = qexec_cast_ct_analyzer_to_idxkey;
    }
  else if (strncasecmp (table->name, CT_LOG_APPLIER_NAME, strlen (CT_LOG_APPLIER_NAME)) == 0)
    {
      ct_cache_info = &Applier_info;
      tmp_cache.cast_func = qexec_cast_ct_applier_to_idxkey;
    }
  else if (strncasecmp (table->name, CT_SHARD_GID_SKEY_INFO_NAME, strlen (CT_SHARD_GID_SKEY_INFO_NAME)) == 0)
    {
      ct_cache_info = &Shard_gid_skey_info;
      tmp_cache.cast_func = qexec_cast_ct_gid_skey_to_idxkey;
    }
  else if (strncasecmp (table->name, CT_INDEX_STATS_NAME, strlen (CT_INDEX_STATS_NAME)) == 0)
    {
  ...
  status = xlocator_find_class_oid (thread_p, table->name, &tmp_cache.class_oid, S_LOCK);
  if (status == LC_CLASSNAME_ERROR || status == LC_CLASSNAME_DELETED)
    {
      error = ER_LC_UNKNOWN_CLASSNAME;
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, error, 1, CT_LOG_ANALYZER_NAME);

      GOTO_EXIT_ON_ERROR;
    }

  error = heap_get_hfid_from_class_oid (thread_p, &tmp_cache.class_oid, &tmp_cache.hfid);
  if (error != NO_ERROR)
    {
      GOTO_EXIT_ON_ERROR;
    }

  error = btree_get_pkey_btid (thread_p, &tmp_cache.class_oid, &tmp_cache.btid);
  if (error != NO_ERROR)
    {
      GOTO_EXIT_ON_ERROR;
    }
  ...
  if (tmp_cache.atts == NULL)
    {
      ...
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, error, 1, table->num_columns * sizeof (int));
      GOTO_EXIT_ON_ERROR;
    }

  error = qexec_get_attrids_from_catalog_table (thread_p, tmp_cache.atts, &tmp_cache.class_oid, table);
  if (error != NO_ERROR)
    {
      GOTO_EXIT_ON_ERROR;
    }
  ...
  if (!LSA_ISNULL (&log_applier->committed_lsa) && bi < 0)
    {
      ...
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, error, 1, "invalid required_lsa");
                          ...
                          HFID * hfid, ATTR_ID * attr_ids, int force_page_allocation, DB_IDXKEY * values)
{
  ...
  error = locator_start_force_scan_cache (thread_p, &scan_cache, hfid, force_page_allocation, class_oid);
  if (error != NO_ERROR)
    {
      GOTO_EXIT_ON_ERROR;
    }
  ...
  for (i = 0; i < values->size; i++)
    {
      error = heap_attrinfo_set (NULL, attr_ids[i], &values->vals[i], &attr_info);
      if (error != NO_ERROR)
        {
          GOTO_EXIT_ON_ERROR;
        }
  ...
  error = locator_attribute_info_force (thread_p, hfid,
                                        &oid, &attr_info, NULL, 0, LC_FLUSH_INSERT, &scan_cache, &force_count);
  if (error != NO_ERROR)
    {
      GOTO_EXIT_ON_ERROR;
    }
                          ...
                          HFID * hfid, ATTR_ID * attr_ids, OID * oid, DB_IDXKEY * values)
{
  ...
  error = locator_start_force_scan_cache (thread_p, &scan_cache, hfid, 0, class_oid);
  if (error != NO_ERROR)
    {
      GOTO_EXIT_ON_ERROR;
    }
  ...
  error = locator_attribute_info_force (thread_p, hfid,
                                        oid, &attr_info, NULL, 0, LC_FLUSH_UPDATE, &scan_cache, &force_count);
  if (error != NO_ERROR)
    {
      GOTO_EXIT_ON_ERROR;
    }
 ...
 */
static int
qexec_get_attrids_from_catalog_table (THREAD_ENTRY * thread_p, int *att_ids, OID * class_oid, CATCLS_TABLE * ct_table)
{
  ...
  if (heap_get (thread_p, class_oid, &class_record, &scan_cache, PEEK) != S_SUCCESS)
    {
      error = er_errid ();
      GOTO_EXIT_ON_ERROR;
    }
                                ...
                                int shard_groupid, DB_VALUE * shard_key, bool req_from_migrator)
{
  ...
  switch (xasl->type)
    ...
    case BUILDVALUE_PROC:
      ...
      error = qexec_lock_table_select_for_update (thread_p, xasl, &num_shard_table, &for_update);
      if (error == NO_ERROR)
        {
          if (is_shard_table)
            {
    ...
    case INSERT_PROC:
      if (shard_groupid == GLOBAL_GROUPID && logtb_is_blocked_global_dml () && req_from_migrator != true)
        {
      ...
      if (lock_object (thread_p, &lock_val, S_LOCK, LK_UNCOND_LOCK) != LK_GRANTED)
        {
          error = er_errid ();
          if (error == NO_ERROR)
            {
      ...
      assert (heap_classrepr_is_shard_table (thread_p, DB_GET_OID (&lock_val)) == is_shard_table);
      for_update = true;
      break;
    ...
    case UPDATE_PROC:
      ...
      error = qexec_lock_table_update_delete (thread_p, xasl->aptr_list,
                                              xasl->proc.update.class_info, is_shard_table, is_catalog_table);
      for_update = true;
      break;
    ...
    case DELETE_PROC:
      ...
      error = qexec_lock_table_update_delete (thread_p, xasl->aptr_list,
                                              xasl->proc.delete_.class_info, is_shard_table, is_catalog_table);
      for_update = true;
      break;
  ...
  if (is_shard_table && is_catalog_table)
    {
      ...
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, error, 1, "qexec_lock_table_and_shard_key(): Invalid shard_key");
      return error;
    }

  /* defense code; check iff valid DML
   */
  if ((shard_groupid != GLOBAL_GROUPID && is_shard_table) || (shard_groupid == GLOBAL_GROUPID && !is_shard_table))
    {
      ;                         /* OK */
    }
  else
  ...
  error = lock_shard_key_lock (thread_p, shard_groupid, shard_key,
                               class_oid, is_shard_table, is_catalog_table, for_update);
  if (error != NO_ERROR)
    {
      return error;
    }
...
qexec_init_xasl_state (THREAD_ENTRY * thread_p, XASL_STATE * xasl_state,
                       int dbval_cnt, DB_VALUE * dbval_ptr, QUERY_ID query_id, int shard_groupid)
{
  struct timeb tloc;
...
qexec_execute_query (THREAD_ENTRY * thread_p, XASL_NODE * xasl,
                     ...
                     QUERY_ID query_id, int shard_groupid, DB_VALUE * shard_key, bool req_from_migrator)
{
  int stat;
...
#if defined(SERVER_MODE)
  if (LOG_IS_HA_CLIENT (logtb_find_current_client_type (thread_p)) || shard_groupid < GLOBAL_GROUPID)
    {
   ...
   */
  er_clear ();
  qexec_init_xasl_state (thread_p, &xasl_state, dbval_cnt, dbval_ptr, query_id, shard_groupid);
  ...
  xasl->query_in_progress = true;
  stat = qexec_lock_table_and_shard_key (thread_p, xasl, shard_groupid, shard_key, req_from_migrator);
  if (stat == NO_ERROR)
    {
      int tran_index;
      QMGR_QUERY_ENTRY *query_p = NULL;
  ...
  if (stat != NO_ERROR)
    {
      switch (er_errid ())
        ...
        case NO_ERROR:
          {
            snprintf (err_buf, 511, "Query execution failure #%d.", xasl_state.qp_xasl_line);
            er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_PT_EXECUTE, 2, err_buf, "");
            break;
          }
        case ER_INTERRUPTED:
          /*
...
qexec_initialize_xasl_cache (THREAD_ENTRY * thread_p)
{
  ...
  mnt_stats_gauge (thread_p, MNT_STATS_PLAN_CACHE_QUERY_STRING_HASH_ENTRIES, 0);
  ...
  if (xasl_ent_cache.xid_ht)
  ...
  else
    {
      ...
      xasl_ent_cache.xid_ht = mht_create ("XASL stream cache (xasl file id)",
                                          xasl_ent_cache.max_entries, xasl_id_hash, xasl_id_hash_cmpeq);
    }
  ...
  if (xasl_ent_cache.oid_ht)
  ...
  else
    {
      ...
      xasl_ent_cache.oid_ht = mht_create ("XASL stream cache (class oid)",
                                          xasl_ent_cache.max_entries, oid_hash, oid_compare_equals);
    }
  ...
  return ((xasl_ent_cache.qstr_ht && xasl_ent_cache.xid_ht
           && xasl_ent_cache.oid_ht && xasl_cache_entry_pool.pool) ? NO_ERROR : ER_FAILED);
}
...
static int
qexec_print_xasl_cache_ent (FILE * fp, UNUSED_ARG const void *key, void *data, UNUSED_ARG void *args)
{
 ...
 */
int
qexec_dump_xasl_cache_internal (UNUSED_ARG THREAD_ENTRY * thread_p, FILE * fp, int mask)
{
 ...
 */
static int
qexec_free_xasl_cache_ent (THREAD_ENTRY * thread_p, void *data, UNUSED_ARG void *args)
{
  /* this function should be called within CSECT_QP_XASL_CACHE */
  UNUSED_VAR int ret = NO_ERROR;
  POOLED_XASL_CACHE_ENTRY *pent;
 ...
 */
XASL_CACHE_ENTRY *
qexec_lookup_xasl_cache_ent (THREAD_ENTRY * thread_p, const char *qstr, const OID * user_oid)
{
  XASL_CACHE_ENTRY *ent;
  ...
  if (ent)
    {
      ...
      if (ent && prm_get_integer_value (PRM_ID_XASL_PLAN_CACHE_TIMEOUT) >= 0
          && (difftime (time (NULL),
                        ent->time_created.tv_sec) > prm_get_integer_value (PRM_ID_XASL_PLAN_CACHE_TIMEOUT)))
        {
...
qexec_update_xasl_cache_ent (THREAD_ENTRY * thread_p,
                             ...
                             XASL_STREAM * stream,
                             const OID * oid, int n_oids, const OID * class_oids, const int *tcards, int dbval_cnt)
{
  ...
  if (class_oids != NULL)
    {
      ent->class_oid_list = memcpy (XASL_CACHE_ENTRY_CLASS_OID_LIST (ent), class_oids, n_oids * sizeof (OID));
    }

  if (tcards != NULL)
    {
      ent->tcard_list = memcpy (XASL_CACHE_ENTRY_TCARD_LIST (ent), tcards, n_oids * sizeof (int));
    }

  ent->sql_info.sql_hash_text =
    memcpy (XASL_CACHE_ENTRY_SQL_HASH_TEXT (ent), context->sql_hash_text, sql_hash_text_len);

  if (context->sql_plan_text)
    {
      ent->sql_info.sql_plan_text =
        memcpy (XASL_CACHE_ENTRY_SQL_PLAN_TEXT (ent), context->sql_plan_text, sql_plan_text_len);
    }

  if (context->sql_user_text)
    {
      ent->sql_info.sql_user_text =
        memcpy (XASL_CACHE_ENTRY_SQL_USER_TEXT (ent), context->sql_user_text, sql_user_text_len);
    }
  ...
  mnt_stats_gauge (thread_p, MNT_STATS_PLAN_CACHE_QUERY_STRING_HASH_ENTRIES, mht_count (xasl_ent_cache.qstr_ht));
  ...
  mnt_stats_gauge (thread_p, MNT_STATS_PLAN_CACHE_XASL_ID_HASH_ENTRIES, mht_count (xasl_ent_cache.xid_ht));
  ...
  mnt_stats_gauge (thread_p, MNT_STATS_PLAN_CACHE_CLASS_OID_HASH_ENTRIES, mht_count (xasl_ent_cache.oid_ht));
  ...
  if (all_entries_are_fixed)
    {
      ...
      er_set (ER_NOTIFICATION_SEVERITY, ARG_FILE_LINE,
              ER_ALL_PLAN_CACHE_ENTRIES_ARE_FIXED, 2, current, xasl_ent_cache.max_entries);
    }
#endif
...
qexec_remove_my_tran_id_in_xasl_entry (UNUSED_ARG THREAD_ENTRY * thread_p,
                                       UNUSED_ARG XASL_CACHE_ENTRY * ent, UNUSED_ARG bool unfix_all)
{
#if defined(SERVER_MODE)
  int tran_index;
...
qexec_check_xasl_cache_ent_by_xasl (THREAD_ENTRY * thread_p,
                                    const XASL_ID * xasl_id, int dbval_cnt, XASL_CACHE_CLONE ** clop, bool save_clop)
{
  XASL_CACHE_ENTRY *ent;
  XASL_CACHE_CLONE *clo;
      ...
      if (ent && !CACHE_TIME_EQ (&(ent->xasl_id.time_stored), &(xasl_id->time_stored)))
        {
          assert ((xasl_id->time_stored.sec < ent->xasl_id.time_stored.sec)
                  || (xasl_id->time_stored.usec < ent->xasl_id.time_stored.usec));
      ...
      if (ent)
        {
          ...
          if (qexec_delete_xasl_cache_ent (thread_p, ent, &force_remove) == NO_ERROR)
            {
 ...
 */
static int
qexec_mark_delete_xasl_entry (UNUSED_ARG THREAD_ENTRY * thread_p, XASL_CACHE_ENTRY * ent, bool removed_from_hash)
{
...
qexec_insert_tuple_into_list (THREAD_ENTRY * thread_p,
                              QFILE_LIST_ID * list_id,
                              OUTPTR_LIST * outptr_list, VAL_DESCR * vd, QFILE_TUPLE_RECORD * tplrec)
{
  QPROC_TPLDESCR_STATUS tpldescr_status;
 ...
 */
int
qexec_get_tuple_column_value (QFILE_TUPLE tpl, int index, TP_DOMAIN * index_domain, DB_VALUE * valp)
{
  QFILE_TUPLE_VALUE_FLAG flag;
  char *ptr;
  int length;
  ...
  if (flag == V_BOUND)
    {
      ...
      if ((*(pr_type->data_readval)) (&buf, valp, index_domain, -1, false) != NO_ERROR)
        {
          return ER_FAILED;
        }
    }
 ...
 */
static void
qexec_gby_finalize_group_val_list (UNUSED_ARG THREAD_ENTRY * thread_p, GROUPBY_STATE * gbstate, int N)
{
  int i;
...
qexec_gby_finalize_group (THREAD_ENTRY * thread_p, GROUPBY_STATE * gbstate, int N, bool keep_list_file)
{
  ...
  if (qdata_finalize_aggregate_list (thread_p,
                                     gbstate->g_dim[N].d_agg_list, &xasl_state->vd, keep_list_file) != NO_ERROR)
    {
      QEXEC_GOTO_EXIT_ON_ERROR;
    }
  ...
  tpldescr_status = qexec_generate_tuple_descriptor (thread_p,
                                                     gbstate->output_file, gbstate->g_outptr_list, &xasl_state->vd);
  if (tpldescr_status == QPROC_TPLDESCR_FAILURE)
    {
      QEXEC_GOTO_EXIT_ON_ERROR;
    }
  ...
  switch (tpldescr_status)
    ...
    case QPROC_TPLDESCR_RETRY_BIG_REC:
      ...
      if (qdata_copy_valptr_list_to_tuple (thread_p, gbstate->output_file,
                                           gbstate->g_outptr_list, &xasl_state->vd, gbstate->output_tplrec) != NO_ERROR)
        {
          QEXEC_GOTO_EXIT_ON_ERROR;
        }
...
qexec_gby_init_group_dim (GROUPBY_STATE * gbstate)
{
  ...
  gbstate->g_dim = (GROUPBY_DIMENSION *) malloc (gbstate->g_dim_levels * sizeof (GROUPBY_DIMENSION));
      ...
      if (gbstate->g_output_agg_list)
        {
          ...
          gbstate->g_dim[i].d_agg_list = aggp = (AGGREGATE_TYPE *) malloc (sizeof (AGGREGATE_TYPE));
          if (aggp == NULL)
            {
              return ER_FAILED;
            }
      ...
      for (i = 0; i < ncolumns; i++)
        {
          ...
          if (qexec_get_tuple_column_value (tuple_rec.tpl, i, i_domain, &i_val) != NO_ERROR)
            {
              gbstate.state = ER_FAILED;
              goto exit_on_error;
            }
  ...
  if (multi_range_opt->no_attrs == 0)
    {
      ...
      multi_range_opt->is_desc_order = (bool *) malloc (count * sizeof (bool));
  ...
  for (index = 0, orderby_list = xasl->orderby_list; index < count; index++, orderby_list = orderby_list->next)
    {
       ...
       * sorting
       */
      for (regu_list = isidp->rest_regu_list; regu_list != NULL; regu_list = regu_list->next)
        {
      ...
      if (att_id != -1)
        {
          ...
          for (i = 0; i < btid->classrepr->indexes[btid->indx_id].n_atts; i++)
            {
              if (att_id == btid->classrepr->indexes[btid->indx_id].atts[i]->id)
                {
                  sort_index_pos = i;
                  break;
                }
...
query_multi_range_opt_check_specs (UNUSED_ARG THREAD_ENTRY * thread_p, XASL_NODE * xasl)
{
          ...
          if (specp->type == TARGET_CLASS)
            {
              ...
              if (heap_classrepr_is_shard_table (thread_p, DB_GET_OID (&lock_val)))
                {
                  (*num_shard_table)++;
                }
...
qexec_lock_table_update_delete (THREAD_ENTRY * thread_p,
                                XASL_NODE * aptr_list,
                                UPDDEL_CLASS_INFO * upd_cls, bool is_shard_table, UNUSED_ARG bool is_catalog_table)
{
  XASL_NODE *aptr = NULL;
  ACCESS_SPEC_TYPE *specp = NULL;
  OID *class_oid = NULL;
 ...
 */
static int
qexec_init_internal_class (UNUSED_ARG THREAD_ENTRY * thread_p, UPDDEL_CLASS_INFO_INTERNAL * class)
{
  assert (class != NULL);
 ...
 */
static void
qexec_clear_internal_class (THREAD_ENTRY * thread_p, UPDDEL_CLASS_INFO_INTERNAL * class)
{
...
qexec_upddel_setup_current_class (THREAD_ENTRY * thread_p,
                                  UPDDEL_CLASS_INFO * query_class, UPDDEL_CLASS_INFO_INTERNAL * internal_class)
{
  int error = NO_ERROR;
  ...
  if (internal_class->scan_cache_inited)
    {
      (void) locator_end_force_scan_cache (thread_p, &internal_class->scan_cache);
      internal_class->scan_cache_inited = false;
    }

  error = locator_start_force_scan_cache (thread_p,
                                          &internal_class->scan_cache,
                                          internal_class->class_hfid, 0, internal_class->class_oid);
  if (error != NO_ERROR)
    {
      return error;
    }
...
qexec_evaluate_aggregates_optimize (THREAD_ENTRY * thread_p,
                                    AGGREGATE_TYPE * agg_list, ACCESS_SPEC_TYPE * spec, bool * is_scan_needed)
{
  AGGREGATE_TYPE *agg_ptr;
  int error = NO_ERROR;
      ...
      if (agg_ptr->flag_agg_optimize)
        {
          ...
          if (agg_ptr->function == PT_COUNT_STAR || agg_ptr->function == PT_COUNT)
            {
              agg_ptr->flag_agg_optimize = false;
              *is_scan_needed = true;
              continue;
          ...
          if (qdata_evaluate_aggregate_optimize
              (thread_p, agg_ptr, &ACCESS_SPEC_CLS_OID (spec), &ACCESS_SPEC_HFID (spec)) != NO_ERROR)
            {
              agg_ptr->flag_agg_optimize = false;
              *is_scan_needed = true;
            }
...
qexec_setup_topn_proc (THREAD_ENTRY * thread_p, XASL_NODE * xasl, VAL_DESCR * vd)
{
  ...
  if (XASL_IS_FLAGED (xasl, XASL_SKIP_ORDERBY_LIST) || XASL_IS_FLAGED (xasl, XASL_USES_MRO))
    {
      return NO_ERROR;
    }
  ...
  for (var_list = xasl->outptr_list->valptrp; var_list != NULL; var_list = var_list->next)
    {
 ...
 */
static void
qexec_clear_topn_tuple (UNUSED_ARG THREAD_ENTRY * thread_p, TOPN_TUPLE * tuple, int count)
{
  int i;
  if (tuple == NULL)
    {
