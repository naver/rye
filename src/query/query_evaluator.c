		   ...
		   VAL_DESCR * vd, DB_VALUE * dbval1, DB_VALUE * dbval2)
{
  ...
  if (et_comp->comp_lhs->type == TYPE_LIST_ID)
      ...
      if (et_comp->comp_rhs->type == TYPE_LIST_ID)
	{
	  ...
	  if (CHECK_REGU_VARIABLE_XASL_STATUS (et_comp->comp_rhs) !=
	      XASL_SUCCESS)
	    {
	      return V_ERROR;
	    }
		       ...
		       VAL_DESCR * vd)
{
  ...
  if (fetch_peek_dbval
      (thread_p, rv, vd, NULL, NULL, &limit_row_count_valp) != NO_ERROR)
    {
      return V_UNKNOWN;
    }
  ...
  switch (pr->type)
	...
	case T_COMP_EVAL_TERM:
	  ...
	  switch (et_comp->comp_rel_op)
	    ...
	    case R_NULL:
	      if (fetch_peek_dbval
		  (thread_p, et_comp->comp_lhs, vd, obj_oid, NULL,
		   &peek_val1) != NO_ERROR)
		{
		  result = V_ERROR;
		  goto exit;
		...
		{
		  /* execute linked query */
		  EXECUTE_REGU_VARIABLE_XASL (thread_p, et_comp->comp_lhs,
					      vd);
		  if (CHECK_REGU_VARIABLE_XASL_STATUS (et_comp->comp_lhs) !=
		      XASL_SUCCESS)
		    {
		      result = V_ERROR;
		      goto exit;
	    ...
	    default:
	       ...
	       */
	      if (et_comp->comp_lhs->type == TYPE_LIST_ID
		  || et_comp->comp_rhs->type == TYPE_LIST_ID)
		{
		  assert (false);	/* impossible */
		  result = V_ERROR;
		  goto exit;
		...
		{
		  if (et_comp->comp_rel_op != R_EQ_TORDER
		      && et_comp->comp_rel_op != R_NULLSAFE_EQ)
		    {
		      result = V_UNKNOWN;
		      goto exit;
		    }
	      ...
	      if (fetch_peek_dbval
		  (thread_p, et_comp->comp_rhs, vd, obj_oid, NULL,
		   &peek_val2) != NO_ERROR)
		{
		  result = V_ERROR;
		  goto exit;
	...
	case T_ALSM_EVAL_TERM:
	  ...
	  if (et_alsm->elemset->type != TYPE_LIST_ID)
	    {
	      ...
	      if (fetch_peek_dbval (thread_p, et_alsm->elemset, vd,
	      ...
	      if (!TP_IS_SET_TYPE (DB_VALUE_DOMAIN_TYPE (peek_val2)))
		{
		  ...
		  er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE,
			  ER_TP_CANT_COERCE, 2,
			  pr_type_name (DB_VALUE_DOMAIN_TYPE (peek_val2)),
	  ...
	  /* fetch item value */
	  if (fetch_peek_dbval (thread_p, et_alsm->elem, vd, obj_oid,
	...
	case T_LIKE_EVAL_TERM:
	  ...
	  if (fetch_peek_dbval (thread_p, et_like->src, vd, obj_oid,
				NULL, &peek_val1) != NO_ERROR)
	    {
	      result = V_ERROR;
	      goto exit;
	  ...
	  if (fetch_peek_dbval (thread_p, et_like->pattern, vd, obj_oid,
	  ...
	  if (et_like->esc_char)
	    {
	      ...
	      if (fetch_peek_dbval (thread_p, et_like->esc_char, vd,
		 ...
		 OID * obj_oid)
{
   ...
   */
  if (fetch_peek_dbval (thread_p, et_comp->comp_lhs, vd, obj_oid, NULL,
			&peek_val1) != NO_ERROR)
    {
      return V_ERROR;
    }
  ...
  if (fetch_peek_dbval (thread_p, et_comp->comp_rhs, vd, obj_oid, NULL,
			&peek_val2) != NO_ERROR)
    {
      return V_ERROR;
    }
		 ...
		 OID * obj_oid)
{
  ...
  if (et_comp->comp_lhs->type == TYPE_LIST_ID)
      ...
      if (!TP_IS_SET_TYPE (DB_VALUE_DOMAIN_TYPE (peek_val1)))
	{
	  ...
	  er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE,
		  ER_TP_CANT_COERCE, 2,
		  pr_type_name (DB_VALUE_DOMAIN_TYPE (peek_val1)),
		  pr_type_name (DB_TYPE_SEQUENCE));
		 ...
		 OID * obj_oid)
{
   ...
   */
  if (fetch_peek_dbval (thread_p, et_alsm->elemset, vd, obj_oid, NULL,
			&peek_val2) != NO_ERROR)
    {
      return V_ERROR;
    }
  ...
  /* fetch item value */
  if (fetch_peek_dbval (thread_p, et_alsm->elem, vd, obj_oid, NULL,
			&peek_val1) != NO_ERROR)
    {
      return V_ERROR;
    }
		 ...
		 OID * obj_oid)
{
  ...
  /* fetch source text expression */
  if (fetch_peek_dbval (thread_p, et_like->src, vd, obj_oid, NULL,
			&peek_val1) != NO_ERROR)
    {
      return V_ERROR;
    }
  ...
  /* fetch pattern regular expression */
  if (fetch_peek_dbval (thread_p, et_like->pattern, vd, obj_oid, NULL,
			&peek_val2) != NO_ERROR)
    {
      return V_ERROR;
    }
  ...
  if (et_like->esc_char)
    {
      ...
      if (fetch_peek_dbval (thread_p,
			    et_like->esc_char, vd, obj_oid, NULL,
			    &peek_val3) != NO_ERROR)
	{
	  return V_ERROR;
	}
		  ...
		  OID * obj_oid)
{
  ...
  /* fetch source text expression */
  if (fetch_peek_dbval (thread_p, et_rlike->src, vd, obj_oid, NULL,
			&peek_val1) != NO_ERROR)
    {
      return V_ERROR;
    }
  ...
  /* fetch pattern */
  if (fetch_peek_dbval (thread_p, et_rlike->pattern, vd, obj_oid, NULL,
			&peek_val2) != NO_ERROR)
    {
      return V_ERROR;
    }
  ...
  if (fetch_peek_dbval (thread_p, et_rlike->case_sensitive, vd, obj_oid,
  ...
  if (pr->type == T_EVAL_TERM)
	...
	case T_COMP_EVAL_TERM:
	  ...
	  if (et_comp->comp_rel_op == R_EXISTS
	      || et_comp->comp_lhs->type == TYPE_LIST_ID
	      || et_comp->comp_rhs->type == TYPE_LIST_ID)
	    {
	      assert (false);	/* is impossible */
	      return NULL;
 ...
 */
DB_LOGICAL
eval_key_filter (THREAD_ENTRY * thread_p, const DB_IDXKEY * key,
		 FILTER_INFO * filterp)
{
