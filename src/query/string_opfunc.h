...
extern int db_string_substring_index (DB_VALUE * src_string,
                                      DB_VALUE * delim_string, const DB_VALUE * count, DB_VALUE * result);
...
extern int db_string_like (const DB_VALUE * src_string,
                           const DB_VALUE * pattern, const DB_VALUE * esc_char, int *result);
...
extern int db_string_replace (const DB_VALUE * src_string,
                              const DB_VALUE * srch_string, const DB_VALUE * repl_string, DB_VALUE * replaced_string);
...
extern int db_string_make_empty_typed_string (THREAD_ENTRY * thread_p,
                                              DB_VALUE * db_val,
                                              const DB_TYPE db_type, int precision, int collation_id);
...
extern void qstr_bit_to_hex_coerce (char *buffer, int buffer_size,
                                    const char *src, int src_length, int pad_flag, int *copy_size, int *truncation);
#endif
...
extern void qstr_make_typed_string (const DB_TYPE db_type, DB_VALUE * value,
                                    const int precision,
                                    const DB_C_VARCHAR src, const int s_unit, const int collation_id);
extern int db_last_day (const DB_VALUE * src_date, DB_VALUE * result_day);
extern int db_str_to_date (const DB_VALUE * src_date,
                           const DB_VALUE * src_format, const DB_VALUE * date_lang, DB_VALUE * result_date);
extern int db_time_format (const DB_VALUE * src_time,
                           const DB_VALUE * src_format, const DB_VALUE * date_lang, DB_VALUE * result_time);
...
extern int db_to_date (const DB_VALUE * src_str,
                       const DB_VALUE * format_str, const DB_VALUE * date_lang, DB_VALUE * result_date);
...
extern int db_to_datetime (const DB_VALUE * src_str,
                           const DB_VALUE * format_str, const DB_VALUE * date_lang, DB_VALUE * result_datetime);
extern int db_to_number (const DB_VALUE * src_str,
                         const DB_VALUE * format_str, const DB_VALUE * number_lang, DB_VALUE * result_num);
...
extern int db_date_format (const DB_VALUE * date_value,
                           const DB_VALUE * format, const DB_VALUE * date_lang, DB_VALUE * result);
...
extern int db_convert_to_time (const DB_VALUE * src_hour,
                               const DB_VALUE * src_minute, const DB_VALUE * src_second, DB_VALUE * result);
...
extern int db_compress_like_pattern (const DB_VALUE * const pattern,
                                     DB_VALUE * compressed_pattern, const bool has_escape_char, const char *escape_str);
...
extern int db_like_bound (const DB_VALUE * src_pattern,
                          const DB_VALUE * src_escape, DB_VALUE * const result_bound, const bool compute_lower_bound);
...
#if defined(ENABLE_UNUSED_FUNCTION)
extern int db_get_date_format (const DB_VALUE * format_str, DATETIME_FORMAT * format);
#endif
extern int db_string_index_prefix (const DB_VALUE * string1,
                                   const DB_VALUE * string2, const DB_VALUE * index_type, DB_VALUE * prefix_index);
...
extern DB_VALUE *db_value_cast_arg (DB_VALUE const *src, DB_VALUE * dest, DB_TYPE desired_type, int *errorp);
...
extern int db_get_datetime_from_dbvalue (const DB_VALUE * src_date,
                                         ...
                                         int *hour, int *minute, int *second, int *millisecond, const char **endp);
