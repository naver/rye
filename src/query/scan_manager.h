...
extern int scan_open_index_scan (THREAD_ENTRY * thread_p, SCAN_ID * scan_id,
                                 ...
                                 HEAP_CACHE_ATTRINFO * cache_pred,
                                 int num_attrs_rest,
                                 ATTR_ID * attrids_rest, HEAP_CACHE_ATTRINFO * cache_rest, QUERY_ID query_id);
...
extern void scan_init_index_scan (INDX_SCAN_ID * isidp, OID * oid_buf, int oid_buf_size);
...
extern int btree_range_search (THREAD_ENTRY * thread_p, BTID * btid,
                               KEY_VAL_RANGE * key_val_range, FILTER_INFO * filter, INDX_SCAN_ID * isidp);
#endif
