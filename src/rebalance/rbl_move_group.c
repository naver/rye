...
static int rbl_copy_shard_table (RBL_COPY_CONTEXT * ctx, TABLE_INFO * tables, int num_tbl);
static int rbl_copy_global_table (RBL_COPY_CONTEXT * ctx, TABLE_INFO * tables, int num_tbl);
static int rbl_lock_src_global_table (RBL_COPY_CONTEXT * ctx, char *table_name);
static TABLE_COPY_STMT *rbl_make_table_copy_stmt (RBL_COPY_CONTEXT * ctx, TABLE_INFO * tables, int num_table);
static int rbl_prepare_select_sql (CCI_CONN * conn, TABLE_INFO * table, TABLE_COPY_STMT * stmt);
static int rbl_prepare_insert_sql (const char *table_name, int num_cols, CCI_CONN * dest_conn, CCI_STMT * stmt);
static void rbl_free_table_copy_stmt (TABLE_COPY_STMT * info, int n);
static int rbl_copy_table_rows (RBL_COPY_CONTEXT * ctx, TABLE_COPY_STMT * stmt, char *skey);
static int rbl_copy_row (RBL_COPY_CONTEXT * ctx, CCI_STMT * sel_stmt,
                         CCI_STMT * ins_stmt, int num_cols, T_CCI_COL_INFO * col_info);
static int rbl_bind_value (CCI_STMT * sel_stmt, T_CCI_COL_INFO * col_info, CCI_STMT * ins_stmt, int index);
static int rbl_bind_null (CCI_STMT * ins_stmt, int index);
static int rbl_bind_string (CCI_STMT * sel_stmt, CCI_STMT * ins_stmt, int index);
static int rbl_bind_int (CCI_STMT * sel_stmt, CCI_STMT * ins_stmt, int index);
static int rbl_bind_bigint (CCI_STMT * sel_stmt, CCI_STMT * ins_stmt, int index);
static int rbl_bind_double (CCI_STMT * sel_stmt, CCI_STMT * ins_stmt, int index);
static int rbl_bind_date (CCI_STMT * sel_stmt, CCI_STMT * ins_stmt, int index);
static int rbl_bind_time (CCI_STMT * sel_stmt, CCI_STMT * ins_stmt, int index);
static int rbl_bind_datetime (CCI_STMT * sel_stmt, CCI_STMT * ins_stmt, int index);
static int rbl_bind_numeric (CCI_STMT * sel_stmt, CCI_STMT * ins_stmt, int index);
...
static int
rbl_copy_shard_table (RBL_COPY_CONTEXT * ctx, TABLE_INFO * tables, int num_tbl)
{
  char **skeys;
  ...
  for (i = 0; i < ctx->num_skeys; i++)
    {
      ...
      RBL_DEBUG (ARG_FILE_LINE, "shard data migration success: shard key=%s\n", skeys[i]);
...
static int
rbl_copy_global_table (RBL_COPY_CONTEXT * ctx, TABLE_INFO * tables, int num_tbl)
{
  TABLE_COPY_STMT *stmts;
  int i, error;
  ...
  for (i = 0; i < num_tbl; i++)
    {
      ...
      if (error != NO_ERROR)
        {
          RBL_DEBUG (ARG_FILE_LINE, "global data migration fail: table=%s\n", tables[i].table_name);
...
rbl_lock_src_global_table (RBL_COPY_CONTEXT * ctx, char *table_name)
{
  ...
  if (cci_prepare (ctx->src_conn, &stmt, sql, CCI_PREPARE_FROM_MIGRATOR) < 0)
    {
      RBL_ERROR (ARG_FILE_LINE, RBL_CCI_ERROR, ctx->src_conn->err_buf.err_code, ctx->src_conn->err_buf.err_msg);
...
rbl_make_pk_bind_info (CCI_CONN * conn, char *table_name, int *num_pk_cols)
{
  ...
  sprintf (sql, "SELECT c.def_order FROM db_column c, db_index_key i "
           ...
           "AND c.col_name = i.key_col_name ORDER BY i.key_order", table_name);
...
rbl_make_table_copy_stmt (RBL_COPY_CONTEXT * ctx, TABLE_INFO * tables, int num_table)
{
  TABLE_COPY_STMT *stmt_array;
  TABLE_COPY_STMT *stmt;
  int i;

  stmt_array = (TABLE_COPY_STMT *) malloc (sizeof (TABLE_COPY_STMT) * num_table);
  if (stmt_array == NULL)
    {
      RBL_ERROR (ARG_FILE_LINE, RBL_OUT_OF_MEMORY, sizeof (TABLE_COPY_STMT) * num_table);
      return NULL;
    }
  ...
  for (i = 0; i < num_table; i++)
    {
      ...
      stmt->pk_bind_info = rbl_make_pk_bind_info (ctx->src_conn, tables[i].table_name, &stmt->num_pk_cols);
      if (stmt->pk_bind_info == NULL)
        {
          goto error_exit;
        }

      if (rbl_prepare_select_sql (ctx->src_conn, &tables[i], stmt) != NO_ERROR)
        {
          goto error_exit;
        }

      if (rbl_prepare_insert_sql (tables[i].table_name, stmt->num_cols, ctx->dest_conn, &stmt->ins_stmt) != NO_ERROR)
        {
          goto error_exit;
        }
    }
...
static int
rbl_prepare_select_sql (CCI_CONN * conn, TABLE_INFO * table, TABLE_COPY_STMT * stmt)
{
  int i;
  char sql[SQL_BUF_SIZE];
  ...
  if (table->skey_col_name == NULL)
    {
      sprintf (sql, "SELECT * FROM [%s] LIMIT %d PRIMARY KEY NEXT (", table->table_name, LIMIT_COUNT);
    }
  else
    {
      sprintf (sql, "SELECT * FROM [%s] WHERE [%s] = ? "
               "LIMIT %d PRIMARY KEY NEXT (", table->table_name, table->skey_col_name, LIMIT_COUNT);
    }
  ...
  stmt->col_info = cci_get_result_info (&stmt->sel_stmt, NULL, &stmt->num_cols);
...
static int
rbl_prepare_insert_sql (const char *table_name, int num_cols, CCI_CONN * dest_conn, CCI_STMT * stmt)
{
  char sql[SQL_BUF_SIZE];
  int i;
      ...
      for (j = 0; j < info[i].num_pk_cols; j++)
        {
          if (info[i].pk_bind_info[j].is_null != true
              && (info[i].pk_bind_info[j].type == CCI_TYPE_VARCHAR || info[i].pk_bind_info[j].type == CCI_TYPE_NUMERIC))
            {
  ...
  for (i = 0, bind_idx = start_index; i < stmt->num_pk_cols; i++, bind_idx++)
    {
      if (stmt->pk_bind_info[i].is_null == true)
        {
          if (cci_bind_param (sel_stmt, bind_idx, CCI_TYPE_NULL, NULL, 0) < 0)
            {
              RBL_ERROR (ARG_FILE_LINE, RBL_CCI_ERROR, sel_stmt->err_buf.err_code, sel_stmt->err_buf.err_msg);
              return ER_FAILED;
            }
      ...
      if (cci_bind_param (sel_stmt, bind_idx, stmt->pk_bind_info[i].type, val, 0) < 0)
        {
          RBL_ERROR (ARG_FILE_LINE, RBL_CCI_ERROR, sel_stmt->err_buf.err_code, sel_stmt->err_buf.err_msg);
          return ER_FAILED;
        }
...
static int
rbl_copy_table_rows (RBL_COPY_CONTEXT * ctx, TABLE_COPY_STMT * stmt, char *skey)
{
  CCI_STMT *sel_stmt;
  ...
  do
    {
      ...
      if (skey != NULL)
        {
          if (cci_bind_param (sel_stmt, bind_index, CCI_TYPE_VARCHAR, skey, 0) < 0)
            {
              RBL_ERROR (ARG_FILE_LINE, RBL_CCI_ERROR, sel_stmt->err_buf.err_code, sel_stmt->err_buf.err_msg);
              goto error_exit;
            }
      ...
      for (i = 0; i < n; i++)
        {
          ...
          if (rbl_copy_row (ctx, sel_stmt, &stmt->ins_stmt, stmt->num_cols, stmt->col_info) != NO_ERROR)
            {
              goto error_exit;
            }
        }
...
static int
rbl_copy_row (RBL_COPY_CONTEXT * ctx, CCI_STMT * sel_stmt, CCI_STMT * ins_stmt, int num_cols, T_CCI_COL_INFO * col_info)
{
  int i, error;
  ...
  if (error < 0)
    {
      if (ins_stmt->err_buf.err_code == ER_BTREE_UNIQUE_FAILED)
        ...
        {
          RBL_ERROR (ARG_FILE_LINE, RBL_CCI_ERROR, ins_stmt->err_buf.err_code, ins_stmt->err_buf.err_msg);
          return ER_FAILED;
        }
    }
  else
...
static int
rbl_bind_value (CCI_STMT * sel_stmt, T_CCI_COL_INFO * col_info, CCI_STMT * ins_stmt, int index)
{
  T_CCI_TYPE type;
...
rbl_dump_stats (UNUSED_ARG RBL_COPY_CONTEXT * ctx)
{
  ...
  if (ctx->num_skeys == ctx->num_copied_keys || (ctx->num_copied_keys % 100) == 0)
    {
      ...
      RBL_DEBUG (ARG_FILE_LINE, "Migration status: \n"
                 ...
                 (ctx->num_copied_rows + ctx->sync_ctx->num_synced_rows) / elapsed_time.tv_sec);
    }
#endif
}
