...
extern int rbl_conf_init (const char *mgmt_host, int mgmt_port,
                          ...
                          const char *dest_dbname, int src_node_ha_staus, bool copy_schema);
...
extern int rbl_conf_update_src_groupid (RBL_COPY_CONTEXT * ctx, bool on_off, bool do_commit);
extern int rbl_conf_update_dest_groupid (RBL_COPY_CONTEXT * ctx);
extern int rbl_conf_check_repl_delay (CCI_CONN * conn);
extern int rbl_conf_insert_gid_removed_info_srcdb (int group_id, bool run_slave);
#endif /* RBL_CONF_H_ */
