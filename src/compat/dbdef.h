...
/*
 * dbdef.h -Supporting definitions for the Rye API functions.
 *
 */
...
typedef struct db_shardkey_info SHARDKEY_INFO;
struct db_shardkey_info
{
  DB_NODE *value;
};
...
struct db_session
{
  ...
  /* shard info */
  int groupid;
  SHARDKEY_INFO *shardkeys;
  int num_shardkeys;
  int shardkey_coll_id;
  bool shardkey_required;
  bool shardkey_exhausted;
  bool from_migrator;
};
...
/* These are handles to attribute descriptors that can
