...
extern int db_update_group_id (int migrator_id, int group_id, int target, int on_off);
...
extern int db_constrain_non_null (DB_OBJECT * classobj, const char *name, int on_or_off);
#endif
#if 0                           /* unused */
...
extern DB_ATTRIBUTE *db_get_attribute_by_name (const char *class_name, const char *atrribute_name);
...
extern int db_get_class_num_objs_and_pages (DB_OBJECT * classmop, int approximation, int64_t * nobjs, int *npages);
...
extern int dbt_constrain_non_null (DB_CTMPL * def, const char *name, int on_or_off);
 ...
 */
extern int db_get_attribute_descriptor (DB_OBJECT * obj, const char *attname, int for_update, DB_ATTDESC ** descriptor);
...
extern void db_session_set_autocommit_mode (DB_SESSION * session, bool autocommit_mode);
...
extern int db_get_shard_key_values (DB_SESSION * session,
                                    ...
                                    char **value_out_buf,
                                    int value_out_buf_size, int *pos_out_buf, int pos_out_buf_size);
extern bool db_is_select_for_update (DB_SESSION * session);

extern int db_execute (const char *RSQL_query, DB_QUERY_RESULT ** result, DB_QUERY_ERROR * query_error);

extern int db_execute_statement (DB_SESSION * session, DB_QUERY_RESULT ** result);

extern int db_execute_and_keep_statement (DB_SESSION * session, DB_QUERY_RESULT ** result);
...
extern int db_update_persist_conf_file (const char *proc_name,
                                        const char *sect_name, const char *key, const char *value);
extern int db_delete_proc_persist_conf_file (const char *proc_name);
extern int db_delete_sect_persist_conf_file (const char *proc_name, const char *sect_name);
extern int db_delete_key_persist_conf_file (const char *proc_name, const char *sect_name, const char *key);
extern int db_read_server_persist_conf_file (const char *sect_name, const bool reload);
extern int db_read_broker_persist_conf_file (INI_TABLE * ini);
extern int db_dump_persist_conf_file (FILE * fp, const char *proc_name, const char *sect_name);

extern int db_set_system_parameters (char *prm_names, int len, const char *data, const bool persist);
extern int db_get_system_parameters (char *data, int len);
extern int db_get_system_parameter_value (char *value, int max_len, const char *param_name);
...
extern void db_session_set_from_migrator (DB_SESSION * session, bool from_migrator);
