...
#define OR_GET_OID(ptr, oid) \
   do { \
     ...
     (oid)->groupid = OR_GET_INT(((char *)(ptr)) + OR_OID_GROUPID); \
   } while (0)
...
#define OR_PUT_OID(ptr, oid) \
   do { \
     ...
     OR_PUT_INT(((char *)(ptr)) + OR_OID_GROUPID, (oid)->groupid); \
   } while (0)
...
#define OR_PUT_NULL_OID(ptr) \
   do { \
     ...
     OR_PUT_INT(((char *)(ptr)) + OR_OID_GROUPID,  NULL_GROUPID); \
   } while (0)
...
#define OR_GET_GRPID(ptr) \
  (OR_GET_INT((ptr) + OR_GRPID_OFFSET))
...
/* class */
#define ORC_HFID_FILEID_OFFSET		8
#define ORC_HFID_VOLID_OFFSET		12
#define ORC_HFID_PAGEID_OFFSET		16
#define ORC_REPID_OFFSET		20
#define ORC_FIXED_COUNT_OFFSET		24
#define ORC_VARIABLE_COUNT_OFFSET       28
#define ORC_FIXED_LENGTH_OFFSET		32
...
#define ORC_ATTRIBUTES_INDEX		4
#define ORC_QUERY_SPEC_INDEX            5
#define ORC_CONSTRAINTS_INDEX	        6
...
#define ORC_ATT_VAR_ATT_COUNT           5
...
#define ORC_ATT_PROPERTIES_INDEX        4
...
/* class constraint */
#define ORC_CLASS_CONSTRAINTS_TYPE_OFFSET       0
#define ORC_CLASS_CONSTRAINTS_STATUS_OFFSET     4

#define ORC_CLASS_CONSTRAINTS_VAR_ATT_COUNT 3
#define ORC_CLASS_CONSTRAINTS_NAME_INDEX  0
#define ORC_CLASS_CONSTRAINTS_BTID_INDEX  1
#define ORC_CLASS_CONSTRAINTS_ATTS_INDEX  2

#define ORC_CLASS_CONSTRAINTS_ATT_ATTID_OFFSET       0
#define ORC_CLASS_CONSTRAINTS_ATT_ASCDESC_OFFSET     4

#define ORC_CLASS_CONSTRAINTS_ATTS_VAR_ATT_COUNT 1
#define ORC_CLASS_CONSTRAINTS_ATTS_RESERVED_INDEX  0
...
/*
 * DB_REFERENCE
 *    This is a common structure header used by DB_SET.
...
extern int or_grp_id (const RECDES * record);
#if defined(ENABLE_UNUSED_FUNCTION)
extern int or_set_grp_id (RECDES * record, int grpid);
#endif
...
extern char *or_pack_hfid (char *ptr, const HFID * hfid);
...
#endif
extern char *or_pack_listid (char *ptr, const void *listid);
extern char *or_pack_lock (char *ptr, LOCK lock);

/* should be using the or_pack_value family instead ! */
extern char *or_pack_db_value (char *buffer, const DB_VALUE * var);
extern char *or_unpack_db_value (char *buffer, DB_VALUE * var);
extern int or_db_value_size (const DB_VALUE * var);

extern char *or_pack_db_idxkey (char *buffer, DB_IDXKEY * key);
extern char *or_unpack_db_idxkey (char *buffer, DB_IDXKEY * key);
extern int or_db_idxkey_size (DB_IDXKEY * key);
...
extern char *or_unpack_string_nocopy (char *ptr, const char **string);
...
extern char *or_packed_get_varchar (OR_BUF * buf, int *length_ptr);
#if defined(ENABLE_UNUSED_FUNCTION)
