			...
			const char *dbname)
{
  ...
  if (dbname == NULL)
    ...
    err = monitor_copy_global_stats (&cur_global_stats,}
				     ...
				     if (cumulative)
				     {
				     mnt_server_dump_stats (&cur_global_stats,
							    stream, substr);}
				     else
				     {
					 ...
					 NO_ERROR)
				     {
				     mnt_server_dump_stats (&diff_result,
							    stream, substr);}
 ...
 */
				     ...
				     for (i = 0;
					  i <
					  MNT_SIZE_OF_SERVER_EXEC_STATS - 1;
					  i++)
				     {
 ...
 */
				     void
				     xmnt_server_clear_stats (THREAD_ENTRY *
							      thread_p,
							      MNT_SERVER_ITEM
							      item)
				     {
				     ...
				     (MONITOR_STATS * monitor_stats,
				      MNT_SERVER_EXEC_STATS * server_stats)
				     {
				     int i;
				     for (i = 0;
					  i < MNT_SIZE_OF_SERVER_EXEC_STATS;
					  i++)
				     {
				     monitor_stats[i].value =
				     server_stats->values[i];
				     monitor_stats[i].acc_time =
				     server_stats->acc_time[i];}
				     }
 ...
 */
				     void
				     mnt_stats_counter (THREAD_ENTRY *
							thread_p,
				     ...
				     tran_index =
				     logtb_get_current_tran_index (thread_p);
				     monitor_stats_counter (tran_index, item,
							    value, 0);}
 ...
 */
				     ...
				     (tran_index, item, value, start_time);
				     parent_item =
				     MNT_GET_PARENT_ITEM_FETCHES (item);
				     if (parent_item != item)
				     {
				     assert (parent_item ==
					     MNT_STATS_DATA_PAGE_FETCHES);
				     monitor_stats_count_with_time
				     (tran_index, parent_item, value,
				      start_time);
				     thread_mnt_track_counter (thread_p,
							       value,
							       start_time);}
				     }
 ...
 */
				     void
				     mnt_stats_gauge (THREAD_ENTRY * thread_p,
						      MNT_SERVER_ITEM item,
						      INT64 value)
				     {
				     int tran_index;
				     ...
				     tran_index =
				     logtb_get_current_tran_index (thread_p);
				     monitor_stats_gauge (tran_index, item,
							  value);}
 ...
 */
				     ...
				     assert (tran_index >= 0);
				     return
				     monitor_get_stats_with_time (acc_time,
								  tran_index,
								  item);}
 ...
 */
				     INT64
				     mnt_get_stats (THREAD_ENTRY * thread_p,
						    MNT_SERVER_ITEM item)
				     {
				     return mnt_get_stats_with_time (thread_p,
								     item,
								     NULL);}
 ...
 */
				     static void
				     mnt_calc_hit_ratio (MNT_SERVER_EXEC_STATS
							 * stats)
				     {
				     if (stats->
 ...
 */
				     ...
				     {
				     int i;
				     if (!diff_stats || !new_stats
					 || !old_stats)
				     {
				     assert (0); return ER_FAILED;}
				     ...
				     {
				     /* calc diff values */
				     if (IS_CUMMULATIVE_VALUE
					 (mnt_Stats_info[i].value_type))
				     {
				     if (new_stats->values[i] >=
					 old_stats->values[i])
				     {
				     diff_stats->values[i] =
				     (new_stats->values[i] -
				     ...
				     /* calc diff acc_time */
				     assert (new_stats->acc_time[i] >=
					     old_stats->acc_time[i]);
				     if (new_stats->acc_time[i] >=
					 old_stats->acc_time[i])
				     {
				     diff_stats->acc_time[i] =
				     (new_stats->acc_time[i] -
				      old_stats->acc_time[i]);}
				     else
				     {
 ...
 */
				     bool
				     mnt_stats_is_cumulative (MNT_SERVER_ITEM
							      item)
				     {
				     if (item < MNT_SIZE_OF_SERVER_EXEC_STATS)
				     {
				     return (IS_CUMMULATIVE_VALUE
					     (mnt_Stats_info[item].
					      value_type));}
				     else
				     {
 ...
 */
				     ...
				     MNT_SERVER_ITEM
				     mnt_csect_type_to_server_item (const
								    CSECT_TYPE
								    ctype)
				     {
...
return MNT_STATS_CSECT_ER_LOG_FILE; case CSECT_ER_MSG_CACHE:	/* 1 */
return MNT_STATS_CSECT_ER_MSG_CACHE; case CSECT_WFG:	/* 2 */
return MNT_STATS_CSECT_WFG; case CSECT_LOG:	/* 3 */
return MNT_STATS_CSECT_LOG; case CSECT_LOG_BUFFER:	/* 4 */
return MNT_STATS_CSECT_LOG_BUFFER; case CSECT_LOG_ARCHIVE:	/* 5 */
return MNT_STATS_CSECT_LOG_ARCHIVE; case CSECT_LOCATOR_SR_CLASSNAME_TABLE:	/* 6 */
return MNT_STATS_CSECT_SR_LOCATOR_CLASSNAME_TABLE; case CSECT_FILE_NEWFILE:	/* 7 */
return MNT_STATS_CSECT_FILE_NEWFILE; case CSECT_QPROC_QUERY_TABLE:	/* 8 */
return MNT_STATS_CSECT_QPROC_QUERY_TABLE; case CSECT_BOOT_SR_DBPARM:	/* 9 */
return MNT_STATS_CSECT_BOOT_SR_DBPARM; case CSECT_DISK_REFRESH_GOODVOL:	/* 10 */
return MNT_STATS_CSECT_DISK_REFRESH_GOODVOL; case CSECT_CNV_FMT_LEXER:	/* 11 */
return MNT_STATS_CSECT_CNV_FMT_LEXER; case CSECT_CT_OID_TABLE:	/* 12 */
return MNT_STATS_CSECT_CT_OID_TABLE; case CSECT_HA_SERVER_STATE:	/* 13 */
return MNT_STATS_CSECT_HA_SERVER_STATE; case CSECT_SESSION_STATE:	/* 14 */
return MNT_STATS_CSECT_SESSION_STATE; case CSECT_ACL:	/* 15 */
return MNT_STATS_CSECT_ACL; case CSECT_EVENT_LOG_FILE:	/* 16 */
return MNT_STATS_CSECT_EVENT_LOG_FILE; case CSECT_ACCESS_STATUS:	/* 17 */
return MNT_STATS_CSECT_ACCESS_STATUS; case CSECT_TEMPFILE_CACHE:	/* 18 */
return MNT_STATS_CSECT_TEMPFILE_CACHE; case CSECT_CSS_ACTIVE_CONN:	/* 19 */
return MNT_STATS_CSECT_CSS_ACTIVE_CONN; case CSECT_CSS_FREE_CONN:	/* 20 */
return MNT_STATS_CSECT_CSS_FREE_CONN; case CSECT_UNKNOWN:	/* 21 */
assert (false); return MNT_STATS_CSECT_UNKNOWN; default:
				     break;}
				     ...
				     (const CSECT_TYPE ctype)
				     {
				     assert (ctype < CSECT_LAST);
				     switch (ctype)
				     {
case CSECT_ER_LOG_FILE:	/* 0 */
return MNT_STATS_CSECT_WAITS_ER_LOG_FILE; case CSECT_ER_MSG_CACHE:	/* 1 */
return MNT_STATS_CSECT_WAITS_ER_MSG_CACHE; case CSECT_WFG:	/* 2 */
return MNT_STATS_CSECT_WAITS_WFG; case CSECT_LOG:	/* 3 */
return MNT_STATS_CSECT_WAITS_LOG; case CSECT_LOG_BUFFER:	/* 4 */
return MNT_STATS_CSECT_WAITS_LOG_BUFFER; case CSECT_LOG_ARCHIVE:	/* 5 */
return MNT_STATS_CSECT_WAITS_LOG_ARCHIVE; case CSECT_LOCATOR_SR_CLASSNAME_TABLE:	/* 6 */
return MNT_STATS_CSECT_WAITS_SR_LOCATOR_CLASSNAME_TABLE; case CSECT_FILE_NEWFILE:	/* 7 */
return MNT_STATS_CSECT_WAITS_FILE_NEWFILE; case CSECT_QPROC_QUERY_TABLE:	/* 8 */
return MNT_STATS_CSECT_WAITS_QPROC_QUERY_TABLE; case CSECT_BOOT_SR_DBPARM:	/* 9 */
return MNT_STATS_CSECT_WAITS_BOOT_SR_DBPARM; case CSECT_DISK_REFRESH_GOODVOL:	/* 10 */
return MNT_STATS_CSECT_WAITS_DISK_REFRESH_GOODVOL; case CSECT_CNV_FMT_LEXER:	/* 11 */
return MNT_STATS_CSECT_WAITS_CNV_FMT_LEXER; case CSECT_CT_OID_TABLE:	/* 12 */
return MNT_STATS_CSECT_WAITS_CT_OID_TABLE; case CSECT_HA_SERVER_STATE:	/* 13 */
return MNT_STATS_CSECT_WAITS_HA_SERVER_STATE; case CSECT_SESSION_STATE:	/* 14 */
return MNT_STATS_CSECT_WAITS_SESSION_STATE; case CSECT_ACL:	/* 15 */
return MNT_STATS_CSECT_WAITS_ACL; case CSECT_EVENT_LOG_FILE:	/* 16 */
return MNT_STATS_CSECT_WAITS_EVENT_LOG_FILE; case CSECT_ACCESS_STATUS:	/* 17 */
return MNT_STATS_CSECT_WAITS_ACCESS_STATUS; case CSECT_TEMPFILE_CACHE:	/* 18 */
return MNT_STATS_CSECT_WAITS_TEMPFILE_CACHE; case CSECT_CSS_ACTIVE_CONN:	/* 19 */
return MNT_STATS_CSECT_WAITS_CSS_ACTIVE_CONN; case CSECT_CSS_FREE_CONN:	/* 20 */
return MNT_STATS_CSECT_WAITS_CSS_FREE_CONN; case CSECT_UNKNOWN:	/* 21 */
assert (false); return MNT_STATS_CSECT_WAITS_UNKNOWN; default:
				     break;}
				     ...
				     (const MNT_SERVER_ITEM item)
				     {
				     switch (item)
				     {
case MNT_STATS_DATA_PAGE_FETCHES_FILE_HEADER:	/* 1 file header page             */
return PAGE_FILE_HEADER; case MNT_STATS_DATA_PAGE_FETCHES_FILE_TAB:	/* 2 file allocset table page             */
return PAGE_FILE_TAB; case MNT_STATS_DATA_PAGE_FETCHES_HEAP_HEADER:	/* 3 heap header page                            */
return PAGE_HEAP_HEADER; case MNT_STATS_DATA_PAGE_FETCHES_HEAP:	/* 4 heap page                            */
return PAGE_HEAP; case MNT_STATS_DATA_PAGE_FETCHES_VOLHEADER:	/* 5 volume header page                   */
return PAGE_VOLHEADER; case MNT_STATS_DATA_PAGE_FETCHES_VOLBITMAP:	/* 6 volume bitmap page                   */
return PAGE_VOLBITMAP; case MNT_STATS_DATA_PAGE_FETCHES_XASL:	/* 7 xasl stream page                     */
return PAGE_XASL; case MNT_STATS_DATA_PAGE_FETCHES_QRESULT:	/* 8 query result page                    */
return PAGE_QRESULT; case MNT_STATS_DATA_PAGE_FETCHES_EHASH:	/* 9 ehash bucket/dir page                */
return PAGE_EHASH; case MNT_STATS_DATA_PAGE_FETCHES_OVERFLOW:	/* 10 overflow page (with ovf_keyval)      */
return PAGE_OVERFLOW; case MNT_STATS_DATA_PAGE_FETCHES_AREA:	/* 11 area page                            */
return PAGE_AREA; case MNT_STATS_DATA_PAGE_FETCHES_CATALOG:	/* 12 catalog page                         */
return PAGE_CATALOG; case MNT_STATS_DATA_PAGE_FETCHES_BTREE_ROOT:	/* 13 b+tree index root page                    */
return PAGE_BTREE_ROOT; case MNT_STATS_DATA_PAGE_FETCHES_BTREE:	/* 14 b+tree index page                    */
return PAGE_BTREE; case MNT_STATS_DATA_PAGE_FETCHES_UNKNOWN:	/* 0 unknown                     */
return PAGE_UNKNOWN; default:
				     break;}

				     assert (false); return PAGE_UNKNOWN;}
...
return MNT_STATS_DATA_PAGE_FETCHES_FILE_HEADER; case PAGE_FILE_TAB:	/* 2 file allocset table page             */
return MNT_STATS_DATA_PAGE_FETCHES_FILE_TAB; case PAGE_HEAP_HEADER:	/* 3 heap header page               */
return MNT_STATS_DATA_PAGE_FETCHES_HEAP_HEADER; case PAGE_HEAP:	/* 4 heap page                            */
return MNT_STATS_DATA_PAGE_FETCHES_HEAP; case PAGE_VOLHEADER:	/* 5 volume header page                   */
return MNT_STATS_DATA_PAGE_FETCHES_VOLHEADER; case PAGE_VOLBITMAP:	/* 6 volume bitmap page                   */
return MNT_STATS_DATA_PAGE_FETCHES_VOLBITMAP; case PAGE_XASL:	/* 7 xasl stream page                     */
return MNT_STATS_DATA_PAGE_FETCHES_XASL; case PAGE_QRESULT:	/* 8 query result page                    */
return MNT_STATS_DATA_PAGE_FETCHES_QRESULT; case PAGE_EHASH:	/* 9 ehash bucket/dir page                */
return MNT_STATS_DATA_PAGE_FETCHES_EHASH; case PAGE_OVERFLOW:	/* 10 overflow page                        */
return MNT_STATS_DATA_PAGE_FETCHES_OVERFLOW; case PAGE_AREA:	/* 11 area page                            */
return MNT_STATS_DATA_PAGE_FETCHES_AREA; case PAGE_CATALOG:	/* 12 catalog page                         */
return MNT_STATS_DATA_PAGE_FETCHES_CATALOG; case PAGE_BTREE_ROOT:	/* 13 b+tree index root page               */
return MNT_STATS_DATA_PAGE_FETCHES_BTREE_ROOT; case PAGE_BTREE:	/* 14 b+tree index page                    */
return MNT_STATS_DATA_PAGE_FETCHES_BTREE; case PAGE_UNKNOWN:	/* 0 used for initialized page            */
return MNT_STATS_DATA_PAGE_FETCHES_UNKNOWN; default:
				     break;}
				     ...
				     {
case PAGE_FILE_HEADER:		/* 1 file header page                     */
return MNT_STATS_DATA_PAGE_FETCHES_WAITS_FILE_HEADER; case PAGE_FILE_TAB:	/* 2 file allocset table page             */
return MNT_STATS_DATA_PAGE_FETCHES_WAITS_FILE_TAB; case PAGE_HEAP_HEADER:	/* 3 heap header page               */
return MNT_STATS_DATA_PAGE_FETCHES_WAITS_HEAP_HEADER; case PAGE_HEAP:	/* 4 heap page                            */
return MNT_STATS_DATA_PAGE_FETCHES_WAITS_HEAP; case PAGE_VOLHEADER:	/* 5 volume header page                   */
return MNT_STATS_DATA_PAGE_FETCHES_WAITS_VOLHEADER; case PAGE_VOLBITMAP:	/* 6 volume bitmap page                   */
return MNT_STATS_DATA_PAGE_FETCHES_WAITS_VOLBITMAP; case PAGE_XASL:	/* 7 xasl stream page                     */
return MNT_STATS_DATA_PAGE_FETCHES_WAITS_XASL; case PAGE_QRESULT:	/* 8 query result page                    */
return MNT_STATS_DATA_PAGE_FETCHES_WAITS_QRESULT; case PAGE_EHASH:	/* 9 ehash bucket/dir page                */
return MNT_STATS_DATA_PAGE_FETCHES_WAITS_EHASH; case PAGE_OVERFLOW:	/* 10 overflow page                        */
return MNT_STATS_DATA_PAGE_FETCHES_WAITS_OVERFLOW; case PAGE_AREA:	/* 11 area page                            */
return MNT_STATS_DATA_PAGE_FETCHES_WAITS_AREA; case PAGE_CATALOG:	/* 12 catalog page                         */
return MNT_STATS_DATA_PAGE_FETCHES_WAITS_CATALOG; case PAGE_BTREE_ROOT:	/* 13 b+tree index root page               */
return MNT_STATS_DATA_PAGE_FETCHES_WAITS_BTREE_ROOT; case PAGE_BTREE:	/* 14 b+tree index page                    */
return MNT_STATS_DATA_PAGE_FETCHES_WAITS_BTREE; case PAGE_UNKNOWN:	/* 0 used for initialized page            */
return MNT_STATS_DATA_PAGE_FETCHES_WAITS_UNKNOWN; default:
				     break;}
