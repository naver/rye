  ...
  if (key != NULL)
    {
      switch (db_value_type (val))
        ...
        case DB_TYPE_INTEGER:
          hash = (unsigned int) DB_GET_INTEGER (val);
          break;
        case DB_TYPE_BIGINT:
          {
            DB_BIGINT bigint;
        ...
        case DB_TYPE_VARCHAR:
          ...
          break;
        case DB_TYPE_VARBIT:
          hash = mht_1str_pseudo_key (db_pull_varbit (val, &t_n), -1);
          break;
        case DB_TYPE_TIME:
...
mht_compare_dbvalues_are_equal (const void *key1, const void *key2)
{
  ...
  c = tp_value_compare ((DB_VALUE *) key1, (DB_VALUE *) key2, 0, 1, &can_compare);
...
/*
 * mht_rehash - rehash all entries of a hash table
 *   return: error code
      ...
      switch (db_value_type (val))
        ...
        default:               /* impossible */
          ...
          er_log_debug (ARG_FILE_LINE, "mht_get_hash_number: ERROR type = %d"
                        " is Unsupported partition column type.", db_value_type (val));
#if defined(NDEBUG)
          hashcode = 0;
#else /* NDEBUG */
          /* debugging purpose */
