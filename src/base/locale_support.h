...
#define LOC_DATA_COLL_RYE_TAILOR_COUNT_GROW 8
...
/*
 * RYE_TAILOR_RULE - Structure used for representing the rules for
 *			absolute tailoring e.g. manually setting the weights
 *			and collation elements for unicode character or
 *  			character ranges.
*/
typedef struct rye_tailor_rule RYE_TAILOR_RULE;
struct rye_tailor_rule
{
...
struct coll_tailoring
{
  ...
  RYE_TAILOR_RULE *rye_rules;   /* absolute tailoring rules */
  int rye_count_rules;          /* # of tailorings */
  int rye_max_rules;            /* # of max (allocated tailorings) */
