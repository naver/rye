	   ...
	   bh_key_comparator cmp_func, BH_CMP_ARG cmp_arg)
{
  ...
  heap = (BINARY_HEAP *) malloc (sizeof (BINARY_HEAP));
  if (heap == NULL)
    {
      return NULL;
    }

  heap->members = (BH_ELEM *) malloc (max_capacity * sizeof (BH_ELEM));
 ...
 */
void
bh_destroy (UNUSED_ARG THREAD_ENTRY * thread_p, BINARY_HEAP * heap)
{
  if (heap != NULL)
    {
      if (heap->members != NULL)
