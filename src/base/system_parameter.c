...
static INT64 prm_max_flush_size_per_second_upper = (INT64) INT_MAX * IO_MAX_PAGE_SIZE;
...
static INT64 prm_pb_sync_on_flush_size_upper = (INT64) INT_MAX * IO_MAX_PAGE_SIZE;
...
static int prm_print_value (const SYSPRM_PARAM * prm, void *prm_value, char *buf, size_t len);
...
static int prm_print_bigint_value (char *buf, size_t len, INT64 val, const SYSPRM_PARAM * prm);
...
static int prm_print_keyword_value (char *buf, size_t len, int val, const SYSPRM_PARAM * prm);
...
static SYSPRM_ERR sysprm_generate_new_value (SYSPRM_PARAM * prm,
                                             const char *value, bool check, SYSPRM_VALUE * new_value);
...
static int sysprm_print_sysprm_value (PARAM_ID prm_id, SYSPRM_VALUE value,
                                      char *buf, size_t len, PRM_PRINT_MODE print_mode);
...
static void sysprm_init_param (PARAM_ID param_id, const char *name,
                               ...
                               void *default_value, void *value, void *upper_limit, void *lower_limit);
...
static int prm_split_node_str_internal (PRM_NODE_LIST * node_list, const char *node_list_str, bool include_local_host);
...
sysprm_initialize_prm_def ()
{
  sysprm_init_param (PRM_ID_ER_LOG_DEBUG, PRM_NAME_ER_LOG_DEBUG,
                     (PRM_FOR_CLIENT | PRM_FOR_SERVER | PRM_USER_CHANGE |
                      PRM_HIDDEN), PRM_BOOLEAN, &prm_er_log_debug_default, &PRM_ER_LOG_DEBUG, NULL, NULL);

  sysprm_init_param (PRM_ID_ER_LOG_LEVEL, PRM_NAME_ER_LOG_LEVEL,
                     (PRM_FOR_CLIENT | PRM_FOR_SERVER | PRM_USER_CHANGE),
                     PRM_KEYWORD,
                     &prm_er_log_level_default, &PRM_ER_LOG_LEVEL, &prm_er_log_level_upper, &prm_er_log_level_lower);

  sysprm_init_param (PRM_ID_ER_LOG_WARNING, PRM_NAME_ER_LOG_WARNING,
                     (PRM_FOR_CLIENT | PRM_FOR_SERVER | PRM_USER_CHANGE),
                     PRM_BOOLEAN, &prm_er_log_warning_default, &PRM_ER_LOG_WARNING, NULL, NULL);

  sysprm_init_param (PRM_ID_ER_EXIT_ASK, PRM_NAME_ER_EXIT_ASK,
                     (PRM_FOR_CLIENT | PRM_FOR_SERVER | PRM_USER_CHANGE |
                      PRM_HIDDEN), PRM_INTEGER, &prm_er_exit_ask_default, &PRM_ER_EXIT_ASK, NULL, NULL);

  sysprm_init_param (PRM_ID_ER_LOG_SIZE, PRM_NAME_ER_LOG_SIZE,
                     (PRM_FOR_CLIENT | PRM_FOR_SERVER | PRM_USER_CHANGE |
                      PRM_SIZE_UNIT), PRM_BIGINT,
                     &prm_er_log_size_default, &PRM_ER_LOG_SIZE, NULL, &prm_er_log_size_lower);
  ...
  sysprm_init_param (PRM_ID_SORT_BUFFER_SIZE, PRM_NAME_SORT_BUFFER_SIZE,
                     ...
                     &prm_sort_buffer_size_default, &PRM_SORT_BUFFER_SIZE, NULL, &prm_sort_buffer_size_lower);
  ...
  sysprm_init_param (PRM_ID_PB_BUFFER_FLUSH_RATIO,
                     ...
                     &PRM_PB_BUFFER_FLUSH_RATIO, &prm_pb_buffer_flush_ratio_upper, &prm_pb_buffer_flush_ratio_lower);

  sysprm_init_param (PRM_ID_PAGE_BUFFER_SIZE,
                     PRM_NAME_PAGE_BUFFER_SIZE,
                     (PRM_FOR_SERVER | PRM_SIZE_UNIT | PRM_RELOADABLE),
                     PRM_BIGINT,
                     &prm_page_buffer_size_default, &PRM_PAGE_BUFFER_SIZE, NULL, &prm_page_buffer_size_lower);
  ...
  sysprm_init_param (PRM_ID_HF_MAX_BESTSPACE_ENTRIES,
                     ...
                     &PRM_HF_MAX_BESTSPACE_ENTRIES, NULL, &prm_hf_max_bestspace_entries_lower);
  ...
  sysprm_init_param (PRM_ID_BT_OID_BUFFER_SIZE,
                     ...
                     &prm_bt_oid_buffer_size_upper, &prm_bt_oid_buffer_size_lower);

  sysprm_init_param (PRM_ID_BOSR_MAXTMP_SIZE,
                     PRM_NAME_BOSR_MAXTMP_SIZE,
                     (PRM_FOR_SERVER | PRM_SIZE_UNIT),
                     PRM_BIGINT, &prm_bosr_maxtmp_size_default, &PRM_BOSR_MAXTMP_SIZE, NULL, NULL);

  sysprm_init_param (PRM_ID_LK_TIMEOUT,
                     PRM_NAME_LK_TIMEOUT,
                     (PRM_FOR_CLIENT | PRM_USER_CHANGE | PRM_TIME_UNIT),
                     PRM_BIGINT,
                     &prm_lk_timeout_default, &PRM_LK_TIMEOUT, &prm_lk_timeout_upper, &prm_lk_timeout_lower);
  ...
  sysprm_init_param (PRM_ID_LK_RUN_DEADLOCK_INTERVAL,
                     ...
                     &PRM_LK_RUN_DEADLOCK_INTERVAL, NULL, &prm_lk_run_deadlock_interval_lower);

  sysprm_init_param (PRM_ID_LOG_BUFFER_SIZE,
                     PRM_NAME_LOG_BUFFER_SIZE,
                     (PRM_FOR_SERVER | PRM_SIZE_UNIT | PRM_RELOADABLE),
                     PRM_BIGINT, &prm_log_buffer_size_default, &PRM_LOG_BUFFER_SIZE, NULL, &prm_log_buffer_size_lower);

  sysprm_init_param (PRM_ID_LOG_CHECKPOINT_SIZE,
                     PRM_NAME_LOG_CHECKPOINT_SIZE,
                     (PRM_FOR_SERVER | PRM_USER_CHANGE | PRM_SIZE_UNIT |
                      PRM_RELOADABLE), PRM_BIGINT,
                     &prm_log_checkpoint_size_default, &PRM_LOG_CHECKPOINT_SIZE, NULL, &prm_log_checkpoint_size_lower);
  ...
  sysprm_init_param (PRM_ID_LOG_CHECKPOINT_INTERVAL,
                     ...
                     &PRM_LOG_CHECKPOINT_INTERVAL, NULL, &prm_log_checkpoint_interval_lower);
  ...
  sysprm_init_param (PRM_ID_LOG_CHECKPOINT_FLUSH_INTERVAL,
                     ...
                     &PRM_LOG_CHECKPOINT_FLUSH_INTERVAL, NULL, &prm_log_checkpoint_flush_interval_lower);

  sysprm_init_param (PRM_ID_COMMIT_ON_SHUTDOWN,
                     PRM_NAME_COMMIT_ON_SHUTDOWN,
                     (PRM_FOR_CLIENT | PRM_USER_CHANGE),
                     PRM_BOOLEAN, &prm_commit_on_shutdown_default, &PRM_COMMIT_ON_SHUTDOWN, NULL, NULL);

  sysprm_init_param (PRM_ID_SHUTDOWN_WAIT_TIME,
                     PRM_NAME_SHUTDOWN_WAIT_TIME,
                     (PRM_FOR_SERVER | PRM_USER_CHANGE | PRM_TIME_UNIT |
                      PRM_HIDDEN), PRM_BIGINT,
                     &prm_shutdown_wait_time_default, &PRM_SHUTDOWN_WAIT_TIME, NULL, &prm_shutdown_wait_time_lower);

  sysprm_init_param (PRM_ID_RSQL_AUTO_COMMIT,
                     PRM_NAME_RSQL_AUTO_COMMIT,
                     (PRM_FOR_CLIENT | PRM_USER_CHANGE),
                     PRM_BOOLEAN, &prm_rsql_auto_commit_default, &PRM_RSQL_AUTO_COMMIT, NULL, NULL);

  sysprm_init_param (PRM_ID_WS_HASHTABLE_SIZE,
                     PRM_NAME_WS_HASHTABLE_SIZE,
                     (PRM_FOR_CLIENT | PRM_HIDDEN),
                     PRM_INTEGER,
                     &prm_ws_hashtable_size_default, &PRM_WS_HASHTABLE_SIZE, NULL, &prm_ws_hashtable_size_lower);

  sysprm_init_param (PRM_ID_WS_MEMORY_REPORT,
                     PRM_NAME_WS_MEMORY_REPORT,
                     (PRM_FOR_CLIENT | PRM_USER_CHANGE | PRM_HIDDEN),
                     PRM_BOOLEAN, &prm_ws_memory_report_default, &PRM_WS_MEMORY_REPORT, NULL, NULL);

  sysprm_init_param (PRM_ID_RYE_PORT_ID,
                     PRM_NAME_RYE_PORT_ID,
                     (PRM_FOR_CLIENT | PRM_FOR_SERVER),
                     PRM_INTEGER, &prm_rye_port_id_default, &PRM_RYE_PORT_ID, NULL, NULL);
  ...
  sysprm_init_param (PRM_ID_TCP_CONNECTION_TIMEOUT,
                     ...
                     &PRM_TCP_CONNECTION_TIMEOUT, NULL, &prm_tcp_connection_timeout_lower);

  sysprm_init_param (PRM_ID_OPTIMIZATION_LEVEL,
                     PRM_NAME_OPTIMIZATION_LEVEL,
                     (PRM_FOR_CLIENT | PRM_USER_CHANGE),
                     PRM_INTEGER, &prm_optimization_level_default, &PRM_OPTIMIZATION_LEVEL, NULL, NULL);

  sysprm_init_param (PRM_ID_QO_DUMP,
                     PRM_NAME_QO_DUMP,
                     (PRM_FOR_CLIENT | PRM_USER_CHANGE | PRM_HIDDEN),
                     PRM_BOOLEAN, &prm_qo_dump_default, &PRM_QO_DUMP, NULL, NULL);
  ...
  sysprm_init_param (PRM_ID_THREAD_STACKSIZE,
                     ...
                     &PRM_THREAD_STACKSIZE, &prm_thread_stacksize_upper, &prm_thread_stacksize_lower);

  sysprm_init_param (PRM_ID_IO_BACKUP_NBUFFERS,
                     PRM_NAME_IO_BACKUP_NBUFFERS,
                     (PRM_FOR_CLIENT | PRM_FOR_SERVER | PRM_HIDDEN),
                     PRM_INTEGER,
                     &prm_io_backup_nbuffers_default, &PRM_IO_BACKUP_NBUFFERS, NULL, &prm_io_backup_nbuffers_lower);

  sysprm_init_param (PRM_ID_IO_BACKUP_SLEEP,
                     PRM_NAME_IO_BACKUP_SLEEP,
                     (PRM_FOR_SERVER | PRM_USER_CHANGE | PRM_TIME_UNIT |
                      PRM_HIDDEN), PRM_BIGINT,
                     &prm_io_backup_sleep_default, &PRM_IO_BACKUP_SLEEP, NULL, &prm_io_backup_sleep_lower);
  ...
  sysprm_init_param (PRM_ID_MAX_PAGES_IN_TEMP_FILE_CACHE,
                     ...
                     &PRM_MAX_PAGES_IN_TEMP_FILE_CACHE, NULL, &prm_max_pages_in_temp_file_cache_lower);
  ...
  sysprm_init_param (PRM_ID_MAX_ENTRIES_IN_TEMP_FILE_CACHE,
                     ...
                     &PRM_MAX_ENTRIES_IN_TEMP_FILE_CACHE, NULL, &prm_max_entries_in_temp_file_cache_lower);
  ...
  sysprm_init_param (PRM_ID_TEMP_MEM_BUFFER_SIZE,
                     ...
                     &PRM_TEMP_MEM_BUFFER_SIZE, &prm_temp_mem_buffer_size_upper, &prm_temp_mem_buffer_size_lower);
  ...
  sysprm_init_param (PRM_ID_INDEX_SCAN_KEY_BUFFER_SIZE,
                     ...
                     &PRM_INDEX_SCAN_KEY_BUFFER_SIZE, NULL, &prm_index_scan_key_buffer_size_lower);

  sysprm_init_param (PRM_ID_ENABLE_HISTO,
                     PRM_NAME_ENABLE_HISTO,
                     (PRM_FOR_CLIENT | PRM_USER_CHANGE),
                     PRM_BOOLEAN, &prm_enable_histo_default, &PRM_ENABLE_HISTO, NULL, NULL);
  ...
  sysprm_init_param (PRM_ID_PB_NUM_LRU_CHAINS,
                     ...
                     &PRM_PB_NUM_LRU_CHAINS, &prm_pb_num_LRU_chains_upper, &prm_pb_num_LRU_chains_lower);
  ...
  sysprm_init_param (PRM_ID_PAGE_BG_FLUSH_INTERVAL,
                     ...
                     &PRM_PAGE_BG_FLUSH_INTERVAL, NULL, &prm_page_bg_flush_interval_lower);

  sysprm_init_param (PRM_ID_ADAPTIVE_FLUSH_CONTROL,
                     PRM_NAME_ADAPTIVE_FLUSH_CONTROL,
                     (PRM_FOR_SERVER | PRM_USER_CHANGE),
                     PRM_BOOLEAN, &prm_adaptive_flush_control_default, &PRM_ADAPTIVE_FLUSH_CONTROL, NULL, NULL);
  ...
  sysprm_init_param (PRM_ID_MAX_FLUSH_SIZE_PER_SECOND,
                     ...
                     &prm_max_flush_size_per_second_upper, &prm_max_flush_size_per_second_lower);
  ...
  sysprm_init_param (PRM_ID_PB_SYNC_ON_FLUSH_SIZE,
                     ...
                     &PRM_PB_SYNC_ON_FLUSH_SIZE, &prm_pb_sync_on_flush_size_upper, &prm_pb_sync_on_flush_size_lower);
  ...
  sysprm_init_param (PRM_ID_PB_DEBUG_PAGE_VALIDATION_LEVEL,
                     ...
                     PRM_KEYWORD,
                     &prm_pb_debug_page_validation_level_default, &PRM_PB_DEBUG_PAGE_VALIDATION_LEVEL, NULL, NULL);

  sysprm_init_param (PRM_ID_ANSI_QUOTES,
                     PRM_NAME_ANSI_QUOTES,
                     (PRM_FOR_CLIENT | PRM_TEST_CHANGE),
                     PRM_BOOLEAN, &prm_ansi_quotes_default, &PRM_ANSI_QUOTES, NULL, NULL);

  sysprm_init_param (PRM_ID_TEST_MODE,
                     PRM_NAME_TEST_MODE,
                     (PRM_FOR_CLIENT | PRM_FOR_SERVER | PRM_HIDDEN),
                     PRM_BOOLEAN, &prm_test_mode_default, &PRM_TEST_MODE, NULL, NULL);
  ...
  sysprm_init_param (PRM_ID_GROUP_CONCAT_MAX_LEN,
                     ...
                     &PRM_GROUP_CONCAT_MAX_LEN, &prm_group_concat_max_len_upper, &prm_group_concat_max_len_lower);
  ...
  sysprm_init_param (PRM_ID_LIKE_TERM_SELECTIVITY,
                     ...
                     &PRM_LIKE_TERM_SELECTIVITY, &prm_like_term_selectivity_upper, &prm_like_term_selectivity_lower);
  ...
  sysprm_init_param (PRM_ID_SUPPRESS_FSYNC,
                     ...
                     &PRM_SUPPRESS_FSYNC, &prm_suppress_fsync_upper, &prm_suppress_fsync_lower);

  sysprm_init_param (PRM_ID_CALL_STACK_DUMP_ON_ERROR,
                     PRM_NAME_CALL_STACK_DUMP_ON_ERROR,
                     (PRM_FOR_CLIENT | PRM_FOR_SERVER | PRM_USER_CHANGE),
                     PRM_BOOLEAN, &prm_call_stack_dump_on_error_default, &PRM_CALL_STACK_DUMP_ON_ERROR, NULL, NULL);

  sysprm_init_param (PRM_ID_CALL_STACK_DUMP_ACTIVATION,
                     PRM_NAME_CALL_STACK_DUMP_ACTIVATION,
                     (PRM_FOR_CLIENT | PRM_FOR_SERVER | PRM_USER_CHANGE),
                     PRM_INTEGER_LIST,
                     &prm_call_stack_dump_activation_default, &PRM_CALL_STACK_DUMP_ACTIVATION, NULL, NULL);

  sysprm_init_param (PRM_ID_CALL_STACK_DUMP_DEACTIVATION,
                     PRM_NAME_CALL_STACK_DUMP_DEACTIVATION,
                     (PRM_FOR_CLIENT | PRM_FOR_SERVER | PRM_USER_CHANGE),
                     PRM_INTEGER_LIST,
                     &prm_call_stack_dump_deactivation_default, &PRM_CALL_STACK_DUMP_DEACTIVATION, NULL, NULL);

  sysprm_init_param (PRM_ID_MIGRATOR_COMPRESSED_PROTOCOL,
                     PRM_NAME_MIGRATOR_COMPRESSED_PROTOCOL,
                     (PRM_FOR_CLIENT | PRM_USER_CHANGE),
                     PRM_BOOLEAN,
                     &prm_migrator_compressed_protocol_default, &PRM_MIGRATOR_COMPRESSED_PROTOCOL, NULL, NULL);

  sysprm_init_param (PRM_ID_AUTO_RESTART_SERVER,
                     PRM_NAME_AUTO_RESTART_SERVER,
                     (PRM_FOR_SERVER | PRM_USER_CHANGE),
                     PRM_BOOLEAN, &prm_auto_restart_server_default, &PRM_AUTO_RESTART_SERVER, NULL, NULL);
  ...
  sysprm_init_param (PRM_ID_XASL_MAX_PLAN_CACHE_ENTRIES,
                     ...
                     &PRM_XASL_MAX_PLAN_CACHE_ENTRIES, NULL, &prm_xasl_max_plan_cache_entries_lower);

  sysprm_init_param (PRM_ID_XASL_MAX_PLAN_CACHE_CLONES,
                     PRM_NAME_XASL_MAX_PLAN_CACHE_CLONES,
                     (PRM_FOR_SERVER | PRM_HIDDEN),
                     PRM_INTEGER, &prm_xasl_max_plan_cache_clones_default, &PRM_XASL_MAX_PLAN_CACHE_CLONES, NULL, NULL);

  sysprm_init_param (PRM_ID_XASL_PLAN_CACHE_TIMEOUT,
                     PRM_NAME_XASL_PLAN_CACHE_TIMEOUT,
                     (PRM_FOR_SERVER),
                     PRM_INTEGER, &prm_xasl_plan_cache_timeout_default, &PRM_XASL_PLAN_CACHE_TIMEOUT, NULL, NULL);

  sysprm_init_param (PRM_ID_USE_ORDERBY_SORT_LIMIT,
                     PRM_NAME_USE_ORDERBY_SORT_LIMIT,
                     (PRM_FOR_SERVER | PRM_USER_CHANGE),
                     PRM_BOOLEAN, &prm_use_orderby_sort_limit_default, &PRM_USE_ORDERBY_SORT_LIMIT, NULL, NULL);

  sysprm_init_param (PRM_ID_LOGWR_COMPRESSED_PROTOCOL,
                     PRM_NAME_LOGWR_COMPRESSED_PROTOCOL,
                     (PRM_FOR_CLIENT | PRM_USER_CHANGE),
                     PRM_BOOLEAN, &prm_logwr_compressed_protocol_default, &PRM_LOGWR_COMPRESSED_PROTOCOL, NULL, NULL);

  sysprm_init_param (PRM_ID_HA_MODE,
                     PRM_NAME_HA_MODE,
                     (PRM_FOR_CLIENT | PRM_FOR_SERVER | PRM_FORCE_SERVER
                      | PRM_RELOADABLE),
                     PRM_KEYWORD, &prm_ha_mode_default, &PRM_HA_MODE, &prm_ha_mode_upper, &prm_ha_mode_lower);

  sysprm_init_param (PRM_ID_HA_NODE_LIST,
                     PRM_NAME_HA_NODE_LIST,
                     (PRM_FOR_SERVER | PRM_FOR_CLIENT | PRM_USER_CHANGE |
                      PRM_RELOADABLE), PRM_STRING, &prm_ha_node_list_default, &PRM_HA_NODE_LIST_STR, NULL, NULL);

  sysprm_init_param (PRM_ID_HA_REPLICA_LIST,
                     PRM_NAME_HA_REPLICA_LIST,
                     (PRM_FOR_CLIENT | PRM_USER_CHANGE | PRM_RELOADABLE),
                     PRM_STRING, &prm_ha_replica_list_default, &PRM_HA_REPLICA_LIST_STR, NULL, NULL);

  sysprm_init_param (PRM_ID_HA_DB_LIST,
                     PRM_NAME_HA_DB_LIST,
                     (PRM_FOR_CLIENT | PRM_FOR_SERVER | PRM_RELOADABLE),
                     PRM_STRING, &prm_ha_db_list_default, &PRM_HA_DB_LIST, NULL, NULL);
  ...
  sysprm_init_param (PRM_ID_HA_INIT_TIMER,
                     ...
                     &PRM_HA_INIT_TIMER, &prm_ha_init_timer_upper, &prm_ha_init_timer_lower);
  ...
  sysprm_init_param (PRM_ID_HA_HEARTBEAT_INTERVAL,
                     ...
                     &PRM_HA_HEARTBEAT_INTERVAL, &prm_ha_heartbeat_interval_upper, &prm_ha_heartbeat_interval_lower);
  ...
  sysprm_init_param (PRM_ID_HA_CALC_SCORE_INTERVAL,
                     ...
                     &PRM_HA_CALC_SCORE_INTERVAL, &prm_ha_calc_score_interval_upper, &prm_ha_calc_score_interval_lower);
  ...
  sysprm_init_param (PRM_ID_HA_FAILOVER_WAIT_TIME,
                     ...
                     &PRM_HA_FAILOVER_WAIT_TIME, &prm_ha_failover_wait_time_upper, &prm_ha_failover_wait_time_lower);
  ...
  sysprm_init_param (PRM_ID_HA_PROCESS_START_CONFIRM_INTERVAL,
                     ...
                     &prm_ha_process_start_confirm_interval_upper, &prm_ha_process_start_confirm_interval_lower);
  ...
  sysprm_init_param (PRM_ID_HA_PROCESS_DEREG_CONFIRM_INTERVAL,
                     ...
                     &prm_ha_process_dereg_confirm_interval_upper, &prm_ha_process_dereg_confirm_interval_lower);
  ...
  sysprm_init_param (PRM_ID_HA_MAX_PROCESS_START_CONFIRM,
                     ...
                     PRM_INTEGER,
                     &prm_ha_max_process_start_confirm_default, &PRM_HA_MAX_PROCESS_START_CONFIRM, NULL, NULL);

  sysprm_init_param (PRM_ID_HA_MAX_PROCESS_DEREG_CONFIRM,
                     PRM_NAME_HA_MAX_PROCESS_DEREG_CONFIRM,
                     (PRM_FOR_CLIENT | PRM_HIDDEN | PRM_RELOADABLE),
                     PRM_INTEGER,
                     &prm_ha_max_process_dereg_confirm_default, &PRM_HA_MAX_PROCESS_DEREG_CONFIRM, NULL, NULL);
  ...
  sysprm_init_param (PRM_ID_HA_CHANGESLAVE_MAX_WAIT_TIME,
                     ...
                     &prm_ha_changeslave_max_wait_time_upper, &prm_ha_changeslave_max_wait_time_lower);
  ...
  sysprm_init_param (PRM_ID_SERVER_STATE_SYNC_INTERVAL,
                     ...
                     &prm_server_state_sync_interval_upper, &prm_server_state_sync_interval_lower);

  sysprm_init_param (PRM_ID_HA_MAX_HEARTBEAT_GAP,
                     PRM_NAME_HA_MAX_HEARTBEAT_GAP,
                     (PRM_FOR_CLIENT | PRM_HIDDEN | PRM_RELOADABLE),
                     PRM_INTEGER, &prm_ha_max_heartbeat_gap_default, &PRM_HA_MAX_HEARTBEAT_GAP, NULL, NULL);

  sysprm_init_param (PRM_ID_HA_PING_HOSTS,
                     PRM_NAME_HA_PING_HOSTS,
                     (PRM_FOR_CLIENT | PRM_RELOADABLE),
                     PRM_STRING, &prm_ha_ping_hosts_default, &PRM_HA_PING_HOSTS, NULL, NULL);

  sysprm_init_param (PRM_ID_HA_IGNORE_ERROR_LIST,
                     PRM_NAME_HA_IGNORE_ERROR_LIST,
                     (PRM_FOR_CLIENT | PRM_FOR_SERVER | PRM_USER_CHANGE |
                      PRM_RELOADABLE), PRM_INTEGER_LIST,
                     &prm_ha_ignore_error_list_default, &PRM_HA_IGNORE_ERROR_LIST, NULL, NULL);
  ...
  sysprm_init_param (PRM_ID_HA_COPY_LOG_MAX_ARCHIVES,
                     ...
                     &prm_ha_copy_log_max_archives_upper, &prm_ha_copy_log_max_archives_lower);
  ...
  sysprm_init_param (PRM_ID_HA_COPY_LOG_TIMEOUT,
                     ...
                     &PRM_HA_COPY_LOG_TIMEOUT, &prm_ha_copy_log_timeout_upper, &prm_ha_copy_log_timeout_lower);
  ...
  sysprm_init_param (PRM_ID_HA_REPLICA_DELAY,
                     ...
                     &PRM_HA_REPLICA_DELAY, &prm_ha_replica_delay_upper, &prm_ha_replica_delay_lower);

  sysprm_init_param (PRM_ID_HA_REPLICA_TIME_BOUND,
                     PRM_NAME_HA_REPLICA_TIME_BOUND,
                     (PRM_FOR_CLIENT | PRM_RELOADABLE),
                     PRM_STRING, &prm_ha_replica_time_bound_default, &PRM_HA_REPLICA_TIME_BOUND, NULL, NULL);
  ...
  sysprm_init_param (PRM_ID_HA_DELAY_LIMIT_DELTA,
                     ...
                     &PRM_HA_DELAY_LIMIT_DELTA, &prm_ha_delay_limit_delta_upper, &prm_ha_delay_limit_delta_lower);
  ...
  sysprm_init_param (PRM_ID_HA_CHECK_DISK_FAILURE_INTERVAL,
                     ...
                     &prm_ha_check_disk_failure_interval_upper, &prm_ha_check_disk_failure_interval_lower);
  ...
  sysprm_init_param (PRM_ID_LOG_ASYNC_LOG_FLUSH_INTERVAL,
                     ...
                     &prm_log_async_log_flush_interval_upper, &prm_log_async_log_flush_interval_lower);

  sysprm_init_param (PRM_ID_LOG_COMPRESS,
                     PRM_NAME_LOG_COMPRESS,
                     (PRM_FOR_SERVER), PRM_BOOLEAN, &prm_log_compress_default, &PRM_LOG_COMPRESS, NULL, NULL);

  sysprm_init_param (PRM_ID_BLOCK_NOWHERE_STATEMENT,
                     PRM_NAME_BLOCK_NOWHERE_STATEMENT,
                     (PRM_FOR_CLIENT | PRM_USER_CHANGE),
                     PRM_BOOLEAN, &prm_block_nowhere_statement_default, &PRM_BLOCK_NOWHERE_STATEMENT, NULL, NULL);

  sysprm_init_param (PRM_ID_BLOCK_DDL_STATEMENT,
                     PRM_NAME_BLOCK_DDL_STATEMENT,
                     (PRM_FOR_CLIENT | PRM_USER_CHANGE),
                     PRM_BOOLEAN, &prm_block_ddl_statement_default, &PRM_BLOCK_DDL_STATEMENT, NULL, NULL);
  ...
  sysprm_init_param (PRM_ID_RSQL_HISTORY_NUM,
                     ...
                     &PRM_RSQL_HISTORY_NUM, &prm_rsql_history_num_upper, &prm_rsql_history_num_lower);

  sysprm_init_param (PRM_ID_LOG_TRACE_DEBUG,
                     PRM_NAME_LOG_TRACE_DEBUG,
                     (PRM_FOR_SERVER | PRM_USER_CHANGE | PRM_HIDDEN),
                     PRM_BOOLEAN, &prm_log_trace_debug_default, &PRM_LOG_TRACE_DEBUG, NULL, NULL);

  sysprm_init_param (PRM_ID_ER_PRODUCTION_MODE,
                     PRM_NAME_ER_PRODUCTION_MODE,
                     (PRM_FOR_SERVER | PRM_FOR_CLIENT | PRM_USER_CHANGE),
                     PRM_BOOLEAN, &prm_er_production_mode_default, &PRM_ER_PRODUCTION_MODE, NULL, NULL);

  sysprm_init_param (PRM_ID_ER_STOP_ON_ERROR,
                     PRM_NAME_ER_STOP_ON_ERROR,
                     (PRM_FOR_SERVER | PRM_FOR_CLIENT | PRM_USER_CHANGE |
                      PRM_HIDDEN), PRM_INTEGER,
                     &prm_er_stop_on_error_default, &PRM_ER_STOP_ON_ERROR, &prm_er_stop_on_error_upper, NULL);

  sysprm_init_param (PRM_ID_TCP_RCVBUF_SIZE,
                     PRM_NAME_TCP_RCVBUF_SIZE,
                     (PRM_FOR_SERVER | PRM_FOR_CLIENT | PRM_HIDDEN),
                     PRM_INTEGER, &prm_tcp_rcvbuf_size_default, &PRM_TCP_RCVBUF_SIZE, NULL, NULL);

  sysprm_init_param (PRM_ID_TCP_SNDBUF_SIZE,
                     PRM_NAME_TCP_SNDBUF_SIZE,
                     (PRM_FOR_SERVER | PRM_FOR_CLIENT | PRM_HIDDEN),
                     PRM_INTEGER, &prm_tcp_sndbuf_size_default, &PRM_TCP_SNDBUF_SIZE, NULL, NULL);

  sysprm_init_param (PRM_ID_TCP_NODELAY,
                     PRM_NAME_TCP_NODELAY,
                     (PRM_FOR_SERVER | PRM_FOR_CLIENT | PRM_HIDDEN),
                     PRM_BOOLEAN, &prm_tcp_nodelay_default, &PRM_TCP_NODELAY, NULL, NULL);

  sysprm_init_param (PRM_ID_TCP_KEEPALIVE,
                     PRM_NAME_TCP_KEEPALIVE,
                     (PRM_FOR_SERVER | PRM_FOR_CLIENT),
                     PRM_BOOLEAN, &prm_tcp_keepalive_default, &PRM_TCP_KEEPALIVE, NULL, NULL);

  sysprm_init_param (PRM_ID_RSQL_SINGLE_LINE_MODE,
                     PRM_NAME_RSQL_SINGLE_LINE_MODE,
                     (PRM_FOR_CLIENT | PRM_USER_CHANGE),
                     PRM_BOOLEAN, &prm_rsql_single_line_mode_default, &PRM_RSQL_SINGLE_LINE_MODE, NULL, NULL);

  sysprm_init_param (PRM_ID_XASL_DEBUG_DUMP,
                     PRM_NAME_XASL_DEBUG_DUMP,
                     (PRM_FOR_CLIENT | PRM_USER_CHANGE | PRM_HIDDEN),
                     PRM_BOOLEAN, &prm_xasl_debug_dump_default, &PRM_XASL_DEBUG_DUMP, NULL, NULL);

  sysprm_init_param (PRM_ID_LOG_MAX_ARCHIVES,
                     PRM_NAME_LOG_MAX_ARCHIVES,
                     (PRM_FOR_SERVER | PRM_USER_CHANGE),
                     PRM_INTEGER,
                     &prm_log_max_archives_default, &PRM_LOG_MAX_ARCHIVES, NULL, &prm_log_max_archives_lower);

  sysprm_init_param (PRM_ID_FORCE_REMOVE_LOG_ARCHIVES,
                     PRM_NAME_FORCE_REMOVE_LOG_ARCHIVES,
                     (PRM_FOR_SERVER | PRM_USER_CHANGE),
                     PRM_BOOLEAN, &prm_force_remove_log_archives_default, &PRM_FORCE_REMOVE_LOG_ARCHIVES, NULL, NULL);
  ...
  sysprm_init_param (PRM_ID_REMOVE_LOG_ARCHIVES_INTERVAL,
                     ...
                     &PRM_REMOVE_LOG_ARCHIVES_INTERVAL, NULL, &prm_remove_log_archives_interval_lower);

  sysprm_init_param (PRM_ID_EVENT_HANDLER,
                     PRM_NAME_EVENT_HANDLER,
                     (PRM_FOR_CLIENT | PRM_FOR_SERVER),
                     PRM_STRING, &prm_event_handler_default, &PRM_EVENT_HANDLER, NULL, NULL);

  sysprm_init_param (PRM_ID_EVENT_ACTIVATION,
                     PRM_NAME_EVENT_ACTIVATION,
                     (PRM_FOR_CLIENT | PRM_FOR_SERVER),
                     PRM_INTEGER_LIST, &prm_event_activation_default, &PRM_EVENT_ACTIVATION, NULL, NULL);

  sysprm_init_param (PRM_ID_MNT_WAITING_THREAD,
                     PRM_NAME_MNT_WAITING_THREAD,
                     (PRM_FOR_SERVER | PRM_USER_CHANGE | PRM_HIDDEN),
                     PRM_INTEGER,
                     &prm_mnt_waiting_thread_default, &PRM_MNT_WAITING_THREAD, NULL, &prm_mnt_waiting_thread_lower);
  ...
  sysprm_init_param (PRM_ID_SESSION_STATE_TIMEOUT,
                     ...
                     &PRM_SESSION_STATE_TIMEOUT, &prm_session_timeout_upper, &prm_session_timeout_lower);
  ...
  sysprm_init_param (PRM_ID_MULTI_RANGE_OPT_LIMIT,
                     ...
                     &PRM_MULTI_RANGE_OPT_LIMIT, &prm_multi_range_opt_limit_upper, &prm_multi_range_opt_limit_lower);
  ...
  sysprm_init_param (PRM_ID_DB_VOLUME_SIZE,
                     ...
                     &PRM_DB_VOLUME_SIZE, &prm_db_volume_size_upper, &prm_db_volume_size_lower);

  sysprm_init_param (PRM_ID_CHECK_PEER_ALIVE,
                     PRM_NAME_CHECK_PEER_ALIVE,
                     (PRM_FOR_CLIENT | PRM_FOR_SERVER | PRM_USER_CHANGE),
                     PRM_KEYWORD, &prm_check_peer_alive_default, &PRM_CHECK_PEER_ALIVE, NULL, NULL);
  ...
  sysprm_init_param (PRM_ID_SQL_TRACE_SLOW,
                     ...
                     &PRM_SQL_TRACE_SLOW, &prm_sql_trace_slow_upper, &prm_sql_trace_slow_lower);

  sysprm_init_param (PRM_ID_SQL_TRACE_EXECUTION_PLAN,
                     PRM_NAME_SQL_TRACE_EXECUTION_PLAN,
                     (PRM_USER_CHANGE | PRM_FOR_SERVER),
                     PRM_BOOLEAN, &prm_sql_trace_execution_plan_default, &PRM_SQL_TRACE_EXECUTION_PLAN, NULL, NULL);
  ...
  sysprm_init_param (PRM_ID_LOG_TRACE_FLUSH_TIME,
                     ...
                     &PRM_LOG_TRACE_FLUSH_TIME, NULL, &prm_log_trace_flush_time_lower);
  ...
  sysprm_init_param (PRM_ID_GENERIC_VOL_PREALLOC_SIZE,
                     ...
                     &prm_generic_vol_prealloc_size_upper, &prm_generic_vol_prealloc_size_lower);
  ...
  sysprm_init_param (PRM_ID_SORT_LIMIT_MAX_COUNT,
                     ...
                     &PRM_SORT_LIMIT_MAX_COUNT, &prm_sort_limit_max_count_upper, &prm_sort_limit_max_count_lower);

  sysprm_init_param (PRM_ID_SQL_TRACE_IOREADS,
                     PRM_NAME_SQL_TRACE_IOREADS,
                     (PRM_USER_CHANGE | PRM_FOR_SERVER),
                     PRM_INTEGER,
                     &prm_sql_trace_ioreads_default, &PRM_SQL_TRACE_IOREADS, NULL, &prm_sql_trace_ioreads_lower);

  sysprm_init_param (PRM_ID_QUERY_TRACE,
                     PRM_NAME_QUERY_TRACE,
                     (PRM_FOR_CLIENT | PRM_USER_CHANGE),
                     PRM_BOOLEAN, &prm_query_trace_default, &PRM_QUERY_TRACE, NULL, NULL);
  ...
  sysprm_init_param (PRM_ID_QUERY_TRACE_FORMAT,
                     ...
                     &PRM_QUERY_TRACE_FORMAT, &prm_query_trace_format_upper, &prm_query_trace_format_lower);

  sysprm_init_param (PRM_ID_MAX_RECURSION_SQL_DEPTH,
                     PRM_NAME_MAX_RECURSION_SQL_DEPTH,
                     (PRM_FOR_SERVER | PRM_USER_CHANGE | PRM_HIDDEN),
                     PRM_INTEGER, &prm_max_recursion_sql_depth_default, &PRM_MAX_RECURSION_SQL_DEPTH, NULL, NULL);

  sysprm_init_param (PRM_ID_BTREE_MERGE_ENABLED,
                     PRM_NAME_BTREE_MERGE_ENABLED,
                     (PRM_FOR_SERVER | PRM_USER_CHANGE),
                     PRM_BOOLEAN, &prm_btree_merge_enabled_default, &PRM_BTREE_MERGE_ENABLED, NULL, NULL);
  ...
  sysprm_init_param (PRM_ID_OPTIMIZER_ENABLE_AGGREGATE_OPTIMIZATION,
                     ...
                     &PRM_OPTIMIZER_ENABLE_AGGREGATE_OPTIMIZATION, NULL, NULL);

  sysprm_init_param (PRM_ID_FAULT_INJECTION,
                     PRM_NAME_FAULT_INJECTION,
                     (PRM_USER_CHANGE | PRM_FOR_SERVER | PRM_FOR_CLIENT |
                      PRM_HIDDEN), PRM_STRING, &prm_fault_injection_default, &PRM_FAULT_INJECTION, NULL, NULL);

  sysprm_init_param (PRM_ID_RYE_SHM_KEY,
                     PRM_NAME_RYE_SHM_KEY,
                     (PRM_FOR_CLIENT | PRM_FOR_SERVER),
                     PRM_STRING, &prm_rye_shm_key_default, &PRM_RYE_SHM_KEY, NULL, NULL);
  ...
  sysprm_init_param (PRM_ID_HA_MAX_LOG_APPLIER,
                     ...
                     &PRM_HA_MAX_LOG_APPLIER, &prm_ha_max_log_applier_upper, &prm_ha_max_log_applier_lower);

  sysprm_init_param (PRM_ID_MAX_CLIENTS,
                     PRM_NAME_MAX_CLIENTS,
                     (PRM_FOR_SERVER),
                     PRM_INTEGER,
                     &prm_max_clients_default, &PRM_MAX_CLIENTS, &prm_max_clients_upper, &prm_max_clients_lower);
  ...
  sysprm_init_param (PRM_ID_MAX_COPYLOG_CONNECTIONS,
                     ...
                     &PRM_MAX_COPYLOG_CONNECTIONS, &prm_max_copylog_conns_upper, &prm_max_copylog_conns_lower);
  ...
  sysprm_init_param (PRM_ID_MIGRATOR_MAX_REPL_DELAY,
                     ...
                     &prm_migrator_max_repl_delay_upper, &prm_migrator_max_repl_delay_lower);

  sysprm_init_param (PRM_ID_HA_NODE_MYSELF,
                     PRM_NAME_HA_NODE_MYSELF,
                     (PRM_FOR_CLIENT | PRM_FOR_SERVER | PRM_RELOADABLE),
                     PRM_STRING, &prm_ha_node_myself_default, &PRM_HA_NODE_MYSELF, NULL, NULL);
  sysprm_init_param (PRM_ID_HA_NODE_MYSELF_PORT,
                     PRM_NAME_HA_NODE_MYSELF_PORT,
                     (PRM_FOR_CLIENT | PRM_FOR_SERVER | PRM_RELOADABLE),
                     PRM_INTEGER, &prm_ha_node_myself_port_default, &PRM_HA_NODE_MYSELF_PORT, NULL, NULL);

  sysprm_set_post_assign_fn (PRM_ID_HA_NODE_LIST, prm_node_list_post_assign);
  sysprm_set_post_assign_fn (PRM_ID_HA_REPLICA_LIST, prm_node_list_post_assign);
  sysprm_set_post_assign_fn (PRM_ID_HA_NODE_MYSELF, prm_ha_node_myself_post_assign);
  sysprm_set_post_assign_fn (PRM_ID_HA_NODE_MYSELF_PORT, prm_ha_node_myself_port_post_assign);
 ...
 */
int
sysprm_dump_persist_conf_file (FILE * fp, const char *proc_name, const char *sect_name)
{
  int error = NO_ERROR;
  ...
  if (root == NULL)
    {
      ...
      snprintf (err_msg, sizeof (err_msg),
                ...
                file_being_dealt_with, root_err.line, root_err.column, root_err.text);
  ...
  /* STEP 2: iter object
   */
  for (p = json_object_iter (root); p && error == NO_ERROR; p = json_object_iter_next (root, p))
    {
      ...
      for (s = json_object_iter (proc); s && error == NO_ERROR; s = json_object_iter_next (proc, s))
        {
          ...
          for (e = json_object_iter (sect); e && error == NO_ERROR; e = json_object_iter_next (sect, e))
            {
...
exit_on_error:
  if (error == NO_ERROR)
    {
      error = ER_GENERIC_ERROR;
      er_set_with_oserror (ER_ERROR_SEVERITY, ARG_FILE_LINE, error, 1, "sysprm_dump_persist_conf_file:");
    }
  ...
  if (log_tm_p != NULL)
    {
      snprintf (error_log_name, PATH_MAX - 1,
                ...
                log_tm_p->tm_mon + 1, log_tm_p->tm_mday, log_tm_p->tm_hour, log_tm_p->tm_min);
  ...
  if (log_tm_p != NULL)
    {
      snprintf (error_log_name, PATH_MAX - 1,
                ...
                log_tm_p->tm_year + 1900, log_tm_p->tm_mon + 1, log_tm_p->tm_mday, log_tm_p->tm_hour, log_tm_p->tm_min);
      prm_set (er_log_file, error_log_name, true);
    }
}
  ...
  for (i = 0; i < NUM_PRM; i++)
    {
      ...
      if (!PRM_IS_SET (prm_Def[i].flag) && !PRM_IS_OBSOLETED (prm_Def[i].flag))
        {
          if (prm_set_default (&prm_Def[i]) != NO_ERROR)
            {
              fprintf (stderr,
                       msgcat_message (MSGCAT_CATALOG_RYE, MSGCAT_SET_PARAMETERS, PRM_ERR_NO_VALUE), prm_Def[i].name);
              assert (0);
              return ER_FAILED;
            }
        }
 ...
 */
int
prm_read_and_parse_server_persist_conf_file (const char *sect_name, const bool reload)
{
  int error = NO_ERROR;
  char file_being_dealt_with[PATH_MAX];
  int conf_fd;
          ...
          for (e = json_object_iter (sect); e && error == NO_ERROR; e = json_object_iter_next (sect, e))
            {
              ...
              if (PRM_IS_DEPRECATED (prm->flag))
                {
                  prm_report_bad_entry (key, -1, PRM_ERR_DEPRICATED, file_being_dealt_with);
                }

              error = prm_set (prm, new_value, true);
              if (error != NO_ERROR)
                {
                  prm_report_bad_entry (key, -1, error, file_being_dealt_with);
                  goto exit_on_error;
                }
            }
        }
...
exit_on_error:
  if (error == NO_ERROR)
    {
      error = ER_GENERIC_ERROR;
      er_set_with_oserror (ER_ERROR_SEVERITY, ARG_FILE_LINE, error, 1, "prm_read_and_parse_server_persist_conf_file:");
    }
          ...
          for (e = json_object_iter (sect); e && error == NO_ERROR; e = json_object_iter_next (sect, e))
            {
              ...
              snprintf (key_name, LINE_MAX - 1, "%s:%s", sect_name, ini_str_lower (ini_str_trim (key)));
              if (ini_table_set (ini, key_name, json_string_value (val), lineno) < 0)
                {
                  assert (false);
                  goto exit_on_error;
                }
...
exit_on_error:
  if (error == NO_ERROR)
    {
      error = ER_GENERIC_ERROR;
      er_set_with_oserror (ER_ERROR_SEVERITY, ARG_FILE_LINE, error, 1, "prm_read_and_parse_broker_persist_conf_file:");
    }
  ...
  do
    {
      ...
      if (!check || PRM_USER_CAN_CHANGE (prm->flag) || (PRM_TEST_CHANGE_ONLY (prm->flag) && PRM_TEST_MODE))
        {
 ...
 */
int
sysprm_change_persist_conf_file (const char *proc_name, const char *sect_name, const char *key, const char *value)
{
  int error = NO_ERROR;
  char file_being_dealt_with[PATH_MAX];
  int conf_fd;
  ...
  assert ((sect_name != NULL && key != NULL && value != NULL) || (value == NULL));
  ...
  if (conf_fd == NULL_VOLDES)
    {
      ...
      er_set_with_oserror (ER_ERROR_SEVERITY, ARG_FILE_LINE, error, 1, file_being_dealt_with);
      goto exit_on_error;
    }

  lockf_type = fileio_lock ("db-name" /* TODO */ , file_being_dealt_with, conf_fd,
                            true);
  if (lockf_type == FILEIO_NOT_LOCKF)
    {
   ...
   */
  if (json_dump_file (root, file_being_dealt_with, JSON_INDENT (2) | JSON_PRESERVE_ORDER) != 0)
    {
      assert (false);
      goto exit_on_error;
    }
...
exit_on_error:
  if (error == NO_ERROR)
    {
      error = ER_GENERIC_ERROR;
      er_set_with_oserror (ER_ERROR_SEVERITY, ARG_FILE_LINE, error, 1, "sysprm_change_persist_conf_file:");
    }
...
int
sysprm_get_persist_conf (char *value, int max_size, const char *proc_name, const char *sect_name, const char *key_name)
{
  int error = NO_ERROR;
  char file_being_dealt_with[PATH_MAX];
  json_t *root = NULL;
...
exit_on_error:
  if (error == NO_ERROR)
    {
      error = ER_GENERIC_ERROR;
      er_set_with_oserror (ER_ERROR_SEVERITY, ARG_FILE_LINE, error, 1, "sysprm_get_persist_conf:");
    }
  ...
  for (av = assignments; av != NULL && error == NO_ERROR; av = av->next)
    {
      ...
      if (!check
          ...
          || av->persist
#endif
          || PRM_USER_CAN_CHANGE (prm->flag) || (PRM_TEST_CHANGE_ONLY (prm->flag) && PRM_TEST_MODE))
        {
          /* We allow changing the parameter value. */
      ...
      if (av->persist)
        {
          ...
          if (sysprm_compare_values (prm->value, prm->default_value, prm->datatype) != 0)
            {
              prm_print (prm, buf, LINE_MAX, PRM_PRINT_NONE, PRM_PRINT_CURR_VAL);

              error = sysprm_change_persist_conf_file ("server", "common", prm->name, buf);
            }
          else
            {
              /* delete default key */
              error = sysprm_change_persist_conf_file ("server", "common", prm->name, NULL);
            }
        }
#endif
    }
 ...
 */
static int
prm_print_value (const SYSPRM_PARAM * prm, void *prm_value, char *buf, size_t len)
{
  int error = NO_ERROR;
    ...
    case PRM_INTEGER:
      {
        ...
        error = prm_print_int_value (value_string, sizeof (value_string), val);
        if (error != NO_ERROR)
          {
            return error;
          }
    ...
    case PRM_BIGINT:
      {
        ...
        error = prm_print_bigint_value (value_string, sizeof (value_string), val, prm);
        if (error != NO_ERROR)
          {
            return error;
          }
    ...
    case PRM_BOOLEAN:
      {
        ...
        error = prm_print_boolean_value (value_string, sizeof (value_string), val);
        if (error != NO_ERROR)
          {
            return error;
          }
    ...
    case PRM_FLOAT:
      {
        ...
        error = prm_print_float_value (value_string, sizeof (value_string), val);
        if (error != NO_ERROR)
          {
            return error;
          }
    ...
    case PRM_STRING:
      {
        ...
        if (n < 0)
          {
            error = ER_GENERIC_ERROR;
            er_set_with_oserror (ER_ERROR_SEVERITY, ARG_FILE_LINE, error, 1, "invalid buffer size");
            return error;
          }
        value = value_string;
      }
    ...
    case PRM_KEYWORD:
      {
        ...
        error = prm_print_keyword_value (value_string, sizeof (value_string), val, prm);
        if (error != NO_ERROR)
          {
            return error;
          }
    ...
    case PRM_INTEGER_LIST:
      {
        ...
        error = prm_print_int_list_value (value_string, sizeof (value_string), val);
        if (error != NO_ERROR)
          {
            return error;
          }
           ...
           PRM_PRINT_MODE print_mode, PRM_PRINT_VALUE_MODE print_value_mode)
{
  ...
  if (print_mode == PRM_PRINT_ID)
    {
      snprintf (left_side, PRM_DEFAULT_BUFFER_SIZE, "%d(%s)=", prm->param_id, prm_flag);
    }
  else if (print_mode == PRM_PRINT_NAME)
    {
      snprintf (left_side, PRM_DEFAULT_BUFFER_SIZE, "%s(%s)=", prm->name, prm_flag);
    }
  ...
  n = prm_print_value (prm, prm_value, value_string, len - strlen (left_side));
  if (n > 0)
    {
 ...
 */
static int
prm_print_bigint_value (char *buf, size_t len, INT64 val, const SYSPRM_PARAM * prm)
{
  int error = NO_ERROR;
  int n;
...
prm_print_boolean_value (char *buf, size_t len, bool val)
{
  ...
  if (n < 0)
    {
      error = ER_GENERIC_ERROR;
      er_set_with_oserror (ER_ERROR_SEVERITY, ARG_FILE_LINE, error, 1, "invalid buffer length");
      return error;
    }
 ...
 */
static int
prm_print_keyword_value (char *buf, size_t len, int val, const SYSPRM_PARAM * prm)
{
  int error = NO_ERROR;
  int n;
  ...
  if (intl_mbs_casecmp (prm->name, PRM_NAME_ER_LOG_LEVEL) == 0)
    {
      keyvalp = prm_keyword (val, NULL, er_log_level_words, DIM (er_log_level_words));
    }
  else if (intl_mbs_casecmp (prm->name, PRM_NAME_PB_DEBUG_PAGE_VALIDATION_LEVEL) == 0)
    {
      keyvalp = prm_keyword (val, NULL,
                             pgbuf_debug_page_validation_level_words, DIM (pgbuf_debug_page_validation_level_words));
    }
    ...
    {
      keyvalp = prm_keyword (val, NULL, check_peer_alive_words, DIM (check_peer_alive_words));
    }
  else if (intl_mbs_casecmp (prm->name, PRM_NAME_QUERY_TRACE_FORMAT) == 0)
    {
      keyvalp = prm_keyword (val, NULL, query_trace_format_words, DIM (query_trace_format_words));
    }
  else
    {
      assert (false);
...
sysprm_print_sysprm_value (PARAM_ID prm_id, SYSPRM_VALUE value, char *buf, size_t len, PRM_PRINT_MODE print_mode)
{
  ...
  if (print_mode == PRM_PRINT_ID)
    {
      snprintf (left_side, PRM_DEFAULT_BUFFER_SIZE, "%d(%s)=", prm_id, prm_flag);
    }
  else if (print_mode == PRM_PRINT_NAME)
    {
      snprintf (left_side, PRM_DEFAULT_BUFFER_SIZE, "%s(%s)=", prm->name, prm_flag);
        ...
        if (PRM_IS_FOR_SERVER (prm->flag) && PRM_IS_FOR_CLIENT (prm->flag))
          {
            ...
            error = prm_print_int_value (val2_buffer, sizeof (val2_buffer), val);
            if (error != NO_ERROR)
              {
                return error;
              }
    ...
    case PRM_FLOAT:
      {
        ...
        error = prm_print_float_value (val1_buffer, sizeof (val1_buffer), val);
        if (error != NO_ERROR)
          {
            return error;
          }
        ...
        if (PRM_IS_FOR_SERVER (prm->flag) && PRM_IS_FOR_CLIENT (prm->flag))
          {
            ...
            error = prm_print_float_value (val2_buffer, sizeof (val2_buffer), val);
            if (error != NO_ERROR)
              {
                return error;
              }
    ...
    case PRM_BOOLEAN:
      {
        ...
        error = prm_print_boolean_value (val1_buffer, sizeof (val1_buffer), val);
        if (error != NO_ERROR)
          {
            return error;
          }
        ...
        if (PRM_IS_FOR_SERVER (prm->flag) && PRM_IS_FOR_CLIENT (prm->flag))
          {
            ...
            error = prm_print_boolean_value (val2_buffer, sizeof (val2_buffer), val);
            if (error != NO_ERROR)
              {
                return error;
              }
    ...
    case PRM_KEYWORD:
      {
        ...
        error = prm_print_keyword_value (val1_buffer, sizeof (val1_buffer), val, prm);
        if (error != NO_ERROR)
          {
            return error;
          }
        ...
        if (PRM_IS_FOR_SERVER (prm->flag) && PRM_IS_FOR_CLIENT (prm->flag))
          {
            ...
            error = prm_print_keyword_value (val2_buffer, sizeof (val2_buffer), val, prm);
            if (error != NO_ERROR)
              {
                return error;
              }
    ...
    case PRM_BIGINT:
      {
        ...
        error = prm_print_bigint_value (val1_buffer, sizeof (val1_buffer), val, prm);
        if (error != NO_ERROR)
          {
            return error;
          }
        ...
        if (PRM_IS_FOR_SERVER (prm->flag) && PRM_IS_FOR_CLIENT (prm->flag))
          {
            ...
            error = prm_print_bigint_value (val2_buffer, sizeof (val2_buffer), val, prm);
            if (error != NO_ERROR)
              {
                return error;
              }
    ...
    case PRM_INTEGER_LIST:
      {
        ...
        error = prm_print_int_list_value (val1_buffer, sizeof (val1_buffer), val);
        if (error != NO_ERROR)
          {
            return error;
          }
        ...
        if (PRM_IS_FOR_SERVER (prm->flag) && PRM_IS_FOR_CLIENT (prm->flag))
          {
            ...
            error = prm_print_int_list_value (val2_buffer, sizeof (val2_buffer), val);
            if (error != NO_ERROR)
              {
                return error;
              }
  ...
  if (val2_string != NULL)
    {
      n = snprintf (buf, len, "%s(%s|%s)", left_side, val1_string, val2_string);
    }
 ...
 */
static SYSPRM_ERR
sysprm_generate_new_value (SYSPRM_PARAM * prm, const char *value, UNUSED_ARG bool check, SYSPRM_VALUE * new_value)
{
  char *end = NULL;
  char buf[LINE_MAX];
  ...
  if (!PRM_IS_BIGINT (prm) && (PRM_HAS_SIZE_UNIT (prm->flag) || PRM_HAS_TIME_UNIT (prm->flag)))
    {
      assert (false);
    ...
    case PRM_STRING:
      {
        ...
        if (prm_keyword (-1, value, null_words, DIM (null_words)) != NULL)
        ...
        else
          {
            ...
            if (val == NULL)
              {
                er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_OUT_OF_VIRTUAL_MEMORY, 1, strlen (value));
                ...
                if (*s == ',' || *s == '\0')
                  {
                    ...
                    *s = '\0';
                    if (intl_mbs_casecmp ("DEFAULT", p) == 0)
                      {
                        if (prm->param_id == PRM_ID_CALL_STACK_DUMP_ACTIVATION)
                          {
                            memcpy (&val[list_size + 1],
                                    call_stack_dump_error_codes, sizeof (call_stack_dump_error_codes));
                            list_size += DIM (call_stack_dump_error_codes);
                          }
                        else if (prm->param_id == PRM_ID_HA_IGNORE_ERROR_LIST)
                          {
                            memcpy (&val[list_size + 1], ha_ignore_error_codes, sizeof (ha_ignore_error_codes));
          ...
          if (value.integer_list != NULL)
            {
              ...
              value.integer_list = (int *) malloc ((integer_list[0] + 1) * sizeof (int));
              if (value.integer_list == NULL)
                {
                  er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE,
                          ER_OUT_OF_VIRTUAL_MEMORY, 1, (integer_list[0] + 1) * sizeof (int));
                  return PRM_ERR_NO_MEM_FOR_PRM;
                }
...
prm_set_force (SYSPRM_PARAM * prm, const char *value)
{
  ...
  if (prm->force_value == NULL)
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_OUT_OF_VIRTUAL_MEMORY, 1, strlen (value) + 1);
      return PRM_ERR_NO_MEM_FOR_PRM;
    }
...
prm_tune_parameters (void)
{
  ...
  /* check parameters */
  if (ha_mode_prm == NULL || test_mode_prm == NULL || auto_restart_server_prm == NULL)
    {
      assert (false);
...
prm_tune_parameters (void)
{
  ...
  ha_check_disk_failure_interval_prm = prm_find (PRM_NAME_HA_CHECK_DISK_FAILURE_INTERVAL, NULL);
  rye_shm_key_prm = prm_find (PRM_NAME_RYE_SHM_KEY, NULL);
  ...
  ha_check_disk_failure_interval_value = PRM_GET_BIGINT (ha_check_disk_failure_interval_prm->value);
  ...
  else if (ha_check_disk_failure_interval_value - ha_copy_log_timeout_value < HB_MIN_DIFF_CHECK_DISK_FAILURE_INTERVAL)
    {
      ha_check_disk_failure_interval_value = ha_copy_log_timeout_value + HB_MIN_DIFF_CHECK_DISK_FAILURE_INTERVAL;
  ...
  if (sysprm_get_force_server_parameters (&force_server_values) == NO_ERROR && force_server_values != NULL)
    {
      ...
      (void) sysprm_change_parameter_values (force_server_values, false, true);
    }
...
prm_get_value (PARAM_ID prm_id)
{
  ...
  if (!PRM_IS_BIGINT (&prm_Def[prm_id])
      && (PRM_HAS_SIZE_UNIT (prm_Def[prm_id].flag) || PRM_HAS_TIME_UNIT (prm_Def[prm_id].flag)))
    {
      assert (false);
    }
    ...
    case PRM_INTEGER_LIST:
      {
        ...
        if (temp == -1)
        ...
        else
          {
            ...
            if (value->integer_list == NULL)
              {
                er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_OUT_OF_VIRTUAL_MEMORY, 1, (temp + 1) * OR_INT_SIZE);
                return NULL;
              }
            else
              {
...
#endif
      ptr = sysprm_pack_sysprm_value (ptr, av->value, GET_PRM_DATATYPE (av->prm_id));
    }
  ...
  for (av = assign_values; av != NULL; av = av->next)
    {
      ...
      size += sysprm_packed_sysprm_value_length (av->value, GET_PRM_DATATYPE (av->prm_id), size + offset);
    }
  ...
  for (i = 0; i < count; i++)
    {
      ...
      ptr = sysprm_unpack_sysprm_value (ptr, &av->value, GET_PRM_DATATYPE (av->prm_id));
      if (ptr == NULL)
        {
      ...
      if (PRM_GET_STRING (prm->value) != NULL)
        {
          ...
          if (prm_value->str == NULL)
            {
              er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE,
                      ER_OUT_OF_VIRTUAL_MEMORY, 1, strlen (PRM_GET_STRING (prm->value)) + 1);
            }
        }
      else
        {
...
sysprm_print_assign_values (SYSPRM_ASSIGN_VALUE * prm_values, char *buffer, int length)
{
  ...
  for (av = prm_values; av != NULL; av = av->next)
    {
      n += sysprm_print_sysprm_value (av->prm_id, av->value, buffer + n, length - n, PRM_PRINT_NAME);
      if (av->next)
        {
 ...
 */
int
sysprm_print_assign_names (char *buffer, int length, SYSPRM_ASSIGN_VALUE * prm_values)
{
  const SYSPRM_ASSIGN_VALUE *av;
  int n = 0;
  int num_printed = 0;
  ...
  for (av = prm_values; av != NULL; av = av->next)
    {
      n = snprintf (buffer + num_printed, length - num_printed, "%s", prm_get_name (av->prm_id));
      if (n < 0)
        {
          assert (false);
          return n;
...
prm_get_myself_node_info ()
{
  ...
  PRM_NODE_INFO_SET (&node_info, prm_get_ha_node_myself (), prm_get_ha_node_myself_port ());
  return node_info;
}
...
prm_node_list_post_assign (SYSPRM_PARAM * prm)
{
  ...
  assert (prm->param_id == PRM_ID_HA_NODE_LIST || prm->param_id == PRM_ID_HA_REPLICA_LIST);
...
int
prm_split_node_str (PRM_NODE_LIST * node_list, const char *node_list_str, bool include_local_host)
{
  int error;

  error = prm_split_node_str_internal (node_list, node_list_str, include_local_host);
  if (error != NO_ERROR)
    {
      return error;
    }
...
static int
prm_split_node_str_internal (PRM_NODE_LIST * node_list, const char *node_list_str, bool include_local_host)
{
  char **list_pp = NULL;
  char *p;
