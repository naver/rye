...
extern BTID *xbtree_add_index (THREAD_ENTRY * thread_p, BTID * btid,
			       int num_atts, DB_TYPE * att_type,
			       OID * class_oid, int attr_id);
extern int xbtree_load_data (THREAD_ENTRY * thread_p, BTID * btid,
...
extern int xstats_update_statistics (THREAD_ENTRY * thread_p,
				     OID * classoid, bool update_stats,
				     bool with_fullscan);
...
extern DKNPAGES xdisk_get_total_numpages (THREAD_ENTRY * thread_p,
					  VOLID volid);
extern char *xdisk_get_fullname (THREAD_ENTRY * thread_p,
				 const char *database_name, VOLID volid,
				 char *vol_fullname);
...
extern int xsession_clear_query_entry_info (THREAD_ENTRY * thread_p,
					    const QUERY_ID query_id);
extern bool xlogtb_exist_working_tran (UNUSED_ARG THREAD_ENTRY * thread_p,
				       int group_id);
extern int xlogtb_update_group_id (THREAD_ENTRY * thread_p, int migrator_id,
				   int group_id, int target, int on_off);
extern int xlogtb_block_global_dml (THREAD_ENTRY * thread_p,
				    int start_or_end);
