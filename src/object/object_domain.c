...
tp_value_coerce_internal (const DB_VALUE * src, DB_VALUE * dest,
                          const TP_DOMAIN * desired_domain, TP_COERCION_MODE coercion_mode, bool preserve_domain);
 ...
 */
static int
tp_domain_match_internal (const TP_DOMAIN * dom1, const TP_DOMAIN * dom2, TP_MATCH exact, UNUSED_ARG bool match_order)
{
  int match = 0;
...
tp_domain_get_list (DB_TYPE type)
{
  ...
  assert ((type == DB_TYPE_NULL
           ...
           || type == DB_TYPE_VARBIT
           || type == DB_TYPE_BIGINT || type == DB_TYPE_DATETIME) || (tp_Domains[type] == &tp_Null_domain));
#endif
  ...
  while (domain != NULL)
    {
      ...
      switch (TP_DOMAIN_TYPE (domain))
        ...
        case DB_TYPE_NUMERIC:
          assert (domain->precision != transient->precision || domain->scale != transient->scale);
          /*
          ...
          if (domain->precision == DB_DOUBLE_DECIMAL_PRECISION || domain->scale == DB_DEFAULT_NUMERIC_SCALE)
            {
              *ins_pos = domain;
              domain = domain->next_list;
            }
...
tp_swizzle_oid (TP_DOMAIN * domain)
{
  ...
  if ((type == DB_TYPE_OBJECT || type == DB_TYPE_OID) && domain->class_mop == NULL && !OID_ISNULL (&domain->class_oid))
    {
 ...
 */
TP_DOMAIN *
tp_domain_find_object (DB_TYPE type, OID * class_oid, struct db_object * class_mop)
{
  TP_DOMAIN *dom;
...
tp_domain_resolve (DB_TYPE domain_type, DB_OBJECT * class_obj,
                   int precision, int scale, TP_DOMAIN * setdomain, int collation)
{
  TP_DOMAIN *d;
      ...
      switch (value_type)
        ...
        case DB_TYPE_VARBIT:
          ...
          if (value_type == DB_TYPE_VARCHAR)
            {
              if (precision == 0 || precision == TP_FLOATING_PRECISION_VALUE || precision > DB_MAX_VARCHAR_PRECISION)
                {
                  precision = DB_MAX_VARCHAR_PRECISION;
                }
            }
          else
            {
              ...
              if (precision == 0 || precision == TP_FLOATING_PRECISION_VALUE || precision > DB_MAX_VARBIT_PRECISION)
                {
                  precision = DB_MAX_VARBIT_PRECISION;
                }
            }

          domain = tp_domain_find_charbit (value_type, db_get_string_collation (val), precision);
          if (domain == NULL)
            {
 ...
 */
TP_DOMAIN *
tp_domain_select (const TP_DOMAIN * domain_list, const DB_VALUE * value, TP_MATCH exact_match)
{
  TP_DOMAIN *best, *d;
  DB_TYPE vtype;
...
tp_can_steal_string (const DB_VALUE * val, const DB_DOMAIN * desired_domain)
{
  ...
  switch (desired_type)
    {
    case DB_TYPE_VARCHAR:
      return (desired_precision >= original_length && original_type == DB_TYPE_VARCHAR);
    case DB_TYPE_VARBIT:
      return (desired_precision >= original_length && original_type == DB_TYPE_VARBIT);
    default:
      return 0;
    }
}
  ...
  if (string == NULL)
    {
      ...
      if (string == NULL)
        {
          er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_OUT_OF_VIRTUAL_MEMORY, 1, (tp - tmp) + sign + 1);
          return NULL;
        }
    }
  sp = string;
  ...
  switch (desired_type)
    ...
    case DB_TYPE_VARCHAR:
      db_make_varchar (&temp, desired_domain->precision,
                       (char *) new_string, strlen (new_string), TP_DOMAIN_COLLATION (desired_domain));
      break;
...
tp_value_coerce (const DB_VALUE * src, DB_VALUE * dest, const TP_DOMAIN * desired_domain)
{
  ...
  status = tp_value_coerce_internal (src, dest, desired_domain, TP_IMPLICIT_COERCION, false);
...
tp_value_coerce_strict (const DB_VALUE * src, DB_VALUE * dest, const TP_DOMAIN * desired_domain)
{
  ...
  if (!TP_IS_NUMERIC_TYPE (desired_type) && !TP_IS_DATE_OR_TIME_TYPE (desired_type))
    {
      db_make_null (dest);
      return ER_FAILED;
    }
            ...
            if (tp_atonumeric (src, &temp) != NO_ERROR)
            ...
            else
              {
                err = numeric_coerce_num_to_num (db_locate_numeric (&temp),
                                                 ...
                                                 DB_VALUE_SCALE (&temp),
                                                 desired_domain->precision, desired_domain->scale, num);
                if (err == NO_ERROR)
                  {
                    err = DB_MAKE_NUMERIC (target, num, desired_domain->precision, desired_domain->scale);
                  }
              }
            break;
          }
...
static TP_DOMAIN_STATUS
tp_value_coerce_internal (const DB_VALUE * src, DB_VALUE * dest,
                          const TP_DOMAIN * desired_domain, const TP_COERCION_MODE coercion_mode, bool preserve_domain)
{
  ...
  if (target != NULL && TP_IS_CHAR_TYPE (DB_VALUE_DOMAIN_TYPE (target)))
    {
      db_string_put_cs_and_collation (target, TP_DOMAIN_COLLATION (desired_domain));
    }
            ...
            if (tp_atonumeric (src, &temp) != NO_ERROR)
            ...
            else
              {
                if (numeric_coerce_num_to_num (db_locate_numeric (&temp),
                                               DB_VALUE_PRECISION (&temp),
                                               DB_VALUE_SCALE (&temp),
                                               desired_domain->precision, desired_domain->scale, num) != NO_ERROR)
                  {
                    status = DOMAIN_ERROR;
                    goto exit_on_error;
                  }

                if (DB_MAKE_NUMERIC (target, num, desired_domain->precision, desired_domain->scale) != NO_ERROR)
                  {
                    status = DOMAIN_ERROR;
                    goto exit_on_error;
                  }
        ...
        default:
          {
            int error_code = numeric_db_value_coerce_to_num ((DB_VALUE *) src,
                                                             target,
                                                             &data_stat);
            if (error_code == ER_NUM_OVERFLOW || data_stat == DATA_STATUS_TRUNCATED)
              {
                status = DOMAIN_OVERFLOW;
              }
                  ...
                  if (tp_domain_compatible (set_domain, desired_domain))
                  ...
                  else
                    {
                      ...
                      setref = set_coerce (setref, (TP_DOMAIN *)
                                           desired_domain, (coercion_mode == TP_IMPLICIT_COERCION));
                    }
        ...
        case DB_TYPE_VARCHAR:
          if (src == dest && tp_can_steal_string (src, desired_domain))
          ...
          else
            {
              ...
              if (target != NULL && TP_IS_CHAR_TYPE (DB_VALUE_DOMAIN_TYPE (target)))
                {
                  if (db_string_put_cs_and_collation (target, TP_DOMAIN_COLLATION (desired_domain)) != NO_ERROR)
                    {
                      status = DOMAIN_INCOMPATIBLE;
                    }
                }
        ...
        case DB_TYPE_DOUBLE:
          {
            ...
            if (DB_IS_NULL (target))
              ...
              if (desired_domain->precision != TP_FLOATING_PRECISION_VALUE
                  && desired_domain->precision < DB_GET_STRING_LENGTH (target))
              {
                status = DOMAIN_OVERFLOW;
  ...
  if (er_errid () == NO_ERROR)
    {
      switch (status)
        ...
        case DOMAIN_OVERFLOW:
          er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_IT_DATA_OVERFLOW, 1, pr_type_name (desired_type));
          break;
 ...
 */
TP_DOMAIN_STATUS
tp_value_cast (const DB_VALUE * src, DB_VALUE * dest, const TP_DOMAIN * desired_domain)
{
  TP_DOMAIN_STATUS status;
...
//      assert (false);
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_TP_CANT_COERCE, 2,
              pr_type_name (DB_VALUE_TYPE (src)), pr_type_name (DB_TYPE_VARIABLE));

      return DOMAIN_INCOMPATIBLE;
    }

  status = tp_value_coerce_internal (src, dest, desired_domain, TP_EXPLICIT_COERCION, false);
  ...
  if (!ARE_COMPARABLE (vtype1, vtype2))
    {
      ...
      if (tp_more_general_type (vtype1, vtype2) > 0)
        {
          cmp_res = DB_GT;
          er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_TP_CANT_COERCE, 2, pr_type_name (vtype1), pr_type_name (vtype2));
        }
      else
        {
          cmp_res = DB_LT;
          er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_TP_CANT_COERCE, 2, pr_type_name (vtype2), pr_type_name (vtype1));
        }
    }
  else
    {
      ...
      if (pr_type)
        {
          ...
          if (common_coll == -1)
            ...
            {
              cmp_res = (*(pr_type->cmpval)) (v1, v2, do_coercion, total_order, common_coll);
            }
        }
      else
        {
 ...
 */
int
tp_value_str_cast_to_number (DB_VALUE * src, DB_VALUE * dest, DB_TYPE * val_type)
{
...
tp_infer_common_domain (TP_DOMAIN * arg1, TP_DOMAIN * arg2, bool force_arg1)
{
  ...
  if (TP_IS_CHAR_TYPE (arg1_type) && TP_IS_CHAR_TYPE (arg2_type))
    {
      /* check collation */
      LANG_RT_COMMON_COLL (TP_DOMAIN_COLLATION (arg1), TP_DOMAIN_COLLATION (arg2), common_coll);
      if (common_coll == -1)
        {
          er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_QSTR_INCOMPATIBLE_COLLATIONS, 0);
  ...
  else if (TP_IS_CHAR_TYPE (arg1_type) && (TP_IS_NUMERIC_TYPE (arg2_type) || TP_IS_DATE_TYPE (arg2_type)))
    {
      common_type = arg2_type;
      target_domain = tp_domain_copy (arg2);
      need_to_domain_update = true;
    }
  else if ((TP_IS_NUMERIC_TYPE (arg1_type) || TP_IS_DATE_TYPE (arg1_type)) && TP_IS_CHAR_TYPE (arg2_type))
    {
      common_type = arg1_type;
      target_domain = tp_domain_copy (arg1);
      need_to_domain_update = true;
