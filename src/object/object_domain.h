...
extern TP_DOMAIN *tp_domain_construct (DB_TYPE domain_type,
                                       DB_OBJECT * class_obj, int precision, int scale, TP_DOMAIN * setdomain);
...
extern TP_DOMAIN *tp_domain_select (const TP_DOMAIN * domain_list, const DB_VALUE * value, TP_MATCH exact_match);
...
TP_DOMAIN *tp_domain_find_charbit (DB_TYPE type, int collation_id, int precision);
#if 0
TP_DOMAIN *tp_domain_find_object (DB_TYPE type, OID * class_oid, struct db_object *class_);
#endif
...
extern int tp_value_str_cast_to_number (DB_VALUE * src, DB_VALUE * dest, DB_TYPE * val_type);
extern TP_DOMAIN *tp_infer_common_domain (TP_DOMAIN * arg1, TP_DOMAIN * arg2, bool force_arg1);
