...
static SOCKET recv_client_fd_from_broker (SOCKET br_sock_fd, int *client_ip_addr, struct timeval *broker_recv_time);
static int send_broker_response (SOCKET client_sock_fd, int result_code);
static int read_db_connect_msg (T_DB_CONNECT_MSG ** db_connect_msg, SOCKET sock_fd);
static int cas_send_connect_reply_to_driver (SOCKET client_sock_fd, bool send_server_info, T_NET_BUF * net_buf);
...
static int net_read_int_keep_con_auto (SOCKET clt_sock_fd, MSG_HEADER * client_msg_header);
...
static bool check_server_alive (const char *db_name, const PRM_NODE_INFO * node_info);
...
static int
cas_send_connect_reply_to_driver (SOCKET client_sock_fd, bool send_server_info, T_NET_BUF * net_buf)
{
  ...
  if (net_buf->err_code)
    {
      net_write_error (client_sock_fd, cas_Info_size, CAS_ERROR_INDICATOR, net_buf->err_code, NULL);
      return -1;
    }
  ...
  if (as_Info->con_status == CON_STATUS_IN_TRAN)
    ...
    {
      cas_msg_header.info_ptr[CAS_STATUS_INFO_IDX_STATUS] = CAS_STATUS_INACTIVE;
    }
  ...
  *(cas_msg_header.msg_body_size_ptr) = htonl (net_buf->data_size);
  ...
  if (net_write_stream (client_sock_fd, net_buf->data, NET_BUF_CURR_SIZE (net_buf)) < 0)
    {
      return -1;
    }
...
cas_main (void)
{
...
#if defined (PROTOCOL_EXTENDS_DEBUG)
  prev_Cas_info[CAS_STATUS_INFO_IDX_STATUS] = CAS_STATUS_INFO_RESERVED_DEFAULT;
#endif

  ut_get_as_port_name (port_name, shm_Appl->broker_name, shm_As_index, BROKER_PATH_MAX);
  ...
  psize_At_start = as_Info->psize = (int) (os_get_mem_size (getpid (), MEM_VSIZE) / ONE_K);
  ...
  for (;;)
    {
      ...
      client_Sock_fd = recv_client_fd_from_broker (br_sock_fd, (int *) &client_ip_addr, &broker_recv_time);
      ...
      if (err_code < 0)
        {
          net_write_error (client_Sock_fd, cas_Info_size, CAS_ERROR_INDICATOR, err_code, NULL);
          RYE_CLOSE_SOCKET (client_Sock_fd);
          goto finish_cas;
        }
      else
        {
          ...
          if (strcmp (db_connect_msg->db_name, HEALTH_CHECK_DUMMY_DB) == 0)
            {
              cas_sql_log_write_and_end (0, "Incoming health check request from client.");

              cas_send_connect_reply_to_driver (client_Sock_fd, false, &net_buf);
              RYE_CLOSE_SOCKET (client_Sock_fd);

              goto finish_cas;
            }

          strncpy (as_Info->client_version, db_connect_msg->client_version, SRV_CON_VER_STR_MAX_SIZE);

          cas_sql_log_write_and_end (0, "CLIENT VERSION %s", as_Info->client_version);
          ...
          if (br_acl_check_right (shm_Appl, NULL, 0,
                                  ...
                                  db_connect_msg->db_user, (unsigned char *) (&client_ip_addr)) < 0)
            {
              char err_msg[1024];
              ...
              cas_sql_log_write_and_end (0,
                                         ...
                                         db_connect_msg->db_name, db_connect_msg->db_user, db_connect_msg->url);

              if (shm_Appl->access_log == ON)
                {
                  cas_access_log (&cas_start_time,
                                  shm_As_index,
                                  client_ip_addr, db_connect_msg->db_name, db_connect_msg->db_user, ACL_REJECTED);
                }
          ...
          err_code = ux_database_connect (db_connect_msg->db_name,
                                          db_connect_msg->db_user, db_connect_msg->db_passwd, &db_err_msg);

          if (err_code < 0)
            {
              char msg_buf[LINE_MAX];

              net_write_error (client_Sock_fd, cas_Info_size, err_Info.err_indicator, err_Info.err_number, db_err_msg);

              if (db_err_msg == NULL)
                {
                  snprintf (msg_buf, LINE_MAX,
                            "connect db %s user %s url %s, error:%d.",
                            db_connect_msg->db_name, db_connect_msg->db_user, db_connect_msg->url, err_Info.err_number);
                }
              else
                {
                  snprintf (msg_buf, LINE_MAX,
                            ...
                            db_connect_msg->db_name,
                            db_connect_msg->db_user, db_connect_msg->url, err_Info.err_number, db_err_msg);
                }
          ...
          if (shm_Appl->access_log == ON)
            {
              ...
              cas_access_log (&cas_start_time, shm_As_index,
                              client_ip_addr, db_connect_msg->db_name, db_connect_msg->db_user, type);
            }

          connected_node = boot_get_host_connected ();
          prm_node_info_to_str (connected_node_str, sizeof (connected_node_str), &connected_node);
          cas_sql_log_write_and_end (0, "connect db %s user %s url %s"
                                     " connected host %s",
                                     db_connect_msg->db_name,
                                     db_connect_msg->db_user, db_connect_msg->url, connected_node_str);
          ...
          if (cas_send_connect_reply_to_driver (client_Sock_fd, true, &net_buf) < 0)
            {
...
send_broker_response (SOCKET client_sock_fd, int result_code)
{
  ...
  if (net_write_stream (client_sock_fd, res_msg.msg_buffer, res_msg.msg_buffer_size) < 0)
    {
      return -1;
    }
...
static int
read_db_connect_msg (T_DB_CONNECT_MSG ** db_connect_msg, SOCKET client_sock_fd)
{
  ...
  if (*db_connect_msg == NULL || (*db_connect_msg)->msg_size < *(client_msg_header.msg_body_size_ptr))
    {
      RYE_FREE_MEM (*db_connect_msg);
      *db_connect_msg = RYE_MALLOC (sizeof (T_DB_CONNECT_MSG) + *(client_msg_header.msg_body_size_ptr));
    }
  ...
  if (net_read_stream (client_sock_fd, con_msg->msg_buffer, con_msg->msg_size) < 0)
    {
      return CAS_ER_COMMUNICATION;
    }

  argc = net_decode_str (con_msg->msg_buffer, con_msg->msg_size, &func_code, &argv);
  if (argc < 0)
    {
      return argc;
    }
...
static SOCKET
recv_client_fd_from_broker (SOCKET br_sock_fd, int *client_ip_addr, struct timeval *broker_recv_time)
{
  int con_status;
  ...
  if (net_read_int (br_sock_fd, &con_status) < 0)
    {
      cas_sql_log_write_and_end (0, "HANDSHAKE ERROR net_read_int(con_status)");
      return -1;
    }
  if (net_write_int (br_sock_fd, as_Info->con_status) < 0)
    {
      cas_sql_log_write_and_end (0, "HANDSHAKE ERROR net_write_int(con_status)");
      return -1;
    }
  ...
  if (client_sock_fd == -1)
    {
      cas_sql_log_write_and_end (0, "HANDSHAKE ERROR recv_fd %d", client_sock_fd);
      return -1;
    }
  if (net_write_int (br_sock_fd, as_Info->uts_status) < 0)
    {
      cas_sql_log_write_and_end (0, "HANDSHAKE ERROR net_write_int(uts_status)");
      RYE_CLOSE_SOCKET (client_sock_fd);
      return -1;
    }

  if (setsockopt (client_sock_fd, IPPROTO_TCP, TCP_NODELAY, (char *) &one, sizeof (one)) < 0)
    {
      assert (0);
    }
      ...
      for (i = 0; i < shm_Appl->unusable_databases_cnt[u_index]; i++)
        {
          ...
          unusable_db_host = &shm_Appl->unusable_databases[u_index][i].db_node;

          if (strcmp (unusable_db_name, db_name) == 0 && prm_is_same_node (unusable_db_host, node_info) == true)
            {
              return false;
            }
        }
...
process_request (SOCKET sock_fd, T_NET_BUF * net_buf, T_REQ_INFO * req_info)
{
  ...
  if (read_msg == NULL)
    {
      net_write_error (sock_fd, cas_Info_size, CAS_ERROR_INDICATOR, CAS_ER_NO_MORE_MEMORY, NULL);
      return FN_CLOSE_CONN;
    }
  if (net_read_stream (sock_fd, read_msg, *(client_msg_header.msg_body_size_ptr)) < 0)
    {
      RYE_FREE_MEM (read_msg);
      net_write_error (sock_fd, cas_Info_size, CAS_ERROR_INDICATOR, CAS_ER_COMMUNICATION, NULL);
  ...
  STRNCPY (as_Info->log_msg, server_Fn_table[func_code - 1].name, sizeof (as_Info->log_msg));

  server_fn = server_Fn_table[func_code - 1].func;

#if defined (PROTOCOL_EXTENDS_DEBUG)    /* for debug cas <-> JDBC info */
  if (prev_Cas_info[CAS_STATUS_INFO_IDX_STATUS] != CAS_STATUS_INFO_RESERVED_DEFAULT)
    {
      assert (prev_Cas_info[CAS_STATUS_INFO_IDX_STATUS] == client_msg_header.info_ptr[CAS_STATUS_INFO_IDX_STATUS]);
      if (prev_Cas_info[CAS_STATUS_INFO_IDX_STATUS] != client_msg_header.info_ptr[CAS_STATUS_INFO_IDX_STATUS])
        {
  ...
  if (fn_ret == FN_KEEP_CONN && net_buf->err_code == 0
      && as_Info->con_status == CON_STATUS_IN_TRAN && req_info->need_auto_commit != TRAN_NOT_AUTOCOMMIT)
    {
  ...
  if (net_buf->err_code)
    {
      net_write_error (sock_fd, cas_Info_size, CAS_ERROR_INDICATOR, net_buf->err_code, NULL);
      fn_ret = FN_CLOSE_CONN;
      goto exit_on_end;
    }
  ...
  if (cas_Send_result_flag && net_buf->data != NULL)
    {
      ...
      prev_Cas_info[CAS_STATUS_INFO_IDX_STATUS] = cas_msg_header.info_ptr[CAS_STATUS_INFO_IDX_STATUS];
      ...
      *(cas_msg_header.msg_body_size_ptr) = htonl (net_buf->data_size);
      ...
      if (net_write_stream (sock_fd, net_buf->data, NET_BUF_CURR_SIZE (net_buf)) < 0)
        {
          cas_sql_log_write_and_end (0, "COMMUNICATION ERROR net_write_stream()");
        }
    }
  ...
  if (as_Info->reset_flag
    ...
    {
      er_log_debug (ARG_FILE_LINE, "process_request: reset_flag && !CON_STATUS_IN_TRAN");
      fn_ret = FN_KEEP_SESS;
      goto exit_on_end;
    }
...
cas_init ()
{
  ...
  snprintf (filename, sizeof (filename), "%s_%d.err", shm_Appl->broker_name, shm_As_index + 1);
  envvar_ryelog_broker_errorlog_file (err_log_file, sizeof (err_log_file), shm_Appl->broker_name, filename);
...
static int
net_read_int_keep_con_auto (SOCKET clt_sock_fd, MSG_HEADER * client_msg_header)
{
  int ret_value = 0;
          ...
          if (as_Info->con_status == CON_STATUS_OUT_TRAN && is_net_timed_out ())
            {
              if (restart_is_needed ())
                {
                  er_log_debug (ARG_FILE_LINE, "net_read_int_keep_con_auto: " "restart_is_needed()");
                  ret_value = -1;
                  break;
                }
  ...
  CON_STATUS_LOCK (&(shm_Appl->info.as_info[shm_As_index]), CON_STATUS_LOCK_CAS);
  ...
  CON_STATUS_UNLOCK (&(shm_Appl->info.as_info[shm_As_index]), CON_STATUS_LOCK_CAS);
...
cas_init_shm (void)
{
  ...
  shm_Br_master = rye_shm_attach (shm_Appl->shm_key_br_global, RYE_SHM_TYPE_BROKER_GLOBAL, false);
...
cas_shard_info_version ()
{
  ...
  if (idx_Shm_shard_info < 0)
    {
      ...
      for (i = 0; i < shm_Br_master->num_shard_version_info; i++)
        {
          if (strcmp (shm_Br_master->shard_version_info[i].local_dbname, local_dbname) == 0)
            {
  ...
  if (idx_Shm_shard_info >= 0)
    {
      shard_info_version = shm_Br_master->shard_version_info[idx_Shm_shard_info].shard_info_ver;
    }
