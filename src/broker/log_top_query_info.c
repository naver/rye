...
{
  RYE_FREE_MEM (qi->sql);
  RYE_FREE_MEM (qi->organized_sql);
  RYE_FREE_MEM (qi->cas_log);
  qi->start_date[0] = '\0';
}
...
query_info_add_ne ()
{
  ...
  RYE_FREE_MEM (query_info_arr_ne[qi_idx].cas_log);
...
{
  const T_QUERY_INFO *info1 = arg1;
  const T_QUERY_INFO *info2 = arg2;

  return (info1->max < info2->max);
}
