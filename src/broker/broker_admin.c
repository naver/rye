...
broker_admin (int command_type, int argc, char **argv)
{
  ...
  err = broker_config_read (br_info, &num_broker, &shm_key_br_gl, admin_log_file, 1);
  if (err < 0)
    {
...
broker_get_local_mgmt_info (char **broker_key, int *port)
{
  ...
  error = admin_get_broker_key_and_portid (broker_key, port, shm_key_br_gl, BR_LOCAL_MGMT_NAME, LOCAL_MGMT);
  if (error < 0)
    {
      return -1;
    }
