...
static int execute_test_with_query (CCI_CONN * conn, const char *query, bool verbose_mode);
...
static int test_broker (const char *host, int port, const char *dbname,
                        ...
                        const char *broker_name, const char *query, bool test_server_shard_nodeid, bool verbose_mode);
static int
test_local_mgmt (int num_broker, const char *host, int local_mgmt_port,
                 char broker_name[][BROKER_NAME_LEN],
                 const char *db_name, const char *db_user,
                 const char *db_passwd, const char *sql_command, bool test_server_shard_nodeid, bool verbose_mode);
  ...
  if (br_tester_info.tester_shard != NULL)
  ...
  else
    {
      ret = test_local_mgmt (br_tester_info.tester_local->num_broker,
                             ...
                             br_tester_info.db_passwd, br_tester_info.command, false, br_tester_info.verbose_mode);
    }
...
get_option (TESTER_INFO * br_tester_info, int argc, char **argv)
{
  ...
  if (br_tester_info->test_shard_mgmt == false && br_tester_info->db_name == NULL)
    {
      goto usage;
    }
  ...
  if (optind < argc)
    {
      int i, arg_idx = 0;
      br_tester_info->test_broker_arg = malloc (sizeof (char *) * (argc - optind + 1));
      ...
      if (shm_br->br_info[i].broker_type == SHARD_MGMT)
        {
          strncpy (tester_shard->shard_mgmt_info[count].global_dbname,
                   shm_br->br_info[i].shard_global_dbname, SRV_CON_DBNAME_SIZE - 1);
...
init_tester_info_local (T_SHM_BROKER * shm_br, const char **test_broker_arg)
{
  ...
  malloc_size = sizeof (TESTER_LOCAL_MGMT) + (BROKER_NAME_LEN * shm_br->num_broker);
      ...
      if (shm_br->br_info[i].broker_type == NORMAL_BROKER)
        {
          if (is_test_broker_arg (shm_br->br_info[i].name, test_broker_arg))
            {
              strncpy (tester_local->broker_name[count++], shm_br->br_info[i].name, BROKER_NAME_LEN - 1);
            }
        }
...
init_tester_info (TESTER_INFO * br_tester_info)
{
  ...
  br_tester_info->tester_local = init_tester_info_local (shm_br, br_tester_info->test_broker_arg);
...
static int
execute_test_with_query (CCI_CONN * conn, const char *query, bool verbose_mode)
{
  int ret;
  ...
  if (ret < 0)
  ...
  else
    {
      PRINT_RESULT ("%sROW COUNT=%d, EXECUTION TIME=%.6f sec\n",
                    PRINT_INDENT3, ret, timeval_diff_in_msec (NULL, &start_time) / 1000.0);
...
make_connection (CCI_CONN * conn, const char *host, int port,
                 ...
                 const char *db_passwd, const char *broker_name, bool is_shard_mgmt, bool test_server_shard_nodeid)
{
  int ret;
  ...
  snprintf (conn_url, sizeof (conn_url), "cci:rye://%s:%d/%s:dba/%s?%s", host, port, dbname, broker_name, url_property);
  ...
  PRINT_RESULT ("%sCONNECT [%s]\t %s %s\n",
                ...
                (ret < 0 ? "FAIL" : "OK"), tester_server_nodeid, tester_err_msg);
  ...
  for (i = 0; i < br_tester_info->tester_shard->num_shard_mgmt; i++)
    {
      ...
      if (br_tester_info->db_name == NULL)
        ...
        {
          if (strcmp (br_tester_info->db_name, tester_shard->shard_mgmt_info[i].global_dbname) != 0)
            {
              continue;
            }
        }
      ...
      ret = make_connection (&conn, "localhost",
                             ...
                             br_tester_info->db_user, br_tester_info->db_passwd, "rw", true, false);
      if (ret < 0)
        {
          goto test_end;
        }
      ...
      for (j = 0; j < node_info->node_count; j++)
        {
          ret = test_local_mgmt (br_tester_info->tester_local->num_broker,
                                 ...
                                 br_tester_info->command, true, br_tester_info->verbose_mode);
          if (ret < 0)
            {
              goto test_end;
            }
             ...
             const char *broker_name, const char *query, bool test_server_shard_nodeid, bool verbose_mode)
{
  ...
  if (make_connection (&conn, host, port, dbname, db_user, db_passwd, broker_name, false, test_server_shard_nodeid) < 0)
    {
      return -1;
    }
  ...
  for (i = 0; i < num_broker; i++)
    {
      if (test_broker (host,
                       local_mgmt_port,
                       db_name,
                       db_user, db_passwd, broker_name[i], sql_command, test_server_shard_nodeid, verbose_mode) < 0)
        {
          return -1;
        }
    }
