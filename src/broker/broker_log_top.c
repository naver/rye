...
#ifdef MT_MODE
  work_msg = RYE_MALLOC (sizeof (T_WORK_MSG) * num_thread);
  if (work_msg == NULL)
    {
...
static int
log_execute (char *linebuf, char **query_p)
{
  char *p;
  int exec_h_id;
...
static int
read_execute_end_msg (const char *msg_p, int *res_code, int *runtime_msec)
{
  char *p, *next_p;
  int sec, msec;
//  int tuple_count;
