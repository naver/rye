...
static int acl_set_internal (const BROKER_ACL_CONF * br_acl_conf,
			     ...
			     int max_num_acl_info, const char *broker_name,
			     char *admin_err_msg);
static int acl_check_ip (BR_ACL_INFO * acl_info, unsigned char *address);
static int find_acl_info (BR_ACL_INFO * acl_info, int num_acl_info,
...
static const BR_ACL_IP_SECTION *find_acl_ip_sect (const BR_ACL_IP_SECTION *
						  br_sect, int num_br_sect,
						  const char *ip_sect_name);
static int merge_acl_ip_info (ACL_IP_INFO * acl_ip_info1, int num_ip_info1,
			      const ACL_IP_INFO * acl_ip_info2,
			      int num_ip_info2, char *admin_err_msg);
static int acl_info_sort_comp (const void *p1, const void *p2);
static char *acl_read_line (FILE * fp, char *buf, int bufsize);
static int check_acl_conf (BROKER_ACL_CONF * br_acl_conf,
			   char *admin_err_msg);
static int set_group_info (BR_ACL_GROUP * acl_group, char *linebuf,
		 ...
		 const BROKER_ACL_CONF * br_acl_conf)
{
  ...
  if (br_acl_set_shm (shm_appl, NULL, br_acl_conf, shm_appl->broker_name,
		      admin_err_msg) < 0)
    {
      return -1;
    }
		...
		const char *broker_name, char *admin_err_msg)
{
  ...
  num_acl_info = acl_set_internal (br_acl_conf, acl_info,
				   ACL_MAX_ITEM_COUNT, broker_name,
				   admin_err_msg);
  if (num_acl_info == 0)
    {
      num_acl_info = acl_set_internal (br_acl_conf, acl_info,
  ...
  if (shm_appl != NULL)
	...
	{
	  memcpy (shm_appl->acl_info, acl_info,
		  sizeof (BR_ACL_INFO) * num_acl_info);
			 ...
			 bool make_default_acl_file, char *admin_err_msg)
{
  ...
  if (fp == NULL)
    {
      sprintf (admin_err_msg, "cannot open access control file(%s)",
	       filename);
  ...
  while (acl_read_line (fp, buf, sizeof (buf)))
	  ...
	  if (br_acl_conf->num_acl_group == MAX_NUM_ACL_GROUP_CONF)
	    {
	      sprintf (admin_err_msg,
		       ...
		       " - max broker list (%d) exceeded.",
		       MAX_NUM_ACL_GROUP_CONF);
	      goto error;
	    }

	  if (set_group_info
	      (&br_acl_conf->acl_group[br_acl_conf->num_acl_group], buf,
	       admin_err_msg, err_line_info) < 0)
	    {
	      goto error;
	    }
      ...
      else if (buf[0] == '[')
	{
	  ...
	  if (p == NULL)
	    {
	      sprintf (admin_err_msg,
		       "Error while loading ip info file %s", err_line_info);
	      goto error;
	    }
	  ...
	  if (br_acl_conf->num_ip_sect >= MAX_NUM_IP_SECT_CONF)
	    {
	      sprintf (admin_err_msg,
		       ...
		       " - max ip section(%d) exceeded.",
		       err_line_info, MAX_NUM_IP_SECT_CONF);
	      goto error;
	    }
      ...
      else
	{
	  if (cur_ip_sect->num_acl_ip_info >= ACL_MAX_IP_COUNT)
	    {
	      sprintf (admin_err_msg,
		       ...
		       err_line_info, ACL_MAX_IP_COUNT);
	      goto error;
	    }
	  ...
	  if (cur_ip_info == NULL)
	    ...
	    {
	      if (br_acl_conf_read_ip_addr (cur_ip_info, buf,
					    admin_err_msg, err_line_info) < 0)
		{
		  goto error;
		}
...
br_acl_check_right (T_SHM_APPL_SERVER * shm_appl,
		    ...
		    const char *arg_dbname, const char *dbuser,
		    unsigned char *address)
{
  ...
  if (acl_info == NULL)
      ...
      if (acl_Info_chn != shm_appl->acl_chn)
	{
	  ...
	  memcpy (acl_Info, shm_appl->acl_info,
		  sizeof (BR_ACL_INFO) * ACL_MAX_ITEM_COUNT);
  ...
  if (ip_info_idx < 0)
  ...
  else
    {
      if (shm_appl != NULL)
	{
	  shm_appl->acl_info[acl_info_idx].acl_ip_info[ip_info_idx].
	    ip_last_access_time = time (NULL);
	}
...
acl_set_internal (const BROKER_ACL_CONF * br_acl_conf,
		  BR_ACL_INFO * br_acl_info, int max_num_acl_info,
		  const char *broker_name, char *admin_err_msg)
{
  int i;
	  ...
	  if (acl_ip_sect == NULL)
	  ...
	  else
	    {
	      ...
	      update = find_exact_acl_info (br_acl_info, num_acl_info,
					    br_acl_conf->acl_group[i].dbname,
					    br_acl_conf->acl_group[i].dbuser);
	      if (update == NULL)
		{
		  ...
		  strcpy (br_acl_info[num_acl_info].dbname,
			  br_acl_conf->acl_group[i].dbname);
		  strcpy (br_acl_info[num_acl_info].dbuser,
			  br_acl_conf->acl_group[i].dbuser);
		...
		merge_acl_ip_info (update->acl_ip_info,
				   update->num_acl_ip_info,
				   acl_ip_sect->acl_ip_info,
				   acl_ip_sect->num_acl_ip_info,
				   admin_err_msg);
...
static int
set_group_info (BR_ACL_GROUP * acl_group, char *linebuf,
		char *admin_err_msg, const char *err_line_info)
{
  char *save = NULL;
  char *token[4];
  ...
  for (i = 0; i < 4; i++)
	...
	{
	  sprintf (admin_err_msg,
		   "error while loading access control file %s",
		   err_line_info);
	  return -1;
	}
  ...
  if (strlen (token[0]) >= BROKER_NAME_LEN ||
      ...
      strlen (token[2]) >= ACL_MAX_DBUSER_LENGTH ||
      strlen (token[3]) >= MAX_IP_SECT_NAME)
    {
      sprintf (admin_err_msg,
	       "error while loading access control file %s"
	       " - id is too long", err_line_info);
      return -1;
    }
...
merge_acl_ip_info (ACL_IP_INFO * acl_ip_info1, int num_ip_info1,
		   const ACL_IP_INFO * acl_ip_info2, int num_ip_info2,
		   char *admin_err_msg)
{
  ...
  memcpy (&all_ip_info[num_ip_info1], acl_ip_info2,
	  sizeof (ACL_IP_INFO) * num_ip_info2);
  ...
  if (all_count > ACL_MAX_IP_COUNT)
    {
      sprintf (admin_err_msg,
	       "Error while loading ip info file "
	       " - max ip count(%d) exceeded.", ACL_MAX_IP_COUNT);
      return -1;
    }
...
find_acl_ip_sect (const BR_ACL_IP_SECTION * br_sect, int num_br_sect,
		  const char *ip_sect_name)
{
  int i;
...
int
br_acl_conf_read_ip_addr (ACL_IP_INFO * ip_info, char *linebuf,
			  char *admin_err_msg, const char *err_line_info)
{
  char *token;
  char *save = NULL;
...
check_acl_conf (BROKER_ACL_CONF * br_acl_conf, char *admin_err_msg)
{
  ...
  for (i = 0; i < br_acl_conf->num_ip_sect; i++)
	...
	remove_duplicate_ip_info (br_acl_conf->ip_sect[i].acl_ip_info,
				  br_acl_conf->ip_sect[i].num_acl_ip_info);
    }
  ...
  for (i = 0; i < br_acl_conf->num_acl_group; i++)
    {
      if (find_acl_ip_sect (br_acl_conf->ip_sect,
			    br_acl_conf->num_ip_sect,
			    br_acl_conf->acl_group[i].ip_sect_name) == NULL)
	{
	  sprintf (admin_err_msg,
		   "error while loading access control file"
		   " - ip section (%s) not found",
		   br_acl_conf->acl_group[i].ip_sect_name);
	  return -1;
	}
    }
...
static int
find_acl_info (BR_ACL_INFO * acl_info, int num_acl_info,
	       const char *dbname, const char *dbuser)
{
  int i;
  ...
  for (i = 0; i < num_acl_info; i++)
    {
      if (acl_info[i].dbname[0] == '*' ||
	  strncmp (acl_info[i].dbname, dbname,
		   strlen (acl_info[i].dbname)) == 0)
	{
	  if (acl_info[i].dbuser[0] == '*' ||
	      strcmp (acl_info[i].dbuser, dbuser) == 0)
	    {
	      return i;
	    }
	}
...
static BR_ACL_INFO *
find_exact_acl_info (BR_ACL_INFO * acl_info, int num_acl_info,
		     const char *dbname, const char *dbuser)
{
  int i;
  ...
  for (i = 0; i < num_acl_info; i++)
    {
      if (strcmp (acl_info[i].dbname, dbname) == 0 &&
	  strcmp (acl_info[i].dbuser, dbuser) == 0)
	{
	  return &acl_info[i];
	}
...
acl_check_ip (BR_ACL_INFO * acl_info, unsigned char *address)
{
  ...
  for (i = 0; i < acl_info->num_acl_ip_info; i++)
    {
      if (memcmp ((void *) acl_info->acl_ip_info[i].ip_addr,
		  (void *) address, acl_info->acl_ip_info[i].ip_len) == 0)
	{
	  return i;
	}
    }
