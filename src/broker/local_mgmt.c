...
static int local_mg_transfer_req (int target_broker_type,
                                  SOCKET * clt_sock_fd, in_addr_t clt_ip_addr, const T_BROKER_REQUEST_MSG * br_req_msg);

static T_LOCAL_MGMT_JOB_QUEUE *local_mg_create_job_queue (int num_workers,
                                                          T_SHM_MGMT_QUEUE_INFO
                                                          * shm_queue_info, THREAD_FUNC (*worker_func) (void *));
...
static int local_mg_delete_conf (T_LOCAL_MGMT_JOB * job,
                                 const T_MGMT_REQ_ARG * req_arg, T_MGMT_RESULT_MSG * result_msg);
static int local_mg_get_conf (T_LOCAL_MGMT_JOB * job, const T_MGMT_REQ_ARG * req_arg, T_MGMT_RESULT_MSG * result_msg);
...
{
  int opcode;
  int (*func) (T_LOCAL_MGMT_JOB *, const T_MGMT_REQ_ARG *, T_MGMT_RESULT_MSG *);
  const char *log_msg;
...
local_mgmt_init ()
{
  ...
  gethostname (local_Mgmt_server_info.hostname, sizeof (local_Mgmt_server_info.hostname) - 1);
  return 0;
}
  ...
  while (br_Process_flag)
    {
      ...
      if (IS_NORMAL_BROKER_OPCODE (br_req_msg->op_code) || IS_SHARD_MGMT_OPCODE (br_req_msg->op_code))
        {
          ...
          err_code = local_mg_transfer_req (target_broker_type, &clt_sock_fd, clt_ip_addr, br_req_msg);
              ...
              if (br_req_msg->op_code == BRREQ_OP_CODE_CONNECT_DB_SERVER)
                {
                  err_code = local_mg_job_queue_add (job_Q_connect_db,
                                                     &clt_sock_fd, clt_ip_addr, clone_req_msg, NULL, true);
                }
              else
                {
                  err_code = local_mg_job_queue_add (job_Q_mgmt, &clt_sock_fd, clt_ip_addr, clone_req_msg, NULL, true);
                }
            }
...
local_mg_transfer_req (int target_broker_type, SOCKET * clt_sock_fd,
                       in_addr_t clt_ip_addr, const T_BROKER_REQUEST_MSG * br_req_msg)
{
  SOCKET br_sock_fd;
  int status;
  ...
  if (target_broker_type == NORMAL_BROKER)
    {
      ...
      transfer_broker_name = brreq_msg_unpack_port_name (br_req_msg, NULL, NULL);
      br_info_service_broker = ut_find_broker (shm_Br->br_info,
                                               shm_Br->num_broker, transfer_broker_name, NORMAL_BROKER);
    }
  else if (target_broker_type == SHARD_MGMT)
    {
      ...
      if (br_mgmt_get_req_arg (&req_arg, br_req_msg) >= 0)
        {
          br_info_service_broker = ut_find_shard_mgmt (shm_Br->br_info, shm_Br->num_broker, req_arg.clt_dbname);
  ...
  if (br_write_nbytes_to_client (br_sock_fd, br_req_msg->msg_buffer,
                                 BRREQ_MSG_SIZE + br_req_msg->op_code_msg_size, BR_SOCKET_TIMEOUT_SEC) < 0)
    {
  ...
  br_read_nbytes_from_client (br_sock_fd, (char *) &status, sizeof (int), BR_SOCKET_TIMEOUT_SEC);
...
local_mg_create_job_queue (int num_workers, T_SHM_MGMT_QUEUE_INFO * shm_queue_info, THREAD_FUNC (*worker_func) (void *))
{
  ...
  if (job_queue != NULL)
    {
      ...
      if (pthread_mutex_init (&job_queue->lock, NULL) < 0 || pthread_cond_init (&job_queue->cond, NULL) < 0)
        {
...
local_mg_init_mgmt_job_queue ()
{
  ...
  job_Q_mgmt = local_mg_create_job_queue (1, &shm_Local_mgmt_info->admin_req_queue, local_mg_admin_worker);
  job_Q_connect_db = local_mg_create_job_queue (1, &shm_Local_mgmt_info->db_connect_req_queue, local_mg_admin_worker);
  job_Q_child_proc = local_mg_create_job_queue (1, NULL, local_mg_child_process_waiter);

  if (job_Q_mgmt == NULL || job_Q_connect_db == NULL || job_Q_child_proc == NULL)
    {
      return -1;
    }
...
read_outfile (const char *filename, int *size, bool allow_partial_read)
{
  ...
  if ((filename == NULL) || (filename[0] == '\0') ||
      (stat (filename, &st) < 0) ||
      (allow_partial_read == false && st.st_size > FILE_SEND_MAX_SIZE) || ((fd = open (filename, O_RDONLY)) < 0))
    {
      assert (0);
    }
  else
...
static void
set_child_process_result (T_MGMT_RESULT_MSG * result_msg, T_LOCAL_MGMT_CHILD_PROC_INFO * child_proc_info)
{
  ...
  make_child_output_filename (infile, sizeof (infile), false,
                              outfile, sizeof (outfile), errfile, sizeof (errfile), child_proc_info->output_file_id);
          ...
          if (job != NULL && !IS_INVALID_SOCKET (job->clt_sock_fd))
            {
              ...
              br_mgmt_result_msg_set (&result_msg, sizeof (int), &exit_status);
      ...
      if (job != NULL)
        {
          ...
          for (i = 0; local_Mg_admin_func_table[i].func != NULL; i++)
            {
              if (local_Mg_admin_func_table[i].opcode == job->info.req_msg->op_code)
                {
          ...
          if (!IS_INVALID_SOCKET (job->clt_sock_fd))
            {
              br_send_result_to_client (job->clt_sock_fd, err_code, &result_msg);
              RYE_CLOSE_SOCKET (job->clt_sock_fd);
            }
  ...
  for (i = 0; i < SHM_MAX_SHARD_VERSION_INFO_COUNT; i++)
    {
      ...
      last_version = MAX (req_arg->value.shard_mgmt_info.nodeid_ver, req_arg->value.shard_mgmt_info.groupid_ver);

      if (strcmp (shm_Br->shard_version_info[i].local_dbname, req_arg->value.shard_mgmt_info.local_dbname) == 0)
        {
...
local_mg_sync_shard_mgmt_info (T_LOCAL_MGMT_JOB * job,
                               const T_MGMT_REQ_ARG * req_arg, UNUSED_ARG T_MGMT_RESULT_MSG * result_msg)
{
  HA_STATE server_state;
  PRM_NODE_INFO shard_mgmt_node_info;

  PRM_NODE_INFO_SET (&shard_mgmt_node_info, job->clt_ip, req_arg->value.shard_mgmt_info.port);

  rye_master_shm_add_shard_mgmt_info (req_arg->value.shard_mgmt_info.local_dbname,
                                      req_arg->value.shard_mgmt_info.global_dbname,
                                      req_arg->value.shard_mgmt_info.nodeid, &shard_mgmt_node_info);
  ...
  br_mgmt_result_msg_set (result_msg, strlen (local_Mgmt_server_info.hostname) + 1, local_Mgmt_server_info.hostname);

  if (rye_server_shm_get_state (&server_state, req_arg->value.shard_mgmt_info.local_dbname) == NO_ERROR)
    {
...
make_child_output_filename (char *infile, int infile_bufsize, bool mkinfile,
                            char *outfile, int outfile_bufsize, char *errfile, int errfile_bufsize, unsigned int seq)
{
...
#define MAX_APPEND_ARGS	2
  ...
  argv = RYE_MALLOC (sizeof (char *) * (launch_arg->argc + 2 + MAX_APPEND_ARGS));
  if (argv == NULL)
    {
      return BR_ER_NO_MORE_MEMORY;
    }
  ...
  error = run_child_process (job, cmd, argv, launch_arg->num_env, launch_arg->envp, send_outfile);
...
static int
run_child_process (T_LOCAL_MGMT_JOB * job, char *cmd, char **argv, int num_env, char **envp, bool send_outfile)
{
      ...
      if (child_info->output_file_id > 0)
        {
          ...
          make_child_output_filename (child_infile, sizeof (child_infile),
                                      ...
                                      child_errfile, sizeof (child_errfile), child_info->output_file_id);
          ...
          fd_stdout = open (child_outfile, O_CREAT | O_TRUNC | O_WRONLY, 0666);
          ...
          fd_stderr = open (child_errfile, O_CREAT | O_TRUNC | O_WRONLY, 0666);
  ...
  if (pid < 0)
  ...
  else
    {
      ...
      error = local_mg_job_queue_add (job_Q_child_proc, &job->clt_sock_fd, job->clt_ip, NULL, &child_info, false);
  ...
  while (job != NULL && num_child < SHM_MAX_CHILD_INFO)
    {
      if (job->info.child_info != NULL)
        {
          shm_Local_mgmt_info->child_process_info[num_child++] = *job->info.child_info;
        }
      job = job->next;
    }
...
local_mg_get_shard_mgmt_info (UNUSED_ARG T_LOCAL_MGMT_JOB * job,
                              UNUSED_ARG const T_MGMT_REQ_ARG * req_arg, T_MGMT_RESULT_MSG * result_msg)
{
  int i;
  int msg_size;
  char *send_msg;
      ...
      if (shm_Br->br_info[i].broker_type == SHARD_MGMT)
        {
          ...
          p = local_mgmt_pack_str (p, shm_Br->br_info[i].shard_global_dbname, dbname_len);
  ...
  if (write_len == arg_write_conf->size)
    {
      ...
      err = db_update_persist_conf_file ("server", "common", "rye_port_id", buf);
      if (err == NO_ERROR)
        {
          err = db_update_persist_conf_file ("server", "common", "rye_shm_key", rye_shm_key);
        }
                      ...
                      const T_MGMT_REQ_ARG * req_arg, UNUSED_ARG T_MGMT_RESULT_MSG * result_msg)
{
  ...
  err = db_update_persist_conf_file (arg_update_conf->proc_name,
                                     arg_update_conf->sect_name, arg_update_conf->key, arg_update_conf->value);
  ...
  if (arg_delete_conf->key != NULL)
    {
      err = db_delete_key_persist_conf_file (arg_delete_conf->proc_name,
                                             arg_delete_conf->sect_name, arg_delete_conf->key);
    }
  else if (arg_delete_conf->sect_name != NULL)
    {
      err = db_delete_sect_persist_conf_file (arg_delete_conf->proc_name, arg_delete_conf->sect_name);
    }
  else
    {
...
static int
local_mg_get_conf (UNUSED_ARG T_LOCAL_MGMT_JOB * job, const T_MGMT_REQ_ARG * req_arg, T_MGMT_RESULT_MSG * result_msg)
{
  ...
  if (sysprm_get_persist_conf (param_value, sizeof (param_value),
                               arg_get_conf->proc_name, arg_get_conf->sect_name, arg_get_conf->key) == NO_ERROR)
    {
      br_mgmt_result_msg_set (result_msg, strlen (param_value) + 1, param_value);
      return 0;
    }
                        ...
                        const T_MGMT_REQ_ARG * req_arg, UNUSED_ARG T_MGMT_RESULT_MSG * result_msg)
{
  ...
  envvar_tmpdir_file (tmp_acl_filepath, sizeof (tmp_acl_filepath), tmp_filename);
  ...
  if (file_write (tmp_acl_filepath, arg_br_acl_reload->acl, arg_br_acl_reload->size) < 0)
    {
      return BR_ER_BR_ACL_RELOAD;
    }
                            ...
                            const T_MGMT_REQ_ARG * req_arg, UNUSED_ARG T_MGMT_RESULT_MSG * result_msg)
{
  ...
  LOCK_CSS ();
  conn = css_connect_to_rye_server (&node_info, db_name, SVR_CONNECT_TYPE_TRANSFER_CONN);
  UNLOCK_CSS ();
