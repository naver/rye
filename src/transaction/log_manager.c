...
static int log_initialize_internal (THREAD_ENTRY * thread_p,
                                    ...
                                    int ismedia_crash, time_t * stopat, LOG_LSA * stopat_lsa, bool init_emergency);
#if defined(SERVER_MODE)
...
static LOG_LSA *log_get_savepoint_lsa (THREAD_ENTRY * thread_p,
                                       const char *savept_name, LOG_TDES * tdes, LOG_LSA * savept_lsa);
static bool log_can_skip_undo_logging (THREAD_ENTRY * thread_p,
                                       LOG_RCVINDEX rcvindex, const LOG_TDES * tdes, LOG_DATA_ADDR * addr);
...
static void log_append_repl_info (THREAD_ENTRY * thread_p, LOG_TDES * tdes, bool is_commit);
static void log_append_donetime (THREAD_ENTRY * thread_p, LOG_TDES * tdes, LOG_RECTYPE iscommitted);
...
static TRAN_STATE log_complete_system_op (THREAD_ENTRY * thread_p,
                                          LOG_TDES * tdes, LOG_RESULT_TOPOP result, TRAN_STATE back_to_state);
#if defined(RYE_DEBUG)
static void log_client_find_system_error (LOG_RECTYPE record_type, LOG_RECTYPE client_type);
#endif
...
static LOG_PAGE *log_dump_record_client_name (THREAD_ENTRY * thread_p,
                                              FILE * out_fp, LOG_LSA * lsa_p, LOG_PAGE * log_pgptr);
#endif
static LOG_PAGE *log_dump_record_undoredo (THREAD_ENTRY * thread_p,
                                           FILE * out_fp, LOG_LSA * lsa_p, LOG_PAGE * log_page_p, LOG_ZIP * log_zip_p);
static LOG_PAGE *log_dump_record_undo (THREAD_ENTRY * thread_p, FILE * out_fp,
                                       LOG_LSA * lsa_p, LOG_PAGE * log_page_p, LOG_ZIP * log_zip_p);
...
static LOG_PAGE *log_dump_record_postpone (THREAD_ENTRY * thread_p,
                                           FILE * out_fp, LOG_LSA * lsa_p, LOG_PAGE * log_page_p);
...
static LOG_PAGE *log_dump_record_ha_server_state (THREAD_ENTRY * thread_p,
                                                  FILE * out_fp, LOG_LSA * log_lsa, LOG_PAGE * log_page_p);
...
static LOG_PAGE *log_dump_record (THREAD_ENTRY * thread_p, FILE * out_fp,
                                  LOG_RECTYPE record_type, LOG_LSA * lsa_p, LOG_PAGE * log_page_p, LOG_ZIP * log_zip_p);
...
#endif
static void log_cleanup_modified_class (THREAD_ENTRY * thread_p, MODIFIED_CLASS_ENTRY * class, void *arg);
static void log_map_modified_class_list (THREAD_ENTRY * thread_p,
                                         LOG_TDES * tdes, bool release,
                                         void (*map_func) (THREAD_ENTRY *
                                                           thread_p,
                                                           MODIFIED_CLASS_ENTRY * class, void *arg), void *arg);
static void log_cleanup_modified_class_list (THREAD_ENTRY * thread_p,
                                             LOG_TDES * tdes, bool release, bool decache_classrepr);
...
log_create_internal (THREAD_ENTRY * thread_p, const char *db_fullname,
                     const char *logpath, const char *prefix_logname, DKNPAGES npages, INT64 * db_creation)
{
  ...
  if (logpb_initialize_header (&log_Gl.hdr, prefix_logname, npages, db_creation) != NO_ERROR)
    {
      logpb_finalize_pool ();
  ...
  log_Gl.append.vdes = fileio_format (thread_p, db_fullname, log_Name_active,
                                      LOG_DBLOG_ACTIVE_VOLID, npages, true, true, false, LOG_PAGESIZE, 0, false);
  ...
  log_Gl.chkpt_every_npages = prm_get_bigint_value (PRM_ID_LOG_CHECKPOINT_SIZE) / IO_PAGESIZE;
  ...
  catmsg = msgcat_message (MSGCAT_CATALOG_RYE, MSGCAT_SET_LOG, MSGCAT_LOG_LOGINFO_ACTIVE);
  if (catmsg == NULL)
    {
      catmsg = "ACTIVE: %s %d pages\n";
    }
  error_code = log_dump_log_info (log_Name_info, false, catmsg, fileio_get_base_file_name (log_Name_active), npages);
  if (error_code == NO_ERROR || error_code == ER_LOG_MOUNT_FAIL)
    {
      volid = logpb_add_volume (NULL, LOG_DBLOG_ACTIVE_VOLID, log_Name_active, DISK_UNKNOWN_PURPOSE);
                ...
                int ismedia_crash, time_t * stopat, LOG_LSA * stopat_lsa)
{
  (void) log_initialize_internal (thread_p, db_fullname, logpath,
                                  prefix_logname, ismedia_crash, stopat, stopat_lsa, false);
}
                         ...
                         int ismedia_crash, time_t * stopat, LOG_LSA * stopat_lsa, bool init_emergency)
{
      ...
      if (ismedia_crash != false)
        {
          ...
          error_code = logpb_initialize_header (&log_Gl.hdr, prefix_logname, log_npages, &db_creation);
          if (error_code != NO_ERROR)
            {
              goto error;
            }
  ...
  if (strcmp (log_Gl.hdr.prefix_name, prefix_logname) != 0)
    {
      ...
      er_set (ER_NOTIFICATION_SEVERITY, ARG_FILE_LINE,
              ER_LOG_INCOMPATIBLE_PREFIX_NAME, 2, prefix_logname, log_Gl.hdr.prefix_name);
      /* Continue anyhow */
    }
  ...
  if (log_Gl.hdr.db_iopagesize != IO_PAGESIZE || log_Gl.hdr.db_logpagesize != LOG_PAGESIZE)
    {
      ...
      error_code = log_initialize_internal (thread_p, db_fullname, logpath,
                                            prefix_logname, ismedia_crash, stopat, stopat_lsa, init_emergency);
  ...
  if (memcmp (log_Gl.hdr.log_magic, RYE_MAGIC_PREFIX,
              strlen (RYE_MAGIC_PREFIX)) != 0 || rel_check_disk_compatible (&log_Gl.hdr.db_version) != REL_COMPATIBLE)
    {
      char ver_string[REL_MAX_VERSION_LENGTH];
      rel_version_to_string (&log_Gl.hdr.db_version, ver_string, sizeof (ver_string));
      /* Database is incompatible with current release */
      error_code = ER_LOG_INCOMPATIBLE_DATABASE;
      er_set (ER_FATAL_ERROR_SEVERITY, ARG_FILE_LINE, error_code, 2, ver_string, rel_version_string ());
      goto error;
    }
      ...
      if (log_Gl.hdr.is_shutdown == false)
        {
          ...
          rel_version_to_string (&log_Gl.hdr.db_version, log_db_version, sizeof (log_db_version));
          error_code = ER_LOG_RECOVER_ON_OLD_RELEASE;
          er_set (ER_FATAL_ERROR_SEVERITY, ARG_FILE_LINE, error_code, 2, log_db_version, rel_version_string ());
          goto error;
        }
    }
  ...
  if (log_Gl.append.vdes != NULL_VOLDES)
    {
      if (fileio_map_mounted (thread_p,
                              (bool (*)(THREAD_ENTRY *, VOLID,
                                        void *)) log_verify_dbcreation, &log_Gl.hdr.db_creation) != true)
        {
   ...
   */
  if (init_emergency == false && (log_Gl.hdr.is_shutdown == false || ismedia_crash != false))
    {
      /*
   ...
   */
  if (log_Gl.chkpt_every_npages < prm_get_bigint_value (PRM_ID_LOG_BUFFER_SIZE) / LOG_PAGESIZE)
    {
      log_Gl.chkpt_every_npages = prm_get_bigint_value (PRM_ID_LOG_BUFFER_SIZE) / LOG_PAGESIZE;
    }
  ...
  bg_arv_info->vdes = fileio_format (thread_p, log_Db_fullname,
                                     ...
                                     LOGPB_ACTIVE_NPAGES + 1, false, false, false, LOG_PAGESIZE, 0, false);
      ...
      if (i != LOG_SYSTEM_TRAN_INDEX && (tdes = LOG_FIND_TDES (i)) != NULL && tdes->trid != NULL_TRANID)
        {
          ...
          if (thread_has_threads (thread_p, i, tdes->client_id) > 0)
          ...
          else if (LOG_ISTRAN_ACTIVE (tdes) && abort_thread_running[i] == 0)
            {
              ...
              if (conn != NULL)
                {
                  CSS_JOB_ENTRY_SET (job_entry, conn, (CSS_THREAD_FN) log_abort_by_tdes, (CSS_THREAD_ARG) tdes);
...
log_restart_emergency (THREAD_ENTRY * thread_p, const char *db_fullname,
                       const char *logpath, const char *prefix_logname)
{
  (void) log_initialize_internal (thread_p, db_fullname, logpath, prefix_logname, false, NULL, NULL, true);
}
                          ...
                          int redo_length, const void *undo_data, const void *redo_data)
{
  ...
  node = prior_lsa_alloc_and_copy_data (thread_p, LOG_UNDOREDO_DATA,
                                        rcvindex, addr, undo_length, undo_data, redo_length, redo_data);
  if (node == NULL)
    {
      assert (false);
      return;
                      ...
                      LOG_DATA_ADDR * addr, int length, const void *data)
{
  ...
  node = prior_lsa_alloc_and_copy_data (thread_p, LOG_UNDO_DATA, rcvindex, addr, length, data, 0, NULL);
  if (node == NULL)
    {
      assert (false);
      return;
                      ...
                      LOG_DATA_ADDR * addr, int length, const void *data)
{
  ...
  node = prior_lsa_alloc_and_copy_data (thread_p, LOG_REDO_DATA, rcvindex, addr, 0, NULL, length, data);
  if (node == NULL)
    {
      assert (false);
      return;
                            ...
                            int num_redo_crumbs, const LOG_CRUMB * undo_crumbs, const LOG_CRUMB * redo_crumbs)
{
  ...
  node = prior_lsa_alloc_and_copy_crumbs (thread_p, LOG_UNDOREDO_DATA,
                                          rcvindex, addr, num_undo_crumbs, undo_crumbs, num_redo_crumbs, redo_crumbs);
  if (node == NULL)
    {
      return;
    }
                        ...
                        LOG_DATA_ADDR * addr, int num_crumbs, const LOG_CRUMB * crumbs)
{
  ...
  node = prior_lsa_alloc_and_copy_crumbs (thread_p, LOG_UNDO_DATA, rcvindex, addr, num_crumbs, crumbs, 0, NULL);
  if (node == NULL)
    {
      return;
    }
                        ...
                        LOG_DATA_ADDR * addr, int num_crumbs, const LOG_CRUMB * crumbs)
{
  ...
  node = prior_lsa_alloc_and_copy_crumbs (thread_p, LOG_REDO_DATA, rcvindex, addr, 0, NULL, num_crumbs, crumbs);
  if (node == NULL)
    {
      return;
    }
...
log_append_dboutside_redo (THREAD_ENTRY * thread_p, LOG_RCVINDEX rcvindex, int length, const void *data)
{
  ...
  node = prior_lsa_alloc_and_copy_data (thread_p, LOG_DBEXTERN_REDO_DATA, rcvindex, NULL, 0, NULL, length, data);
  if (node == NULL)
    {
      assert (false);
      return;
...
log_append_postpone (THREAD_ENTRY * thread_p, LOG_RCVINDEX rcvindex, LOG_DATA_ADDR * addr, int length, const void *data)
{
  ...
  if (LSA_ISNULL (&tdes->last_lsa)
      || (log_is_in_crash_recovery ()
          && (crash_lsa = log_get_crash_point_lsa ()) != NULL && LSA_LE (&tdes->last_lsa, crash_lsa)))
    {
      log_append_empty_record (thread_p, LOG_DUMMY_HEAD_POSTPONE, addr);
    }

  node = prior_lsa_alloc_and_copy_data (thread_p, LOG_POSTPONE, rcvindex, addr, 0, NULL, length, data);
  if (node == NULL)
    {
      assert (false);
      return;
  ...
  if (tdes->topops.last >= 0)
    {
      if (LSA_ISNULL (&tdes->topops.stack[tdes->topops.last].posp_lsa))
        {
          LSA_COPY (&tdes->topops.stack[tdes->topops.last].posp_lsa, &tdes->last_lsa);
        }
    }
                         ...
                         int length, const void *data, const LOG_LSA * ref_lsa)
{
  ...
  if (tdes->state != TRAN_UNACTIVE_WILL_COMMIT
    ...
    {
      node = prior_lsa_alloc_and_copy_data (thread_p, LOG_RUN_POSTPONE, RV_NOT_DEFINED, NULL, length, data, 0, NULL);
      if (node == NULL)
        {
          assert (false);
          return;
                       ...
                       PGLENGTH offset, PAGE_PTR pgptr, int length, const void *data, LOG_TDES * tdes)
{
  ...
  node = prior_lsa_alloc_and_copy_data (thread_p, LOG_COMPENSATE, rcvindex, NULL, length, data, 0, NULL);
  if (node == NULL)
    {
      assert (false);
      return;
 ...
 */
void
log_append_logical_compensate (THREAD_ENTRY * thread_p, LOG_RCVINDEX rcvindex, LOG_TDES * tdes, LOG_LSA * undo_nxlsa)
{
  struct log_logical_compensate *logical_comp;  /* end of a logical undo   */
  LOG_PRIOR_NODE *node;

  node = prior_lsa_alloc_and_copy_data (thread_p, LOG_LCOMPENSATE, rcvindex, NULL, 0, NULL, 0, NULL);
  if (node == NULL)
    {
      assert (false);
      return;
 ...
 */
void
log_append_del_ovfl_record (THREAD_ENTRY * thread_p, OID * class_oid, RECDES * recdes)
{
  LOG_TDES *tdes;
  LOG_PRIOR_NODE *node;
  ...
  if (rdata == NULL)
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_OUT_OF_VIRTUAL_MEMORY, 1, sizeof (OID) + recdes->length);
      return;
    }
  ...
  node = prior_lsa_alloc_and_copy_data (thread_p, LOG_DUMMY_OVF_RECORD_DEL,
                                        RVHF_DELETE, &addr, 0, NULL, rlength, rdata);
  free (rdata);
 ...
 */
void
log_append_gid_bitmap_update_record (THREAD_ENTRY * thread_p, int migrator_id, int group_id, int target, int on_off)
{
  LOG_TDES *tdes;
  ...
  node = prior_lsa_alloc_and_copy_data (thread_p, LOG_DUMMY_UPDATE_GID_BITMAP, RV_NOT_DEFINED, NULL, 0, NULL, 0, NULL);
  if (node == NULL)
    {
      assert (false);
      return;
...
log_skip_logging_set_lsa (THREAD_ENTRY * thread_p, LOG_DATA_ADDR * addr)
{
  ...
  node = prior_lsa_alloc_and_copy_data (thread_p, LOG_DUMMY_RECORD, RV_NOT_DEFINED, NULL, 0, NULL, 0, NULL);
  if (node == NULL)
    {
      assert (false);
      return;
  ...
  if (!pgbuf_is_lsa_temporary (addr->pgptr) && pgbuf_set_lsa (thread_p, addr->pgptr, &start_lsa) == NULL)
    {
      assert (false);
      return;
    }
...
log_append_savepoint (THREAD_ENTRY * thread_p, const char *savept_name)
{
  ...
  node = prior_lsa_alloc_and_copy_data (thread_p, LOG_SAVEPOINT,
                                        RV_NOT_DEFINED, NULL,
                                        length, (const char *) savept_name, 0, (const char *) NULL);
  if (node == NULL)
    {
      assert (false);
      return NULL;
...
log_start_system_op (THREAD_ENTRY * thread_p)
{
   ...
   */
  tdes->topops.last++;
  LSA_COPY (&tdes->topops.stack[tdes->topops.last].lastparent_lsa, &tdes->last_lsa);
...
log_end_system_op (THREAD_ENTRY * thread_p, LOG_RESULT_TOPOP result)
{
  ...
  if (!LOG_ISTRAN_ACTIVE (tdes))
    {
       ...
       */
      if (tdes->topops.last == 1 && result == LOG_RESULT_TOPOP_ATTACH_TO_OUTER)
        {
          /*
           *
  ...
  if (result != LOG_RESULT_TOPOP_ATTACH_TO_OUTER
      ...
      && (LSA_ISNULL (&tdes->topops.stack[tdes->topops.last].lastparent_lsa)
          || LSA_GT (&tdes->last_lsa, &tdes->topops.stack[tdes->topops.last].lastparent_lsa)))
    {
      /*
      ...
      if (result == LOG_RESULT_TOPOP_COMMIT)
        {
          ...
          log_do_postpone (thread_p, tdes,
                           &tdes->topops.stack[tdes->topops.last].posp_lsa, LOG_COMMIT_TOPOPE_WITH_POSTPONE);
...
log_can_skip_undo_logging (THREAD_ENTRY * thread_p, LOG_RCVINDEX rcvindex, const LOG_TDES * tdes, LOG_DATA_ADDR * addr)
{
  ...
  if (addr->vfid != NULL
      && file_is_new_file_with_has_undolog (thread_p, addr->vfid,
                                            &has_undolog) == FILE_NEW_FILE && has_undolog == false)
    {
      /*
 ...
 */
static bool
log_can_skip_redo_logging (LOG_RCVINDEX rcvindex, UNUSED_ARG const LOG_TDES * ignore_tdes, LOG_DATA_ADDR * addr)
{
  /*
   * Some log record types (rcvindex) should never be skipped.
   * In the case of LINK_PERM_VOLEXT, the link of a permanent temp
...
log_append_topope_commit_postpone (THREAD_ENTRY * thread_p, LOG_TDES * tdes, LOG_LSA * start_postpone_lsa)
{
  ...
  node = prior_lsa_alloc_and_copy_data (thread_p,
                                        LOG_COMMIT_TOPOPE_WITH_POSTPONE, RV_NOT_DEFINED, NULL, 0, NULL, 0, NULL);
  if (node == NULL)
    {
      assert (false);
      return;
      ...
      while (tdes->append_repl_recidx < tdes->cur_repl_record)
        {
          ...
          if ((repl_rec->repl_type == LOG_REPLICATION_DATA
              ...
              &&
              ((is_commit
                && repl_rec->must_flush != LOG_REPL_DONT_NEED_FLUSH) || repl_rec->must_flush == LOG_REPL_NEED_FLUSH))
            {
              node = prior_lsa_alloc_and_copy_data (thread_p,
                                                    repl_rec->repl_type,
                                                    RV_NOT_DEFINED, NULL,
              ...
              if (repl_rec->rcvindex == RVREPL_DATA_DELETE || repl_rec->rcvindex == RVREPL_SCHEMA)
                {
 ...
 */
static void
log_append_donetime (THREAD_ENTRY * thread_p, LOG_TDES * tdes, LOG_RECTYPE iscommitted)
{
      ...
      if (prm_get_bool_value (PRM_ID_LOG_TRACE_DEBUG))
        {
          ...
          er_log_debug (ARG_FILE_LINE, msgcat_message (MSGCAT_CATALOG_RYE,
                        ...
                        log_Gl.hdr.append_lsa.pageid, log_Gl.hdr.append_lsa.offset, time_val);
        }
#endif /* !NDEBUG */
    }
  else
 ...
 */
int
log_add_to_modified_class_list (THREAD_ENTRY * thread_p, const OID * class_oid)
{
  LOG_TDES *tdes;
 ...
 */
static void
log_cleanup_modified_class (THREAD_ENTRY * thread_p, MODIFIED_CLASS_ENTRY * class, void *arg)
{
  ...
  if (qexec_remove_xasl_cache_ent_by_class (thread_p, &class->class_oid, 1) != NO_ERROR)
    {
      er_log_debug (ARG_FILE_LINE,
                    "log_cleanup_modified_class: "
                    "qexec_remove_xasl_cache_ent_by_class"
                    " failed for class { %d %d %d }\n",
                    class->class_oid.pageid, class->class_oid.slotid, class->class_oid.volid);
    }
}
...
log_map_modified_class_list (THREAD_ENTRY * thread_p,
                             LOG_TDES * tdes, bool release,
                             void
                             (*map_func) (THREAD_ENTRY * thread_p, MODIFIED_CLASS_ENTRY * class, void *arg), void *arg)
{
 ...
 */
static void
log_cleanup_modified_class_list (THREAD_ENTRY * thread_p, LOG_TDES * tdes, bool release, bool decache_classrepr)
{
  (void) log_map_modified_class_list (thread_p, tdes, release, log_cleanup_modified_class, &decache_classrepr);
}
...
log_commit_local (THREAD_ENTRY * thread_p, LOG_TDES * tdes)
{
  ...
  if (!LSA_ISNULL (&tdes->last_lsa))
    {
      ...
      log_do_postpone (thread_p, tdes, &tdes->posp_nxlsa, LOG_COMMIT_WITH_POSTPONE);
   ...
   */
  if ((wfg_get_tran_entries (tdes->tran_index) > 0) && !log_is_tran_distributed (tdes))
    {
      wfg_dump ();
    }
#endif
  ...
  if (tdes->topops.last >= 0)
    {
      ...
      er_set (ER_WARNING_SEVERITY, ARG_FILE_LINE,
              ER_LOG_HAS_TOPOPS_DURING_COMMIT_ABORT, 2, tdes->trid, tdes->tran_index);
...
#if defined(RYE_DEBUG)
static void
log_client_find_system_error (LOG_RECTYPE record_type, LOG_RECTYPE client_type)
{
  switch (record_type)
    {
    case LOG_COMMIT_TOPOPE_WITH_POSTPONE:
...
log_dump_data (THREAD_ENTRY * thread_p, FILE * out_fp, int length,
               LOG_LSA * log_lsa, LOG_PAGE * log_page_p, void (*dumpfun) (FILE *, int, void *), LOG_ZIP * log_dump_ptr)
{
...
#if defined (ENABLE_UNUSED_FUNCTION)
static LOG_PAGE *
log_dump_record_client_name (THREAD_ENTRY * thread_p, FILE * out_fp, LOG_LSA * log_lsa, LOG_PAGE * log_page_p)
{
  LOG_READ_ADVANCE_WHEN_DOESNT_FIT (thread_p, LOG_USERNAME_MAX, log_lsa, log_page_p);
                          ...
                          LOG_LSA * log_lsa, LOG_PAGE * log_page_p, LOG_ZIP * log_zip_p)
{
  ...
  fprintf (out_fp,
           ...
           (int) GET_ZIP_LEN (undoredo->ulength), (int) GET_ZIP_LEN (undoredo->rlength));
  ...
  log_dump_data (thread_p, out_fp, undo_length, log_lsa,
                 log_page_p,
                 ((RV_fun[rcvindex].dump_undofun != NULL) ? RV_fun[rcvindex].dump_undofun : log_ascii_dump), log_zip_p);
  /* Print REDO (AFTER) DATA */
  fprintf (out_fp, "-->> Redo (After) Data:\n");
  log_dump_data (thread_p, out_fp, redo_length, log_lsa,
                 log_page_p,
                 ((RV_fun[rcvindex].dump_redofun != NULL) ? RV_fun[rcvindex].dump_redofun : log_ascii_dump), log_zip_p);
                      ...
                      LOG_LSA * log_lsa, LOG_PAGE * log_page_p, LOG_ZIP * log_zip_p)
{
  ...
  fprintf (out_fp, "     Volid = %d Pageid = %d Offset = %d,\n"
           ...
           undo->data.volid, undo->data.pageid, undo->data.offset, (int) GET_ZIP_LEN (undo->length));
                      ...
                      LOG_LSA * log_lsa, LOG_PAGE * log_page_p, LOG_ZIP * log_zip_p)
{
  ...
  fprintf (out_fp, "     Volid = %d Pageid = %d Offset = %d,\n"
           "     Redo (After) length = %d,\n",
           redo->data.volid, redo->data.pageid, redo->data.offset, (int) GET_ZIP_LEN (redo->length));
...
log_dump_record_postpone (THREAD_ENTRY * thread_p, FILE * out_fp, LOG_LSA * log_lsa, LOG_PAGE * log_page_p)
{
  ...
  fprintf (out_fp,
           ...
           run_posp->length, (long long int) run_posp->ref_lsa.pageid, run_posp->ref_lsa.offset);
  ...
  log_dump_data (thread_p, out_fp, redo_length, log_lsa,
                 log_page_p,
                 ((RV_fun[rcvindex].dump_redofun != NULL) ? RV_fun[rcvindex].dump_redofun : log_ascii_dump), NULL);
...
log_dump_record_compensate (THREAD_ENTRY * thread_p, FILE * out_fp, LOG_LSA * log_lsa, LOG_PAGE * log_page_p)
{
  ...
  log_dump_data (thread_p, out_fp, length_compensate, log_lsa,
                 log_page_p,
                 (RV_fun[rcvindex].dump_undofun != NULL) ? RV_fun[rcvindex].dump_undofun : log_ascii_dump, NULL);

  return log_page_p;
}

static LOG_PAGE *
log_dump_record_logical_compensate (THREAD_ENTRY * thread_p, FILE * out_fp, LOG_LSA * log_lsa, LOG_PAGE * log_page_p)
{
  struct log_logical_compensate *logical_comp;

  /* Read the DATA HEADER */
  LOG_READ_ADVANCE_WHEN_DOESNT_FIT (thread_p, sizeof (*logical_comp), log_lsa, log_page_p);
  logical_comp = ((struct log_logical_compensate *) ((char *) log_page_p->area + log_lsa->offset));

  fprintf (out_fp, ", Recv_index = %s,\n", rv_rcvindex_string (logical_comp->rcvindex));
  fprintf (out_fp, "     Next_to_UNDO = %lld|%d\n",
           (long long int) logical_comp->undo_nxlsa.pageid, logical_comp->undo_nxlsa.offset);
...
log_dump_record_save_point (THREAD_ENTRY * thread_p, FILE * out_fp, LOG_LSA * log_lsa, LOG_PAGE * log_page_p)
{
  ...
  fprintf (out_fp, ", Prev_savept_Lsa = %lld|%d, length = %d,\n",
           (long long int) savept->prv_savept.pageid, savept->prv_savept.offset, savept->length);
  ...
  log_dump_data (thread_p, out_fp, length_save_point, log_lsa, log_page_p, log_ascii_dump, NULL);
...
static LOG_PAGE *
log_dump_record_git_bitmap_upadte (THREAD_ENTRY * thread_p, FILE * out_fp, LOG_LSA * log_lsa, LOG_PAGE * log_page_p)
{
  struct log_gid_bitmap_update *git_bitmap;

  /* Get the DATA HEADER */
  LOG_READ_ADVANCE_WHEN_DOESNT_FIT (thread_p, sizeof (*git_bitmap), log_lsa, log_page_p);
  git_bitmap = ((struct log_gid_bitmap_update *) ((char *) log_page_p->area + log_lsa->offset));
  fprintf (out_fp, "  GID Bitmap Update: migrator_id = %d, "
           "group_id = %d, target = %d, on_off = %d\n",
           git_bitmap->migrator_id, git_bitmap->group_id, git_bitmap->target, git_bitmap->on_off);
...
log_dump_record (THREAD_ENTRY * thread_p, FILE * out_fp,
                 LOG_RECTYPE record_type, LOG_LSA * log_lsa, LOG_PAGE * log_page_p, LOG_ZIP * log_zip_p)
{
  switch (record_type)
    {
    case LOG_UNDOREDO_DATA:
    ...
    case LOG_LCOMPENSATE:
      log_page_p = log_dump_record_logical_compensate (thread_p, out_fp, log_lsa, log_page_p);
      break;
    ...
    case LOG_DUMMY_HA_SERVER_STATE:
      ...
      break;
    case LOG_DUMMY_UPDATE_GID_BITMAP:
      log_page_p = log_dump_record_git_bitmap_upadte (thread_p, out_fp, log_lsa, log_page_p);
      break;
 ...
 */
void
xlog_dump (THREAD_ENTRY * thread_p, FILE * out_fp, LOG_PAGEID start_logpageid, DKNPAGES dump_npages)
{
      ...
      while (lsa.pageid == log_lsa.pageid)
        {
            ...
            if (log_startof_nxrec (thread_p, &next_lsa, false) == NULL
                || (!LSA_EQ (&next_lsa, &log_rec->forw_lsa) && !LSA_ISNULL (&log_rec->forw_lsa)))
              {
                ...
                fprintf (out_fp,
                         ...
                         (long long int) next_lsa.pageid, next_lsa.offset, (long long int) lsa.pageid, lsa.offset);
                fprintf (out_fp, "<<<<<****\n");
              }
          }
          ...
          fprintf (out_fp, "\nLSA = %3lld|%3d, Forw log = %3lld|%3d,"
                   ...
                   log_rec->trid,
                   (long long int) log_rec->prev_tranlsa.pageid, log_rec->prev_tranlsa.offset, log_to_string (type));
                     ...
                     VPID * rcv_vpid, LOG_RCV * rcv, LOG_TDES * tdes, LOG_ZIP * log_unzip_ptr)
{
  ...
  if (RCV_IS_LOGICAL_LOG (rcv_vpid, rcvindex)
    ...
    {
      rcv->pgptr = pgbuf_fix (thread_p, rcv_vpid, OLD_PAGE, PGBUF_LATCH_WRITE, PGBUF_UNCONDITIONAL_LATCH, PAGE_UNKNOWN);
    }
      ...
      if (!RCV_IS_LOGICAL_LOG (rcv_vpid, rcvindex))
      ...
      else
        {
...
#if defined(RYE_DEBUG)
          {
            ...
            if (LSA_EQ (&check_tail_lsa, &tdes->last_lsa) && rcvindex != RVFL_CREATE_TMPFILE)
              {
                er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE,
                        ER_LOG_MISSING_COMPENSATING_RECORD, 1, rv_rcvindex_string (rcvindex));
              }
          }
#else /* RYE_DEBUG */
          /* Invoke Undo recovery function */
          ...
          if (rv_err != NO_ERROR)
            {
              er_log_debug (ARG_FILE_LINE,
                            ...
                            log_lsa->offset, rv_rcvindex_string (rcvindex), rv_err);
              er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE,
                      ER_LOG_MAYNEED_MEDIA_RECOVERY, 1, fileio_get_volume_label (rcv_vpid->volid, PEEK));
            }
           ...
           */
          log_append_logical_compensate (thread_p, rcvindex, tdes, &logical_undo_nxlsa);
        }
    }
  else
    {
...
log_dump_record_header_to_string (LOG_RECORD_HEADER * log, char *buf, size_t len)
{
  ...
  snprintf (buf, len, fmt, log->type, log->trid,
            ...
            log->back_lsa.offset, (long long int) log->forw_lsa.pageid, log->forw_lsa.offset);
}
...
log_rollback (THREAD_ENTRY * thread_p, LOG_TDES * tdes, const LOG_LSA * upto_lsa_ptr)
{
  ...
  thread_mnt_track_push (thread_p, MNT_STATS_DATA_PAGE_FETCHES_TRACK_LOG_ROLLBACK, &status);
      ...
      while (prev_tranlsa.pageid == log_lsa.pageid)
        {
          ...
          switch (log_rec->type)
            ...
            case LOG_DIFF_UNDOREDO_DATA:
              /* Read the DATA HEADER */
              LOG_READ_ADD_ALIGN (thread_p, sizeof (*log_rec), &log_lsa, log_pgptr);
              LOG_READ_ADVANCE_WHEN_DOESNT_FIT (thread_p, sizeof (*undoredo), &log_lsa, log_pgptr);
              ...
              LOG_READ_ADD_ALIGN (thread_p, sizeof (*undoredo), &log_lsa, log_pgptr);
            ...
            case LOG_UNDO_DATA:
              /* Read the DATA HEADER */
              LOG_READ_ADD_ALIGN (thread_p, sizeof (*log_rec), &log_lsa, log_pgptr);
              LOG_READ_ADVANCE_WHEN_DOESNT_FIT (thread_p, sizeof (*undo), &log_lsa, log_pgptr);
            ...
            case LOG_LCOMPENSATE:
              ...
              /* Read the DATA HEADER */
              LOG_READ_ADD_ALIGN (thread_p, sizeof (*log_rec), &log_lsa, log_pgptr);
              LOG_READ_ADVANCE_WHEN_DOESNT_FIT (thread_p, sizeof (*logical_comp), &log_lsa, log_pgptr);
              logical_comp = ((struct log_logical_compensate *) ((char *) log_pgptr->area + log_lsa.offset));
          ...
          while (forward_lsa.pageid == log_lsa.pageid)
            {
              ...
              if (forward_lsa.offset == NULL_OFFSET
                  && forward_lsa.pageid != NULL_PAGEID && forward_lsa.pageid < log_lsa.pageid)
                {
...
log_run_postpone_op (THREAD_ENTRY * thread_p, LOG_LSA * log_lsa, LOG_PAGE * log_pgptr)
{
  ...
  rcv.pgptr = pgbuf_fix_with_retry (thread_p, &rcv_vpid, OLD_PAGE, PGBUF_LATCH_WRITE, 10, PAGE_UNKNOWN);
  ...
  if (rcv.pgptr != NULL || (rcv_vpid.volid == NULL_VOLID && rcv_vpid.pageid == NULL_PAGEID))
    {
      if (rcvindex == RVDK_IDDEALLOC_WITH_VOLHEADER)
        {
      ...
      else
        {
           ...
           * the postpone action
           */
          log_append_run_postpone (thread_p, rcvindex, &rvaddr, &rcv_vpid, rcv.length, rcv.data, &ref_lsa);
  ...
  FI_TEST_ARG_INT (thread_p, FI_TEST_LOG_MANAGER_RANDOM_EXIT_AT_RUN_POSTPONE, 100, 0);
  ...
  for (volid = LOG_DBFIRST_VOLID; volid < num_perm_vols; volid++)
    {
      ...
      (void) disk_set_creation (thread_p, volid, vlabel,
                                &log_Gl.hdr.db_creation, &log_Gl.hdr.chkpt_lsa, false, DISK_DONT_FLUSH);
      LOG_CS_ENTER (thread_p);
      logpb_flush_pages_direct (thread_p);
      LOG_CS_EXIT ();
...
log_get_io_page_size (THREAD_ENTRY * thread_p, const char *db_fullname, const char *logpath, const char *prefix_logname)
{
  ...
  if (logpb_find_header_parameters (thread_p, db_fullname, logpath,
                                    ...
                                    &ignore_log_page_size, &ignore_dbcreation, &ignore_db_version, &dummy) == -1)
    {
      /*
 ...
 */
int
log_rv_outside_noop_redo (UNUSED_ARG THREAD_ENTRY * thread_p, UNUSED_ARG LOG_RCV * rcv)
{
  return NO_ERROR;
}
