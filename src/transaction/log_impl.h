...
{                               /* Transaction descriptor */
  ...
  int is_allowed_modification;  /* db_is_Allowed_Modification for each tran */
...
extern int logpb_read_page_from_active_log (THREAD_ENTRY * thread_p,
                                            LOG_PAGEID pageid, int num_pages, LOG_PAGE * log_pgptr);
...
extern PGLENGTH logpb_find_header_parameters (THREAD_ENTRY * thread_p,
                                              ...
                                              INT64 * db_creation, RYE_VERSION * db_version, int *db_charset);
...
extern LOG_PRIOR_NODE *prior_lsa_alloc_and_copy_data (THREAD_ENTRY * thread_p,
                                                      ...
                                                      LOG_RCVINDEX rcvindex,
                                                      LOG_DATA_ADDR * addr,
                                                      int ulength, const char *udata, int rlength, const char *rdata);
extern LOG_PRIOR_NODE *prior_lsa_alloc_and_copy_crumbs (THREAD_ENTRY *
                                                        ...
                                                        const LOG_CRUMB *
                                                        ucrumbs, const int num_rcrumbs, const LOG_CRUMB * rcrumbs);
extern void prior_lsa_free_node (THREAD_ENTRY * thread_p, LOG_PRIOR_NODE * node);
...
extern int prior_lsa_get_current_lsa (THREAD_ENTRY * thread_p, LOG_LSA * current_lsa);
...
extern VOLID logpb_add_volume (const char *db_fullname,
                               VOLID new_volid, const char *new_volfullname, DISK_VOLPURPOSE new_volpurpose);
...
extern void logpb_copy_from_log (THREAD_ENTRY * thread_p, char *area,
                                 int length, LOG_LSA * log_lsa, LOG_PAGE * log_pgptr);
...
extern void log_recovery (THREAD_ENTRY * thread_p, int ismedia_crash, time_t * stopat, LOG_LSA * stopat_lsa);
...
extern void logtb_shutdown_write_normal_clients (THREAD_ENTRY * thread_p, bool force);
...
extern int logtb_get_current_client_ids (THREAD_ENTRY * thread_p, LOG_CLIENTIDS * client_info);
...
extern LOG_LSA *logtb_find_unilaterally_largest_undo_lsa (THREAD_ENTRY * thread_p);
...
extern void logtb_set_backup_session (THREAD_ENTRY * thread_p, BK_BACKUP_SESSION * session);
