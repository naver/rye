...
wfg_internal_detect_cycle (THREAD_ENTRY * thread_p,
                           ...
                           WFG_CYCLE ** list_cycles_p, const int max_cycles_in_cycle_group, const int max_cycles);
...
static int wfg_detect_tran_group_cycle (THREAD_ENTRY * thread_p,
                                        WFG_CYCLE_CASE * cycle_case_p, WFG_CYCLE ** list_cycles);
...
check_duplication_holders (const int holder_index,
                           const int *holder_tran_indices, const int num_holders, const int waiter_tran_index);
      ...
      if (holder_tran_indices_p[i] < 0 || holder_tran_indices_p[i] > wfg_Total_nodes - 1)
        {
          er_log_debug (ARG_FILE_LINE,
                        ...
                        " IGNORED **", i, htran_indices_p[i], wfg_Total_nodes - 1);
          error_code = ER_FAILED;
          goto end;
        }
...
wfg_detect_cycle (THREAD_ENTRY * thread_p, WFG_CYCLE_CASE * cycle_case, WFG_CYCLE ** list_cycles_p)
{
  ...
  return wfg_internal_detect_cycle (thread_p, cycle_case, list_cycles_p,
                                    WFG_PRUNE_CYCLES_IN_CYCLE_GROUP, WFG_MAX_CYCLES_TO_REPORT);
}
      ...
      for (i = 0; i < wfg_Total_tran_groups; i++)
        {
          fprintf (stdout, "TG[%d]:\t Num_holders %d, Num_waiters %d\n",
                   i, wfg_Tran_group[i].num_holders, wfg_Tran_group[i].num_waiters);
              ...
              switch (wfg_Nodes[htran_index].status)
                ...
                case WFG_ON_STACK:
                  ...
                  if (cycle_p->waiters == NULL)
                    {
                      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE,
                              ER_OUT_OF_VIRTUAL_MEMORY, 1, sizeof (WFG_WAITER) * cycle_p->num_trans);
                      error_code = ER_OUT_OF_VIRTUAL_MEMORY;
                  ...
                  for (stack_elem_p = top_p, waiter_p = cycle_p->waiters;
                       j < cycle_p->num_trans; j++, stack_elem_p--, waiter_p++)
                    {
  ...
  for (i = tg_index; i < wfg_Total_tran_groups && *all_waiting == true; i++)
    {
      ...
      for (w_tran_list_p = tg_tmp->waiter_tran_list_p;
           w_tran_list_p != NULL && tg_connected == false; w_tran_list_p = w_tran_list_p->next)
        {
          w_node_p = &wfg_Nodes[w_tran_list_p->tran_index];
          if (w_node_p->status != WFG_NOT_VISITED)
            {
      ...
      for (h_tran_list_p = tg_tmp->holder_tran_list_p;
           h_tran_list_p != NULL && tg_all_waiting == true; h_tran_list_p = h_tran_list_p->next)
        {
  ...
  for (tg1_index = 0; tg1_index < wfg_Total_tran_groups; tg1_index++)
    {
       ...
       */
      for (w_tran_list_p =
           wfg_Tran_group[tg1_index].waiter_tran_list_p; w_tran_list_p != NULL; w_tran_list_p = w_tran_list_p->next)
        {
          /*
          ...
          if (all_waiting == true)
            {
              cycle_p = wfg_detect_tran_group_cycle_internal (cycle_case_p,
                                                              w_tran_list_p,
                                                              add_waiter, tg1_index, num_tran_groups_holders);
              if (cycle_p == NULL)
                {
                  goto error;
                }
