...
static LOG_PAGE *logpb_fix_page (THREAD_ENTRY * thread_p, LOG_PAGEID pageid, int fetch_mode);
...
static int logpb_add_archive_page_info (THREAD_ENTRY * thread_p,
                                        int arv_num, LOG_PAGEID start_page, LOG_PAGEID end_page);
...
static void logpb_fatal_error_internal (THREAD_ENTRY * thread_p,
                                        ...
                                        const char *file_name, const int lineno, const char *fmt, va_list ap);
  ...
  if (num_new_buffers <= 0)
    {
      ...
      if (log_Pb.num_buffers > 0)
        ...
        {
          num_new_buffers = prm_get_bigint_value (PRM_ID_LOG_BUFFER_SIZE) / LOG_PAGESIZE;
        }
    }
...
logpb_initialize_pool (THREAD_ENTRY * thread_p)
{
      ...
      if (!prm_get_bool_value (PRM_ID_LOG_COMPRESS))
      ...
      else
        {
          ...
          if (log_Pb.log_data_ptr == NULL)
            {
              er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_OUT_OF_VIRTUAL_MEMORY, 1, log_Pb.log_data_length);
            }

          if (log_Pb.log_zip_undo == NULL || log_Pb.log_zip_redo == NULL || log_Pb.log_data_ptr == NULL)
            {
              log_Pb.log_zip_support = false;
  ...
  log_Pb.ht = mht_create ("Log buffer pool hash table",
                          log_Pb.num_buffers * 8, mht_logpageidhash, mht_compare_logpageids_are_equal);
  if (log_Pb.ht == NULL)
    {
  ...
  for (i = 0; i < log_Pb.num_buffers; i++)
    {
      ...
      if ((log_bufptr->pageid == LOGPB_HEADER_PAGE_ID
           || log_bufptr->pageid > NULL_PAGEID) && log_bufptr->fcnt <= 0 && log_bufptr->dirty == false)
        {
 ...
 */
void
logpb_set_dirty (UNUSED_ARG THREAD_ENTRY * thread_p, LOG_PAGE * log_pgptr, int free_page)
{
...
logpb_dump_information (FILE * out_fp)
{
  ...
  fprintf (out_fp,
           ...
           (long long int) log_Gl.prior_info.prev_lsa.pageid, log_Gl.prior_info.prev_lsa.offset);
 ...
 */
int
logpb_print_hash_entry (FILE * outfp, const void *key, void *ent, UNUSED_ARG void *ignore)
{
 ...
 */
int
logpb_initialize_header (struct log_header *loghdr, const char *prefix_logname, DKNPAGES npages, INT64 * db_creation)
{
  assert (loghdr != NULL);
  ...
  if (logpb_is_page_in_archive (pageid)
  ...
  else
    {
      ...
      if (fileio_read (thread_p, log_Gl.append.vdes, log_pgptr, phy_pageid, LOG_PAGESIZE) == NULL)
        ...
        {
          mnt_stats_counter_with_time (thread_p, MNT_STATS_LOG_PAGE_IOREADS, 1, perf_start);
      ...
      if (er_errid () == ER_IO_WRITE_OUT_OF_SPACE)
        {
          ...
          er_set (ER_FATAL_ERROR_SEVERITY, ARG_FILE_LINE,
                  ER_LOG_WRITE_OUT_OF_SPACE, 4, logical_pageid, phy_pageid, log_Name_active, nbytes);
        }
      else
        {
          er_set_with_oserror (ER_FATAL_ERROR_SEVERITY, ARG_FILE_LINE,
                               ER_LOG_WRITE, 3, logical_pageid, phy_pageid, log_Name_active);
        }
...
logpb_find_header_parameters (THREAD_ENTRY * thread_p,
                              ...
                              INT64 * creation_time, RYE_VERSION * db_version, int *db_charset)
{
  ...
  if (strcmp (hdr.prefix_name, prefix_logname) != 0)
    {
      ...
      er_set (ER_NOTIFICATION_SEVERITY, ARG_FILE_LINE,
              ER_LOG_INCOMPATIBLE_PREFIX_NAME, 2, log_Name_active, hdr.prefix_name);
      /* Continue anyhow.. */
    }
  ...
  if (rel_check_disk_compatible (db_version) != REL_COMPATIBLE)
    {
      ...
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, error_code, 2, ver_string, rel_version_string ());
      goto error;
    }
...
logpb_fetch_start_append_page (THREAD_ENTRY * thread_p)
{
  ...
  if ((log_Gl.hdr.append_lsa.pageid == START_LOG_PAGEID) && (log_Gl.hdr.append_lsa.offset == 0))
    {
      flag = NEW_PAGE;
    }
  ...
  if (LOGPB_IS_FIRST_PHYSICAL_PAGE (log_Gl.hdr.append_lsa.pageid))
    {
      ...
      assert ((log_Gl.hdr.fpageid - START_LOG_PAGEID) % LOGPB_ACTIVE_NPAGES == 0);
          ...
          if (er_errid () == ER_IO_WRITE_OUT_OF_SPACE)
            {
              er_set (ER_FATAL_ERROR_SEVERITY, ARG_FILE_LINE,
                      ...
                      bufptr->pageid, phy_pageid, log_Name_active, log_Gl.hdr.db_logpagesize);
            }
          else
            {
              er_set_with_oserror (ER_FATAL_ERROR_SEVERITY, ARG_FILE_LINE,
                                   ER_LOG_WRITE, 3, bufptr->pageid, phy_pageid, log_Name_active);
            }
          to_flush = NULL;
        }
    }
...
logpb_flush_all_append_pages (THREAD_ENTRY * thread_p)
{
  ...
  if (thread_p && thread_p->type != TT_DAEMON)
    {
      ...
      thread_p->event_stats.trace_log_flush_time = prm_get_bigint_value (PRM_ID_LOG_TRACE_FLUSH_TIME);
    }
#endif /* SERVER_MODE */
      ...
      if (idxflush != -1 && prv_bufptr != NULL)
        {
          ...
          if ((bufptr->dirty == false)
              || (bufptr->pageid != (prv_bufptr->pageid + 1)) || (bufptr->phy_pageid != (prv_bufptr->phy_pageid + 1)))
            {
              /*
  ...
  mnt_stats_counter_with_time (thread_p, MNT_STATS_LOG_PAGE_IOWRITES, flush_info->num_toflush, perf_start);
  ...
  while (logwr_find_entry_status (writer_info, LOGWR_STATUS_FETCH) != NULL
         && logwr_find_copy_completed_entry (writer_info) == NULL)
    {
      ...
      rv = pthread_cond_timedwait (&writer_info->wr_list_cond, &writer_info->wr_list_mutex, &to);
    }
      ...
      if (all_writer_thr_end_time - flush_start_time > thread_p->event_stats.trace_log_flush_time)
        {
          event_log_log_flush_thr_wait (thread_p, flush_page_count,
                                        ...
                                        flush_start_time,
                                        all_writer_thr_end_time -
                                        flush_completed_time, writer_info->last_writer_elapsed_time);
        }
    }
      ...
      while (LSA_LT (&nxio_lsa, flush_lsa))
        {
          ...
          wakeup_time = timespec_add_msec (&wakeup_time, max_wait_time_in_msec);
          ...
          (void) pthread_cond_timedwait (&group_commit_info->gc_cond, &group_commit_info->gc_mutex, &wakeup_time);
          pthread_mutex_unlock (&group_commit_info->gc_mutex);
...
logpb_create_log_info (const char *logname_info, const char *db_fullname)
{
  ...
  if (fp != NULL)
    {
      fclose (fp);
      catmsg = msgcat_message (MSGCAT_CATALOG_RYE, MSGCAT_SET_LOG, MSGCAT_LOG_LOGINFO_COMMENT);
      if (db_name == NULL)
        {
      ...
      error_code = log_dump_log_info (logname_info, false, catmsg,
                                      RYE_MAGIC_LOG_INFO, fileio_get_base_file_name (db_name));
      if (error_code != NO_ERROR)
        {
          return;
        }
...
logpb_recreate_volume_info (THREAD_ENTRY * thread_p)
{
  ...
  do
    {
      if (logpb_add_volume (NULL, next_volid, next_vol_fullname, DISK_PERMVOL_GENERIC_PURPOSE) != next_volid)
        {
          error_code = ER_FAILED;
          goto error;
        }

      if (disk_get_link (thread_p, log_Db_fullname, next_volid, next_vol_fullname) == NULL)
        {
          error_code = ER_FAILED;
          goto error;
        }
  ...
  while (true)
    {
      ...
      COMPOSE_FULL_NAME (temp_vol_fullname, sizeof (temp_vol_fullname), temp_path, vol_name);
      ...
      if ((volid + 1) != NULL_VOLID && (volid + 1) != (VOLID) read_int_volid && num_vols != 0)
        {
          er_set (ER_WARNING_SEVERITY, ARG_FILE_LINE, ER_BO_UNSORTED_VOLINFO,
                  4, volinfo_fullname, num_vols, read_int_volid, temp_vol_fullname);
          num_vols = -1;
          break;
        }
...
logpb_set_unavailable_archive (int arv_num)
{
  ...
  assert (LOG_CS_OWN_WRITE_MODE (NULL) || (LOG_CS_OWN_READ_MODE (NULL) && LOG_ARCHIVE_CS_OWN_WRITE_MODE (NULL)));
...
logpb_fetch_from_archive (THREAD_ENTRY * thread_p, LOG_PAGEID pageid,
                          LOG_PAGE * log_pgptr, int *ret_arv_num, struct log_arv_header * ret_arv_hdr, bool is_fatal)
{
      ...
      else
        {
          ...
          while (retry != 0 && retry != 1
                 && (vdes = fileio_mount (thread_p, log_Db_fullname, arv_name,
                                          LOG_DBLOG_ARCHIVE_VOLID, false, false)) == NULL_VOLDES)
            {
            ...
            retry_prompt:
              if (log_default_input_for_archive_log_location >= 0)
                ...
                {
                  fprintf (stdout, msgcat_message (MSGCAT_CATALOG_RYE, MSGCAT_SET_LOG, MSGCAT_LOG_STARTS));
                  fprintf (stdout, msgcat_message (MSGCAT_CATALOG_RYE,
                                                   MSGCAT_SET_LOG, MSGCAT_LOG_LOGARCHIVE_NEEDED), arv_name);
                  fprintf (stdout, msgcat_message (MSGCAT_CATALOG_RYE, MSGCAT_SET_LOG, MSGCAT_LOG_STARTS));
              ...
              switch (retry)
                ...
                case 3:        /* Relocate */
                  fprintf (stdout, msgcat_message (MSGCAT_CATALOG_RYE, MSGCAT_SET_LOG, MSGCAT_LOG_NEWLOCATION));
                ...
                default:       /* Something strange.  Get user to try again. */
                  fprintf (stdout, msgcat_message (MSGCAT_CATALOG_RYE,
                                                   MSGCAT_SET_LOG, MSGCAT_LOG_INPUT_RANGE_ERROR), 0, 3);
                  goto retry_prompt;
                }
            }
                  ...
                  if (difftime64 ((time_t) arv_hdr->db_creation, (time_t) log_Gl.hdr.db_creation) != 0)
                    {
                       ...
                       */
                      er_set (ER_FATAL_ERROR_SEVERITY, ARG_FILE_LINE,
                              ER_LOG_DOESNT_CORRESPOND_TO_DATABASE, 1, fileio_get_base_file_name (arv_name));
                      arv_hdr = NULL;
                    }
                }
            }
...
logpb_archive_active_log (THREAD_ENTRY * thread_p)
{
  ...
  if (log_Gl.hdr.nxarv_pageid >= log_Gl.hdr.append_lsa.pageid)
    {
      er_log_debug (ARG_FILE_LINE, "log_archive_active_log: WARNING "
                    "Trying to archive ONLY the append page" " which is incomplete\n");
      return;
    }
  ...
  if (log_Gl.archive.vdes != NULL_VOLDES)
    {
      assert (LOG_CS_OWN_WRITE_MODE (thread_p)
              || (LOG_CS_OWN_READ_MODE (thread_p) && LOG_ARCHIVE_CS_OWN_WRITE_MODE (thread_p)));
  ...
  if (bg_arv_info->vdes != NULL_VOLDES)
  ...
  else
    {
      vdes = fileio_format (thread_p, log_Db_fullname, arv_name,
                            LOG_DBLOG_ARCHIVE_VOLID, arvhdr->npages + 1, false, false, false, LOG_PAGESIZE, 0, false);
      if (vdes == NULL_VOLDES)
        {
          /* Unable to create archive log to archive */
          er_set (ER_FATAL_ERROR_SEVERITY, ARG_FILE_LINE,
                  ER_LOG_CREATE_LOGARCHIVE_FAIL, 3,
                  fileio_get_base_file_name (arv_name), arvhdr->fpageid, arvhdr->fpageid + arvhdr->npages - 1);
          goto error;
        }
    }
  ...
  if (fileio_write (thread_p, vdes, malloc_arv_hdr_pgptr, 0, LOG_PAGESIZE) == NULL)
    {
      ...
      er_set (ER_FATAL_ERROR_SEVERITY, ARG_FILE_LINE, ER_LOG_WRITE, 3, 0, 0, fileio_get_base_file_name (arv_name));
      goto error;
    }

  if (bg_arv_info->vdes != NULL_VOLDES && arvhdr->fpageid == bg_arv_info->start_page_id)
    {
      pageid = bg_arv_info->current_page_id;
      ...
      if (fileio_write_pages (thread_p, vdes, (char *) log_pgptr, ar_phy_pageid, num_pages, LOG_PAGESIZE) == NULL)
        {
          er_set (ER_FATAL_ERROR_SEVERITY, ARG_FILE_LINE, ER_LOG_WRITE, 3,
                  pageid, ar_phy_pageid, fileio_get_base_file_name (arv_name));
          goto error;
        }
    }
        ...
        if (unneeded_arvnum >= 0)
          {
            ...
            catmsg = msgcat_message (MSGCAT_CATALOG_RYE,
                                     MSGCAT_SET_LOG, MSGCAT_LOG_LOGINFO_COMMENT_UNUSED_ARCHIVE_NAME);
            if (catmsg == NULL)
              {
                catmsg = "29 COMMENT: Log archive %s,"
                  " which contains log pages before %lld," " is not needed any longer by any HA utilities.\n";
              }
            error_code =
              log_dump_log_info (log_Name_info, true,
                                 catmsg, fileio_get_base_file_name (unneeded_logarv_name), min_fpageid);
            if (error_code != NO_ERROR && error_code != ER_LOG_MOUNT_FAIL)
              {
                goto error;
              }
  ...
  bg_arv_info->vdes = fileio_format (thread_p, log_Db_fullname,
                                     ...
                                     LOGPB_ACTIVE_NPAGES, false, false, false, LOG_PAGESIZE, 0, true);
  if (bg_arv_info->vdes != NULL_VOLDES)
    {
...
logpb_remove_archive_logs_exceed_limit (THREAD_ENTRY * thread_p, int max_count)
{
  ...
  if (last_arv_num_to_delete >= 0 && last_arv_num_to_delete >= first_arv_num_to_delete)
    {
      catmsg = msgcat_message (MSGCAT_CATALOG_RYE, MSGCAT_SET_LOG, MSGCAT_LOG_MAX_ARCHIVES_HAS_BEEN_EXCEEDED);
      if (catmsg == NULL)
        {
          catmsg = "Number of active log archives has been exceeded" " the max desired number.";
        }
      deleted_count = logpb_remove_archive_logs_internal (thread_p,
                                                          first_arv_num_to_delete, last_arv_num_to_delete, catmsg);
    }
...
logpb_remove_archive_logs (THREAD_ENTRY * thread_p, const char *info_reason)
{
  ...
  pgbuf_flush_checkpoint (thread_p, &flush_upto_lsa, NULL, &newflush_upto_lsa);

  if ((!LSA_ISNULL (&newflush_upto_lsa)
       && LSA_LT (&newflush_upto_lsa, &flush_upto_lsa)) || fileio_synchronize_all (thread_p, false) != NO_ERROR)
    {
 ...
 */
static int
logpb_get_archive_num_from_info_table (UNUSED_ARG THREAD_ENTRY * thread_p, LOG_PAGEID page_id)
{
  int i, count;
  ...
  for (i = logpb_Arv_page_info_table.rear, count = 0;
       count < logpb_Arv_page_info_table.item_count; i = ((i == 0) ? ARV_PAGE_INFO_TABLE_SIZE - 1 : i - 1), count++)
    {
      ...
      if (first == last)
        {
          error_code =
            log_dump_log_info (log_Name_info, true, catmsg,
                               ...
                               fileio_get_base_file_name (logarv_name), info_reason);
        }
      else
        {
          ...
          error_code =
            log_dump_log_info (log_Name_info, true, catmsg,
                               ...
                               last, fileio_get_base_file_name (logarv_name), info_reason);
        }
      if (error_code != NO_ERROR && error_code != ER_LOG_MOUNT_FAIL)
        {
          return;
  ...
  if ((int) (strlen (dbname) + 1 + volmax_suffix) > filename_max)
    {
      ...
      er_set (ER_FATAL_ERROR_SEVERITY, ARG_FILE_LINE,
              ER_LOG_NOFULL_DATABASE_NAME_IS_TOO_LONG, 2, dbname, filename_max - volmax_suffix - 1);
  ...
  if ((int) (strlen (log_prefix) + 1) > filename_max || (int) (strlen (log_prefix) + 1) > MAXLOGNAME)
    {
      ...
      er_set (ER_FATAL_ERROR_SEVERITY, ARG_FILE_LINE,
              ...
              log_prefix, ((MAXLOGNAME > filename_max) ? filename_max - 1 : MAXLOGNAME - 1));
  ...
  if ((int) (strlen (db_fullname) + 1 + volmax_suffix) > pathname_max)
    {
      ...
      er_set (ER_FATAL_ERROR_SEVERITY, ARG_FILE_LINE,
              ER_BO_FULL_DATABASE_NAME_IS_TOO_LONG, 3, NULL, db_fullname, strlen (db_fullname) + 1, pathname_max);
...
logpb_checkpoint (THREAD_ENTRY * thread_p)
{
  ...
  if (pgbuf_flush_checkpoint (thread_p, &newchkpt_lsa, &chkpt_redo_lsa, &tmp_chkpt.redo_lsa) != NO_ERROR)
    {
      LOG_CS_ENTER (thread_p);
      goto exit_on_error;
    }
  ...
  for (i = 0, ntrans = 0, ntops = 0; i < log_Gl.trantable.num_total_indices; i++)
    {
      ...
      if (act_tdes != NULL && act_tdes->trid != NULL_TRANID && !LSA_ISNULL (&act_tdes->last_lsa))
        {
          ...
          if (LSA_ISNULL (&smallest_lsa) || LSA_GT (&smallest_lsa, &act_tdes->begin_lsa))
            {
  ...
  node = prior_lsa_alloc_and_copy_data (thread_p, LOG_END_CHKPT,
                                        ...
                                        (char *) chkpt_trans, length_all_tops, (char *) chkpt_topops);
  if (node == NULL)
    {
      assert (false);
      free_and_init (chkpt_trans);
          ...
          if (first_arv_num_not_needed == last_arv_num_not_needed)
            {
              ...
              catmsg = msgcat_message (MSGCAT_CATALOG_RYE, MSGCAT_SET_LOG, MSGCAT_LOG_LOGINFO_COMMENT_ARCHIVE_NONEEDED);
              if (catmsg == NULL)
                {
                  catmsg =
                    "COMMENT: Log archive %s is not needed any longer" " unless a database media crash occurs.\n";
                }
              error_code = log_dump_log_info (log_Name_info, true, catmsg, fileio_get_base_file_name (logarv_name));
            }
          else
            {
              ...
              catmsg = msgcat_message (MSGCAT_CATALOG_RYE,
                                       MSGCAT_SET_LOG, MSGCAT_LOG_LOGINFO_COMMENT_MANY_ARCHIVES_NONEEDED);
              if (catmsg == NULL)
                {
              ...
              error_code =
                log_dump_log_info (log_Name_info, true, catmsg,
                                   ...
                                   (logarv_name_first), fileio_get_base_file_name (logarv_name));
            }
          if (error_code != NO_ERROR && error_code != ER_LOG_MOUNT_FAIL)
            {
              goto exit_on_error;
...
logpb_delete (THREAD_ENTRY * thread_p, VOLID num_perm_vols,
              const char *db_fullname, const char *logpath, const char *prefix_logname, bool force_delete)
{
  ...
  if (num_perm_vols < 0 || log_Gl.trantable.area == NULL || log_Pb.pool == NULL)
    {
      ...
      if (fileio_is_volume_exist (log_Name_active) == false
      ...
      else
        {
           ...
           */
          if (rel_check_disk_compatible (&loghdr->db_version) != REL_COMPATIBLE)
            {
              loghdr = NULL;
            }
      ...
      if (db_volinfo_fp != NULL)
        {
          while (true)
            {
              if (fscanf (db_volinfo_fp, format_string, &read_int_volid, vol_name) != 2)
                {
                  break;
                }
  ...
  if (loghdr != NULL)
    {
      for (i = loghdr->last_deleted_arv_num + 1; i < loghdr->nxarv_num; i++)
        {
          fileio_make_log_archive_name (temp_vol_fullname, log_Archive_path, log_Prefix, i);
          fileio_unformat (thread_p, temp_vol_fullname);
        }
    }
...
logpb_fatal_error (THREAD_ENTRY * thread_p, bool log_exit,
                   const char *file_name, const int lineno, const char *fmt, ...)
{
  va_list ap;
...
logpb_fatal_error_internal (THREAD_ENTRY * thread_p, bool log_exit,
                            bool need_flush, const char *file_name, const int lineno, const char *fmt, va_list ap)
{
  const char *msglog;
  char msg[LINE_MAX];
      ...
      if (in_fatal == false)
        {
           ...
           * record.
           */
          (void) pgbuf_flush_checkpoint (thread_p, &tmp_lsa1, NULL, &tmp_lsa2);
          in_fatal = false;
        }
    }
...
logpb_initialize_flush_info (void)
{
  ...
  flush_info->num_toflush = 0;
  flush_info->toflush = (LOG_PAGE **) calloc (log_Pb.num_buffers, sizeof (LOG_PAGE *));
  if (flush_info->toflush == NULL)
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_OUT_OF_VIRTUAL_MEMORY, 1, log_Pb.num_buffers * sizeof (LOG_PAGE *));
