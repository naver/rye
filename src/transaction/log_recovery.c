...
log_rv_redo_record (THREAD_ENTRY * thread_p, LOG_LSA * log_lsa,
                    ...
                    LOG_RCV * rcv, LOG_LSA * rcv_lsa_ptr, int undo_length, char *undo_data, LOG_ZIP * redo_unzip_ptr);
...
static bool log_rv_get_unzip_log_data (THREAD_ENTRY * thread_p, int length,
                                       LOG_LSA * log_lsa, LOG_PAGE * log_page_p, LOG_ZIP * undo_unzip_ptr);
...
static int log_rv_analysis_run_postpone (THREAD_ENTRY * thread_p, int tran_id,
                                         LOG_LSA * log_lsa, LOG_PAGE * log_page_p, LOG_LSA * check_point);
static int log_rv_analysis_compensate (THREAD_ENTRY * thread_p, int tran_id, LOG_LSA * log_lsa, LOG_PAGE * log_page_p);
static int log_rv_analysis_lcompensate (THREAD_ENTRY * thread_p, int tran_id, LOG_LSA * log_lsa, LOG_PAGE * log_page_p);
...
static int log_rv_analysis_complete (THREAD_ENTRY * thread_p, int tran_id,
                                     ...
                                     LOG_LSA * prev_lsa,
                                     bool is_media_crash,
                                     const time_t * stopat, const LOG_LSA * stopat_lsa, bool * did_incom_recovery);
static int log_rv_analysis_complte_topope (THREAD_ENTRY * thread_p, int tran_id, LOG_LSA * log_lsa);
...
static void log_recovery_analysis (THREAD_ENTRY * thread_p,
                                   ...
                                   time_t * stopat,
                                   const LOG_LSA * stopat_lsa, bool * did_incom_recovery, INT64 * num_redo_log_records);
...
static void log_recovery_resetlog (THREAD_ENTRY * thread_p,
                                   LOG_LSA * new_append_lsa, bool is_new_append_page, LOG_LSA * last_lsa);
static int log_recovery_find_first_postpone (THREAD_ENTRY * thread_p,
                                             LOG_LSA * ret_lsa, LOG_LSA * start_postpone_lsa, LOG_TDES * tdes);
/*
...
log_rv_undo_record (THREAD_ENTRY * thread_p, LOG_LSA * log_lsa,
                    LOG_PAGE * log_page_p, LOG_RCVINDEX rcvindex,
                    const VPID * rcv_vpid, LOG_RCV * rcv,
                    UNUSED_ARG const LOG_LSA * rcv_undo_lsa, LOG_TDES * tdes, LOG_ZIP * undo_unzip_ptr)
{
  char *area = NULL;
 ...
 */
void
log_recovery (THREAD_ENTRY * thread_p, int ismedia_crash, time_t * stopat, LOG_LSA * stopat_lsa)
{
  ...
  log_recovery_analysis (thread_p, &rcv_lsa, &start_redolsa, &end_redo_lsa,
                         ismedia_crash, stopat, stopat_lsa, &did_incom_recovery, &num_redo_log_records);

  er_set (ER_NOTIFICATION_SEVERITY, ARG_FILE_LINE,
          ER_LOG_RECOVERY_STARTED, 3, num_redo_log_records, start_redolsa.pageid, end_redo_lsa.pageid);
 ...
 */
static int
log_rv_analysis_lcompensate (THREAD_ENTRY * thread_p, int tran_id, LOG_LSA * log_lsa, LOG_PAGE * log_page_p)
{
  LOG_TDES *tdes;
  ...
  if (tdes == NULL)
    {
      logpb_fatal_error (thread_p, true, ARG_FILE_LINE, "log_rv_analysis_lcompensate");
      return ER_FAILED;
    }
  ...
  /* Read the DATA HEADER */
  LOG_READ_ADD_ALIGN (thread_p, sizeof (LOG_RECORD_HEADER), log_lsa, log_page_p);
  LOG_READ_ADVANCE_WHEN_DOESNT_FIT (thread_p, sizeof (struct log_logical_compensate), log_lsa, log_page_p);
                          ...
                          const LOG_LSA * stopat_lsa, bool * did_incom_recovery)
{
  ...
  if (stopat != NULL && *stopat != (time_t) (-1) && difftime (*stopat, last_at_time) < 0)
    {
      found_stopat = true;
    }
  else if (stopat_lsa != NULL && !LSA_ISNULL (stopat_lsa) && LSA_LE (stopat_lsa, log_lsa))
    {
      found_stopat = true;
    }
 ...
 */
static int
log_rv_analysis_complte_topope (THREAD_ENTRY * thread_p, int tran_id, LOG_LSA * log_lsa)
{
  LOG_TDES *tdes;
  ...
  if (tdes == NULL)
    {
      logpb_fatal_error (thread_p, true, ARG_FILE_LINE, "log_rv_analysis_complte_topope");
      return ER_FAILED;
    }
...
log_rv_analysis_end_checkpoint (THREAD_ENTRY * thread_p, LOG_LSA * log_lsa,
                                ...
                                LOG_LSA * start_redo_lsa, bool * may_use_checkpoint)
{
  LOG_TDES *tdes;
...
log_rv_analysis_record (THREAD_ENTRY * thread_p, LOG_RECTYPE log_type,
                        ...
                        const LOG_LSA * stopat_lsa, bool * did_incom_recovery, bool * may_use_checkpoint)
{
  switch (log_type)
    {
    ...
    case LOG_LCOMPENSATE:
      (void) log_rv_analysis_lcompensate (thread_p, tran_id, log_lsa, log_page_p);
      break;
    ...
    case LOG_ABORT:
      (void) log_rv_analysis_complete (thread_p, tran_id, log_lsa, log_page_p,
                                       prev_lsa, is_media_crash, stopat, stopat_lsa, did_incom_recovery);
      break;
    ...
    case LOG_END_CHKPT:
      log_rv_analysis_end_checkpoint (thread_p, log_lsa, log_page_p,
                                      checkpoint_lsa, start_redo_lsa, may_use_checkpoint);
      break;
              ...
              if (log_rec != NULL)
                {
                  ...
                  last_log_tdes = LOG_FIND_TDES (logtb_find_tran_index (thread_p, log_rec->trid));
                  if (last_log_tdes != NULL)
                    {
                      LSA_COPY (&last_log_tdes->last_lsa, &log_rec->prev_tranlsa);
      ...
      while (!LSA_ISNULL (&lsa) && lsa.pageid == log_lsa.pageid)
        {
          ...
          if (!LOG_IS_VALID_LOG_RECORD (&log_lsa, log_rec))
            {
              ...
              er_log_debug (ARG_FILE_LINE,
                            ...
                            (long) log_lsa.pageid, log_lsa.offset, (long) lsa.pageid, lsa.offset);
              logpb_fatal_error (thread_p, true, ARG_FILE_LINE, "log_recovery_analysis");
              LSA_SET_NULL (&lsa);
              break;
            }
          ...
          log_rv_analysis_record (thread_p, log_rtype, tran_id, &log_lsa,
                                  ...
                                  is_media_crash, stopat, stopat_lsa, did_incom_recovery, &may_use_checkpoint);
          if (*did_incom_recovery == true)
            {
              LSA_SET_NULL (&lsa);
              break;
          ...
          if (!LOG_IS_VALID_LOG_RECORD (&log_lsa, log_rec))
            {
              ...
              er_log_debug (ARG_FILE_LINE,
                            ...
                            (long long int) log_lsa.pageid, log_lsa.offset, (long long int) lsa.pageid, lsa.offset);
              logpb_fatal_error (thread_p, true, ARG_FILE_LINE, "log_recovery_redo");
              LSA_SET_NULL (&lsa);
              break;
            }
          ...
          switch (log_rec->type)
            ...
            case LOG_DIFF_UNDOREDO_DATA:
              ...
              if (rcv.pgptr != NULL)
                {
                  ...
                  if (LSA_LE (&rcv_lsa, rcv_page_lsaptr)
                      && (end_redo_lsa == NULL || LSA_ISNULL (end_redo_lsa) || LSA_LE (rcv_page_lsaptr, end_redo_lsa)))
                    {
                      /* It is already done */
                      pgbuf_unfix (thread_p, rcv.pgptr);
                      break;
                  ...
                  if (rcv_page_lsaptr != NULL)
                    {
                      fprintf (stdout, "      page_lsa = %lld|%d\n",
                               (long long int) rcv_page_lsaptr->pageid, rcv_page_lsaptr->offset);
                    }
                  else
                    {
              ...
              if (is_diff_rec)
                ...
                {
                  log_rv_redo_record (thread_p, &log_lsa,
                                      log_pgptr, RV_fun[rcvindex].redofun, &rcv, &rcv_lsa, 0, NULL, redo_unzip_ptr);
                }
              if (rcv.pgptr != NULL)
                {
                  pgbuf_unfix (thread_p, rcv.pgptr);
            ...
            case LOG_DBEXTERN_REDO_DATA:
              ...
              log_rv_redo_record (thread_p, &log_lsa,
                                  log_pgptr, RV_fun[rcvindex].redofun, &rcv, &rcv_lsa, 0, NULL, NULL);
              break;
              ...
              if (rcv_vpid.pageid != NULL_PAGEID && rcv_vpid.volid != NULL_VOLID)
                {
                  ...
                  rcv.pgptr = pgbuf_fix (thread_p, &rcv_vpid, OLD_PAGE,
                                         PGBUF_LATCH_WRITE, PGBUF_UNCONDITIONAL_LATCH, PAGE_UNKNOWN);
                  if (rcv.pgptr == NULL)
                    {
                      break;
                    }
            ...
            case LOG_COMPENSATE:
              ...
              log_rv_redo_record (thread_p, &log_lsa,
                                  log_pgptr, RV_fun[rcvindex].undofun, &rcv, &rcv_lsa, 0, NULL, NULL);
              if (rcv.pgptr != NULL)
                {
                  pgbuf_unfix (thread_p, rcv.pgptr);
                }
          ...
          if (tdes->state == TRAN_UNACTIVE_WILL_COMMIT || tdes->state == TRAN_UNACTIVE_COMMITTED_WITH_POSTPONE)
            {
              ...
              log_do_postpone (thread_p, tdes, &first_postpone_to_apply, LOG_COMMIT_WITH_POSTPONE);
          ...
          else
            {
              ...
              log_do_postpone (thread_p, tdes, &first_postpone_to_apply, LOG_COMMIT_TOPOPE_WITH_POSTPONE);
      ...
      if (tran_index != LOG_SYSTEM_TRAN_INDEX)
        {
          ...
          if (tdes != NULL
              ...
              && (tdes->state == TRAN_UNACTIVE_UNILATERALLY_ABORTED
                  || tdes->state == TRAN_UNACTIVE_ABORTED) && LSA_ISNULL (&tdes->undo_nxlsa))
            {
          ...
          switch (log_rec->type)
            ...
            case LOG_DIFF_UNDOREDO_DATA:
              ...
              log_rv_undo_record (thread_p, &log_lsa, log_pgptr,
                                  rcvindex, &rcv_vpid, &rcv, &rcv_lsa, tdes, undo_unzip_ptr);
            ...
            case LOG_LCOMPENSATE:
              ...
              /* Get the DATA HEADER */
              LOG_READ_ADD_ALIGN (thread_p, sizeof (LOG_RECORD_HEADER), &log_lsa, log_pgptr);
              LOG_READ_ADVANCE_WHEN_DOESNT_FIT (thread_p, sizeof (struct log_logical_compensate), &log_lsa, log_pgptr);
        ...
        if (start_arv_num == i - 1)
          {
            ...
            error_code =
              log_dump_log_info (log_Name_info, true, catmsg,
                                 ...
                                 start_arv_num, fileio_get_base_file_name (logarv_name), info_reason);
        ...
        else
          {
            ...
            error_code =
              log_dump_log_info (log_Name_info, true, catmsg,
                                 ...
                                 (logarv_name_first), i - 1, fileio_get_base_file_name (logarv_name), info_reason);
            if (error_code != NO_ERROR && error_code != ER_LOG_MOUNT_FAIL)
              {
                return;
              }
...
log_recovery_notpartof_volumes (THREAD_ENTRY * thread_p)
{
  ...
  (void) fileio_map_mounted (thread_p,
                             (bool (*)(THREAD_ENTRY *, VOLID, void *)) log_unformat_ahead_volumes, &start_volid);
  ...
  if (log_Gl.append.vdes == NULL_VOLDES || log_Gl.hdr.fpageid > log_Gl.hdr.append_lsa.pageid)
    {
      ...
      catmsg = msgcat_message (MSGCAT_CATALOG_RYE, MSGCAT_SET_LOG, MSGCAT_LOG_RESETLOG_DUE_INCOMPLTE_MEDIA_RECOVERY);
      ...
      if (log_Gl.append.vdes == NULL_VOLDES)
        {
          ...
          log_Gl.append.vdes = fileio_format (thread_p, log_Db_fullname,
                                              ...
                                              (-1), false, true, false, LOG_PAGESIZE, 0, false);
...
log_startof_nxrec (THREAD_ENTRY * thread_p, LOG_LSA * lsa, bool canuse_forwaddr)
{
  ...
  switch (type)
    ...
    case LOG_LCOMPENSATE:
      {
        ...
        LOG_READ_ADD_ALIGN (thread_p, sizeof (struct log_logical_compensate), &log_lsa, log_pgptr);
        break;
      }
    ...
    case LOG_DUMMY_UPDATE_GID_BITMAP:
      LOG_READ_ADVANCE_WHEN_DOESNT_FIT (thread_p, sizeof (struct log_gid_bitmap_update), &log_lsa, log_pgptr);
      LOG_READ_ADD_ALIGN (thread_p, sizeof (struct log_gid_bitmap_update), &log_lsa, log_pgptr);
      break;

    case LOG_DUMMY_FILLPAGE_FORARCHIVE:        /* for backward compatibility */
      {
        /* Get to start of next page */
        LOG_READ_ADVANCE_WHEN_DOESNT_FIT (thread_p, LOG_PAGESIZE, &log_lsa, log_pgptr);
        break;
      }
                  ...
                  switch (log_rec->type)
                    ...
                    case LOG_RUN_POSTPONE:
                      ...
                      LOG_READ_ADVANCE_WHEN_DOESNT_FIT (thread_p,
                                                        sizeof (struct log_run_postpone), &log_lsa, log_pgptr);
