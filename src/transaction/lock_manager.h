...
#endif
extern int lock_object (THREAD_ENTRY * thread_p, const DB_VALUE * val,
...
extern void lock_unlock_object (THREAD_ENTRY * thread_p, const DB_VALUE * val,
				LK_UNLOCK_TYPE unlock_type);
extern void lock_unlock_all (THREAD_ENTRY * thread_p);
extern bool lock_has_xlock (THREAD_ENTRY * thread_p);
extern LOCK lock_get_current_lock (THREAD_ENTRY * thread_p,
				   DB_VALUE * oid_val);
...
extern int lock_shard_key_lock (THREAD_ENTRY * thread_p,
				...
				OID * class_oid, bool is_shard_table,
				bool is_catalog_table, bool for_update);
extern int xlock_system_ddl_lock (THREAD_ENTRY * thread_p);
#if defined (ENABLE_UNUSED_FUNCTION)
extern int xlock_change_class_xlock_to_ulock (THREAD_ENTRY *
					      thread_p, OID * class_oid);
#endif
