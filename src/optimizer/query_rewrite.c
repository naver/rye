 ...
 */
static PT_NODE *
qo_get_name_by_spec_id (UNUSED_ARG PARSER_CONTEXT * parser, PT_NODE * node, void *arg, int *continue_walk)
{
 ...
 */
PT_NODE *
qo_check_nullable_expr (UNUSED_ARG PARSER_CONTEXT * parser, PT_NODE * node, void *arg, UNUSED_ARG int *continue_walk)
{
 ...
 */
static PT_NODE *
qo_collect_name_spec_post (UNUSED_ARG PARSER_CONTEXT * parser, PT_NODE * node, void *arg, int *continue_walk)
{
  PT_NAME_SPEC_INFO *info = (PT_NAME_SPEC_INFO *) arg;
...
static PT_NODE *
qo_reduce_equality_terms_helper (PARSER_CONTEXT * parser, PT_NODE * node, PT_NODE * where)
{
  PT_NODE *from;
                  ...
                  if (attr && col && qo_is_reduceable_const (col))
                    {
                      ...
                      where = parser_append_node (parser_copy_tree (parser, expr), where);
              ...
              if (temp->or_next == NULL)
                {
                  ...
                  (void) parser_walk_tree (parser, temp->info.expr.arg1,
                                           qo_collect_name_spec, &info1, qo_collect_name_spec_post, &info1);
                  ...
                  if (info1.query_num == 0)
                    {
                      (void) parser_walk_tree (parser, temp->info.expr.arg2,
                                               qo_collect_name_spec, &info2, qo_collect_name_spec_post, &info2);
                    }
          ...
          if (PT_IS_CONST (arg2))
            {
              ...
              if (parser->set_host_var == 0)
...
#endif
                {
                  ...
                  if (tp_value_coerce (dbval, &dbval_res, dom) != DOMAIN_COMPATIBLE)
                    {
                      continue; /* give up */
                    }
          ...
          node->info.query.q.select.list =
            pt_lambda_with_arg (parser, node->info.query.q.select.list,
                                ...
                                (temp->info.name.location > 0 ? true
                                 : false), 1 /* type: check normal func data_type */ ,
                                true /* dont_replace */ );
        }
      where =
        pt_lambda_with_arg (parser, where, arg1, arg2,
                            (temp->info.name.location > 0 ? true : false), 1 /* type: check normal func data_type */ ,
...
qo_reduce_equality_terms (PARSER_CONTEXT * parser, PT_NODE * node)
{
  ...
  switch (node->node_type)
    {
    case PT_SELECT:
      node->info.query.q.select.where = qo_reduce_equality_terms_helper (parser, node, node->info.query.q.select.where);
      node->info.query.q.select.having =
        qo_reduce_equality_terms_helper (parser, node, node->info.query.q.select.having);
      break;

    case PT_UPDATE:
      node->info.update.search_cond = qo_reduce_equality_terms_helper (parser, node, node->info.update.search_cond);
      break;

    case PT_DELETE:
      node->info.delete_.search_cond = qo_reduce_equality_terms_helper (parser, node, node->info.delete_.search_cond);
      break;
      ...
      if (subquery_ptr && subquery_ptr->node_type == PT_SELECT)
        {
          subquery_ptr->info.query.q.select.where =
            qo_reduce_equality_terms_helper (parser, node, subquery_ptr->info.query.q.select.where);
        }
      break;
  ...
  if (node->info.query.orderby_for)
    {
      ...
      node->info.query.orderby_for =
        pt_lambda_with_arg (parser, node->info.query.orderby_for, ord_num, grp_num, false /* loc_check: DEFAULT */ ,
                            0 /* type: DEFAULT */ ,
                            false /* dont_replace: DEFAULT */ );
       ...
       */
      node->info.query.q.select.list =
        pt_lambda_with_arg (parser, node->info.query.q.select.list, ord_num, grp_num, false /* loc_check: DEFAULT */ ,
                            0 /* type: DEFAULT */ ,
                            false /* dont_replace: DEFAULT */ );
  ...
  if (node->info.query.all_distinct == PT_DISTINCT)
  ...
  else
    {
      ...
      if (node->info.query.q.select.group_by
          ...
          && node->info.query.q.select.group_by->with_rollup == 0 && node->info.query.order_by)
        {
          bool ordbynum_flag;
              ...
              if (node->info.query.orderby_for)
                {
                  ...
                  node->info.query.orderby_for =
                    pt_lambda_with_arg (parser, node->info.query.orderby_for,
                                        ord_num, ins_num, false /* loc_check: DEFAULT */ ,
                                        0 /* type: DEFAULT */ ,
                                        false /* dont_replace: DEFAULT */ );
          ...
          while ((arg1 = dnf_node->info.expr.arg1) && (arg2 = dnf_node->info.expr.arg2))
            {
              ...
              arg1_arg1 = ((pt_is_expr_node (arg1)
                            && arg1->info.expr.op == PT_UNARY_MINUS) ? arg1->info.expr.arg1 : NULL);
              arg2_arg1 = ((pt_is_expr_node (arg2)
                            && arg2->info.expr.op == PT_UNARY_MINUS) ? arg2->info.expr.arg1 : NULL);
                  ...
                  parser_free_tree (parser, arg2);
                       ...
                       && (pt_is_attr (arg1_arg1)
                           || (pt_is_expr_node (arg1_arg1)
                               && arg1_arg1->info.expr.op == PT_UNARY_MINUS)) && pt_is_const (arg2))
                {
                  ...
                  dnf_node->info.expr.arg2 = arg1;
                       ...
                       && (pt_is_attr (arg2->info.expr.arg1)
                           || (pt_is_expr_node (arg2_arg1)
                               && arg2_arg1->info.expr.op == PT_UNARY_MINUS)) && pt_is_const (arg1))
                {
  ...
  for (node = start; node; node = node->next)
    {
      ...
      if (node->info.expr.op == op_type1 || node->info.expr.op == op_type2)
        {
          if (pt_is_attr (arg_prior) && (pt_check_path_eq (parser, arg_prior, arg_prior_start) == 0))
            {
...
qo_reduce_comp_pair_terms (PARSER_CONTEXT * parser, PT_NODE ** wherep)
{
  ...
  for (node = *wherep; node; node = node->next)
    {
      if (node->node_type != PT_EXPR || !pt_is_attr (node->info.expr.arg1) || node->or_next != NULL)
        {
      ...
      if (pt_comp_to_between_op (lower->info.expr.op, upper->info.expr.op,
                                 PT_REDUCE_COMP_PAIR_TERMS, &pair->info.expr.op) != 0)
        {
...
qo_allocate_like_bound_for_index_scan (PARSER_CONTEXT * const parser,
                                       PT_NODE * const like,
                                       PT_NODE * const pattern, PT_NODE * const escape, const bool allocate_lower_bound)
{
  PT_NODE *bound = NULL;
...
qo_rewrite_like_for_index_scan (PARSER_CONTEXT * const parser,
                                PT_NODE * const like, PT_NODE * const pattern, PT_NODE * const escape)
{
 ...
 */
static PT_NODE *
qo_check_like_expression_pre (UNUSED_ARG PARSER_CONTEXT * parser, PT_NODE * node, void *arg, int *continue_walk)
{
              ...
              if (pt_is_pseudo_const (pattern)
                  && (escape == NULL || PT_IS_NULL_NODE (escape) || pt_is_pseudo_const (escape)))
                {
                  ...
                  (void *) parser_walk_tree (parser, pattern,
                                             qo_check_like_expression_pre, &like_expression_not_safe, NULL, NULL);
                  ...
                  qo_rewrite_like_for_index_scan (parser, crt_expr, pattern, escape);
                }
            }
        }
    }
  ...
  switch (op_type)
    ...
    case PT_LE:
      ...
      between_and->info.expr.op = (op_type == PT_GT ? PT_BETWEEN_GT_INF :
                                   (op_type == PT_GE ? PT_BETWEEN_GE_INF :
                                    (op_type == PT_LT ? PT_BETWEEN_INF_LT : PT_BETWEEN_INF_LE)));
      between_and->info.expr.arg1 = node->info.expr.arg2;
      between_and->info.expr.arg2 = NULL;
      between_and->info.expr.location = node->info.expr.location;
#if defined(RYE_DEBUG)
    ...
    case PT_IS_IN:
      ...
      if (PT_IS_COLLECTION_TYPE (node->type_enum)
          || PT_IS_QUERY_NODE_TYPE (in_arg2->node_type) || !PT_IS_COLLECTION_TYPE (in_arg2->type_enum))
        {
  ...
  while ((sibling = prev->or_next))
    {
      ...
      if ((node_prior->node_type != sibling_prior->node_type)
          || (pt_is_attr (node_prior)
              && pt_is_attr (sibling_prior) && pt_check_path_eq (parser, node_prior, sibling_prior)))
        {
      ...
      while ((dnf_node = (dnf_prev ? dnf_prev->or_next : cnf_node)))
        {
          ...
          if (dnf_node == cnf_node && dnf_node->or_next == NULL && !pt_is_instnum (arg1_prior))
            {
              if (dnf_node->info.expr.op == PT_EQ)
                {
                  ...
                  continue;
                       ...
                       || dnf_node->info.expr.op == PT_LT || dnf_node->info.expr.op == PT_LE)
                {
          ...
          if (dnf_node == cnf_node && dnf_node->or_next == NULL
              && dnf_node->info.expr.op == PT_EQ && !pt_is_instnum (arg1_prior))
            {
              /* do not convert one predicate '=' term to RANGE */
              dnf_prev = dnf_prev ? dnf_prev->or_next : dnf_node;
              continue;
          ...
          switch (dnf_node->info.expr.op)
            ...
            case PT_RANGE:
              ...
              if (dnf_node->info.expr.op == PT_IS_IN
                  ...
                  && PT_IS_COLLECTION_TYPE (dnf_node->info.expr.arg2->type_enum) && dnf_node->or_next == NULL)
                {
                  /*
                   * skip merge in list
 ...
 */
static PT_NODE *
qo_rewrite_outerjoin (PARSER_CONTEXT * parser, PT_NODE * node, UNUSED_ARG void *arg, int *continue_walk)
{
              ...
              for (expr = node->info.query.q.select.where; expr; expr = expr->next)
                {
                  if (expr->node_type == PT_EXPR
                      && expr->info.expr.location == 0 && expr->info.expr.op != PT_IS_NULL && expr->or_next == NULL)
                    {
                      (void) parser_walk_leaves (parser, expr,
                                                 qo_get_name_by_spec_id, &info, qo_check_nullable_expr, &nullable_cnt);
                      ...
                      if (info.appears && nullable_cnt == 0)
                        {
                          ...
                          (void) parser_walk_tree (parser,
                                                   node->info.query.q.select.where, qo_reset_location,
                              ...
                              if (ns->info.spec.join_type == PT_JOIN_RIGHT_OUTER)
                                {
                                  ...
                                  (void) parser_walk_tree (parser,
                                                           node->info.query.q.select.where,
                                                           qo_reset_location, &locate_info, NULL, NULL);
                                }
                            }
                          break;
                        }
 ...
 */
static PT_NODE *
qo_reset_location (UNUSED_ARG PARSER_CONTEXT * parser, PT_NODE * node, void *arg, UNUSED_ARG int *continue_walk)
{
  ...
  if (node->node_type == PT_VALUE
      && node->info.value.location >= infop->start && node->info.value.location <= infop->end)
    {
 ...
 */
static PT_NODE *
qo_rewrite_innerjoin (PARSER_CONTEXT * parser, PT_NODE * node, UNUSED_ARG void *arg, int *continue_walk)
{
  PT_NODE *spec, *spec2;
      ...
      while ((next_node = hint_node->next) != NULL)
        {
          ...
          if (PT_IDX_HINT_ORDER (hint_node) > PT_IDX_HINT_ORDER (next_node))
          ...
          else if (hint_node->etc == next_node->etc)
            {
              ...
              if (hint_node->info.name.resolved != NULL && next_node->info.name.resolved != NULL)
                {
                  res_cmp_tbl_names = intl_identifier_casecmp
                    (hint_node->info.name.resolved, next_node->info.name.resolved);
                }
                  ...
                  if (hint_node->info.name.original != NULL && next_node->info.name.original != NULL)
                    {
                      ...
                      res_cmp_idx_names = intl_identifier_casecmp
                        (hint_node->info.name.original, next_node->info.name.original);
      ...
      if (do_rewrite)
        {
          ...
          switch (op_type)
            ...
            case PT_IS_IN:     /* arg1 = set_func_elements, attr */
              ...
              (void) parser_walk_tree (parser,
                                       new_spec->info.spec.derived_table, qo_rewrite_subqueries, idx, NULL, NULL);
              break;
 ...
 */
static PT_NODE *
qo_optimize_queries (PARSER_CONTEXT * parser, PT_NODE * node, UNUSED_ARG void *arg, UNUSED_ARG int *continue_walk)
{
  int level;
              ...
              if (can_move)
                {
                  ...
                  (void) parser_walk_tree (parser, cnf,
                                           ...
                                           &info, pt_find_aggregate_functions_post, &info);
 ...
 */
static PT_NODE *
qo_optimize_queries_post (PARSER_CONTEXT * parser, PT_NODE * tree, UNUSED_ARG void *arg, UNUSED_ARG int *continue_walk)
{
          ...
          if (location > 0)
            {
              for (spec = tree->info.query.q.select.from;
                   spec && spec->info.spec.location != location; spec = spec->next)
                /* nop */ ;
              if (spec)
                {
                  if (spec->info.spec.join_type == PT_JOIN_LEFT_OUTER
