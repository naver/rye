...
static QO_SEGMENT *qo_insert_segment (QO_NODE * head, PT_NODE * node, QO_ENV * env);
...
qo_optimize_helper (QO_ENV * env)
{
  ...
  for (n = 1; n < env->nnodes; n++)
    {
      ...
      if (QO_NODE_PT_JOIN_TYPE (node) != PT_JOIN_NONE
          && QO_NODE_PT_JOIN_TYPE (node) != PT_JOIN_CROSS && !BITSET_MEMBER (nodeset, n))
        {
  ...
  (void) parser_walk_tree (parser, tree->info.query.q.select.list,
                           qo_add_final_segment, &local_env, pt_continue_walk, NULL);
   ...
   */
  qo_get_optimization_param (&level, QO_PARAM_LEVEL);
  if (PLAN_DUMP_ENABLED (level) && DETAILED_DUMP (level) && env->plan_dump_enabled)
    {
 ...
 */
static PT_NODE *
graph_size_select (UNUSED_ARG PARSER_CONTEXT * parser, PT_NODE * tree, void *arg, int *continue_walk)
{
 ...
 */
static PT_NODE *
build_query_graph (UNUSED_ARG PARSER_CONTEXT * parser, PT_NODE * tree, void *arg, int *continue_walk)
{
  QO_ENV *env = (QO_ENV *) * (long *) arg;
 ...
 */
static PT_NODE *
build_query_graph_post (UNUSED_ARG PARSER_CONTEXT * parser, PT_NODE * tree, UNUSED_ARG void *arg, int *continue_walk)
{
  *continue_walk = PT_CONTINUE_WALK;
              ...
              if (xasl)
                {
                  ...
                  QO_NODE_TCARD (node) = (unsigned long)
                    ((QO_NODE_NCARD (node) * (double) xasl->projected_size) / (double) IO_PAGESIZE);
                }
              break;
            default:
              break;
 ...
 */
static PT_NODE *
qo_add_final_segment (UNUSED_ARG PARSER_CONTEXT * parser, PT_NODE * tree, void *arg, int *continue_walk)
{
  QO_ENV *env = (QO_ENV *) * (long *) arg;
...
qo_add_term (PT_NODE * conjunct, QO_ENV * env)
{
  ...
  QO_TERM_LOCATION (term) = (conjunct->node_type == PT_EXPR ?
                             conjunct->info.expr.location : conjunct->info.value.location);
 ...
 */
static PT_NODE *
check_subquery_pre (UNUSED_ARG PARSER_CONTEXT * parser, PT_NODE * node, void *arg, int *continue_walk)
{
 ...
 */
static PT_NODE *
get_local_subqueries_pre (UNUSED_ARG PARSER_CONTEXT * parser, PT_NODE * node, void *arg, int *continue_walk)
{
  QO_ENV *env;
  BITSET segs;
 ...
 */
static PT_NODE *
get_local_subqueries_post (UNUSED_ARG PARSER_CONTEXT * parser, PT_NODE * node, void *arg, int *continue_walk)
{
...
add_hint (QO_ENV * env, PT_NODE * tree)
{
      ...
      if (tree->info.query.q.select.ordered_hint)
        {
          ...
          for (arg = tree->info.query.q.select.ordered_hint; arg->next; arg = arg->next)
            {
              ;                 /* nop */
            }
          for (i = 0; i < env->nnodes; i++)
          ...
          for (i = 0; i < env->nnodes; i++)
            {
              ...
              /* check for arg list */
              p_arg = NULL;
              for (arg = tree->info.query.q.select.ordered_hint, j = 0; arg; arg = arg->next, j++)
                {
                  if (spec->info.spec.id == arg->info.name.spec_id)
                    {
                      if (p_arg)
  ...
  if (hint & PT_HINT_USE_NL)
    {
      add_hint_args (env, tree->info.query.q.select.use_nl_hint, PT_HINT_USE_NL);
    }

  if (hint & PT_HINT_USE_IDX)
    {
      add_hint_args (env, tree->info.query.q.select.use_idx_hint, PT_HINT_USE_IDX);
    }
  ...
  for (indexp_nokl = using_index; indexp_nokl; indexp_nokl = indexp_nokl->next)
    {
      ...
      else if (indexp_nokl->info.name.original && !indexp_nokl->info.name.indx_key_limit)
        {
          ...
          for (indexp = using_index; indexp; indexp = indexp->next)
            {
              if (indexp->info.name.original
                  ...
                  && !intl_identifier_casecmp (indexp->info.name.original,
                                               indexp_nokl->info.name.original)
...
qo_get_class_info (QO_ENV * env, QO_NODE * node)
{
  ...
  if (info == NULL)
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_OUT_OF_VIRTUAL_MEMORY, 1, SIZEOF_CLASS_INFO (1));
      return NULL;
    }
  ...
  for (; p; p = p->next)
    {
      ...
      if (smclass->stats == NULL)
        ...
        {
          if (!info->normal_class || (((hfid = sm_get_heap (info->mop)) && !HFID_IS_NULL (hfid))))
            {
...
qo_get_attr_info (QO_ENV * env, QO_SEGMENT * seg)
{
  ...
  if (cum_statsp->pkeys_size == 0 ||    /* the first found */
      cum_statsp->pkeys[cum_statsp->pkeys_size - 1] < bt_statsp->pkeys[bt_statsp->pkeys_size - 1])
    {
      ...
      if (cum_statsp->pkeys == NULL)
        {
          er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE,
                  ER_OUT_OF_VIRTUAL_MEMORY, 1, SIZEOF_ATTR_CUM_STATS_PKEYS (cum_statsp->pkeys_size));
      ...
      if (index_entryp != NULL)
        {
          ...
          attr_statsp = &QO_GET_CLASS_STATS (class_info_entryp)->attr_stats[k];
...
qo_estimate_statistics (UNUSED_ARG MOP class_mop, CLASS_STATS * statblock)
{
  ...
  statblock->heap_num_objects = (NOMINAL_HEAP_SIZE * DB_PAGESIZE) / NOMINAL_OBJECT_SIZE;
  ...
  for (i = 0; i < env->nterms; i++)
    {
      ...
      if (!(QO_TERM_CLASS (term) == QO_TC_SARG
            || QO_TERM_CLASS (term) == QO_TC_JOIN || QO_TERM_CLASS (term) == QO_TC_OTHER))
        {
...
qo_find_index_segs (QO_ENV * env,
                    ...
                    int *seg_idx, int seg_idx_num, int *nseg_idxp, BITSET * segs)
{
  QO_SEGMENT *segp;
  SM_ATTRIBUTE *attrp;
  BITSET working;
      ...
      for (e = 0; e < E; ++e)
        {
          ...
          if (bitset_subset
              (&(QO_PARTITION_NODES (part)), &(QO_TERM_NODES (edge))) && !bitset_is_empty (&(QO_TERM_NODES (edge))))
            {
      ...
      for (attr = cons->attributes; *attr != NULL; attr++)
        {
          ...
          if ((*attr)->sma_domain != NULL && TP_TYPE_HAS_COLLATION (TP_DOMAIN_TYPE ((*attr)->sma_domain)))
            {
              LANG_COLLATION *lang_coll = lang_get_collation (TP_DOMAIN_COLLATION ((*attr)->sma_domain));
