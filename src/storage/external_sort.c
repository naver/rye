 ...
 *   slotid(out):A slot identifier or NULL_SLOTID
		      ...
		      PAGE_PTR pgptr, INT16 length, INT16 type)
{
  ...
  assert (out_sptr != NULL && out_space != NULL && out_slotid != NULL);

  /* init out parameter */
  *out_sptr = NULL;
  *out_space = 0;
  *out_slotid = NULL_SLOTID;
  ...
  if (slotid != sphdr->nslots || sphdr->tfree != sphdr->cfree)
    {
      assert (false);
	       ...
	       SORT_DUP_OPTION option, long *srun_limit)
{
  ...
  st_p->srun = (SRUN *) malloc (cnt * sizeof (SRUN));
  if (st_p->srun == NULL)
    {
      return ER_OUT_OF_VIRTUAL_MEMORY;
    }
		...
		char **index_area, int numrecs, int rec_type)
{
  int error;
  int run_size;
  RECDES out_recdes = RECDES_INITIALIZER;
  int i;
  SORT_REC *key, *next;
  int flushed_items = 0;
  int should_continue = true;
  INT16 slotid = NULL_SLOTID;
		   ...
		   bool force_alloc)
{
   ...
   */
  file_numpages = file_get_numpages (thread_p, vfid, NULL, NULL, NULL);
		 ...
		 INT32 num_pages, char *area_start)
{
  ...
  file_size = file_get_numpages (thread_p, vfid, NULL, NULL, NULL);
