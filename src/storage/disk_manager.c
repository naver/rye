...
static INT16 disk_cache_get_purpose_info (THREAD_ENTRY * thread_p,
                                          DISK_VOLPURPOSE vol_purpose,
                                          INT16 * nvols, int *total_pages, int *free_pages);
#endif
...
static int disk_map_init (THREAD_ENTRY * thread_p, INT16 volid,
                          INT32 at_fpageid, INT32 at_lpageid, INT32 nalloc_bits, DISK_VOLPURPOSE vol_purpose);
static int disk_map_dump (THREAD_ENTRY * thread_p, FILE * fp, VPID * vpid,
                          const char *at_name, INT32 at_fpageid, INT32 at_lpageid, INT32 all_fid, INT32 all_lid);
...
static bool disk_get_hint_contiguous_free_numpages (THREAD_ENTRY * thread_p,
                                                    INT16 volid, INT32 arecontiguous_npages, INT32 * num_freepgs);

static bool disk_check_sector_has_npages (THREAD_ENTRY * thread_p,
                                          INT16 volid, INT32 at_pg1, INT32 low_allid, INT32 high_allid, int exp_npages);
static INT32 disk_id_alloc (THREAD_ENTRY * thread_p, INT16 volid,
                            DISK_VAR_HEADER * vhdr, INT32 nalloc,
                            INT32 low_allid, INT32 high_allid, int allid_type, int exp_npages, int skip_pageid);
static int disk_id_dealloc (THREAD_ENTRY * thread_p, INT16 volid,
                            INT32 at_pg1, INT32 deallid, INT32 ndealloc, int deallid_type, DISK_PAGE_TYPE page_type);
static INT32 disk_id_get_max_contiguous (THREAD_ENTRY * thread_p, INT16 volid,
                                         INT32 at_pg1, INT32 low_allid, INT32 high_allid, INT32 nunits_quit);
#if defined (ENABLE_UNUSED_FUNCTION)
...
static int disk_dump_goodvol_system (THREAD_ENTRY * thread_p, FILE * fp,
                                     INT16 volid, INT32 fs_sectid, INT32 ls_sectid, INT32 fs_pageid, INT32 ls_pageid);
static bool disk_dump_goodvol_all (THREAD_ENTRY * thread_p, INT16 volid, void *arg);
 ...
 */
static bool
disk_cache_goodvol_refresh_onevol (THREAD_ENTRY * thread_p, INT16 volid, UNUSED_ARG void *ignore)
{
  void *ptr;
  DISK_VOLPURPOSE vol_purpose;
  ...
  if ((disk_Cache->nvols + 1) > disk_Cache->max_nvols)
    {
      ...
      if (ptr == NULL)
        {
          er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_OUT_OF_VIRTUAL_MEMORY, 1, sizeof (DISK_VOLFREEPGS) * vol_index);
          return false;
        }
...
disk_goodvol_refresh (THREAD_ENTRY * thread_p, int hint_max_nvols)
{
  ...
  if (disk_Cache->vols == NULL || hint_max_nvols < disk_Cache->max_nvols)
    {
      ptr = realloc (disk_Cache->vols, sizeof (DISK_VOLFREEPGS) * hint_max_nvols);
      if (ptr == NULL)
        {
  ...
  for (vol_purpose = DISK_PERMVOL_DATA_PURPOSE;
       vol_purpose < DISK_UNKNOWN_PURPOSE; vol_purpose = (DB_VOLPURPOSE) (vol_purpose + 1))
    {
...
//  assert (pgbuf_get_num_hold_cnt (thread_p, PAGE_VOLHEADER) == 0);
  ...
  if (csect_enter_as_reader (thread_p, CSECT_DISK_REFRESH_GOODVOL, INF_WAIT) == NO_ERROR)
    {
 ...
 */
int
disk_cache_set_auto_extend_volid (UNUSED_ARG THREAD_ENTRY * thread_p, VOLID volid)
{
  assert (pgbuf_get_num_hold_cnt (thread_p, PAGE_VOLHEADER) == 0);
...
disk_cache_goodvol_update (UNUSED_ARG THREAD_ENTRY * thread_p, INT16 volid,
                           DISK_VOLPURPOSE vol_purpose,
                           INT32 nfree_pages_toadd, bool do_update_total, UNUSED_ARG bool * need_to_add_generic_volume)
{
  int start_at = -1;
  int end_at = -1;
  int i;
...
//  assert (pgbuf_get_num_hold_cnt (thread_p, PAGE_VOLHEADER) == 0);
  ...
  if (nfree_pages_toadd < 0 && need_to_add_generic_volume
      ...
      && (vol_purpose == DISK_PERMVOL_DATA_PURPOSE
          || vol_purpose == DISK_PERMVOL_INDEX_PURPOSE || vol_purpose == DISK_PERMVOL_GENERIC_PURPOSE))
    {
  ...
  if (need_to_check_auto_volume_ext == true)
    {
      ...
      start_at = (disk_Cache->purpose[DISK_PERMVOL_DATA_PURPOSE].nvols +
                  disk_Cache->purpose[DISK_PERMVOL_INDEX_PURPOSE].nvols);
      end_at = start_at + disk_Cache->purpose[DISK_PERMVOL_GENERIC_PURPOSE].nvols;
...
#if defined (SERVER_MODE)
  if (need_to_check_auto_volume_ext == true
      && total_free_pages < (int) (prm_get_bigint_value (PRM_ID_GENERIC_VOL_PREALLOC_SIZE) / IO_PAGESIZE))
    {
...
disk_probe_disk_cache_to_find_desirable_vol (THREAD_ENTRY * thread_p,
                                             INT16 * best_volid,
                                             INT32 * best_numpages,
                                             INT16 undesirable_volid, INT32 exp_numpages, int start_at, int num_vols)
{
  int i;
  ...
  for (i = start_at; i < num_vols + start_at; i++)
    {
      if (disk_Cache->vols[i].hint_freepages >= exp_numpages
          && disk_Cache->vols[i].hint_freepages > *best_numpages && undesirable_volid != disk_Cache->vols[i].volid)
        {
          ...
          if (exp_numpages == 1
              || disk_get_hint_contiguous_free_numpages (thread_p,
                                                         disk_Cache->vols[i].volid, exp_numpages,
                                                         &disk_Cache->vols[i].hint_freepages) == true)
            {
              /*
               ...
               * volume
               */
              if (contiguous_best_volid == NULL_VOLID || disk_Cache->vols[i].hint_freepages > *best_numpages)
                {
      ...
      else
        {
          ...
          if (contiguous_best_volid == NULL_VOLID
              && undesirable_volid != disk_Cache->vols[i].volid && disk_Cache->vols[i].hint_freepages > *best_numpages)
            {
              *best_numpages = disk_Cache->vols[i].hint_freepages;
              *best_volid = disk_Cache->vols[i].volid;
            }
...
retry:
  ...
  volid = disk_find_goodvol_from_disk_cache (thread_p, hint_volid,
                                             undesirable_volid, exp_numpages, setpage_type, vol_purpose);
...
//  assert (pgbuf_get_num_hold_cnt (thread_p, PAGE_VOLHEADER) == 0);
  ...
  if (disk_Cache->max_nvols <= 0 && disk_goodvol_refresh (thread_p, -1) == false)
    {
      return NULL_VOLID;
    }
  ...
  switch (vol_purpose)
    ...
    case DISK_PERMVOL_DATA_PURPOSE:
      ...
      if (num_data_vols > 0
          && disk_probe_disk_cache_to_find_desirable_vol (thread_p,
                                                          ...
                                                          &best_numpages,
                                                          undesirable_volid,
                                                          exp_numpages, 0, num_data_vols) != NULL_VOLID)
        {
          break;
        }

      num_generic_vols = disk_Cache->purpose[DISK_PERMVOL_GENERIC_PURPOSE].nvols;
      ...
      if (num_generic_vols > 0
          && disk_probe_disk_cache_to_find_desirable_vol (thread_p,
                                                          ...
                                                          exp_numpages,
                                                          (num_data_vols +
                                                           num_index_vols), num_generic_vols) != NULL_VOLID)
        {
          break;
        }
    ...
    case DISK_PERMVOL_INDEX_PURPOSE:
      ...
      if (num_index_vols > 0
          && disk_probe_disk_cache_to_find_desirable_vol (thread_p,
                                                          ...
                                                          &best_numpages,
                                                          undesirable_volid,
                                                          exp_numpages, num_data_vols, num_index_vols) != NULL_VOLID)
        {
          break;
        }
    ...
    case DISK_TEMPVOL_TEMP_PURPOSE:
      if (hint_volid >= disk_Cache->nvols
          && disk_get_hint_contiguous_free_numpages (thread_p, hint_volid, exp_numpages, &best_numpages) == true)
        {
      ...
      if (num_ttemp_vols
          && disk_probe_disk_cache_to_find_desirable_vol (thread_p,
                                                          ...
                                                          (num_data_vols +
                                                           num_index_vols +
                                                           num_generic_vols +
                                                           num_ptemp_vols), num_ttemp_vols) != NULL_VOLID)
        {
          break;
        }
    ...
    case DISK_PERMVOL_TEMP_PURPOSE:
      ...
      if (num_ptemp_vols > 0
          && disk_probe_disk_cache_to_find_desirable_vol (thread_p,
                                                          ...
                                                          (num_data_vols +
                                                           num_index_vols +
                                                           num_generic_vols), num_ptemp_vols) != NULL_VOLID)
        {
          break;
        }
...
disk_cache_get_purpose_info (UNUSED_ARG THREAD_ENTRY * thread_p,
                             DISK_VOLPURPOSE vol_purpose, INT16 * nvols, int *total_pages, int *free_pages)
{
//  assert (pgbuf_get_num_hold_cnt (thread_p, PAGE_VOLHEADER) == 0);
  ...
  if (vol_purpose != DISK_UNKNOWN_PURPOSE)
  ...
  else
    {
      ...
      *free_pages = 0;
      ...
      for (vol_purpose = DISK_PERMVOL_DATA_PURPOSE;
           vol_purpose < DISK_UNKNOWN_PURPOSE; vol_purpose = (DISK_VOLPURPOSE) (vol_purpose + 1))
        {
          *nvols += disk_Cache->purpose[vol_purpose].nvols;
          *total_pages += disk_Cache->purpose[vol_purpose].total_pages;
          *free_pages += disk_Cache->purpose[vol_purpose].free_pages;
 ...
 */
INT32
disk_get_max_numpages (UNUSED_ARG THREAD_ENTRY * thread_p, DISK_VOLPURPOSE vol_purpose)
{
  INT32 maxpgs = 0;
  INT32 (*fun) (void);
...
disk_format (THREAD_ENTRY * thread_p, const char *dbname, INT16 volid, DBDEF_VOL_EXT_INFO * ext_info)
{
  ...
  if (DB_PAGESIZE < sizeof (DISK_VAR_HEADER))
    {
      er_log_debug (ARG_FILE_LINE,
                    ...
                    " is set to %d", sizeof (DISK_VAR_HEADER), sizeof (INT32), DB_PAGESIZE);
#if defined(NDEBUG)
      exit (EXIT_FAILURE);
#else /* NDEBUG */
      /* debugging purpose */
  ...
  if (vol_purpose > DISK_TEMPVOL_TEMP_PURPOSE)
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_DISK_UNKNOWN_PURPOSE, 3,
              vol_purpose, DISK_PERMVOL_DATA_PURPOSE, DISK_TEMPVOL_TEMP_PURPOSE);
      return NULL_VOLID;
    }
  ...
  if (vol_purpose == DISK_TEMPVOL_TEMP_PURPOSE)
    {
      vdes = fileio_format (thread_p, dbname, vol_fullname, volid,
                            max_npages, false, false, false, IO_PAGESIZE, kbytes_to_be_written_per_sec, false);
    }
  else
    {
      vdes = fileio_format (thread_p, dbname, vol_fullname, volid,
                            extend_npages, true, false, false, IO_PAGESIZE, kbytes_to_be_written_per_sec, false);
    }
  ...
  if (disk_set_alloctables (vol_purpose,
                            ...
                            &vhdr->sect_alloctb_page1, &vhdr->page_alloctb_page1, &vhdr->sys_lastpage) != NO_ERROR)
    {
  ...
  if (vol_purpose != DISK_TEMPVOL_TEMP_PURPOSE
      && vhdr->total_sects > DISK_HINT_START_SECT && (vhdr->total_sects - vhdr->free_sects) < DISK_HINT_START_SECT)
    {
  ...
  if (vol_purpose == DISK_TEMPVOL_TEMP_PURPOSE)
    {
      fileio_make_volume_temp_name (temp_vol_fullname, temp_path, temp_name, vhdr->volid);
    }
  else
    {
      fileio_make_volume_ext_name (temp_vol_fullname, temp_path, temp_name, vhdr->volid);
    }
  ...
  if (disk_map_init (thread_p, volid, vhdr->sect_alloctb_page1,
      ...
      || (vol_purpose != DISK_TEMPVOL_TEMP_PURPOSE && volid > 0 && disk_set_link (thread_p, volid - 1) != NO_ERROR))
    {
          ...
          for (vpid.volid = volid, vpid.pageid = DISK_VOLHEADER_PAGE; vpid.pageid <= vhdr->sys_lastpage; vpid.pageid++)
            {
              pgptr =
                pgbuf_fix (thread_p, &vpid, OLD_PAGE, PGBUF_LATCH_WRITE,
                           ...
                           (vpid.pageid == DISK_VOLHEADER_PAGE) ? PAGE_VOLHEADER : PAGE_UNKNOWN);
              if (pgptr != NULL)
                {
...
disk_expand_tmp (THREAD_ENTRY * thread_p, INT16 volid, INT32 min_pages, INT32 max_pages)
{
  ...
  hdr_pgptr = pgbuf_fix (thread_p, &vpid, OLD_PAGE, PGBUF_LATCH_WRITE, PGBUF_UNCONDITIONAL_LATCH, PAGE_VOLHEADER);
  if (hdr_pgptr == NULL)
    {
      return -1;
    }
  ...
  if (vhdr->purpose != DISK_TEMPVOL_TEMP_PURPOSE)
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE,
              ER_DISK_CANNOT_EXPAND_PERMVOLS, 2, fileio_get_volume_label (volid, PEEK), vhdr->purpose);
      goto error;
    }
...
#if defined(SERVER_MODE)
  mnt_stats_counter_with_time (thread_p, MNT_STATS_DISK_TEMP_EXPAND, npages_toadd, perf_start);
#endif /* SERVER_MODE */
...
disk_expand_perm (THREAD_ENTRY * thread_p, INT16 volid, INT32 npages)
{
  ...
  /* Update total_pages and free_pages on disk_Cache. */
  disk_cache_goodvol_update (thread_p, volid, save_purpose, npages, true, NULL);
...
disk_reinit (THREAD_ENTRY * thread_p, INT16 volid, UNUSED_ARG void *ignore)
{
  ...
  pgptr = pgbuf_fix (thread_p, &vpid, OLD_PAGE, PGBUF_LATCH_WRITE, PGBUF_UNCONDITIONAL_LATCH, PAGE_VOLHEADER);
  if (pgptr == NULL)
    {
      return true;
    }
  ...
  if (disk_map_init (thread_p, volid, vhdr->sect_alloctb_page1,
      ...
      || disk_map_init (thread_p, volid, vhdr->page_alloctb_page1,
                        (vhdr->page_alloctb_page1 +
                         vhdr->page_alloctb_npages - 1), vhdr->sys_lastpage + 1, vhdr->purpose) != NO_ERROR)
    {
      DISK_VERIFY_VAR_HEADER (vhdr);
...
disk_set_creation (THREAD_ENTRY * thread_p, INT16 volid,
                   ...
                   const INT64 * new_dbcreation, const LOG_LSA * new_chkptlsa, bool logchange, DISK_FLUSH_TYPE flush)
{
  DISK_VAR_HEADER *vhdr;
  LOG_DATA_ADDR addr = LOG_ADDR_INITIALIZER;
  VPID vpid;
...
disk_set_link (THREAD_ENTRY * thread_p, INT16 volid)
{
  ...
  addr.pgptr = pgbuf_fix (thread_p, &vpid, OLD_PAGE, PGBUF_LATCH_WRITE, PGBUF_UNCONDITIONAL_LATCH, PAGE_VOLHEADER);
  if (addr.pgptr == NULL)
    {
      return ER_FAILED;
    }
  ...
  assert (vhdr->next_volid == vhdr->volid + 1 || vhdr->next_volid == NULL_VOLDES);

  next_volid = vhdr->volid + 1;
  assert (next_volid > 0);

  log_append_undoredo_data (thread_p, RVDK_LINK_PERM_VOLEXT, &addr,
                            sizeof (INT16), sizeof (INT16), &(vhdr->next_volid), &next_volid);
...
disk_get_boot_hfid (THREAD_ENTRY * thread_p, HFID * hfid)
{
  ...
  pgptr = pgbuf_fix (thread_p, &vpid, OLD_PAGE, PGBUF_LATCH_READ, PGBUF_UNCONDITIONAL_LATCH, PAGE_VOLHEADER);
  if (pgptr == NULL)
    {
      return NULL;
    }
 ...
 */
char *
disk_get_link (THREAD_ENTRY * thread_p, const char *db_fullname, INT16 volid, char *next_volext_fullname)
{
  DISK_VAR_HEADER *vhdr;
  PAGE_PTR pgptr = NULL;
  VPID vpid;
  ...
  if (vhdr->next_volid != NULL_VOLDES)
    {
      fileio_make_volume_ext_name (next_volext_fullname, temp_path, temp_name, vhdr->next_volid);
    }
  else
    {
...
disk_map_init (THREAD_ENTRY * thread_p, INT16 volid, INT32 at_fpageid,
               INT32 at_lpageid, INT32 nalloc_bits, DISK_VOLPURPOSE vol_purpose)
{
  ...
  for (vpid.pageid = at_fpageid; vpid.pageid <= at_lpageid; vpid.pageid++)
    {
      ...
      log_append_redo_data (thread_p, RVDK_INITMAP, &addr, sizeof (nalloc_bits), &nalloc_bits);
      pgbuf_set_dirty (thread_p, addr.pgptr, FREE);
      addr.pgptr = NULL;
    }
...
disk_get_checkpoint (THREAD_ENTRY * thread_p, INT16 volid, LOG_LSA * vol_lsa)
{
   ...
   * that this is the only page among the volume system pages that is locked.
   */
  hdr_pgptr = pgbuf_fix (thread_p, &vpid, OLD_PAGE, PGBUF_LATCH_READ, PGBUF_UNCONDITIONAL_LATCH, PAGE_VOLHEADER);
  if (hdr_pgptr == NULL)
    {
      return ER_FAILED;
    }
...
xdisk_get_purpose_and_sys_lastpage (THREAD_ENTRY * thread_p, INT16 volid,
                                    DISK_VOLPURPOSE * vol_purpose, INT32 * sys_lastpage)
{
  DISK_VAR_HEADER *vhdr;
  PAGE_PTR hdr_pgptr = NULL;
  VPID vpid;
 ...
 */
char *
xdisk_get_fullname (THREAD_ENTRY * thread_p, const char *database_name, INT16 volid, char *vol_fullname)
{
  DISK_VAR_HEADER *vhdr;
  PAGE_PTR hdr_pgptr = NULL;
  VPID vpid;
...
disk_alloc_sector (THREAD_ENTRY * thread_p, INT16 volid, INT32 nsects, int exp_npages)
{
  ...
  alloc_sect = disk_id_alloc (thread_p, volid, vhdr, nsects,
                              vhdr->hint_allocsect, vhdr->total_sects - 1, DISK_SECTOR, exp_npages, NULL_PAGEID);
  if (alloc_sect == NULL_SECTID)
    {
      alloc_sect = disk_id_alloc (thread_p, volid, vhdr, nsects,
                                  1, vhdr->hint_allocsect - 1, DISK_SECTOR, exp_npages, NULL_PAGEID);
    }
  ...
  mnt_stats_counter_with_time (thread_p, MNT_STATS_DISK_SECTOR_ALLOCS, 1, perf_start);
...
disk_alloc_page (THREAD_ENTRY * thread_p, INT16 volid, INT32 sectid,
                 INT32 npages, INT32 near_pageid, DISK_PAGE_TYPE alloc_page_type)
{
  DISK_VAR_HEADER *vhdr;
  INT32 fpageid;
  INT32 lpageid;
  ...
  if (npages <= 0)
    {
      er_log_debug (ARG_FILE_LINE,
                    "disk_alloc_page: ** SYSTEM_ERROR.. Bad interface" " trying to allocate %d pages", npages);
  ...
  if (new_pageid == NULL_PAGEID)
  ...
  else
    {
      ...
      if (vhdr->purpose == DISK_PERMVOL_GENERIC_PURPOSE)
        {
          if (alloc_page_type == DISK_PAGE_DATA_TYPE)
            ...
            {
              assert_release (alloc_page_type == DISK_PAGE_DATA_TYPE || alloc_page_type == DISK_PAGE_INDEX_TYPE);
            }
        }
      ...
      disk_cache_goodvol_update (thread_p, volid, vol_purpose, -npages, false, &need_to_add_generic_volume);

      if (need_to_add_generic_volume)
        {
          (void) boot_add_auto_volume_extension (thread_p, 1,
                                                 DISK_CONTIGUOUS_PAGES, DISK_PERMVOL_GENERIC_PURPOSE, false);
        }
    }
  ...
  mnt_stats_counter_with_time (thread_p, MNT_STATS_DISK_PAGE_ALLOCS, 1, perf_start);
...
disk_id_alloc (THREAD_ENTRY * thread_p, INT16 volid, DISK_VAR_HEADER * vhdr,
               INT32 nalloc, INT32 low_allid, INT32 high_allid, int allid_type, int exp_npages, int skip_pageid)
{
  int i;
  ...
  for (vpid.pageid = (low_allid / DISK_PAGE_BIT) + at_pg1; nfound < nalloc && low_allid <= high_allid; vpid.pageid++)
    {
      addr.pgptr = pgbuf_fix (thread_p, &vpid, OLD_PAGE, PGBUF_LATCH_READ, PGBUF_UNCONDITIONAL_LATCH, PAGE_VOLBITMAP);
      if (addr.pgptr == NULL)
        {
          nfound = 0;
          break;
              ...
              if (allid_type == DISK_SECTOR && nalloc == 1 && nfound == 1
                  && exp_npages > 0 && allid > DISK_SECTOR_WITH_ALL_PAGES)
                {
                  ...
                  if (disk_check_sector_has_npages (thread_p, volid,
                                                    vhdr->page_alloctb_page1, fpageid, lpageid, exp_npages) == false)
                    {
                      nfound = 0;
                      allid = NULL_PAGEID;
                    }
      ...
      for (vpid.pageid = (low_allid / DISK_PAGE_BIT) + at_pg1; low_allid < allid + nalloc; vpid.pageid++)
        {
          addr.pgptr = pgbuf_fix (thread_p, &vpid, OLD_PAGE,
                                  PGBUF_LATCH_WRITE, PGBUF_UNCONDITIONAL_LATCH, PAGE_VOLBITMAP);
          if (addr.pgptr == NULL)
            {
              allid = NULL_PAGEID;
              break;
...
disk_check_sector_has_npages (THREAD_ENTRY * thread_p, INT16 volid,
                              INT32 at_pg1, INT32 low_allid, INT32 high_allid, int exp_npages)
{
  int i;
      ...
      for (at_chptr = (unsigned char *) addr.pgptr + addr.offset;
           nfound < exp_npages && low_allid <= high_allid && at_chptr < out_chptr; at_chptr++)
        {
          ...
          for (i = low_allid % CHAR_BIT;
               i < CHAR_BIT && nfound < exp_npages && low_allid <= high_allid; i++, low_allid++)
            {
              if (!disk_bit_is_set (at_chptr, i))
                {
                  nfound++;
...
disk_dealloc_sector (THREAD_ENTRY * thread_p, INT16 volid, INT32 sectid, INT32 nsects)
{
  ...
  while ((addr.pgptr = pgbuf_fix (thread_p, &vpid, OLD_PAGE,
                                  PGBUF_LATCH_WRITE, PGBUF_UNCONDITIONAL_LATCH, PAGE_VOLHEADER)) == NULL)
    {
      switch (er_errid ())
        {
        case NO_ERROR:
  ...
  mnt_stats_counter_with_time (thread_p, MNT_STATS_DISK_SECTOR_DEALLOCS, 1, perf_start);
...
disk_dealloc_page (THREAD_ENTRY * thread_p, INT16 volid, INT32 pageid, INT32 npages, DISK_PAGE_TYPE page_type)
{
  ...
  if (pageid <= vhdr->sys_lastpage && pageid >= DISK_VOLHEADER_PAGE)
    {
      ...
      pgbuf_unfix_and_init (thread_p, addr.pgptr);
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE,
              ER_DISK_TRY_DEALLOC_DISK_SYSPAGE, 2, pageid, fileio_get_volume_label (volid, PEEK));
      return ER_FAILED;
    }
  ...
  mnt_stats_counter_with_time (thread_p, MNT_STATS_DISK_PAGE_DEALLOCS, 1, perf_start);
...
disk_id_dealloc (THREAD_ENTRY * thread_p, INT16 volid, INT32 at_pg1,
                 INT32 deallid, INT32 ndealloc, int deallid_type, DISK_PAGE_TYPE page_type)
{
  int i;
  ...
  for (vpid.pageid = (deallid / DISK_PAGE_BIT) + at_pg1; ndealloc > 0; vpid.pageid++)
    {
      ...
      retry = 0;
      while ((addr.pgptr = pgbuf_fix (thread_p, &vpid, OLD_PAGE,
                                      PGBUF_LATCH_WRITE, PGBUF_UNCONDITIONAL_LATCH, PAGE_VOLBITMAP)) == NULL)
        {
          switch (er_errid ())
            {
            case NO_ERROR:
      ...
      /* One byte at a time */
      for (at_chptr = (unsigned char *) addr.pgptr + addr.offset,
           out_chptr = (unsigned char *) addr.pgptr + DB_PAGESIZE; ndealloc > 0 && at_chptr < out_chptr; at_chptr++)
        {
                  ...
                  if (deallid_type == DISK_SECTOR)
                    {
                      er_set (ER_WARNING_SEVERITY, ARG_FILE_LINE,
                              ER_DISK_UNKNOWN_SECTOR, 2, deallid, fileio_get_volume_label (volid, PEEK));
                    }
                  else
                    {
                      er_set (ER_WARNING_SEVERITY, ARG_FILE_LINE,
                              ER_DISK_UNKNOWN_PAGE, 2, deallid, fileio_get_volume_label (volid, PEEK));
                    }
...
disk_get_maxcontiguous_numpages (THREAD_ENTRY * thread_p, INT16 volid, INT32 max_npages)
{
  ...
  npages = disk_id_get_max_contiguous (thread_p, volid,
                                       vhdr->page_alloctb_page1,
                                       vhdr->sys_lastpage + 1, vhdr->total_pages - 1, max_npages);
end:
  ...
  if (arecontiguous_npages > 1)
    {
      npages = disk_id_get_max_contiguous (thread_p, volid,
                                           vhdr->page_alloctb_page1,
                                           vhdr->sys_lastpage + 1, vhdr->total_pages - 1, arecontiguous_npages);
    }
  else
    {
      npages = *num_freepgs;
                            ...
                            INT32 at_pg1, INT32 low_allid, INT32 high_allid, INT32 nunits_quit)
{
  ...
  for (vpid.pageid = (low_allid / DISK_PAGE_BIT) + at_pg1;
    ...
    {
      pgptr = pgbuf_fix (thread_p, &vpid, OLD_PAGE, PGBUF_LATCH_READ, PGBUF_UNCONDITIONAL_LATCH, PAGE_VOLBITMAP);
      if (pgptr == NULL)
        {
          nfound = 0;
          break;
      ...
      for (at_chptr = (unsigned char *) pgptr + offset;
           (nfound < nunits_quit && low_allid <= high_allid && at_chptr < out_chptr); at_chptr++)
        {
          /* One bit at a time */
          for (i = low_allid % CHAR_BIT;
               (i < CHAR_BIT && nfound < nunits_quit && low_allid <= high_allid); i++, low_allid++)
            {
              if (!disk_bit_is_set (at_chptr, i))
                {
                  nfound++;
...
disk_isvalid_page (THREAD_ENTRY * thread_p, INT16 volid, INT32 pageid)
{
  ...
  valid = ((pageid <= vhdr->sys_lastpage)
           ...
           : ((pageid > vhdr->total_pages)
              ? DISK_INVALID : disk_id_isvalid (thread_p, volid, vhdr->page_alloctb_page1, pageid)));
...
disk_id_isvalid (THREAD_ENTRY * thread_p, INT16 volid, INT32 at_pg1, INT32 allid)
{
  ...
  at_pgptr = pgbuf_fix (thread_p, &vpid, OLD_PAGE, PGBUF_LATCH_READ, PGBUF_UNCONDITIONAL_LATCH, PAGE_VOLBITMAP);
  if (at_pgptr != NULL)
    {
...
disk_check (THREAD_ENTRY * thread_p, INT16 volid, bool repair)
{
  ...
  if (nfree != vhdr->free_pages)
    {
      if (nfree == -1)
        ...
        {
          er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE,
                  ER_DISK_INCONSISTENT_NFREE_PAGES, 3, fileio_get_volume_label (volid, PEEK), vhdr->free_pages, nfree);
          valid = DISK_INVALID;
        }
    }
  ...
  if (nfree != vhdr->free_sects)
    {
      if (nfree == -1)
        ...
        {
          er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE,
                  ER_DISK_INCONSISTENT_NFREE_SECTS, 3, fileio_get_volume_label (volid, PEEK), vhdr->free_sects, nfree);
          valid = DISK_INVALID;
        }
    }
  ...
  if (vhdr->sect_npgs != DISK_SECTOR_NPAGES
      ...
      || vhdr->sys_lastpage != (vhdr->page_alloctb_page1 + vhdr->page_alloctb_npages - 1))
    {
      ...
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE,
              ER_DISK_INCONSISTENT_VOL_HEADER, 1, fileio_get_volume_label (volid, PEEK));
      valid = DISK_INVALID;
    }
  else
    {
...
disk_dump_goodvol_system (THREAD_ENTRY * thread_p, FILE * fp, INT16 volid,
                          INT32 fs_sectid, INT32 ls_sectid, INT32 fs_pageid, INT32 ls_pageid)
{
  DISK_VAR_HEADER *vhdr;
  PAGE_PTR hdr_pgptr = NULL;
  VPID vpid;
  ...
  if (disk_map_dump (thread_p, fp, &vpid, "SECTOR ID",
    ...
    {
      (void) fprintf (fp, "Problems dumping sector table of volume = %d\n", vhdr->volid);
    }
  else
    {
      ...
      if (disk_map_dump (thread_p, fp, &vpid, "PAGE ID",
        ...
        {
          (void) fprintf (fp, "Problems dumping page table of volume = %d\n", vhdr->volid);
        }
    }
...
disk_volume_header_next_scan (THREAD_ENTRY * thread_p, int cursor, DB_VALUE ** out_values, int out_cnt, void *ptr)
{
  ...
  snprintf (buf, sizeof (buf), "MAGIC SYMBOL = %s at disk location = %lld",
            vhdr->magic, offsetof (FILEIO_PAGE, page) + (long long) offsetof (DISK_VAR_HEADER, magic));
...
disk_vhdr_dump (FILE * fp, const DISK_VAR_HEADER * vhdr)
{
  ...
  (void) fprintf (fp,
                  ...
                  time_val, (long long int) vhdr->chkpt_lsa.pageid, (int) vhdr->chkpt_lsa.offset);
  (void) fprintf (fp, "Boot_hfid: volid %d, fileid %d header_pageid %d\n",
                  vhdr->boot_hfid.vfid.volid, vhdr->boot_hfid.vfid.fileid, vhdr->boot_hfid.hpgid);
               ...
               const char *at_name, INT32 at_fpageid, INT32 at_lpageid, INT32 all_fid, INT32 all_lid)
{
  ...
  for (vpid->pageid = at_fpageid; vpid->pageid <= at_lpageid; vpid->pageid++)
    {
      at_pgptr = pgbuf_fix (thread_p, vpid, OLD_PAGE, PGBUF_LATCH_WRITE, PGBUF_UNCONDITIONAL_LATCH, PAGE_VOLBITMAP);
      if (at_pgptr == NULL)
        {
          return ER_FAILED;
        }
...
disk_dump_all (THREAD_ENTRY * thread_p, FILE * fp)
{
  ...
  if (fileio_map_mounted (thread_p, disk_dump_goodvol_all, (void *) fp) == true)
    {
      ret = NO_ERROR;
    }
  else
...
disk_dump_goodvol_all (THREAD_ENTRY * thread_p, INT16 volid, void *arg)
{
  ...
  ret = disk_dump_goodvol_system (thread_p, out_fp, volid, NULL_PAGEID, NULL_PAGEID, NULL_PAGEID, NULL_PAGEID);
  if (ret != NO_ERROR)
    {
      ...
      if (vhdr->purpose == DISK_TEMPVOL_TEMP_PURPOSE)
        {
          (void) fileio_format (thread_p, NULL, vol_label,
                                vhdr->volid, vhdr->total_pages, false, false, false, IO_PAGESIZE, 0, false);
        }
      else
        {
          (void) fileio_format (thread_p, NULL, vol_label,
                                vhdr->volid, vhdr->total_pages, true, false, false, IO_PAGESIZE, 0, false);
        }
 ...
 */
void
disk_vhdr_rv_dump_free_sectors (FILE * fp, UNUSED_ARG int length_ignore, void *data)
{
...
disk_vhdr_rv_undoredo_free_pages (THREAD_ENTRY * thread_p, LOG_RCV * rcv)
{
  ...
  if (vhdr->purpose == DISK_PERMVOL_DATA_PURPOSE)
  ...
  else if (vhdr->purpose == DISK_PERMVOL_GENERIC_PURPOSE)
    {
      if (mtb->page_type == DISK_PAGE_DATA_TYPE)
        ...
        {
          assert_release (mtb->page_type == DISK_PAGE_DATA_TYPE || mtb->page_type == DISK_PAGE_INDEX_TYPE);
        }
    }
  else
    {
  ...
  disk_cache_goodvol_update (thread_p, vhdr->volid, vhdr->purpose, mtb->num, false, NULL);
 ...
 */
void
disk_vhdr_rv_dump_free_pages (FILE * fp, UNUSED_ARG int length_ignore, void *data)
{
  DISK_RECV_MTAB_BITS_WITH *mtb;

  mtb = (DISK_RECV_MTAB_BITS_WITH *) data;

  fprintf (fp, "num_pages = %d, page_type = %s\n", mtb->num, disk_page_type_to_string (mtb->page_type));
}
...
disk_rv_alloctable_helper (THREAD_ENTRY * thread_p, LOG_RCV * rcv, DISK_ALLOCTABLE_MODE mode)
{
  ...
  for (at_chptr = (unsigned char *) rcv->pgptr + rcv->offset; num < mtb->num; at_chptr++)
    {
...
disk_rv_alloctable_vhdr_only (THREAD_ENTRY * thread_p, LOG_RCV * rcv,
                              DISK_ALLOCTABLE_MODE mode, bool update_cache, INT32 * delta_out)
{
  const DISK_RECV_MTAB_BITS_WITH *mtb;  /* Recovery structure of bits */
  DISK_VAR_HEADER *vhdr;
  INT32 delta = 0;
  ...
  if (mtb->deallid_type == DISK_SECTOR)
  ...
  else
    {
      ...
      if (vhdr->purpose == DISK_PERMVOL_DATA_PURPOSE)
        ...
        {
          assert (mtb->page_type != DISK_PAGE_DATA_TYPE && mtb->page_type != DISK_PAGE_INDEX_TYPE);
        }

      if (update_cache == true)
        {
          disk_cache_goodvol_update (thread_p, vhdr->volid, vhdr->purpose, delta, false, NULL);
        }
    }
...
disk_rv_alloctable_with_volheader (THREAD_ENTRY * thread_p, LOG_RCV * rcv, LOG_LSA * ref_lsa)
{
  ...
  vhdr_rcv.pgptr = pgbuf_fix_with_retry (thread_p, &vhdr_vpid, OLD_PAGE, PGBUF_LATCH_WRITE, 10, PAGE_VOLHEADER);
  ...
  rcv->pgptr = pgbuf_fix_with_retry (thread_p, &page_vpid, OLD_PAGE, PGBUF_LATCH_WRITE, 10, PAGE_VOLBITMAP);
  if (rcv->pgptr == NULL)
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_LOG_MAYNEED_MEDIA_RECOVERY,
  ...
  disk_rv_alloctable_vhdr_only (thread_p, &vhdr_rcv, DISK_ALLOCTABLE_CLEAR, false, &delta);
  ...
  if (ref_lsa != NULL)
    {
      ...
      log_append_run_postpone (thread_p,
                               RVDK_IDDEALLOC_BITMAP_ONLY, &page_addr, &page_vpid, rcv->length, rcv->data, ref_lsa);

      vhdr_addr.offset = 0;
      vhdr_addr.pgptr = vhdr_rcv.pgptr;
      log_append_run_postpone (thread_p,
                               RVDK_IDDEALLOC_VHDR_ONLY, &vhdr_addr, &vhdr_vpid, rcv->length, rcv->data, ref_lsa);
    }
  ...
  if (delta != 0)
    {
      disk_cache_goodvol_update (thread_p, vhdr_vpid.volid, purpose, delta, false, NULL);
    }
...
{
  return disk_rv_alloctable_vhdr_only (thread_p, rcv, DISK_ALLOCTABLE_SET, true, NULL);
}
...
{
  return disk_rv_alloctable_vhdr_only (thread_p, rcv, DISK_ALLOCTABLE_CLEAR, true, NULL);
}
 ...
 */
void
disk_rv_dump_alloctable_with_vhdr (FILE * fp, UNUSED_ARG int length_ignore, void *data)
{
  DISK_RECV_MTAB_BITS_WITH *mtb;        /* Recovery structure of bits */

  mtb = (DISK_RECV_MTAB_BITS_WITH *) data;
  fprintf (fp, "Start_bit = %u, Num_bits = %d, Deallocation_type = %d\n", mtb->start_bit, mtb->num, mtb->deallid_type);
}
...
disk_rv_dump_set_creation_time (FILE * fp, int length_ignore, void *data)
{
  ...
  fprintf (fp, "Label = %s, Db_creation = %lld, chkpt = %lld|%d\n",
           ...
           (long long int) change->chkpt_lsa.pageid, (int) change->chkpt_lsa.offset);
}
#endif
...
disk_rv_redo_dboutside_init_pages (THREAD_ENTRY * thread_p, LOG_RCV * rcv)
{
  ...
  if (fileio_initialize_pages (thread_p, vol_fd, io_page_p, info->start_pageid, info->npages, IO_PAGESIZE, -1) == NULL)
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_LOG_MAYNEED_MEDIA_RECOVERY,
              1, fileio_get_volume_label (volid, PEEK));
...
disk_get_first_total_free_numpages (THREAD_ENTRY * thread_p,
                                    DISK_VOLPURPOSE purpose, INT32 * ntotal_pages, INT32 * nfree_pages)
{
  INT16 volid;
  int nperm_vols;
  DISK_VAR_HEADER *vhdr;
...
disk_set_alloctables (DISK_VOLPURPOSE vol_purpose,
                      ...
                      INT32 * sect_alloctb_page1, INT32 * page_alloctb_page1, INT32 * sys_lastpage)
{
