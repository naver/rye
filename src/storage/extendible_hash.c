 ...
 *   ehid(in/out): identifier for the extendible hashing structure to
 ...
 *                      Undeclared if key_type is "DB_TYPE_VARCHAR".
		     ...
		     bool is_tmp)
{
  ...
  assert (key_type == DB_TYPE_VARCHAR || key_type == DB_TYPE_OBJECT);
  ...
  dir_page_p = pgbuf_fix2 (thread_p, &dir_vpid, NEW_PAGE, PGBUF_LATCH_WRITE,
			   PGBUF_UNCONDITIONAL_LATCH,
			   MNT_STATS_DATA_PAGE_FETCHES_EHASH);
  if (dir_page_p == NULL)
    {
		    ...
		    const VPID * vpid_p, int latch_mode)
{
  ...
  page_p = pgbuf_fix2 (thread_p, vpid_p, OLD_PAGE, latch_mode,
		       PGBUF_UNCONDITIONAL_LATCH,
		       MNT_STATS_DATA_PAGE_FETCHES_EHASH);
  if (page_p == NULL)
    {
		     ...
		     OID * value_p, int lock_type, VPID * existing_ovf_vpid_p)
{
  ...
  if (dir_header_p->key_type == DB_TYPE_VARCHAR)
    {
			...
			OID * value_p, UNUSED_ARG VPID * existing_ovf_vpid_p)
{
  ...
  RECDES bucket_recdes = RECDES_INITIALIZER;
  RECDES old_bucket_recdes = RECDES_INITIALIZER;
  RECDES log_recdes = RECDES_INITIALIZER;
#if defined (ENABLE_UNUSED_FUNCTION)
  RECDES ovf_recdes = RECDES_INITIALIZER;
  VPID ovf_vpid;
  char *record_p;
...
ehash_delete (THREAD_ENTRY * thread_p, EHID * ehid_p, void *key_p)
{
  ...
  RECDES bucket_recdes = RECDES_INITIALIZER;
  RECDES log_recdes_undo = RECDES_INITIALIZER;
  RECDES log_recdes_redo = RECDES_INITIALIZER;
...
static EHASH_HASH_KEY
ehash_hash_eight_bytes_type (const char *key_p)
{
  EHASH_HASH_KEY hash_key = 0;
  unsigned int i;
  int Int;
 ...
 * folded (for DB_TYPE_VARCHAR, DB_TYPE_OBJECT and DB_TYPE_DOUBLE).
 * This function does not change the value of parameter
 * orig_key, as it might be on a bucket.
 */
static EHASH_HASH_KEY
ehash_hash (const void *original_key_p, DB_TYPE key_type)
{
  const char *key = original_key_p;
  EHASH_HASH_KEY hash_key = 0;

  switch (key_type)
    {
    case DB_TYPE_VARCHAR:
      hash_key = ehash_hash_string_type (key, original_key_p);
      break;
...
eh_count (EHID * ehid)
{
  ...
  numpages = file_get_numpages (&dir_header->bucket_file, NULL, NULL, NULL);
  num_entries = 0;
 ...
 */
void
ehash_dump (THREAD_ENTRY * thread_p, FILE * out_fp, EHID * ehid_p)
{
  EHASH_DIR_HEADER *dir_header_p;
  EHASH_DIR_RECORD *dir_record_p;
  int num_pages;
  ...
  for (i = 0; i < num_ptrs; i++)
      ...
      if (DB_PAGESIZE - dir_offset < SSIZEOF (EHASH_DIR_RECORD))
	{
      ...
      if (VPID_ISNULL (&dir_record_p->bucket_vpid))
	{
	  fprintf (out_fp, "*    Dir loc :  %d   points to bucket page  id:"
		   " NULL    *\n", dir_ptr_no);
	}
      else
	{
...
ehash_print_bucket (THREAD_ENTRY * thread_p, EHID * ehid_p, int nth_ptr)
{
  ...
  ehash_dump_bucket (bucket_page_p, stdout, dir_header_p->key_type);
 ...
 */
static void
ehash_dump_bucket (PAGE_PTR bucket_page_p, FILE * out_fp, DB_TYPE key_type)
{
  EHASH_BUCKET_HEADER *bucket_header_p;
  char *bucket_record_p;
  RECDES recdes = RECDES_INITIALIZER;
...
ehash_rv_delete_redo (THREAD_ENTRY * thread_p, LOG_RCV * recv_p)
{
  PGSLOTID slot_id;
  RECDES recdes = RECDES_INITIALIZER;
  RECDES existing_recdes = RECDES_INITIALIZER;
...
{
  OR_GET_OID (record_p, oid_p);

  return record_p + OR_OID_SIZE;
}

static char *
ehash_write_oid_to_record (char *record_p, OID * oid_p)
{
  OR_PUT_OID (record_p, oid_p);
