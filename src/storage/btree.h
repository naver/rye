...
extern int btree_get_stats (THREAD_ENTRY * thread_p, OID * class_oid, BTREE_STATS * stat_info, bool with_fullscan);

extern int btree_get_pkey_btid (THREAD_ENTRY * thread_p, OID * cls_oid, BTID * pkey_btid);
#if defined (ENABLE_UNUSED_FUNCTION)
extern DISK_ISVALID btree_check_by_class_oid (THREAD_ENTRY * thread_p, OID * cls_oid);
extern DISK_ISVALID btree_check_all (THREAD_ENTRY * thread_p);
#endif

extern PAGE_PTR btree_get_next_page (THREAD_ENTRY * thread_p, BTID_INT * btid, PAGE_PTR page_p);

extern DB_IDXKEY *btree_delete (THREAD_ENTRY * thread_p, BTID_INT * btid, DB_IDXKEY * key);
extern DB_IDXKEY *btree_insert (THREAD_ENTRY * thread_p, BTID_INT * btid, DB_IDXKEY * key);
extern int btree_update (THREAD_ENTRY * thread_p, BTID_INT * btid, DB_IDXKEY * old_key, DB_IDXKEY * new_key);
extern int btree_find_min_or_max_key (THREAD_ENTRY * thread_p,
                                      OID * class_oid, BTID * btid,
                                      const VPID * top_vpid, DB_IDXKEY * idxkey, int flag_minkey);
...
extern int btree_dump_tree (THREAD_ENTRY * thread_p, FILE * fp, BTID_INT * btid, int dump_level, bool dump_key);
extern void btree_dump_page (THREAD_ENTRY * thread_p, FILE * fp,
                             BTID_INT * btid, PAGE_PTR page_ptr, VPID * pg_vpid, int n, bool dump_key);
...
extern int btree_rv_newroot_redo_init (THREAD_ENTRY * thread_p, LOG_RCV * recv);
...
extern int btree_rv_leafrec_redo_delete (THREAD_ENTRY * thread_p, LOG_RCV * recv);
extern int btree_rv_leafrec_redo_insert_key (THREAD_ENTRY * thread_p, LOG_RCV * recv);
...
extern int btree_coerce_idxkey (DB_IDXKEY * key, OR_INDEX * indexp, int num_term, int key_minmax);
extern void btree_get_indexname_on_table (THREAD_ENTRY * thread_p,
                                          const BTID_INT * btid, char *buffer, const int buffer_len);

extern bool btree_clear_key_value (bool * clear_flag, DB_IDXKEY * key);

extern int btree_get_key_length (const DB_IDXKEY * key);
extern int btree_get_oid_from_key (THREAD_ENTRY * thread_p, BTID_INT * btid, const DB_IDXKEY * key, OID * oid);
extern int btree_read_node_header (BTID_INT * btid, PAGE_PTR pg_ptr, BTREE_NODE_HEADER * header);
extern void btree_read_fixed_portion_of_non_leaf_record (RECDES * rec, NON_LEAF_REC * nlf_rec);
...
extern int btree_write_record (THREAD_ENTRY * thread_p, BTID_INT * btid,
                               void *node_rec, const DB_IDXKEY * key, int node_type, RECDES * rec);

extern int btree_find_lower_bound_leaf (THREAD_ENTRY * thread_p,
                                        const VPID * top_vpid, BTREE_SCAN * BTS, BTREE_STATS * stat_info);
extern int btree_find_next_record (THREAD_ENTRY * thread_p, BTREE_SCAN * bts, int direction, BTREE_STATS * stat_info);

extern int btree_init_node_header (THREAD_ENTRY * thread_p, BTREE_NODE_HEADER * node_header);
extern int btree_insert_node_header (THREAD_ENTRY * thread_p, PAGE_PTR pg_ptr, BTREE_NODE_HEADER * header);
extern int btree_write_node_header (BTID_INT * btid, PAGE_PTR pg_ptr, BTREE_NODE_HEADER * header);

extern PAGE_PTR btree_get_new_page (THREAD_ENTRY * thread_p, BTID_INT * btid, VPID * vpid, VPID * near_vpid);

extern PAGE_PTR btree_pgbuf_fix (THREAD_ENTRY * thread_p, const VFID * vfid,
                                 const VPID * vpid,
                                 int requestmode, PGBUF_LATCH_CONDITION condition, const PAGE_TYPE ptype);

extern int btree_search_nonleaf_page (THREAD_ENTRY * thread_p,
                                      BTID_INT * btid, PAGE_PTR page_ptr,
                                      const DB_IDXKEY * key, INT16 * slot_id, VPID * child_vpid);
...
extern int btree_set_vpid_previous_vpid (THREAD_ENTRY * thread_p, BTID_INT * btid, PAGE_PTR page_p, VPID * prev);
extern int btree_split_next_pivot (BTREE_NODE_SPLIT_INFO * split_info, float new_value, int max_index);

extern void btree_write_fixed_portion_of_non_leaf_record (RECDES * rec, NON_LEAF_REC * nlf_rec);

extern int btree_rv_save_keyval (BTID_INT * btid, const DB_IDXKEY * key, char *data, int *length);
extern int btree_rv_util_save_page_records (THREAD_ENTRY * thread_p,
                                            BTID_INT * btid,
                                            PAGE_PTR page_ptr,
                                            INT16 first_slotid, int rec_cnt,
                                            INT16 ins_slotid, char *data, const int data_len, int *length);

#if !defined(NDEBUG)
extern int btree_fence_check_key (THREAD_ENTRY * thread_p,
                                  BTID_INT * btid,
                                  const DB_IDXKEY * left_key, const DB_IDXKEY * right_key, const bool with_eq);
extern int btree_fence_check_page (THREAD_ENTRY * thread_p, BTID_INT * btid, PAGE_PTR page_ptr);
#endif
