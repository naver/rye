...
extern int heap_classrepr_decache_and_lock (THREAD_ENTRY * thread_p, const OID * class_oid);
extern int heap_classrepr_unlock_class (const OID * class_oid);
extern OR_CLASSREP *heap_classrepr_get (THREAD_ENTRY * thread_p,
                                        OID * class_oid, REPR_ID reprid, int *idx_incache, bool use_last_reprid);
...
extern bool heap_classrepr_has_cache_entry (THREAD_ENTRY * thread_p, const OID * class_oid);
...
extern int heap_assign_address (THREAD_ENTRY * thread_p, const HFID * hfid, OID * oid, int expected_length);
...
extern OID *heap_insert (THREAD_ENTRY * thread_p, const HFID * hfid,
                         OID * class_oid, OID * oid, RECDES * recdes, HEAP_SCANCACHE * scan_cache);
...
extern const OID *heap_delete (THREAD_ENTRY * thread_p, const HFID * hfid,
                               const OID * oid, HEAP_SCANCACHE * scan_cache, OID * class_oid);
...
extern SCAN_CODE heap_first (THREAD_ENTRY * thread_p, const HFID * hfid,
                             OID * class_oid, OID * oid, RECDES * recdes, HEAP_SCANCACHE * scan_cache, int ispeeking);
...
extern int heap_get_num_objects (THREAD_ENTRY * thread_p, const HFID * hfid, DB_BIGINT * nobjs);

extern int heap_estimate_num_objects (THREAD_ENTRY * thread_p, const HFID * hfid, DB_BIGINT * num_objects);
#if defined(ENABLE_UNUSED_FUNCTION)
extern INT32 heap_estimate_num_pages_needed (THREAD_ENTRY * thread_p,
                                             int total_nobjs, int avg_obj_size, int num_attrs, int num_var_attrs);
#endif

extern OID *heap_get_class_oid (THREAD_ENTRY * thread_p, OID * class_oid, const OID * oid);
...
extern int heap_attrinfo_start (THREAD_ENTRY * thread_p,
                                const OID * class_oid,
                                int requested_num_attrs, const ATTR_ID * attrid, HEAP_CACHE_ATTRINFO * attr_info);
...
extern int heap_attrinfo_read_dbvalues (THREAD_ENTRY * thread_p,
                                        const OID * inst_oid, RECDES * recdes, HEAP_CACHE_ATTRINFO * attr_info);
#if defined (ENABLE_UNUSED_FUNCTION)
...
extern int heap_attrinfo_set (const OID * inst_oid, ATTR_ID attrid,
                              DB_VALUE * attr_val, HEAP_CACHE_ATTRINFO * attr_info);
...
extern int heap_attrinfo_start_with_btid (THREAD_ENTRY * thread_p,
                                          OID * class_oid, BTID * btid, HEAP_CACHE_ATTRINFO * attr_info);
...
#endif
extern int heap_get_indexname_of_btid (THREAD_ENTRY * thread_p, OID * class_oid, BTID * btid, char **btnamepp);
...
extern int xheap_get_class_num_objects_pages (THREAD_ENTRY * thread_p,
                                              const HFID * hfid, int approximation, DB_BIGINT * nobjs, int *npages);
...
extern int heap_bestspace_sync_all_heap_files_if_needed (THREAD_ENTRY * thread_p);

extern int heap_classrepr_find_index_id (OR_CLASSREP * classrepr, BTID * btid);
extern bool heap_classrepr_is_shard_table (THREAD_ENTRY * thread_p, OID * class_oid);
...
extern OID *heap_ovf_insert (THREAD_ENTRY * thread_p, const HFID * hfid,
                             OID * ovf_oid, RECDES * recdes, const OID * class_oid);
extern const OID *heap_ovf_update (THREAD_ENTRY * thread_p, const HFID * hfid, const OID * ovf_oid, RECDES * recdes);
extern const OID *heap_ovf_delete (THREAD_ENTRY * thread_p, const HFID * hfid, const OID * ovf_oid);
...
extern SCAN_CODE heap_ovf_get (THREAD_ENTRY * thread_p, const OID * ovf_oid, RECDES * recdes);
