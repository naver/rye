...
static int spage_get_saved_spaces (THREAD_ENTRY * thread_p,
                                   SPAGE_HEADER * page_header_p, PAGE_PTR page_p, int *other_saved_spaces);
...
static int spage_find_empty_slot (THREAD_ENTRY * thread_p, PAGE_PTR pgptr,
                                  int length, INT16 type, SPAGE_SLOT ** sptr, int *space, PGSLOTID * slotid);
static int spage_find_empty_slot_at (THREAD_ENTRY * thread_p, PAGE_PTR pgptr,
                                     bool for_recovery, PGSLOTID slotid, int length, INT16 type, SPAGE_SLOT ** sptr);
...
static int spage_update_record_after_compact (PAGE_PTR page_p,
                                              ...
                                              SPAGE_SLOT * slot_p,
                                              const RECDES *
                                              record_descriptor_p, int space, int old_waste, int new_waste);

static SCAN_CODE spage_search_record (PAGE_PTR page_p,
                                      PGSLOTID * out_slot_id_p,
                                      RECDES * record_descriptor_p, int is_peeking, int direction);
...
static bool spage_is_unknown_slot (PGSLOTID slotid, SPAGE_HEADER * sphdr, SPAGE_SLOT * sptr);
static SPAGE_SLOT *spage_find_slot (PAGE_PTR pgptr, SPAGE_HEADER * sphdr, PGSLOTID slotid, bool is_unknown_slot_check);
...
static int spage_put_helper (THREAD_ENTRY * thread_p, PAGE_PTR pgptr,
                             PGSLOTID slotid, int offset, const RECDES * recdes, bool is_append);
#endif
  ...
  if (sphdr->total_free < 0
      ...
      || sphdr->num_records < 0 || sphdr->num_slots < 0 || sphdr->num_records > sphdr->num_slots)
    {
      ...
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_SP_INVALID_HEADER,
              3, vpid.pageid, fileio_get_volume_label (vpid.volid, PEEK), header_info);
    }
...
spage_is_valid_anchor_type (const INT16 anchor_type)
{
  ...
  return (anchor_type == ANCHORED
          || anchor_type == UNANCHORED_KEEP_SEQUENCE || anchor_type == UNANCHORED_KEEP_SEQUENCE_BTREE);
}
 ...
 */
static int
spage_free_all_saved_spaces_helper (const void *vpid_key, void *ent, UNUSED_ARG void *tid)
{
  SPAGE_SAVE_ENTRY *entry, *current;
 ...
 */
static void
spage_dump_saved_spaces_by_other_trans (UNUSED_ARG THREAD_ENTRY * thread_p, FILE * fp, VPID * vpid_p)
{
  SPAGE_SAVE_ENTRY *entry_p;
  SPAGE_SAVE_HEAD *head_p;
...
spage_boot (UNUSED_ARG THREAD_ENTRY * thread_p)
{
  ...
  if (spage_Mht_saving != NULL)
    ...
    {
      spage_Mht_saving = mht_create ("Page space saving table", 50, pgbuf_hash_vpid, pgbuf_compare_vpid);
    }
...
spage_finalize (UNUSED_ARG THREAD_ENTRY * thread_p)
{
  ...
  if (spage_Mht_saving != NULL)
    {
      (void) mht_map (spage_Mht_saving, spage_free_all_saved_spaces_helper, &tranid);
 ...
 */
int
spage_get_free_space_without_saving (UNUSED_ARG THREAD_ENTRY * thread_p, PAGE_PTR page_p)
{
  SPAGE_HEADER *page_header_p;
  int free_space;
...
spage_max_space_for_new_record (THREAD_ENTRY * thread_p, PAGE_PTR page_p)
{
  ...
  if (page_header_p->anchor_type != ANCHORED && page_header_p->num_slots > page_header_p->num_records)
    {
      if (total_free < 0)
        {
          assert (false);       /* TODO - trace */
      ...
      if (slot_array == NULL)
        {
          ...
          er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE,
                  ER_OUT_OF_VIRTUAL_MEMORY, 1, (page_header_p->num_slots) * sizeof (SPAGE_SLOT *));
          return ER_FAILED;
        }
      ...
      if (page_header_p->num_records != j)
        {
          ...
          er_set (ER_FATAL_ERROR_SEVERITY, ARG_FILE_LINE,
                  ...
                  vpid.pageid, fileio_get_volume_label (vpid.volid, PEEK), j, page_header_p->num_records);
          ...
          if (to_offset == slot_array[i]->offset_to_record)
          ...
          else
            {
              ...
              memmove ((char *) page_p + to_offset,
                       (char *) page_p + slot_array[i]->offset_to_record, slot_array[i]->record_length);
  ...
  for (; last_slot_p < slot_p; last_slot_p++)
    {
      spage_set_slot (last_slot_p, (last_slot_p + 1)->offset_to_record,
                      (last_slot_p + 1)->record_length, (last_slot_p + 1)->record_type);
    }
...
spage_shift_slot_down (PAGE_PTR page_p, SPAGE_HEADER * page_header_p, SPAGE_SLOT * slot_p)
{
  ...
  for (; slot_p > last_slot_p; slot_p--)
    {
      spage_set_slot (slot_p, (slot_p - 1)->offset_to_record, (slot_p - 1)->record_length, (slot_p - 1)->record_type);
    }
...
spage_take_slot_in_use (THREAD_ENTRY * thread_p, PAGE_PTR page_p,
                        bool for_recovery,
                        SPAGE_HEADER * page_header_p, PGSLOTID slot_id, SPAGE_SLOT * slot_p, int *out_space_p)
{
  int status;
      ...
      if (page_header_p->anchor_type != ANCHORED)
        {
          assert (false);
          er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE,
                  ER_SP_BAD_INSERTION_SLOT, 3, slot_id, pgbuf_get_page_id (page_p), pgbuf_get_volume_label (page_p));
          return SP_ERROR;
        }
...
spage_find_empty_slot_at (THREAD_ENTRY * thread_p, PAGE_PTR page_p,
                          ...
                          int record_length, INT16 record_type, SPAGE_SLOT ** out_slot_p)
{
  SPAGE_HEADER *page_header_p;
  SPAGE_SLOT *slot_p;
  int waste, space, status;
  ...
  *out_slot_p = NULL;
  ...
  if (slot_id == page_header_p->num_slots)
    ...
    {
      status = spage_take_slot_in_use (thread_p, page_p, for_recovery, page_header_p, slot_id, slot_p, &space);
    }
...
spage_insert (THREAD_ENTRY * thread_p, PAGE_PTR page_p, RECDES * record_descriptor_p, PGSLOTID * out_slot_id_p)
{
  ...
  status = spage_find_slot_for_insert (thread_p, page_p, record_descriptor_p,
                                       out_slot_id_p, (void **) &slot_p, &used_space);
  if (status == SP_SUCCESS)
    {
                            ...
                            PGSLOTID * out_slot_id_p, void **out_slot_p, int *out_used_space_p)
{
  ...
  *out_slot_id_p = NULL_SLOTID;
  ...
  status = spage_find_empty_slot (thread_p, page_p,
                                  ...
                                  record_descriptor_p->type, &slot_p, out_used_space_p, out_slot_id_p);
...
spage_insert_at (THREAD_ENTRY * thread_p, PAGE_PTR page_p, PGSLOTID slot_id, RECDES * record_descriptor_p)
{
  ...
  status = spage_find_empty_slot_at (thread_p, page_p, false, slot_id,
                                     record_descriptor_p->length, record_descriptor_p->type, &slot_p);
  if (status == SP_SUCCESS)
    {
      status = spage_insert_data (thread_p, page_p, record_descriptor_p, slot_p);
    }
                       ...
                       SPAGE_SLOT ** out_slot_p, int *out_space_p, int *out_old_waste_p, int *out_new_waste_p)
{
  ...
  new_waste = DB_WASTED_ALIGN (record_descriptor_p->length, page_header_p->alignment);
  space = record_descriptor_p->length + new_waste - slot_p->record_length - old_waste;
...
spage_update (THREAD_ENTRY * thread_p, PAGE_PTR page_p, PGSLOTID slot_id, const RECDES * record_descriptor_p)
{
  ...
  status = spage_check_updatable (thread_p, page_p, slot_id,
                                  record_descriptor_p, &slot_p, &space, &old_waste, &new_waste);
  if (status != SP_SUCCESS)
    {
      return status;
    }
...
#if !defined(NDEBUG)
  if (page_header_p->anchor_type == UNANCHORED_KEEP_SEQUENCE_BTREE && slot_id == 0)
    {
  ...
  if (record_descriptor_p->length <= (int) slot_p->record_length)
  ...
  else
    {
      status = spage_update_record_after_compact (page_p, page_header_p,
                                                  slot_p, record_descriptor_p, space, old_waste, new_waste);
    }
 ...
 */
bool
spage_is_updatable (THREAD_ENTRY * thread_p, PAGE_PTR page_p, PGSLOTID slot_id, const RECDES * record_descriptor_p)
{
  if (spage_check_updatable (thread_p, page_p, slot_id, record_descriptor_p, NULL, NULL, NULL, NULL) != SP_SUCCESS)
    {
      return false;
    }
      ...
      if (remain_length > page_header_p->cont_free)
      ...
      else
        {
          ...
          memcpy (((char *) page_p + new_slot_p->offset_to_record),
                  (char *) page_p + slot_p->offset_to_record + offset, remain_length);
          new_slot_p->record_length = remain_length;
          slot_p->record_length = offset;
          /* Adjust the header */
...
spage_take_out (THREAD_ENTRY * thread_p, PAGE_PTR page_p, PGSLOTID slot_id, int takeout_offset, int takeout_length)
{
  ...
  if ((takeout_offset + takeout_length) > (int) slot_p->record_length)
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE,
              ER_SP_TAKEOUT_WRONG_OFFSET, 4, takeout_offset, takeout_length, slot_p->record_length, slot_id);
      return SP_ERROR;
    }
      ...
      if (takeout_offset == 0)
      ...
      else
        {
          ...
          memmove ((char *) page_p + slot_p->offset_to_record +
                   takeout_length, (char *) page_p + slot_p->offset_to_record, takeout_offset);
        }
      ...
      if (((int) slot_p->record_length - takeout_offset - takeout_length) > 0)
        {
          ...
          if (slot_p->offset_to_record + takeout_offset +
              (slot_p->record_length - takeout_offset - takeout_length) > DB_PAGESIZE)
            {
              er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_GENERIC_ERROR, 1, "");
              assert_release (false);
              return SP_ERROR;
          ...
          memmove ((char *) page_p + slot_p->offset_to_record +
                   ...
                   takeout_offset + takeout_length, slot_p->record_length - takeout_offset - takeout_length);
        }
      else
        {
...
spage_put_helper (THREAD_ENTRY * thread_p, PAGE_PTR page_p, PGSLOTID slot_id,
                  int offset, const RECDES * record_descriptor_p, bool is_append)
{
  SPAGE_HEADER *page_header_p;
  SPAGE_SLOT *slot_p;
  int old_offset;
      ...
      if (!is_append)
        {
          ...
          if (slot_p->offset_to_record + offset +
              record_descriptor_p->length + (slot_p->record_length - offset) > DB_PAGESIZE)
            {
              er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_GENERIC_ERROR, 1, "");
              assert_release (false);
              return SP_ERROR;
      ...
      if (is_append)
        {
          ...
          memcpy ((char *) page_p + page_header_p->offset_to_free_area,
                  (char *) page_p + slot_p->offset_to_record, slot_p->record_length);
        }
      else
        {
  ...
  else
    {
      ...
      if (is_append)
      ...
      else
        {
          ...
          memcpy ((char *) page_p + page_header_p->offset_to_free_area +
                  offset + record_descriptor_p->length, copyarea + offset, slot_p->record_length - offset);
        }
  ...
  if ((overwrite_offset + record_descriptor_p->length) > (int) slot_p->record_length)
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE,
              ...
              overwrite_offset, record_descriptor_p->length, slot_p->record_length, slot_id);
      return SP_ERROR;
    }
...
spage_merge (THREAD_ENTRY * thread_p, PAGE_PTR page_p, PGSLOTID first_slot_id, PGSLOTID second_slot_id)
{
  ...
  if (first_slot_p == NULL)
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_SP_UNKNOWN_SLOTID, 3,
              first_slot_id, pgbuf_get_page_id (page_p), pgbuf_get_volume_label (page_p));
      assert (false);
      return SP_ERROR;
    }
  ...
  if (second_slot_p == NULL)
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_SP_UNKNOWN_SLOTID, 3,
              second_slot_id, pgbuf_get_page_id (page_p), pgbuf_get_volume_label (page_p));
      assert (false);
      return SP_ERROR;
    }
  ...
  if (spage_is_record_located_at_end (page_header_p, first_slot_p)
  ...
  else if ((int) first_slot_p->record_length + (int) second_slot_p->record_length <= page_header_p->cont_free)
    {
      ...
      memcpy ((char *) page_p + page_header_p->offset_to_free_area,
              (char *) page_p + first_slot_p->offset_to_record, first_slot_p->record_length);
  ...
  else
    {
      ...
      memcpy (copyarea + first_slot_p->record_length,
              (char *) page_p + second_slot_p->offset_to_record, second_slot_p->record_length);
      ...
      if (page_header_p->offset_to_free_area +
          (first_slot_p->record_length + second_slot_p->record_length) > DB_PAGESIZE)
        {
          er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_GENERIC_ERROR, 1, "");
          assert_release (false);
          free_and_init (copyarea);
...
{
  return spage_search_record (page_p, out_slot_id_p, record_descriptor_p, is_peeking, SPAGE_SEARCH_NEXT);
}
...
{
  return spage_search_record (page_p, out_slot_id_p, record_descriptor_p, is_peeking, SPAGE_SEARCH_PREV);
}
      ...
      if (record_descriptor_p->area_size < 0 || record_descriptor_p->area_size < (int) slot_p->record_length)
        {
          ...
          record_descriptor_p->length = (((int) slot_p->record_length) * (-1));
          return S_DOESNT_FIT;
        }
...
spage_get_space_for_record (PAGE_PTR page_p, PGSLOTID slot_id)
{
  ...
  return (slot_p->record_length
          + DB_WASTED_ALIGN (slot_p->record_length, page_header_p->alignment) + spage_slot_size ());
}
  ...
  for (i = 0; i < num_slots; slot_p--, i++)
    {
      (void) fprintf (fp, "\nSlot-id = %2d, offset = %4d, type = %s",
                      i, slot_p->offset_to_record, spage_record_type_string (slot_p->record_type));
      if (slot_p->offset_to_record != SPAGE_EMPTY_OFFSET)
        {
  ...
  if (used_length + page_header_p->total_free > DB_PAGESIZE)
    {
      er_log_debug (ARG_FILE_LINE,
                    ...
                    page_header_p->total_free, DB_PAGESIZE, used_length + page_header_p->total_free, DB_PAGESIZE);
      ret = ER_FAILED;
      assert (false);
    }
       ...
       SSIZEOF (SPAGE_SLOT) * page_header_p->num_slots) > DB_PAGESIZE)
    {
      er_log_debug (ARG_FILE_LINE,
                    ...
                    (page_header_p->cont_free +
                     page_header_p->offset_to_free_area + sizeof (SPAGE_SLOT) * page_header_p->num_slots), DB_PAGESIZE);
      ret = ER_FAILED;
      assert (false);
    }
  ...
  if (page_header_p->is_saving)
    {
      ...
      total_saved = spage_get_saved_spaces (thread_p, page_header_p, page_p, &other_saved_spaces);
#if 1
...
spage_is_unknown_slot (PGSLOTID slot_id, SPAGE_HEADER * page_header_p, SPAGE_SLOT * slot_p)
{
  ...
  if (slot_id < 0 || slot_id >= page_header_p->num_slots
      ...
      || slot_p->offset_to_record < sizeof (SPAGE_HEADER) || slot_p->offset_to_record > max_offset)
    {
      assert (slot_p->offset_to_record == SPAGE_EMPTY_OFFSET);
#if 0
      assert (slot_p->record_type == REC_UNKNOWN || slot_p->record_type == REC_MARKDELETED);
#endif
...
spage_has_enough_contiguous_space (PAGE_PTR page_p, SPAGE_HEADER * page_header_p, int space)
{
  ...
  assert (spage_has_enough_total_space (NULL, page_p, page_header_p, space) == true);
  assert (spage_check_num_slots (NULL, page_p) == true);
