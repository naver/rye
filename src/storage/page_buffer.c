...
static PGBUF_HOLDER *pgbuf_find_thrd_holder (THREAD_ENTRY * thread_p, PGBUF_BCB * bufptr);
...
static int pgbuf_unlatch_bcb_upon_unfix (THREAD_ENTRY * thread_p,
                                         PGBUF_BCB * bufptr,
                                         int holder_status, const char *caller_file, int caller_line);
static int pgbuf_block_bcb (THREAD_ENTRY * thread_p, PGBUF_BCB * bufptr,
                            int request_mode, int request_fcnt, const char *caller_file, int caller_line);
#else /* NDEBUG */
static int pgbuf_latch_bcb_upon_fix (THREAD_ENTRY * thread_p,
...
static int pgbuf_block_bcb (THREAD_ENTRY * thread_p, PGBUF_BCB * bufptr, int request_mode, int request_fcnt);
#endif /* NDEBUG */
...
static PGBUF_BCB *pgbuf_allocate_bcb (THREAD_ENTRY * thread_p,
                                      const VPID * src_vpid, const char *caller_file, int caller_line);
...
static PGBUF_BCB *pgbuf_get_victim_from_lru_list (THREAD_ENTRY * thread_p, const VPID * vpid, int check_count);
...
static int pgbuf_flush_all_helper (THREAD_ENTRY * thread_p, VOLID volid,
                                   bool is_only_fixed,
                                   bool is_set_lsa_as_null, const char *caller_file, int caller_line);
#else /* NDEBUG */
...
static int pgbuf_timed_sleep_error_handling (THREAD_ENTRY * thread_p,
                                             PGBUF_BCB * bufptr,
                                             THREAD_ENTRY * thrd_entry, const char *caller_file, int caller_line);
...
static bool pgbuf_get_check_page_validation (THREAD_ENTRY * thread_p, int page_validation_level);
...
static void pgbuf_set_bcb_page_vpid (THREAD_ENTRY * thread_p, PGBUF_BCB * bufptr);
static bool pgbuf_check_page_ptype_internal (THREAD_ENTRY * thread_p, PAGE_PTR pgptr, PAGE_TYPE ptype, bool no_error);
static bool pgbuf_check_bcb_page_vpid (THREAD_ENTRY * thread_p, PGBUF_BCB * bufptr);
/*
...
{
  pgbuf_Pool.num_buffers = prm_get_bigint_value (PRM_ID_PAGE_BUFFER_SIZE) / IO_PAGESIZE;
  if (pgbuf_Pool.num_buffers < PGBUF_MINIMUM_BUFFERS)
    {
#if defined(RYE_DEBUG)
      er_log_debug (ARG_FILE_LINE, "pgbuf_initialize: WARNING "
                    "Num_buffers = %d is too small. %d was assumed", pgbuf_Pool.num_buffers, PGBUF_MINIMUM_BUFFERS);
#endif /* RYE_DEBUG */
  ...
  pgbuf_Pool.victim_cand_list = ((PGBUF_VICTIM_CANDIDATE_LIST *)
                                 malloc (pgbuf_Pool.num_buffers * sizeof (PGBUF_VICTIM_CANDIDATE_LIST)));
  if (pgbuf_Pool.victim_cand_list == NULL)
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_OUT_OF_VIRTUAL_MEMORY,
              1, (pgbuf_Pool.num_buffers * sizeof (PGBUF_VICTIM_CANDIDATE_LIST)));
      goto error;
    }
                      ...
                      int mode, int retry, UNUSED_ARG const PAGE_TYPE ptype)
{
  ...
  while ((pgptr = pgbuf_fix (thread_p, vpid, newpg, mode, PGBUF_UNCONDITIONAL_LATCH, ptype)) == NULL)
    {
      switch (er_errid ())
        {
...
pgbuf_fix_without_validation_debug (THREAD_ENTRY * thread_p,
                                    ...
                                    UNUSED_ARG const PAGE_TYPE ptype, const char *caller_file, int caller_line)
{
  PAGE_PTR pgptr;
#if defined(SERVER_MODE)
  ...
  pgptr = pgbuf_fix_debug (thread_p, vpid, newpg, request_mode, condition, ptype, caller_file, caller_line);
...
pgbuf_fix_without_validation_release (THREAD_ENTRY * thread_p,
                                      const VPID * vpid, int newpg,
                                      PGBUF_LATCH_MODE request_mode,
                                      PGBUF_LATCH_CONDITION condition, UNUSED_ARG const PAGE_TYPE ptype)
{
  PAGE_PTR pgptr;
#if defined(SERVER_MODE)
  ...
  pgptr = pgbuf_fix_release (thread_p, vpid, newpg, request_mode, condition, ptype);
                       ...
                       UNUSED_ARG const PAGE_TYPE ptype, const char *caller_file, int caller_line)
{
  ...
  return pgbuf_fix_debug (thread_p, vpid, NEW_PAGE,
                          PGBUF_LATCH_WRITE, PGBUF_UNCONDITIONAL_LATCH, ptype, caller_file, caller_line);
}
#else /* NDEBUG */
PAGE_PTR
pgbuf_fix_newpg_release (THREAD_ENTRY * thread_p, const VPID * vpid, UNUSED_ARG const PAGE_TYPE ptype)
{
  return pgbuf_fix_release (thread_p, vpid, NEW_PAGE, PGBUF_LATCH_WRITE, PGBUF_UNCONDITIONAL_LATCH, ptype);
}
#endif /* NDEBUG */
...
pgbuf_fix_debug (THREAD_ENTRY * thread_p, const VPID * vpid, int newpg,
                 PGBUF_LATCH_MODE request_mode,
                 PGBUF_LATCH_CONDITION condition, PAGE_TYPE ptype, const char *caller_file, int caller_line)
#else /* NDEBUG */
PAGE_PTR
pgbuf_fix_release (THREAD_ENTRY * thread_p, const VPID * vpid, int newpg,
                   PGBUF_LATCH_MODE request_mode, PGBUF_LATCH_CONDITION condition, PAGE_TYPE ptype)
#endif                          /* NDEBUG */
{
  ...
  if (FI_TEST_ARRAY_ARG_INT (thread_p, DIM (fi_entries), fi_entries) != NO_ERROR)
    {
      return NULL;
    }
...
try_again:
      ...
      if (newpg != NEW_PAGE)
        {
          ...
          if (fileio_read (thread_p,
                           ...
                           &bufptr->iopage_buffer->iopage, vpid->pageid, IO_PAGESIZE) == NULL)
            {
          ...
          mnt_stats_counter_with_time (thread_p, MNT_STATS_DATA_PAGE_IOREADS, 1, io_perf_start);

          if (pgbuf_is_temporary_volume (vpid->volid) == true && !LSA_IS_INIT_TEMP (&(prv_p->lsa)))
            {
  ...
  if (pgbuf_latch_bcb_upon_fix (thread_p, bufptr, request_mode,
                                buf_lock_acquired, condition, caller_file, caller_line) != NO_ERROR)
#else /* NDEBUG */
  if (pgbuf_latch_bcb_upon_fix (thread_p, bufptr, request_mode, buf_lock_acquired, condition) != NO_ERROR)
#endif /* NDEBUG */
    {
...
#endif                          /* NDEBUG */
{
  ...
  if (bufptr->dirty == true
      ...
      && !log_is_logged_since_restart (&bufptr->iopage_buffer->iopage.prv.lsa))
    {
       ...
       */
      pgbuf_set_lsa (thread_p, (PAGE_PTR) (&bufptr->iopage_buffer->iopage.page[0]), log_get_restart_lsa ());
  ...
  if (memcmp (PGBUF_FIND_BUFFER_GUARD (bufptr), pgbuf_Guard, sizeof (pgbuf_Guard)) != 0)
    {
      er_log_debug (ARG_FILE_LINE, "pgbuf_unfix: SYSTEM ERROR"
                    "buffer of pageid = %d|%d has been OVER RUN", bufptr->vpid.volid, bufptr->vpid.pageid);
  ...
  if ((bufptr->dirty == true && LSA_ISNULL (&bufptr->oldest_unflush_lsa))
      || (bufptr->dirty == false && !LSA_ISNULL (&bufptr->oldest_unflush_lsa)))
    {
#if 1                           /* TODO - ksseo */
      if (LOG_ISRESTARTED ())
        {
          assert (pgbuf_is_lsa_temporary (pgptr) || pgbuf_find_thrd_holder (thread_p, bufptr) != NULL);
        }
#else /* TODO:[happy:remove "#if 0" when postpone op. bug fixed] */
      assert (log_Gl.rcv_phase == LOG_RECOVERY_REDO_PHASE
              || pgbuf_is_lsa_temporary (pgptr) || pgbuf_find_thrd_holder (thread_p, bufptr) != NULL);
#endif
    }

#if 0                           /* TODO:[happy] */
  if (bufptr->latch_mode == PGBUF_LATCH_WRITE && bufptr->write_fcnt == bufptr->fcnt)
    {
      bufptr->write_fcnt--;
          ...
          if (!VPID_ISNULL (&bufptr->vpid) && pgbuf_is_consistent (bufptr, 0) == PGBUF_CONTENT_BAD)
            {
              er_log_debug (ARG_FILE_LINE, "pgbuf_unfix: WARNING"
                            " Pageid = %d|%d seems inconsistent", bufptr->vpid.volid, bufptr->vpid.pageid);
 ...
 */
int
pgbuf_get_num_hold_cnt (UNUSED_ARG THREAD_ENTRY * thread_p, PAGE_TYPE page_type)
{
  int cnt = 0;
  int thrd_index;
  PGBUF_HOLDER_ANCHOR *thrd_holder_info;
      ...
      while (holder != NULL)
        {
          ...
          latch_mode_str =
            ...
            (bufptr->latch_mode == PGBUF_LATCH_VICTIM_INVALID) ? "VictimInv" : "Fault";

          zone_str = ((bufptr->zone == PGBUF_LRU_1_ZONE) ? "LRU_1_Zone" :
                      (bufptr->zone == PGBUF_LRU_2_ZONE) ? "LRU_2_Zone" :
                      (bufptr->zone == PGBUF_INVALID_ZONE) ? "INVALID_Zone" : "VOID_Zone");
          ...
          consistent_str = ((consistent == PGBUF_CONTENT_GOOD) ? "GOOD" :
                            (consistent == PGBUF_CONTENT_BAD) ? "BAD" : "LIKELY BAD");
#else /* RYE_DEBUG */
...
#endif                          /* NDEBUG */
{
  ...
  if (VPID_ISNULL (&bufptr->vpid)
      || !VPID_EQ (&temp_vpid, &bufptr->vpid) || bufptr->fcnt > 0 || bufptr->avoid_victim == true)
    {
      pthread_mutex_unlock (&bufptr->BCB_mutex);
      return NO_ERROR;
    }
 ...
 */
void
pgbuf_invalidate_temporary_file (VOLID volid, PAGEID first_pageid, DKNPAGES npages)
{
  PGBUF_BUFFER_HASH *hash_anchor;
  PGBUF_BCB *bufptr;
  PAGE_PTR pgptr;
...
pgbuf_flush_all_helper (THREAD_ENTRY * thread_p, VOLID volid,
                        bool is_unfixed_only, bool is_set_lsa_as_null, const char *caller_file, int caller_line)
#else /* NDEBUG */
static int
  ...
  for (i = 0; i < pgbuf_Pool.num_buffers; i++)
    {
      ...
      if ((bufptr->dirty == false)
          || (is_unfixed_only && bufptr->fcnt > 0) || (volid != NULL_VOLID && volid != bufptr->vpid.volid))
        {
          pthread_mutex_unlock (&bufptr->BCB_mutex);
          continue;
        }
...
pgbuf_flush_all_unfixed_and_set_lsa_as_null_debug (THREAD_ENTRY * thread_p,
                                                   VOLID volid, const char *caller_file, int caller_line)
{
...
pgbuf_flush_victim_candidate_debug (THREAD_ENTRY * thread_p,
                                    float flush_ratio, UNUSED_ARG const char *caller_file, UNUSED_ARG int caller_line)
#else /* NDEBUG */
int
...
#endif                          /* NDEBUG */
{
  ...
  do
    {
      ...
      while ((bufptr != NULL) && (bufptr->zone != PGBUF_LRU_1_ZONE) && (check_count > 0))
        {
  ...
  qsort ((void *) victim_cand_list, victim_cand_count, sizeof (PGBUF_VICTIM_CANDIDATE_LIST), pgbuf_compare_victim_list);
  ...
  er_log_debug (ARG_FILE_LINE, "pgbuf_flush_victim_candidate: "
                ...
                total_flushed_count, start_lru_idx, pgbuf_Pool.last_flushed_LRU_list_idx);
...
pgbuf_flush_checkpoint_debug (THREAD_ENTRY * thread_p,
                              ...
                              LOG_LSA * smallest_lsa, UNUSED_ARG const char *caller_file, UNUSED_ARG int caller_line)
#else /* NDEBUG */
int
pgbuf_flush_checkpoint (THREAD_ENTRY * thread_p,
                        const LOG_LSA * flush_upto_lsa, const LOG_LSA * prev_chkpt_redo_lsa, LOG_LSA * smallest_lsa)
#endif                          /* NDEBUG */
{
  int status = NO_ERROR;
  PGBUF_BCB *bufptr;
  ...
  thread_mnt_track_push (thread_p, MNT_STATS_DATA_PAGE_FETCHES_TRACK_PGBUF_FLUSH_CHECKPOINT, &status);
          ...
          if (LSA_ISNULL (&bufptr->oldest_unflush_lsa))
            {
              ...
              pthread_mutex_unlock (&bufptr->BCB_mutex);
                   ...
                   && !LSA_ISNULL (prev_chkpt_redo_lsa) && LSA_LT (&bufptr->oldest_unflush_lsa, prev_chkpt_redo_lsa))
            {
              /* invalid page */
              pthread_mutex_unlock (&bufptr->BCB_mutex);
          ...
          else
            {
              ...
              pgptr = pgbuf_fix (thread_p, &vpid, OLD_PAGE, PGBUF_LATCH_READ, PGBUF_UNCONDITIONAL_LATCH, PAGE_UNKNOWN);
              if (pgptr == NULL || pgbuf_flush_with_wal (thread_p, pgptr) == NULL)
                {
                  ...
                  if (LSA_ISNULL (smallest_lsa) || LSA_LT (&bufptr->oldest_unflush_lsa, smallest_lsa))
                    {
  ...
  if (bufptr == NULL)
    {
      ...
      if (do_fetch == true)
        {
          pgptr = pgbuf_fix (thread_p, vpid, OLD_PAGE, PGBUF_LATCH_READ, PGBUF_UNCONDITIONAL_LATCH, PAGE_AREA);
          if (pgptr != NULL)
            {
      ...
      else
        {
          ...
          if (fileio_read_user_area (thread_p,
                                     ...
                                     (vpid->volid), vpid->pageid, start_offset, length, area) == NULL)
            {
              area = NULL;
            }

          mnt_stats_counter_with_time (thread_p, MNT_STATS_DATA_PAGE_IOREADS, 1, perf_start);
        }
#endif
    }
  else
...
pgbuf_copy_from_area (THREAD_ENTRY * thread_p, const VPID * vpid,
                      int start_offset, int length, void *area, UNUSED_ARG bool do_fetch)
{
  PGBUF_BUFFER_HASH *hash_anchor;
  PGBUF_BCB *bufptr;
  PAGE_PTR pgptr;
      ...
      if (do_fetch == false)
        {
          ...
          mnt_stats_counter_with_time (thread_p, MNT_STATS_DATA_PAGE_IOWRITES, 1, perf_start);
  ...
  if (bufptr->vpid.pageid == 0)
    {
      ...
      if (strncmp (vhdr->magic, RYE_MAGIC_DATABASE_VOLUME, RYE_MAGIC_MAX_LENGTH) != 0)
        {
          assert (0);
        }
    }
          ...
          if (LSA_LT (lsa_ptr, &chkpt_redo_lsa))
            {
              ...
              er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE,
                      ...
                      bufptr->vpid.pageid,
                      fileio_get_volume_label (bufptr->vpid.volid, PEEK),
                      lsa_ptr->pageid, lsa_ptr->offset, log_Gl.chkpt_redo_lsa.pageid, log_Gl.chkpt_redo_lsa.offset);
              er_stack_pop ();
  ...
  if (pgbuf_Pool.permvols_tmparea_volids == NULL)
    {
      ...
      if (area == NULL)
        {
          pthread_mutex_unlock (&pgbuf_Pool.volinfo_mutex);
          er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_OUT_OF_VIRTUAL_MEMORY, 1, nbytes);
          return;
        }
 ...
 */
static PGBUF_HOLDER *
pgbuf_find_thrd_holder (UNUSED_ARG THREAD_ENTRY * thread_p, PGBUF_BCB * bufptr)
{
  int thrd_index;
  PGBUF_HOLDER *holder;
  ...
  if (holder == NULL)
    {
      ...
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, err, 3,
              pgptr, bufptr->vpid.pageid, fileio_get_volume_label (bufptr->vpid.volid, PEEK));
 ...
 */
static int
pgbuf_remove_thrd_holder (UNUSED_ARG THREAD_ENTRY * thread_p, PGBUF_HOLDER * holder)
{
  int err = NO_ERROR;
  int thrd_index;
  PGBUF_HOLDER_ANCHOR *thrd_holder_info;
...
static int
pgbuf_latch_bcb_upon_fix (THREAD_ENTRY * thread_p, PGBUF_BCB * bufptr,
                          int request_mode, int buf_lock_acquired, PGBUF_LATCH_CONDITION condition)
#endif                          /* NDEBUG */
{
#if defined(SERVER_MODE)
  ...
  if ((request_mode == PGBUF_LATCH_READ)
      && (bufptr->latch_mode == PGBUF_LATCH_READ
          || bufptr->latch_mode == PGBUF_LATCH_FLUSH || bufptr->latch_mode == PGBUF_LATCH_VICTIM))
    {
      ...
      if (wait_msec == LK_ZERO_WAIT)
        {
          ...
          er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_LK_PAGE_TIMEOUT, 8,
                  ...
                  client_pid,
                  (request_mode == PGBUF_LATCH_READ ? "READ" : "WRITE"), bufptr->vpid.volid, bufptr->vpid.pageid, NULL);
        }
      else
        {
          pthread_mutex_unlock (&bufptr->BCB_mutex);
...
#if !defined(NDEBUG)
      if (pgbuf_block_bcb (thread_p, bufptr, request_mode, request_fcnt, caller_file, caller_line) != NO_ERROR)
#else /* NDEBUG */
      if (pgbuf_block_bcb (thread_p, bufptr, request_mode, request_fcnt) != NO_ERROR)
#endif /* NDEBUG */
        {
          return ER_FAILED;
        }
...
pgbuf_unlatch_bcb_upon_unfix (THREAD_ENTRY * thread_p, PGBUF_BCB * bufptr,
                              int holder_status, UNUSED_ARG const char *caller_file, UNUSED_ARG int caller_line)
#else /* NDEBUG */
static int
  ...
  if (bufptr->fcnt == 0)
    {
       ...
       */
      if (pgbuf_is_exist_blocked_reader_writer (bufptr) == false && bufptr->zone != PGBUF_LRU_1_ZONE)
        {
...
pgbuf_block_bcb (UNUSED_ARG THREAD_ENTRY * thread_p,
                 ...
                 UNUSED_ARG int request_fcnt, UNUSED_ARG const char *caller_file, UNUSED_ARG int caller_line)
#else /* NDEBUG */
static int
pgbuf_block_bcb (THREAD_ENTRY * thread_p, PGBUF_BCB * bufptr, int request_mode, int request_fcnt)
#endif                          /* NDEBUG */
{
#if defined(SERVER_MODE)
...
#if !defined(NDEBUG)
      error = pgbuf_timed_sleep (thread_p, bufptr, cur_thrd_entry, caller_file, caller_line);
#else /* NDEBUG */
...
pgbuf_timed_sleep_error_handling (UNUSED_ARG THREAD_ENTRY * thread_p,
                                  ...
                                  UNUSED_ARG const char *caller_file, UNUSED_ARG int caller_line)
#else /* NDEBUG */
static int
      ...
      if (logtb_is_current_active (thread_p) == true)
        {
          ...
          er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE,
                  ER_LK_UNILATERALLY_ABORTED, 4, tran_index, client_user_name, client_host_name, client_pid);
        }
      else
        {
          /*
  ...
  else if (old_wait_msecs > 0)
    {
      ...
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_LK_PAGE_TIMEOUT, 8,
              ...
              (save_request_latch_mode ==
               PGBUF_LATCH_READ ? "READ" : "WRITE"), bufptr->vpid.volid, bufptr->vpid.pageid, NULL);
    }
  else
    {
      pthread_mutex_unlock (&bufptr->BCB_mutex);
 ...
 */
static int
pgbuf_lock_page (THREAD_ENTRY * thread_p, UNUSED_ARG PGBUF_BUFFER_HASH * hash_anchor, UNUSED_ARG const VPID * vpid)
{
#if defined(SERVER_MODE)
...
pgbuf_unlock_page (UNUSED_ARG PGBUF_BUFFER_HASH * hash_anchor,
                   UNUSED_ARG const VPID * vpid, UNUSED_ARG int need_hash_mutex)
{
#if defined(SERVER_MODE)
  UNUSED_VAR int rv;
...
#endif                          /* NDEBUG */
{
  ...
  check_count =
    MAX (PGBUF_MIN_NUM_VICTIMS, (int) (PGBUF_NUM_PAGES_IN_LRU * prm_get_float_value (PRM_ID_PB_BUFFER_FLUSH_RATIO)));
          ...
          for (sleep_count = 0; sleep_count < PGBUF_SLEEP_MAX; sleep_count++)
            {
               ...
               */
              bufptr = pgbuf_get_victim_from_lru_list (thread_p, &vpid, check_count);
              if (bufptr != NULL)
                {
                  /* the caller is holding bufptr->BCB_mutex. */
...
pgbuf_invalidate_bcb (PGBUF_BCB * bufptr)
{
  ...
  if (bufptr->latch_mode == PGBUF_LATCH_FLUSH_INVALID
      || bufptr->latch_mode == PGBUF_LATCH_VICTIM_INVALID || bufptr->latch_mode == PGBUF_LATCH_INVALID)
    {
      pthread_mutex_unlock (&bufptr->BCB_mutex);
      return NO_ERROR;
    }
...
#endif                          /* NDEBUG */
{
  ...
  if (bufptr->latch_mode == PGBUF_LATCH_INVALID
      || bufptr->latch_mode == PGBUF_LATCH_FLUSH_INVALID || bufptr->latch_mode == PGBUF_LATCH_VICTIM_INVALID)
    {
      pthread_mutex_unlock (&bufptr->BCB_mutex);
      return NO_ERROR;
    }
...
#if !defined(NDEBUG)
          if (pgbuf_block_bcb (thread_p, bufptr, PGBUF_LATCH_FLUSH, 0, caller_file, caller_line) != NO_ERROR)
#else /* NDEBUG */
          if (pgbuf_block_bcb (thread_p, bufptr, PGBUF_LATCH_FLUSH, 0) != NO_ERROR)
#endif /* NDEBUG */
            {
              return ER_FAILED;
            }
 ...
 */
static PGBUF_BCB *
pgbuf_get_victim_from_lru_list (THREAD_ENTRY * thread_p, const VPID * vpid, int check_count)
{
  PGBUF_BCB *bufptr;
  int lru_idx;
  int check_myself;
  ...
  while ((bufptr = pgbuf_Pool.buf_LRU_list[lru_idx].LRU_bottom) != NULL)
    {
      ...
      while (bufptr != NULL && check_myself > 0 && bufptr->zone != PGBUF_LRU_1_ZONE)
        {
          check_myself--;
 ...
 */
static bool
pgbuf_get_check_page_validation (UNUSED_ARG THREAD_ENTRY * thread_p, int page_validation_level)
{
#if !defined(NDEBUG)
...
pgbuf_is_valid_page (THREAD_ENTRY * thread_p, const VPID * vpid,
                     bool no_error, DISK_ISVALID (*fun) (const VPID * vpid, void *args), void *args)
{
  DISK_ISVALID valid;
      ...
      if (valid != DISK_ERROR && !no_error)
        {
          er_set (ER_FATAL_ERROR_SEVERITY, ARG_FILE_LINE,
                  ER_PB_BAD_PAGEID, 2, vpid->pageid, fileio_get_volume_label (vpid->volid, PEEK));
...
pgbuf_dump (void)
{
  ...
  (void) fprintf (stdout,
                  ...
                  pgbuf_Pool.last_perm_volid, pgbuf_Pool.num_permvols_tmparea);
...
pgbuf_initialize_statistics (void)
{
  ...
  ps_info.vol_stat = (PGBUF_VOL_STAT *) malloc (sizeof (PGBUF_VOL_STAT) * ps_info.nvols);
...
pgbuf_add_fixed_at (PGBUF_HOLDER * holder, const char *caller_file, int caller_line)
{
  ...
  if (holder->fix_count > 5)
    {
      assert (false);
      er_log_debug (ARG_FILE_LINE, "pgbuf_add_fixed_at: WARNING " "fix_count = %d is too many.", holder->fix_count);
    }
#endif
...
pgbuf_get_page_ptype (THREAD_ENTRY * thread_p, PAGE_PTR pgptr)
{
  ...
  ptype = fileio_get_page_ptype (thread_p, &(bufptr->iopage_buffer->iopage.prv));
...
//  assert (ptype >= PAGE_UNKNOWN);
  ...
  (void) fileio_set_page_ptype (thread_p, &(bufptr->iopage_buffer->iopage.prv), ptype);

  assert (fileio_get_page_ptype (thread_p, &(bufptr->iopage_buffer->iopage.prv)) == ptype);
}
...
pgbuf_check_page_ptype_internal (THREAD_ENTRY * thread_p, PAGE_PTR pgptr, PAGE_TYPE ptype, bool no_error)
{
  ...
  if (ptype == PAGE_UNKNOWN
      ...
      || ((ptype == PAGE_HEAP_HEADER || ptype == PAGE_HEAP) && (pself == PAGE_HEAP_HEADER || pself == PAGE_HEAP)))
    {
      return true;              /* OK */
    }
...
pgbuf_check_bcb_page_vpid (THREAD_ENTRY * thread_p, PGBUF_BCB * bufptr)
{
  ...
  if (PAGEID_EQ (bufptr->vpid.pageid, prv_p->pageid) && VOLID_EQ (bufptr->vpid.volid, prv_p->volid))
    {
      return true;
    }
