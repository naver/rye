  ...
  if (!need_alloc)
      ...
      if (*ovf_first_page < 0)
	{
	  ...
	  *ovf_first_page = 0;
      ...
      if (file_get_numpages (thread_p, ovf_vfid, NULL, NULL, NULL) <=
	  *ovf_first_page)
	{
  ...
  for (i = 0; i < npages; i++)
	  ...
	  && logging_type == OVERFLOW_NORMAL_LOGGING)
	{
      ...
      log_append_redo_data (thread_p, RVOVF_NEWPAGE_INSERT, &addr,
			    copy_length + CAST_BUFLEN (copyto -
						       (char *) addr.pgptr),
			    (char *) addr.pgptr);
	  ...
	  if (VPID_ISNULL (&next_vpid))
	    {
	      ...
	      if (file_alloc_pages
		  (thread_p, ovf_vfid, &next_vpid, 1, addr_vpid_ptr, NULL,
		   NULL) == NULL)
		{
		  error = er_errid ();
	      ...
	      else
		{
	      ...
	      error =
		FI_TEST_ARG_INT (thread_p, FI_TEST_OVF_FILE_UPDATE_ERROR1, 0,
				 0);
	      if (error != NO_ERROR)
		{
		  GOTO_EXIT_ON_ERROR;
      ...
      else
	{
	  ...
	  while (!(VPID_ISNULL (&next_vpid)))
	      ...
	      if (error != NO_ERROR)
		{
	      ...
	      error =
		file_dealloc_page (thread_p, ovf_vfid, &tmp_vpid,
				   PAGE_OVERFLOW);
	      if (error != NO_ERROR)
		{
		  GOTO_EXIT_ON_ERROR;
 ...
 */
void
overflow_rv_newpage_logical_dump_undo (FILE * fp,
