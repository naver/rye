			  ...
			  const void *file_des)
{
  ...
  if (file_create (thread_p, vfid, exp_numpages, file_type, file_des,
		   NULL, 0) != NULL)
    {
	     ...
	     VPID * first_prealloc_vpid, INT32 prealloc_npages)
{
  ...
  for (ftb_page_index = 1; ftb_page_index < num_ftb_pages; ftb_page_index++)
    {
      ...
      addr.pgptr =
	pgbuf_fix2 (thread_p, vpid_ptr, NEW_PAGE, PGBUF_LATCH_WRITE,
		    PGBUF_UNCONDITIONAL_LATCH,
  ...
  fhdr_pgptr = pgbuf_fix2 (thread_p, &table_vpids[0], NEW_PAGE,
			   PGBUF_LATCH_WRITE, PGBUF_UNCONDITIONAL_LATCH,
  ...
  if (nsects > 0)
    {
      addr.pgptr =
	pgbuf_fix2 (thread_p, &allocset->start_sects_vpid, OLD_PAGE,
		    PGBUF_LATCH_WRITE, PGBUF_UNCONDITIONAL_LATCH,
		    MNT_STATS_DATA_PAGE_FETCHES_FILE_TAB);
      if (addr.pgptr == NULL)
	{
      ...
      for (i = 0; i < nsects; i++)
	{
	  if (aid_ptr >= outptr)
	      ...
	      if (ret != NO_ERROR)
		{
	      ...
	      addr.pgptr = pgbuf_fix2 (thread_p, &vpid, OLD_PAGE,
				       PGBUF_LATCH_WRITE,
				       PGBUF_UNCONDITIONAL_LATCH,
				       MNT_STATS_DATA_PAGE_FETCHES_FILE_TAB);
	      if (addr.pgptr == NULL)
...
file_destroy (THREAD_ENTRY * thread_p, const VFID * vfid)
{
  ...
  fhdr_pgptr = pgbuf_fix2 (thread_p, &allocset_vpid, OLD_PAGE,
			   PGBUF_LATCH_WRITE, PGBUF_UNCONDITIONAL_LATCH,
			   item);
  if (fhdr_pgptr == NULL)
    {
  ...
  if ((file_type == FILE_TMP || file_type == FILE_QUERY_AREA)
      ...
      if (0 < fhdr->num_user_pages)
	{
	  ...
	  while (!VPID_ISNULL (&allocset_vpid))
	    {
	      allocset_pgptr = pgbuf_fix2 (thread_p, &allocset_vpid, OLD_PAGE,
					   PGBUF_LATCH_WRITE,
					   PGBUF_UNCONDITIONAL_LATCH,
					   item);
		...
		{
		  pgptr = pgbuf_fix2 (thread_p, &nxftb_vpid, OLD_PAGE,
				      PGBUF_LATCH_WRITE,
				      PGBUF_UNCONDITIONAL_LATCH,
				      MNT_STATS_DATA_PAGE_FETCHES_FILE_TAB);
		  if (pgptr == NULL)
  ...
  while (!VPID_ISNULL (&nxftb_vpid))
    {
      pgptr = pgbuf_fix2 (thread_p, &nxftb_vpid, OLD_PAGE, PGBUF_LATCH_WRITE,
  ...
  ret = FI_TEST_ARG_INT (thread_p, FI_TEST_FILE_MANAGER_FILE_DESTROY_ERROR1,
			 0, 0);
  if (ret != NO_ERROR)
    {
      GOTO_EXIT_ON_ERROR;
...
file_get_type (THREAD_ENTRY * thread_p, const VFID * vfid)
{
  ...
  fhdr_pgptr = pgbuf_fix2 (thread_p, &vpid, OLD_PAGE, PGBUF_LATCH_READ,
			   PGBUF_UNCONDITIONAL_LATCH,
			   MNT_STATS_DATA_PAGE_FETCHES_FILE_HEADER);
  if (fhdr_pgptr == NULL)
    {
				  ...
				  const FILE_HEADER * fhdr, VPID * last_vpid)
{
  ...
  allocset_pgptr = pgbuf_fix2 (thread_p, &fhdr->last_allocset_vpid, OLD_PAGE,
			    ...
			    int exp_npages)
{
  ...
  while (!VPID_ISNULL (&allocset->end_sects_vpid))
    {
      addr.pgptr = pgbuf_fix2 (thread_p, &allocset->end_sects_vpid, OLD_PAGE,
			       PGBUF_LATCH_WRITE, PGBUF_UNCONDITIONAL_LATCH,
			       MNT_STATS_DATA_PAGE_FETCHES_FILE_TAB);
      if (addr.pgptr == NULL)
	{
			     ...
			     PAGE_PTR allocset_pgptr, INT16 allocset_offset)
{
  ...
  from_pgptr = pgbuf_fix2 (thread_p, &from_vpid, OLD_PAGE, PGBUF_LATCH_WRITE,
			   PGBUF_UNCONDITIONAL_LATCH,
			   MNT_STATS_DATA_PAGE_FETCHES_FILE_TAB);
  if (from_pgptr == NULL)
    {
  ...
  if (((FILE_NUM_EMPTY_SECTS * SSIZEOF (to_vpid.pageid)) -
       ...
       allocset->end_pages_offset) < DB_PAGESIZE)
    {
      ...
      to_vpid = from_vpid;
      to_pgptr = pgbuf_fix2 (thread_p, &to_vpid, OLD_PAGE, PGBUF_LATCH_WRITE,
			     PGBUF_UNCONDITIONAL_LATCH,
			     MNT_STATS_DATA_PAGE_FETCHES_FILE_TAB);
      if (to_pgptr == NULL)
	{
  ...
  while (!(VPID_EQ (&from_vpid, &allocset->end_sects_vpid)
      ...
      if (from_outptr >= from_aid_ptr)
	{
	  ...
	  from_pgptr = pgbuf_fix2 (thread_p, &from_vpid, OLD_PAGE,
				   PGBUF_LATCH_WRITE,
				   PGBUF_UNCONDITIONAL_LATCH,
				   MNT_STATS_DATA_PAGE_FETCHES_FILE_TAB);
	  if (from_pgptr == NULL)
      ...
      if (to_outptr >= to_aid_ptr)
	{
	  ...
	  to_pgptr = pgbuf_fix2 (thread_p, &to_vpid, OLD_PAGE,
				 PGBUF_LATCH_WRITE, PGBUF_UNCONDITIONAL_LATCH,
				 MNT_STATS_DATA_PAGE_FETCHES_FILE_TAB);
	  if (to_pgptr == NULL)
	    {
...
file_expand_ftab (THREAD_ENTRY * thread_p, PAGE_PTR fhdr_pgptr)
{
  ...
  addr.pgptr = pgbuf_fix2 (thread_p, &new_ftb_vpid, NEW_PAGE,
			   PGBUF_LATCH_WRITE, PGBUF_UNCONDITIONAL_LATCH,
			   MNT_STATS_DATA_PAGE_FETCHES_FILE_TAB);
  if (addr.pgptr == NULL)
    {
  ...
  if (!VPID_ISNULL (&fhdr->next_table_vpid))
    {
       ...
       * changes
       */
      addr.pgptr = pgbuf_fix2 (thread_p, &prev_last_ftb_vpid, OLD_PAGE,
			       PGBUF_LATCH_WRITE, PGBUF_UNCONDITIONAL_LATCH,
			       MNT_STATS_DATA_PAGE_FETCHES_FILE_TAB);
      if (addr.pgptr == NULL)
	{
		       ...
		       INT16 volid)
{
  ...
  new_allocset_pgptr = pgbuf_fix2 (thread_p, &new_allocset_vpid, OLD_PAGE,
				   PGBUF_LATCH_WRITE,
				   PGBUF_UNCONDITIONAL_LATCH,
				   MNT_STATS_DATA_PAGE_FETCHES_FILE_TAB);
			       ...
			       INT32 npages, void *args), void *args)
{
  ...
  fhdr_pgptr = pgbuf_fix2 (thread_p, &vpid, OLD_PAGE, PGBUF_LATCH_WRITE,
			   PGBUF_UNCONDITIONAL_LATCH,
			   MNT_STATS_DATA_PAGE_FETCHES_FILE_HEADER);
  if (fhdr_pgptr == NULL)
    {
  ...
  if (isfile_new == FILE_NEW_FILE
      && file_type != FILE_TMP && logtb_is_current_active (thread_p) == true)
    {
      log_end_system_op (thread_p, LOG_RESULT_TOPOP_ATTACH_TO_OUTER);
    }
  else
				   ...
				   void *args, FILE_ALLOC_VPIDS * alloc_vpids)
{
  ...
  *first_alloc_nthpage = fhdr->num_user_pages;
  ...
  is_tmp_file = ((file_type == FILE_TMP
		   ...
		   UNUSED_ARG MNT_SERVER_ITEM dealloc_item)
{
  ...
  if (logtb_is_current_active (thread_p) == false
      && isfile_new == FILE_NEW_FILE && file_type != FILE_TMP)
    {
      ...
      addr.pgptr = pgbuf_fix2 (thread_p, dealloc_vpid, OLD_PAGE,
			       PGBUF_LATCH_WRITE, PGBUF_UNCONDITIONAL_LATCH,
			       dealloc_item);
      if (addr.pgptr != NULL)
	{
      ...
      while (!VPID_ISNULL (&allocset_vpid))
	{
	  ...
	  if (allocset->num_pages <= keep_first_numpages)
	      ...
	      while (!VPID_ISNULL (&ftb_vpid))
		{
		   ...
		   */
		  pgptr = pgbuf_fix2 (thread_p, &ftb_vpid, OLD_PAGE,
				      PGBUF_LATCH_WRITE,
				      PGBUF_UNCONDITIONAL_LATCH,
				      MNT_STATS_DATA_PAGE_FETCHES_FILE_TAB);
		  if (pgptr == NULL)
...
file_dump_ftabs (THREAD_ENTRY * thread_p, FILE * fp, const FILE_HEADER * fhdr)
{
  ...
  while (!VPID_ISNULL (&ftb_vpid))
      ...
      else
	{
      ...
      ftb_pgptr = pgbuf_fix2 (thread_p, &ftb_vpid, OLD_PAGE, PGBUF_LATCH_READ,
			      PGBUF_UNCONDITIONAL_LATCH, item);
      if (ftb_pgptr == NULL)
	{
	  return ER_FAILED;
...
file_tracker_dump (THREAD_ENTRY * thread_p, FILE * fp)
{
  ...
  trk_fhdr_pgptr = pgbuf_fix2 (thread_p, &set_vpids[0], OLD_PAGE,
			       PGBUF_LATCH_READ, PGBUF_UNCONDITIONAL_LATCH,
			       MNT_STATS_DATA_PAGE_FETCHES_FILE_HEADER);
  if (trk_fhdr_pgptr == NULL)
    {
					 ...
					 const VFID * vfid)
{
  ...
  if (file_tracker_unregister (thread_p, vfid) == NULL
      && file_get_tracker_vfid () != NULL)
    {
      ...
      addr.pgptr = pgbuf_fix2 (thread_p, &vpid, OLD_PAGE, PGBUF_LATCH_WRITE,
			       PGBUF_UNCONDITIONAL_LATCH,
			       MNT_STATS_DATA_PAGE_FETCHES_FILE_HEADER);
      if (addr.pgptr == NULL)
	{
      ...
      if (vfid->volid > xboot_find_number_permanent_volumes (thread_p))
	{
	  ...
	  if (!BO_IS_SERVER_RESTARTED ())
	  ...
	  else
	    {
	      ...
	      fhdr_pgptr = pgbuf_fix2 (thread_p, &vpid, OLD_PAGE,
				       PGBUF_LATCH_WRITE,
				       PGBUF_UNCONDITIONAL_LATCH,
				       MNT_STATS_DATA_PAGE_FETCHES_FILE_HEADER);
	      if (fhdr_pgptr == NULL)
					      ...
					      const int attr_id)
{
  ...
  if (heap_get_indexname_of_btid (thread_p, (OID *) class_oid_p, &btid,
				  &index_name_p) != NO_ERROR)
    {
      goto end;
    }
