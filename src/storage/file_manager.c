...
static void file_descriptor_dump_multi_page_object_heap (FILE * fp,
							 const
							 FILE_OVF_HEAP_DES *
							 ovf_hfile_des_p);
		    ...
		    const FILE_TYPE file_type)
{
  ...
  if (entry != NULL)
    {
      if (file_new_declare_as_old_internal
	  (thread_p, vfid, entry->tran_index, true) != NO_ERROR)
	{
	  csect_exit (CSECT_FILE_NEWFILE);
	  return NULL;
	}
 ...
 */
static int
file_new_declare_as_old_internal (UNUSED_ARG THREAD_ENTRY * thread_p,
				  const VFID * vfid, int tran_id,
				  bool hold_csect)
{
  FILE_NEWFILE *entry, *tmp;
  ...
  do
      ...
      if (hint_volid != NULL_VOLID)
	{
	  ...
	  pageid = disk_alloc_page (thread_p, hint_volid,
				    DISK_SECTOR_WITH_ALL_PAGES, num_ftb_pages,
				    hint_pageid, alloc_page_type);
 ...
 */
static int
file_descriptor_insert (UNUSED_ARG THREAD_ENTRY * thread_p,
			FILE_HEADER * fhdr, const void *xfile_des)
{
 ...
 */
static int
file_descriptor_get (UNUSED_ARG THREAD_ENTRY * thread_p,
		     const FILE_HEADER * fhdr, void *xfile_des, int maxsize)
{
 ...
 */
static int
file_descriptor_destroy_rest_pages (UNUSED_ARG THREAD_ENTRY * thread_p,
				    FILE_HEADER * fhdr)
{
#if defined (ENABLE_UNUSED_FUNCTION)
  VPID rest_vpid;
  ...
  while (!VPID_ISNULL (&rest_vpid))
    {
      pgptr =
	pgbuf_fix (thread_p, &rest_vpid, OLD_PAGE, PGBUF_LATCH_WRITE,
		   PGBUF_UNCONDITIONAL_LATCH);
      if (pgptr == NULL)
	{
	  return ER_FAILED;
				     ...
				     FILE_HEADER * fhdr)
{
  ...
  if (!VPID_ISNULL (&fhdr->des.next_part_vpid))
	...
	{
	  pgptr =
	    pgbuf_fix (thread_p, &rest_vpid, OLD_PAGE, PGBUF_LATCH_READ,
		       PGBUF_UNCONDITIONAL_LATCH);
	  if (pgptr == NULL)
	    {
	      return -1;
			       ...
			       const FILE_HEADER * fhdr)
{
  ...
  if (fhdr->des.total_length == fhdr->des.first_length)
	...
	default:
	  /* Dump the first part */
	  for (i = 0, dumpfrom = fhdr->des.piece; i < fhdr->des.first_length;
	       i++)
	    {
			  ...
			  const void *file_des)
{
  ...
  if (file_create (thread_p, vfid, exp_numpages, file_type, file_des, NULL, 0)
      != NULL)
    {
		 ...
		 const void *file_des)
{
  ...
  return file_create_tmp_internal (thread_p, vfid, FILE_TMP, exp_numpages,
				   file_des);
}
		       ...
		       INT32 exp_numpages, const void *file_des)
{
  ...
  return file_create_tmp_internal (thread_p, vfid, FILE_QUERY_AREA,
...
file_create (THREAD_ENTRY * thread_p, VFID * vfid, INT32 exp_numpages,
	     const FILE_TYPE file_type, const void *file_des,
	     VPID * first_prealloc_vpid, INT32 prealloc_npages)
{
  ...
  setpage_type =
    ((prealloc_npages <
      0) ? DISK_CONTIGUOUS_PAGES : DISK_NONCONTIGUOUS_SPANVOLS_PAGES);
  ...
  length =
    (length + (sizeof (sectid) * (nsects + FILE_NUM_EMPTY_SECTS)) +
     sizeof (vpid.pageid));
  ...
  if (table_vpids == NULL)
    {
      ret = ER_OUT_OF_VIRTUAL_MEMORY;
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ret, 1,
	      sizeof (vpid) * num_ftb_pages);
      goto exit_on_error;
    }

  ret =
    file_ftabvpid_alloc (thread_p, vfid->volid, NULL_PAGEID, table_vpids,
			 num_ftb_pages, file_type);
  if (ret != NO_ERROR)
    {
      goto exit_on_error;
    }
  ...
  if (FILE_SIZEOF_FHDR_WITH_DES_COMMENTS (fhdr) < DB_PAGESIZE)
	...
	(INT16) FILE_SIZEOF_FHDR_WITH_DES_COMMENTS (fhdr);
      allocset->start_sects_offset =
	DB_ALIGN (allocset->start_sects_offset, DOUBLE_ALIGNMENT);
    }
  else
    {
  ...
  ret = file_calculate_offset (allocset->end_sects_offset,
			       ...
			       &allocset->start_pages_offset,
			       &allocset->start_pages_vpid, table_vpids,
			       num_ftb_pages, &ftb_page_index);
  if (ret != NO_ERROR)
    {
      goto exit_on_error;
  ...
  if (nsects > 0)
      ...
      if (addr.pgptr == NULL)
	{
      ...
      ret =
	file_find_limits (addr.pgptr, allocset, &aid_ptr, &outptr,
			  FILE_SECTOR_TABLE);
      if (ret != NO_ERROR)
	{
  ...
  if (prealloc_npages != 0)
      ...
      if (prealloc_npages > 0)
	{
	  ...
	  if (file_alloc_pages_at_volid (thread_p, vfid, first_prealloc_vpid,
					 prealloc_npages, NULL, vfid->volid,
					 NULL, NULL) == NULL)
	    {
	      /*
...
file_destroy (THREAD_ENTRY * thread_p, const VFID * vfid)
{
  ...
  fhdr_pgptr =
    pgbuf_fix (thread_p, &allocset_vpid, OLD_PAGE, PGBUF_LATCH_WRITE,
  ...
  if (!VFID_EQ (vfid, &fhdr->vfid))
    {
      ...
      er_set (ER_FATAL_ERROR_SEVERITY, ARG_FILE_LINE, ret, 2, vfid->volid,
	      vfid->fileid);
      GOTO_EXIT_ON_ERROR;
    }
	  ...
	  if (allocset->num_sects > 0)
	      ...
	      while (!VPID_ISNULL (&nxftb_vpid))
		{
		  ...
		  /* Calculate the starting offset and length of the page to
		     check */
		  ret =
		    file_find_limits (pgptr, allocset, &aid_ptr, &outptr,
				      FILE_SECTOR_TABLE);
		  if (ret != NO_ERROR)
		    {
		      pgbuf_unfix_and_init (thread_p, pgptr);
  ...
  while (!VPID_ISNULL (&nxftb_vpid))
    {
      pgptr =
	pgbuf_fix (thread_p, &nxftb_vpid, OLD_PAGE, PGBUF_LATCH_WRITE,
		   PGBUF_UNCONDITIONAL_LATCH, ptype);
      if (pgptr == NULL)
	{
	  GOTO_EXIT_ON_ERROR;
  ...
  if (file_type == FILE_TMP && logtb_is_current_active (thread_p) == true)
    {
      ...
      /* remove header page */
      if (disk_dealloc_page
	  (thread_p, vfid->volid, vfid->fileid, 1, page_type) != NO_ERROR)
	{
	  GOTO_EXIT_ON_ERROR;
	}
  ...
  ret =
    FI_TEST_ARG_INT (thread_p, FI_TEST_FILE_MANAGER_FILE_DESTROY_ERROR1, 0,
		     0);
  if (ret != NO_ERROR)
    {
      GOTO_EXIT_ON_ERROR;
...
file_get_numpages (THREAD_ENTRY * thread_p, const VFID * vfid,
		   int *num_table_vpids, int *num_user_pages_mrkdelete,
		   int *num_allocsets)
{
  PAGE_PTR fhdr_pgptr = NULL;
  FILE_HEADER *fhdr;
  ...
  if (vfid == NULL)
    {
      ...
      num_overhead_pages =
	(((nsects + npages) * sizeof (INT32)) + sizeof (*fhdr));
    }
  else
    {
      ...
      if (fhdr_pgptr == NULL)
	{
      ...
      allocset_pgptr = pgbuf_fix (thread_p, &fhdr->last_allocset_vpid,
				  ...
				  PGBUF_UNCONDITIONAL_LATCH, PAGE_FILE_TAB);
      if (allocset_pgptr == NULL)
	{
	  pgbuf_unfix_and_init (thread_p, fhdr_pgptr);
	  return -1;
      ...
      allocset =
	(FILE_ALLOCSET *) ((char *) allocset_pgptr +
		       ...
		       INT32 start_nthpage, INT32 num_desired_pages)
{
  ...
  while (!VPID_ISNULL (&vpid) && num_returned < num_desired_pages)
      ...
      if (pgptr == NULL)
	{
      ...
      if (file_find_limits
	  (pgptr, allocset, &aid_ptr, &outptr, FILE_PAGE_TABLE) != NO_ERROR)
	{
	  pgbuf_unfix_and_init (thread_p, pgptr);
	  return -1;
	}
				  ...
				  const FILE_HEADER * fhdr, VPID * last_vpid)
{
  ...
  allocset =
    (FILE_ALLOCSET *) ((char *) allocset_pgptr + fhdr->last_allocset_offset);
  if (allocset->num_pages <= 0
      || file_allocset_nthpage (thread_p, fhdr, allocset, last_vpid,
				allocset->num_pages - 1, 1) != 1)
    {
      VPID_SET_NULL (last_vpid);
      pgbuf_unfix_and_init (thread_p, allocset_pgptr);
      return NULL;
			    ...
			    int exp_npages)
{
  ...
  if (allocset->end_sects_offset >= allocset->start_pages_offset
      && VPID_EQ (&allocset->end_sects_vpid, &allocset->start_pages_vpid))
    {
      ...
      if (file_allocset_expand_sector
	  (thread_p, fhdr_pgptr, allocset_pgptr, allocset_offset) == NULL)
	{
			     ...
			     PAGE_PTR allocset_pgptr, INT16 allocset_offset)
{
  ...
  from_pgptr =
    pgbuf_fix (thread_p, &from_vpid, OLD_PAGE, PGBUF_LATCH_WRITE,
	       PGBUF_UNCONDITIONAL_LATCH, PAGE_FILE_TAB);
  if (from_pgptr == NULL)
    {
      return NULL;
  ...
  if (((FILE_NUM_EMPTY_SECTS * SSIZEOF (to_vpid.pageid)) -
       ...
       allocset->end_pages_offset) < DB_PAGESIZE)
    {
      ...
      to_vpid = from_vpid;
      to_pgptr =
	pgbuf_fix (thread_p, &to_vpid, OLD_PAGE, PGBUF_LATCH_WRITE,
		   PGBUF_UNCONDITIONAL_LATCH, PAGE_FILE_TAB);
      if (to_pgptr == NULL)
	{
	  pgbuf_unfix_and_init (thread_p, from_pgptr);
      ...
      if (nsects < 0)
	{
      ...
      allocset->end_pages_offset =
	(INT16) ((nsects * sizeof (to_vpid.pageid)) + sizeof (*chain));
    }
  ...
  if (file_find_limits
      (to_pgptr, allocset, &to_outptr, &to_aid_ptr,
       FILE_PAGE_TABLE) != NO_ERROR)
    {
      pgbuf_unfix_and_init (thread_p, from_pgptr);
  ...
  while (!
	 (VPID_EQ (&from_vpid, &allocset->end_sects_vpid)
		       ...
		       INT16 volid)
{
  ...
  new_allocset =
    (FILE_ALLOCSET *) ((char *) new_allocset_pgptr + new_allocset_offset);
  ...
  if ((new_allocset_offset + SSIZEOF (*new_allocset)) < DB_PAGESIZE)
    {
      ...
      new_allocset->start_sects_offset =
	(new_allocset_offset + sizeof (*new_allocset));
    }
  else
    {
      if (file_expand_ftab (thread_p, fhdr_pgptr) == NULL)
	  ...
	  if (outptr > aid_ptr)
	    {
	      length =
		(int) ((npages >
			(outptr - aid_ptr)) ? (outptr - aid_ptr) : (npages -
								    i));
	      log_append_undo_data (thread_p, RVFL_IDSTABLE, &addr,
			   ...
			   FILE_ALLOC_VPIDS * alloc_vpids)
{
  ...
  near_pageid =
    ((near_vpid != NULL
      && allocset->volid ==
      near_vpid->volid) ? near_vpid->pageid : NULL_PAGEID);
  ...
  if (alloc_pageid == DISK_NULL_PAGEID_WITH_ENOUGH_DISK_PAGES
      ...
      && ((allocset->num_pages + npages) <=
	  ((allocset->num_sects - 1) * DISK_SECTOR_NPAGES)))
    {
  ...
  if (alloc_pageid != NULL_PAGEID
      && alloc_pageid != DISK_NULL_PAGEID_WITH_ENOUGH_DISK_PAGES)
    {
      ...
      if (file_allocset_add_pageids (thread_p, fhdr_pgptr, allocset_pgptr,
				     allocset_offset, alloc_pageid, npages,
			       ...
			       void *args), void *args)
{
  ...
  if (fun != NULL
      && (*fun) (thread_p, vfid, file_type, first_alloc_vpid, npages,
		 args) == false)
    {
				   ...
				   void *args, FILE_ALLOC_VPIDS * alloc_vpids)
{
  ...
  *first_alloc_nthpage = fhdr->num_user_pages;
  ...
  is_tmp_file =
    ((file_type == FILE_TMP || file_type == FILE_QUERY_AREA) ? true : false);
  ...
  while (npages > 0)
    {
       ...
       */
      allocate_npages =
	(is_tmp_file ? npages : MIN (npages, DISK_SECTOR_NPAGES));
	  ...
	  if (pageid == *aid_ptr)
	      ...
	      if (fun)
		{
		  if ((*fun) (thread_p, fhdr, allocset, fhdr_pgptr,
			      allocset_pgptr, allocset_offset, pgptr, aid_ptr,
...
file_allocset_dealloc_page (THREAD_ENTRY * thread_p, FILE_HEADER * fhdr,
			    ...
			    UNUSED_ARG INT32 ignore_pageid, void *args)
{
  int *ret;
...
file_allocset_dealloc_contiguous_pages (THREAD_ENTRY * thread_p,
					...
					PAGE_PTR allocset_pgptr,
					INT16 allocset_offset,
					PAGE_PTR pgptr, INT32 * first_aid_ptr,
					INT32 ncont_page_entries)
{
#if defined(SERVER_MODE)
  bool old_val;
  ...
  if (ret == NO_ERROR)
    {
      ...
      ret =
	disk_dealloc_page (thread_p, allocset->volid, *first_aid_ptr,
...
file_allocset_remove_pageid (THREAD_ENTRY * thread_p, FILE_HEADER * fhdr,
			     ...
			     PAGE_PTR allocset_pgptr, INT16 allocset_offset,
			     PAGE_PTR pgptr, INT32 * aid_ptr,
			     UNUSED_ARG INT32 ignore_pageid,
		   ...
		   UNUSED_ARG PAGE_TYPE dealloc_ptype)
{
  ...
  if (logtb_is_current_active (thread_p) == false
      && isfile_new == FILE_NEW_FILE && file_type != FILE_TMP)
    {
      /*
		      ...
		      VPID * allocset_vpid, INT16 allocset_offset)
{
  ...
  *allocset_vpid = allocset->next_allocset_vpid;
  ...
  allocset_pgptr =
    pgbuf_fix (thread_p, prev_allocset_vpid, OLD_PAGE, PGBUF_LATCH_WRITE,
	       PGBUF_UNCONDITIONAL_LATCH);
  if (allocset_pgptr == NULL)
    {
      goto exit_on_error;
				  ...
				  bool rm_freespace_sectors)
{
  ...
  while (!VPID_EQ (&from_vpid, &allocset->end_pages_vpid)
      ...
      if (from_aid_ptr >= from_outptr)
	{
	  ...
	  /* Find the location for portion of page table */
	  ret =
	    file_find_limits (from_pgptr, allocset, &from_aid_ptr,
		       ...
		       INT16 * ftb_offset)
{
  ...
  while (true)
      ...
      else
	{
	  ...
	  ftb_pgptr =
	    pgbuf_fix (thread_p, ftb_vpid, OLD_PAGE, PGBUF_LATCH_WRITE,
		       PGBUF_UNCONDITIONAL_LATCH);
	  if (ftb_pgptr == NULL)
	    {
	      goto exit_on_error;
		      ...
		      bool validate_vfid)
{
  ...
  fhdr_pgptr =
    pgbuf_fix (thread_p, &set_vpids[0], OLD_PAGE, PGBUF_LATCH_READ,
	       PGBUF_UNCONDITIONAL_LATCH);
  if (fhdr_pgptr == NULL)
    {
      goto exit_on_error;
  ...
  if (allvalid == DISK_VALID)
      ...
      for (i = 0; i < fhdr->num_user_pages; i += num_found)
	{
	  ...
	  for (j = 0; j < num_found; j++)
	    {
	      valid =
		disk_isvalid_page (thread_p, set_vpids[j].volid,
				   set_vpids[j].pageid);
	      if (valid != DISK_VALID)
		{
		  if (valid == DISK_INVALID)
...
file_dump_fhdr (THREAD_ENTRY * thread_p, FILE * fp, const FILE_HEADER * fhdr)
{
  ...
  (void) fprintf (fp, "File_type = %s\nCreation_time = %s",
		  file_type_to_string (fhdr->type), time_val);
...
file_dump_ftabs (THREAD_ENTRY * thread_p, FILE * fp, const FILE_HEADER * fhdr)
{
  ...
  while (!VPID_ISNULL (&ftb_vpid))
      ...
      else
	{
      ...
      ftb_pgptr =
	pgbuf_fix (thread_p, &ftb_vpid, OLD_PAGE, PGBUF_LATCH_READ,
		   PGBUF_UNCONDITIONAL_LATCH, ptype);
      if (ftb_pgptr == NULL)
	{
	  return ER_FAILED;
...
file_dump (THREAD_ENTRY * thread_p, FILE * fp, const VFID * vfid)
{
  ...
  while (!VPID_ISNULL (&allocset_vpid))
      ...
      if (ret != NO_ERROR)
	{
      ...
      (void) fprintf (fp,
		      "First page in this allocation set is the"
		      " nthpage = %d\n", num_pages);
...
file_tracker_register (THREAD_ENTRY * thread_p, const VFID * vfid)
{
  ...
  if (file_allocset_add_pageids (thread_p, fhdr_pgptr, allocset_pgptr,
				 fhdr->last_allocset_offset, vfid->fileid, 1,
				 NULL) == NULL_PAGEID)
    {
      goto exit_on_error;
    }
...
file_get_numfiles (THREAD_ENTRY * thread_p)
{
  ...
  return file_get_numpages (thread_p, file_get_tracker_vfid (), NULL, NULL,
			    NULL);
}
...
file_find_nthfile (THREAD_ENTRY * thread_p, VFID * vfid, int nthfile)
{
  ...
  count =
    file_find_nthpages (thread_p, file_get_tracker_vfid (), &vpid, nthfile,
			1);
  if (count == -1)
    {
      VFID_SET_NULL (vfid);
...
file_tracker_cross_check_with_disk_idsmap (THREAD_ENTRY * thread_p)
{
  ...
  num_files =
    file_get_numpages (thread_p, file_Tracker->vfid, NULL, NULL, NULL);
 ...
 */
static DISK_ISVALID
file_make_idsmap_image (THREAD_ENTRY * thread_p, const VFID * vfid,
			char **vol_ids_map, int nperm_vols)
{
  DISK_ISVALID valid = DISK_VALID;
#if defined (SA_MODE)
  FILE_HEADER *fhdr;
  ...
  for (i = 0; i < num_user_pages; i += num_found)
	...
	file_find_nthpages (thread_p, vfid, &set_vpids[0], i,
			    ((num_user_pages - i <
			      FILE_SET_NUMVPIDS) ? (num_user_pages -
						    i) : FILE_SET_NUMVPIDS));
...
set_bitmap (char *vol_ids_map, INT32 pageid)
{
  ...
  byte_offset =
    ((pageid - (page_num * DISK_PAGE_BIT)) / CHAR_BIT) +
...
file_tracker_dump (THREAD_ENTRY * thread_p, FILE * fp)
{
  ...
  num_files =
    file_get_numpages (thread_p, file_get_tracker_vfid (), NULL, NULL, NULL);
...
file_tracker_compress (THREAD_ENTRY * thread_p)
{
  ...
  trk_fhdr_pgptr =
    pgbuf_fix (thread_p, &set_vpids[0], OLD_PAGE, PGBUF_LATCH_WRITE,
	       PGBUF_UNCONDITIONAL_LATCH);
  if (trk_fhdr_pgptr == NULL)
    {
      return ER_FAILED;
...
file_dump_all_capacities (THREAD_ENTRY * thread_p, FILE * fp)
{
  ...
  for (i = 0; i < num_files; i++)
      ...
      if (size < 0)
	{
      ...
      fprintf (fp, "%4d|%4d %5d  %-22s ", vfid.volid, vfid.fileid, num_pages,
 ...
 */
static int
file_rv_tracker_unregister_logical_undo (THREAD_ENTRY * thread_p,
					 const VFID * vfid)
{
  LOG_DATA_ADDR addr = LOG_ADDR_INITIALIZER;
  VPID vpid;
 ...
 */
void
file_rv_allocset_dump_sector (FILE * fp, UNUSED_ARG int length_ignore,
			      void *data)
{
 ...
 */
void
file_rv_allocset_dump_page (FILE * fp, UNUSED_ARG int length_ignore,
			    void *data)
{
  ...
  fprintf (fp, "Page Table   (Start): Page = %d|%d, Offset = %d\n",
	   rvstb->start_pages_vpid.volid, rvstb->start_pages_vpid.pageid,
	   rvstb->start_pages_offset);
 ...
 */
void
file_rv_fhdr_dump_expansion (FILE * fp, UNUSED_ARG int length_ignore,
			     void *data)
{
 ...
 */
void
file_rv_allocset_dump_link (FILE * fp, UNUSED_ARG int length_ignore,
			    void *data)
{
 ...
 */
void
file_rv_fhdr_dump_last_allocset (FILE * fp, UNUSED_ARG int length_ignore,
				 void *data)
{
 ...
 */
void
file_rv_allocset_dump_add_pages (FILE * fp, UNUSED_ARG int length_ignore,
				 void *data)
{
 ...
 */
void
file_rv_fhdr_dump_add_pages (FILE * fp, UNUSED_ARG int length_ignore,
			     void *data)
{
 ...
 */
void
file_rv_fhdr_dump_mark_deleted_pages (FILE * fp, UNUSED_ARG int length_ignore,
				      void *data)
{
 ...
 */
void
file_rv_allocset_dump_delete_pages (FILE * fp, UNUSED_ARG int length_ignore,
				    void *data)
{
 ...
 */
void
file_rv_allocset_dump_sectortab (FILE * fp, UNUSED_ARG int length_ignore,
				 void *data)
{
  ...
  fprintf (fp,
	   ...
	   rvsect->start_sects_vpid.volid, rvsect->start_sects_vpid.pageid,
 ...
 */
void
file_rv_descriptor_dump_firstrest_nextvpid (FILE * fp,
					    UNUSED_ARG int length_ignore,
					    void *data)
{
  VPID *vpid;

  vpid = (VPID *) data;
  fprintf (fp, "First Rest of file Desc VPID: %d|%d", vpid->volid,
	   vpid->pageid);
}
 ...
 */
void
file_rv_descriptor_dump_nrest_nextvpid (FILE * fp,
					UNUSED_ARG int length_ignore,
					void *data)
{
  VPID *vpid;
 ...
 */
void
file_rv_tracker_dump_undo_register (FILE * fp, UNUSED_ARG int length_ignore,
				    void *data)
{
  VFID *vfid;
					      ...
					      const int attr_id)
{
  ...
  if (heap_get_indexname_of_btid
      (thread_p, (OID *) class_oid_p, &btid, &index_name_p) != NO_ERROR)
    {
      goto end;
    }
...
file_update_used_pages_of_vol_header (THREAD_ENTRY * thread_p)
{
  ...
  if (space_info == NULL)
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_OUT_OF_VIRTUAL_MEMORY, 1,
	      sizeof (VOL_SPACE_INFO) * nperm_vols);
      return DISK_ERROR;
    }
  ...
  for (i = 0; i < num_files && allvalid != DISK_ERROR; i += num_found)
      ...
      for (j = 0; j < num_found && allvalid != DISK_ERROR; j++)
	{
	  ...
	  valid =
	    file_construct_space_info (thread_p, space_info, &vfid,
				       nperm_vols);
	  if (valid != DISK_VALID)
	    {
	      allvalid = valid;
