 ...
 */
int
btree_write_node_header (PAGE_PTR pg_ptr, BTREE_NODE_HEADER * header)
{
 ...
 */
int
btree_read_node_header (PAGE_PTR pg_ptr, BTREE_NODE_HEADER * header)
{
  RECDES peek_rec = RECDES_INITIALIZER;
  int error_code = NO_ERROR;
...
btree_pgbuf_fix (THREAD_ENTRY * thread_p, const VFID * vfid,
		 const VPID * vpid,
		 int newpg, int requestmode, PGBUF_LATCH_CONDITION condition)
{
  PAGE_PTR page_ptr = NULL;

  page_ptr = pgbuf_fix (thread_p, vpid, newpg, requestmode, condition);
  ...
  if (vfid != NULL && page_ptr != NULL)
    {
      ...
      if (newpg == OLD_PAGE && spage_number_of_records (page_ptr) > 0)
	{
	  BTREE_NODE_HEADER node_header;
			   ...
			   void *args)
{
  ...
  pgptr = btree_pgbuf_fix (thread_p, vfid, vpid, NEW_PAGE, PGBUF_LATCH_WRITE,
			   ...
			   INT16 * slot_id, VPID * child_vpid)
{
  ...
  ret = btree_read_node_header (page_ptr, &node_header);
  if (ret != NO_ERROR)
    {
      goto exit_on_error;
    }
		  ...
		  DB_TYPE * att_type, OID * class_oid, int attr_id)
{
   ...
   */
  root_page =
    btree_pgbuf_fix (thread_p, &(btid->vfid), &root_vpid, OLD_PAGE,
			    ...
			    const DB_IDXKEY * key)
{
  ...
  assert (node_header.node_level == 1);
  BTREE_CHECK_KEY_CNT (leaf_pg, node_header.node_level, node_header.key_cnt);

  if (btree_write_node_header (leaf_pg, &node_header) != NO_ERROR)
    {
      GOTO_EXIT_ON_ERROR;
    }
		  ...
		  short node_type, int is_left_merge, VPID * child_vpid)
{
  ...
  if (P != NULL)
      ...
      if (btree_read_node_header (left_pg, &node_header) != NO_ERROR)
	{
      ...
      BTREE_CHECK_KEY_CNT (left_pg, node_header.node_level,
			   node_header.key_cnt);
    }
  ...
  assert (node_header.key_cnt >= 0);
  BTREE_CHECK_KEY_CNT (right_pg, node_header.node_level, node_header.key_cnt);
#endif
  ...
  assert_release (node_header.key_cnt >= 0);
  BTREE_CHECK_KEY_CNT (left_pg, node_header.node_level, node_header.key_cnt);
  ...
  *child_vpid = left_vpid;
  ...
  if (P != NULL)
      ...
      if (btree_read_node_header (P, &node_header) != NO_ERROR)
	{
      ...
      assert_release (node_header.key_cnt >= 0);
      assert (node_header.node_level > 1);
      BTREE_CHECK_KEY_CNT (P, node_header.node_level, node_header.key_cnt);
...
btree_delete (THREAD_ENTRY * thread_p, BTID_INT * btid, DB_IDXKEY * key)
{
  ...
  BTREE_CHECK_KEY_CNT (P, root_header.node_level, root_header.key_cnt);
  ...
  while (node_header[i].node_level > 1)
      ...
      if (Q == NULL)
	{
      ...
      /* read the header record */
      if (btree_read_node_header (Q, &(node_header[i])) != NO_ERROR)
	{
	  goto error;
	}
  ...
  /* read the header record */
  if (btree_read_node_header (P, &(node_header[i])) != NO_ERROR)
    {
      goto error;
    }
	...
	{			/* left sibling accessible */
	  ...
	  for (m_slot_id = p_slot_id; m_slot_id > 1; m_slot_id--)
	      ...
	      if (pheader.node_level > 2)
		{
		  ...
		  Left =
		    btree_pgbuf_fix (thread_p, &(btid->sys_btid->vfid),
				     &Left_vpid, OLD_PAGE, PGBUF_LATCH_WRITE,
				     PGBUF_UNCONDITIONAL_LATCH);
		  if (Left == NULL)
		    {
		      GOTO_EXIT_ON_ERROR;
	      ...
	      else
		{
		  ...
		  if (Left == NULL)
		      ...
		      if (Left == NULL)
			{
		      ...
		      Q =
			btree_pgbuf_fix (thread_p, &(btid->sys_btid->vfid),
					 &Q_vpid, OLD_PAGE, PGBUF_LATCH_WRITE,
					 PGBUF_UNCONDITIONAL_LATCH);
		      if (Q == NULL)
			{
			  GOTO_EXIT_ON_ERROR;
		...
		1 ? BTREE_NON_LEAF_NODE : BTREE_LEAF_NODE;
	      ...
	      /* read the header record */
	      if (btree_read_node_header (Left, &left_header) != NO_ERROR)
		{
		  GOTO_EXIT_ON_ERROR;
		}
		  ...
		  INT16 * slot_id, bool * found)
{
  ...
  *slot_id = NULL_SLOTID;
  ...
  P =
    btree_pgbuf_fix (thread_p, &(btid_int->sys_btid->vfid), &P_vpid, OLD_PAGE,
		     PGBUF_LATCH_READ, PGBUF_UNCONDITIONAL_LATCH);
  if (P == NULL)
    {
      goto error;
  ...
  while (node_header.node_level > 1)
      ...
      if (Q == NULL)
	{
      ...
      /* read the header record */
      if (btree_read_node_header (Q, &node_header) != NO_ERROR)
	{
	  goto error;
	}
      ...
      BTREE_STATS_ADD_WAIT_TIME (node_header.node_level > 1
			     ...
			     BTREE_STATS * stat_info)
{
  ...
  if (btree_read_node_header (BTS->C_page, &node_header) != NO_ERROR)
    {
      GOTO_EXIT_ON_ERROR;
    }
  ...
  assert (key_cnt >= 0);
  assert (node_header.node_level == 1);
  BTREE_CHECK_KEY_CNT (BTS->C_page, node_header.node_level,
		       node_header.key_cnt);
  if (!(key_cnt >= 0)
      || (node_header.node_level > 1)
		       ...
		       BTREE_STATS * stat_info)
{
  ...
  key_cnt = node_header.key_cnt;
  assert_release (key_cnt >= 0);
  BTREE_CHECK_KEY_CNT (P_page, node_header.node_level, node_header.key_cnt);
  ...
  while (node_type == BTREE_NON_LEAF_NODE)
      ...
      if (spage_get_record (P_page, 1, &rec, PEEK) != S_SUCCESS)
	{
      ...
      C_vpid = nleaf.pnt;
      C_page =
	btree_pgbuf_fix (thread_p, &(btid->sys_btid->vfid), &C_vpid, OLD_PAGE,
			 PGBUF_LATCH_READ, PGBUF_UNCONDITIONAL_LATCH);
      if (C_page == NULL)
	{
	  goto error;
	...
	node_header.node_level > 1 ? BTREE_NON_LEAF_NODE : BTREE_LEAF_NODE;
      key_cnt = node_header.key_cnt;
      assert_release (key_cnt >= 0);
      BTREE_CHECK_KEY_CNT (C_page, node_header.node_level,
			   node_header.key_cnt);
		  ...
		  int slot_id, RECDES * rec, int is_peeking)
{
  ...
  BTREE_CHECK_KEY_CNT (page_p, node_header.node_level, node_header.key_cnt);
#endif
		      ...
		      const VPID * root_vpid, VPID * pg_vpid)
{
  ...
  while (node_type == BTREE_NON_LEAF_NODE)
      ...
      if (spage_get_record (P, num_records - 1, &rec, PEEK) != S_SUCCESS)
	{
      ...
      Q_vpid = nleaf.pnt;
      Q =
	btree_pgbuf_fix (thread_p, &(btid->sys_btid->vfid), &Q_vpid, OLD_PAGE,
			 PGBUF_LATCH_READ, PGBUF_UNCONDITIONAL_LATCH);
      if (Q == NULL)
	{
	  goto error;
      ...
      if (node_type == BTREE_LEAF_NODE)
	{
	  ...
	  key_cnt = node_header.key_cnt;
	  assert_release (key_cnt >= 0);
	  BTREE_CHECK_KEY_CNT (Q, node_header.node_level,
			       node_header.key_cnt);
	  ...
	  if (key_cnt <= 0)
				    ...
				    &rec, PEEK) == NO_ERROR)
		{
		  ...
		  Q =
		    btree_pgbuf_fix (thread_p, &(btid->sys_btid->vfid),
				     &Q_vpid, OLD_PAGE, PGBUF_LATCH_READ,
				     PGBUF_UNCONDITIONAL_LATCH);
		  if (Q == NULL)
		    {
		      goto error;
      ...
      if (bts->C_vpid.pageid != NULL_PAGEID)
	{
	  bts->C_page =
	    btree_pgbuf_fix (thread_p, &(bts->btid_int.sys_btid->vfid),
			     &bts->C_vpid, OLD_PAGE, PGBUF_LATCH_READ,
			     PGBUF_CONDITIONAL_LATCH);
	  if (bts->C_page == NULL)
	    {
	      if (er_errid () == NO_ERROR)
			      ...
			      PAGE_PTR page_p, VPID * prev)
{
  ...
  if (btree_write_node_header (page_p, &node_header) != NO_ERROR)
    {
      return ER_FAILED;
    }
