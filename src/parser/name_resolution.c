...
static PT_NODE *pt_bind_name_or_path_in_scope (PARSER_CONTEXT * parser,
                                               PT_BIND_NAMES_ARG * bind_arg, PT_NODE * in_node);
...
static PT_NODE *pt_get_resolution (PARSER_CONTEXT * parser,
                                   ...
                                   PT_NODE * scope, PT_NODE * in_node, PT_NODE ** p_entity, int col_name);
...
static PT_NODE *pt_make_subclass_list (PARSER_CONTEXT * parser,
                                       ...
                                       int col_num, UINTPTR id, PT_MISC_TYPE meta_class, MHT_TABLE * names_mht);
...
static PT_NODE *pt_find_outer_entity_in_scopes (PARSER_CONTEXT * parser,
                                                SCOPES * scopes, UINTPTR spec_id, short *scope_location);
          ...
          if (node->info.name.spec_id == spec->info.spec.id &&
              !PT_NAME_INFO_IS_FLAGED (node, PT_NAME_INFO_FILL_DEFAULT))
            {
              ...
              PT_ERRORmf (parser, node, MSGCAT_SET_PARSER_SEMANTIC,
                          MSGCAT_SEMANTIC_IS_NOT_DEFINED, pt_short_print (parser, node));
 ...
 */
static PT_NODE *
pt_undef_names_post (UNUSED_ARG PARSER_CONTEXT * parser, PT_NODE * node, void *arg, UNUSED_ARG int *continue_walk)
{
  PT_NODE *spec = (PT_NODE *) arg;
  short *level_p = NULL;
              ...
              if (er_errid () != NO_ERROR)
              ...
              else
                {
                  PT_ERRORmf (parser, in_node, MSGCAT_SET_PARSER_SEMANTIC,
                              MSGCAT_SEMANTIC_IS_NOT_DEFINED, pt_short_print (parser, in_node));
                }
            }
        }
    }
          ...
          if (temp != node)
            {
              ...
              PT_ERRORmf (parser, node, MSGCAT_SET_PARSER_SEMANTIC,
                          MSGCAT_SEMANTIC_OUTERJOIN_PATH_EXPR, pt_short_print (parser, node));
              node = NULL;
            }
          else
            {
              ...
              if (!entity)
                {
                  ...
                  PT_ERRORmf (parser, node, MSGCAT_SET_PARSER_SEMANTIC,
                              MSGCAT_SEMANTIC_OUTERJOIN_SCOPE, pt_short_print (parser, node));
                  node = NULL;
                }
              else if (entity->info.spec.location < 0
                       || (entity->info.spec.location >
                           temp->info.name.location) || scope_location > entity->info.spec.location)
                {
                  /* cannot resolve within the outer join scope */
                  PT_ERRORmf (parser, node, MSGCAT_SET_PARSER_SEMANTIC,
                              MSGCAT_SEMANTIC_OUTERJOIN_SCOPE, pt_short_print (parser, node));
  ...
  if (col_cnt != attr_cnt)
    {
      PT_ERRORmf3 (parser, spec, MSGCAT_SET_PARSER_SEMANTIC,
                   MSGCAT_SEMANTIC_ATT_CNT_NE_DERIVED_C, pt_short_print (parser, spec), attr_cnt, col_cnt);
    }
  else
    {
 ...
 */
static PT_NODE *
pt_mark_location (UNUSED_ARG PARSER_CONTEXT * parser, PT_NODE * node, void *arg, UNUSED_ARG int *continue_walk)
{
    ...
    case PT_UPDATE:
      {
        ...
        for (temp = assignments; temp && error == NO_ERROR; temp = temp->next)
          {
            for (lhs = temp->info.expr.arg1; lhs && error == NO_ERROR; lhs = lhs->next)
              {
      ...
      if (crt_list->info.node_list.list_type == PT_IS_DEFAULT_VALUE)
      ...
      else
        {
          for (crt_attr = attrs_list,
               ...
               crt_attr != NULL && crt_value != NULL; crt_attr = crt_attr->next, crt_value = crt_value->next)
            {
  ...
  switch (node->node_type)
              ...
              if (!node->info.query.q.select.list)
                {
                  ...
                  PT_ERRORmf (parser, from, MSGCAT_SET_PARSER_SEMANTIC,
                              MSGCAT_SEMANTIC_NO_ATTRIBUTES_IN_CLS, pt_short_print (parser, from));
                          ...
                          if (derived_table == NULL)
                            {
                              ...
                              if (pt_str_compare (attr->info.name.original,
                                                  flat->info.name.resolved, CASE_INSENSITIVE) == 0)
                                {
                                  /* find spec
                                   * set attr's spec_id */
                            ...
                            {   /* derived table */
                              range_var = spec->info.spec.range_var;
                              if (pt_str_compare (attr->info.name.original,
                                                  range_var->info.name.original, CASE_INSENSITIVE) == 0)
                                {
                                  break;
                                }
                            }
                          ...
                          if (derived_table == NULL)
                            {
                              resolved_attrs = parser_append_node
                                (attr->next, pt_resolve_star (parser, node->info.query.q.select.from, attr));
                            }
                          else
                            {
                              ...
                              resolved_attrs = parser_append_node
                                (attr->next, parser_copy_tree_list (parser, spec->info.spec.as_attr_list));
                            }
    ...
    case PT_INSERT:
      ...
      parser_walk_tree (parser, node->info.insert.value_clauses,
                        pt_undef_names_pre, node->info.insert.spec, pt_undef_names_post, node->info.insert.spec);
    ...
    case PT_DOT_:
      {
        ...
        if (!(node->node_type == PT_DOT_ && (node->info.dot.arg2->node_type == PT_FUNCTION)))
          {
            /* don't revisit leaves */
            *continue_walk = PT_LIST_WALK;
          }
      ...
      if (node->info.function.function_type == PT_GENERIC)
        {
          ...
          if (parser_function_code != PT_EMPTY)
          ...
          else
            {
              PT_ERRORmf (parser, node,
                          MSGCAT_SET_PARSER_SEMANTIC,
                          MSGCAT_SEMANTIC_UNKNOWN_FUNCTION, node->info.function.generic_name);
            }
        }
#endif
      break;
          ...
          switch (node->info.spec.join_type)
            ...
            case PT_JOIN_RIGHT_OUTER:
              parser_walk_tree (parser, node->info.spec.on_cond,
                                pt_mark_location, &(node->info.spec.location), NULL, NULL);
              break;
        ...
        if (entity_name && entity_name->node_type == PT_NAME)
          {
            ...
            if (entity_name->info.name.db_object && db_is_system_table (entity_name->info.name.db_object))
              {
            ...
            if (derived_table->info.query.correlation_level == 1)
              {
                PT_ERRORmf (parser, derived_table,
                            MSGCAT_SET_PARSER_SEMANTIC,
                            MSGCAT_SEMANTIC_IS_NOT_DEFINED, pt_short_print (parser, derived_table));
              }
          }
      }
      ...
      while (q)
        {                       /* check that p->range !=
          ...
          if (!pt_str_compare (p->info.spec.range_var->info.name.original,
                               q->info.spec.range_var->info.name.original, CASE_INSENSITIVE))
            {
              PT_ERRORmf (parser, q, MSGCAT_SET_PARSER_SEMANTIC,
                          MSGCAT_SEMANTIC_AMBIGUOUS_EXPOSED_NM, q->info.spec.range_var->info.name.original);
              return 0;
            }
...
pt_add_class_to_entity_list (PARSER_CONTEXT * parser,
                             DB_OBJECT * class_,
                             PT_NODE * entity, const PT_NODE * parent, UINTPTR id, PT_MISC_TYPE meta_class)
{
  PT_NODE *flat_list;
          ...
          if (db)
            {
              ...
              result->info.data_type.entity
                = pt_add_class_to_entity_list (parser, db,
                                               result->info.data_type.entity, result, (UINTPTR) result, PT_CLASS);
            }
              ...
              if (result)
                {
                  ...
                  s->info.data_type.entity =
                    pt_name_list_union (parser, s->info.data_type.entity, result->info.data_type.entity);
                  s->next = result;
                  result->info.data_type.entity = NULL;
                }
              result = s;
 ...
 */
PT_NODE *
pt_flat_spec_pre (PARSER_CONTEXT * parser, PT_NODE * node, void *chk_parent, int *continue_walk)
{
      ...
      while (node)
        {
          ...
          if (!derived_table)
          ...
          else
            {
              ...
              parser_walk_tree (parser, derived_table, pt_flat_spec_pre, chk_parent, pt_continue_walk, NULL);
            }
...
pt_get_all_attributes_and_types (PARSER_CONTEXT * parser, PT_NODE * cls, PT_NODE * from)
{
  ...
  if (cls == NULL
      ...
      || (object = cls->info.name.db_object) == NULL || from == NULL || from->node_type != PT_SPEC)
    {
      return NULL;
    }
...
pt_resolve_correlation (PARSER_CONTEXT * parser,
                        PT_NODE * in_node,
                        UNUSED_ARG PT_NODE * scope, PT_NODE * exposed_spec, int col_name, PT_NODE ** p_entity)
{
      ...
      if (col_name && exposed_spec->info.spec.derived_table && exposed_spec->info.spec.range_var != in_node)
        {
          if (PT_NAME_INFO_IS_FLAGED (in_node, PT_NAME_FOR_UPDATE))
            ...
            {
              PT_ERRORmf (parser, in_node, MSGCAT_SET_PARSER_SEMANTIC,
                          MSGCAT_SEMANTIC_WANT_NO_REF_TO_DRVTB, pt_short_print (parser, in_node));
              return NULL;
            }
        }
...
pt_get_resolution (PARSER_CONTEXT * parser,
                   PT_BIND_NAMES_ARG * bind_arg, PT_NODE * scope, PT_NODE * in_node, PT_NODE ** p_entity, int col_name)
{
          ...
          if (pt_find_name_in_spec (parser, spec, in_node))
            {
              if (savespec)
                {
                  PT_ERRORmf (parser, in_node, MSGCAT_SET_PARSER_SEMANTIC,
                              MSGCAT_SEMANTIC_AMBIGUOUS_REF_TO, in_node->info.name.original);
                  return NULL;
                }
              savespec = spec;
            }
      ...
      if (PT_IS_OID_NAME (arg1))
      ...
      else
        {
          ...
          PT_ERRORmf3 (parser, in_node, MSGCAT_SET_PARSER_SEMANTIC,
                       ...
                       pt_short_print (parser, in_node), pt_short_print (parser, arg2), pt_short_print (parser, arg1));
  ...
  for (specs = scope; specs; specs = specs->next)
    {
      ...
      if (specs->info.spec.range_var
          && pt_str_compare (nam->info.name.original,
                             specs->info.spec.range_var->info.name.original, CASE_INSENSITIVE) == 0)
        {
          if (!owner)
            {
              return specs;
          ...
          else
            {
              ...
              if (entity_name && entity_name->node_type == PT_NAME && entity_name->info.name.resolved
                  ...
                  && (pt_str_compare (entity_name->info.name.resolved,
                                      owner->info.name.original, CASE_INSENSITIVE) == 0))
                {
                  return specs;
                }
            }
...
pt_make_subclass_list (PARSER_CONTEXT * parser,
                       DB_OBJECT * db,
                       int line_num, int col_num, UINTPTR id, PT_MISC_TYPE meta_class, MHT_TABLE * names_mht)
{
 ...
 */
static PT_NODE *
pt_make_flat_name_list (PARSER_CONTEXT * parser, PT_NODE * spec, UNUSED_ARG PT_NODE * spec_parent)
{
  ...
  while (spec)
    {
      ...
      if (derived_table)
      ...
      else
        {
          ...
          spec_att = pt_get_all_attributes_and_types (parser, flat_list, spec);
        }
 ...
 */
static int
pt_resolve_hint_args (PARSER_CONTEXT * parser, PT_NODE ** arg_list, PT_NODE * spec_list)
{
...
pt_resolve_using_index (PARSER_CONTEXT * parser, PT_NODE * index, PT_NODE * from)
{
  ...
  if (index->info.name.original == NULL && index->etc != (void *) PT_IDX_HINT_CLASS_NONE)
    {
              ...
              if (index->info.name.original)
                {
                  ...
                  if (cons == NULL || cons->index_status != INDEX_STATUS_COMPLETED)
                    {
 ...
 */
static PT_NODE *
pt_mark_pt_name (PARSER_CONTEXT * parser, PT_NODE * node, UNUSED_ARG void *chk_parent, int *continue_walk)
{
  *continue_walk = PT_CONTINUE_WALK;
 ...
 */
static PT_NODE *
pt_mark_group_having_pt_name (PARSER_CONTEXT * parser, PT_NODE * node, UNUSED_ARG void *chk_parent, int *continue_walk)
{
  *continue_walk = PT_CONTINUE_WALK;
  ...
  if (col == NULL)
    {
      PT_ERRORmf (parser, (*node_p), MSGCAT_SET_PARSER_SEMANTIC,
                  MSGCAT_SEMANTIC_IS_NOT_DEFINED, pt_short_print (parser, (*node_p)));
    }
}
 ...
 */
static PT_NODE *
pt_resolve_group_having_alias (PARSER_CONTEXT * parser, PT_NODE * node, UNUSED_ARG void *chk_parent, int *continue_walk)
{
  PT_NODE *pt_cur;
...
pt_resolve_names (PARSER_CONTEXT * parser, PT_NODE * statement, SEMANTIC_CHK_INFO * sc_info)
{
  ...
  statement = parser_walk_tree (parser, statement, pt_flat_spec_pre, &chk_parent, pt_continue_walk, NULL);
  ...
  if (!pt_has_error (parser))
    {
      ...
      if (statement->node_type == PT_CREATE_INDEX
          || statement->node_type == PT_ALTER_INDEX || statement->node_type == PT_DROP_INDEX)
        {
