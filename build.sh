...
# Build and package script for Rye
# Requirements
# - Bash shell
...
function build_increase_version ()
{
  ...
  if [ "$increase_build_number" = "yes" ]; then
    ...
    sed --in-place -r "s/[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+/${build_number}/g" $source_dir/contrib/php4/src/php_rye_version.h
    sed --in-place -r "s/[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+/${build_number}/g" $source_dir/contrib/php5/php_rye_version.h

    # RPM spec's release number
    sed --in-place -r -e "s/rye_version [0-9]+\.[0-9]+\.[0-9]+\.[0-9]+/rye_version ${build_number}/g" -e "s/build_version  [0-9]+\.[0-9]+\.[0-9]+/build_version  ${major}.${minor}.${maintanance}/g" $source_dir/contrib/rpm/rye.spec
...
function build_autogen ()
{
  ...
  # run autogen of rye manager if needed
  print_check "Checking manager server directory"
  if [ -d "$source_dir/ryemanager" -a -d "$source_dir/ryemanager/server" ]; then 
    print_result "OK"
    
    print_check "Running autogen.sh of rye manager" 
    (cd $source_dir/ryemanager/server && sh ./autogen.sh) 
    [ $? -eq 0 ] && print_result "OK" || print_fatal "Result failed"
  else
...
function build_configure ()
{
  ...
  print_check "Checking manager server directory"
  if [ ! -d "$source_dir/ryemanager" -o ! -d "$source_dir/ryemanager/server" ]; then
...
#  (cd $build_dir && make -j) # Disable -j to compile rsql_grammar.y
  (cd $build_dir && make)
...
rm -f ./${PRODUCT_NAME}_Setup.sh ${PRODUCT_NAME}-product.tar.gz ${PRODUCT_NAME}-product.tar ./install.${PRODUCT_NAME}.tmp ./version.sh check_glibc_version COPYING
...
. ./version.sh
echo "Since Rye broker and server versions should match, please make sure that you are running the same version if you operate them in separate machines. For installation of Rye tools like Query Browser, Manager and Web Manager, please refer to http://www.rye.org/wiki_tools."
...
if [ ! -w $target_dir/databases/databases.txt ]; then
    if [ -w $target_dir/RYE_DATABASES/databases.txt ]; then
        cp_old_dbtxt="yes"
    fi
fi
...
case $SHELL in
    */csh | */tcsh ) 
    echo "setenv    RYE                  $target_dir"             > $XDBMS_ENV_FILE1
    echo "setenv    RYE_DATABASES        $target_dir/databases"          >> $XDBMS_ENV_FILE1

    echo 'if (${?LD_LIBRARY_PATH}) then' >> $XDBMS_ENV_FILE1
    echo 'setenv    LD_LIBRARY_PATH         $RYE/lib:${LD_LIBRARY_PATH}'  >> $XDBMS_ENV_FILE1
    echo 'else'                                                                                     >> $XDBMS_ENV_FILE1
    echo 'setenv    LD_LIBRARY_PATH         $RYE/lib'     >> $XDBMS_ENV_FILE1
    ...
    echo 'set       path=($RYE/{bin,ryemanager} $path)'      >> $XDBMS_ENV_FILE1
    ;;
esac

#
# make $XDBMS_ENV_FILE2 (.rye.sh) to make demodb/subway in this script
#
echo "RYE=$target_dir"                   > $XDBMS_ENV_FILE2
echo "RYE_DATABASES=$target_dir/databases"      >> $XDBMS_ENV_FILE2

echo 'ld_lib_path=`printenv LD_LIBRARY_PATH`'          >> $XDBMS_ENV_FILE2
echo 'if [ "$ld_lib_path" = "" ]'                      >> $XDBMS_ENV_FILE2
echo 'then'                                             >> $XDBMS_ENV_FILE2
echo 'LD_LIBRARY_PATH=$RYE/lib'               >> $XDBMS_ENV_FILE2
echo 'else'                                             >> $XDBMS_ENV_FILE2
echo 'LD_LIBRARY_PATH=$RYE/lib:$LD_LIBRARY_PATH' >> $XDBMS_ENV_FILE2
echo 'fi'                                               >> $XDBMS_ENV_FILE2
echo 'SHLIB_PATH=$LD_LIBRARY_PATH'                      >> $XDBMS_ENV_FILE2
echo 'LIBPATH=$LD_LIBRARY_PATH'                         >> $XDBMS_ENV_FILE2
echo 'PATH=$RYE/bin:$RYE/ryemanager:$PATH' >> $XDBMS_ENV_FILE2
echo "export RYE"                                    >> $XDBMS_ENV_FILE2
echo "export RYE_DATABASES"                          >> $XDBMS_ENV_FILE2
...
if [ -r "$XDBMS_ENV_FILE2" ]; then
    ...
    if [ $cp_old_dbtxt = "yes" ]; then
        ...
        cp $target_dir/RYE_DATABASES/databases.txt $target_dir/databases
    fi

    if [ $is_protego = "no" ]; then
        if [ -r $RYE/demo/make_rye_demo.sh ]; then
#       echo 'Create demodb...'
            (mkdir -p $RYE_DATABASES/demodb ; cd $RYE_DATABASES/demodb ; $RYE/demo/make_rye_demo.sh > /dev/null 2>&1)
            if [ $? = 0 ]; then
...
function build_bin_pack ()
{
  ...
  print_install_sh > $install_dir/Rye_Install.sh
  chmod a+x $install_dir/Rye_Install.sh
  print_setup_sh > $install_dir/Rye_Setup.sh
  chmod a+x $install_dir/Rye_Setup.sh
  ...
  (cd $install_dir/$product_name &&
    tar zcf $install_dir/Rye-product.tar.gz * &&
    cd $install_dir && mv -f Rye_Install.sh $package_file &&
    tar cf - Rye-product.tar.gz Rye_Setup.sh COPYING version.sh check_glibc_version >> $package_file)
...
function build_package ()
{
  ...
  for package in $packages; do
	...
	if [ "$package" = "shell" -a "$without_cmserver" = "true" ]; then
	  print_info "Rye manager server is disabled. Skip shell package"
	else
	...
	if [ "$without_cmserver" = "true" ]; then
	  print_info "Rye manager server is disabled. Skip SRPM build"
      ...
      rpm)
	if [ "$without_cmserver" = "true" ]; then
	  print_info "Rye manager server is disabled. Skip RPM build"
	elif [ ! "$build_mode" = "release" ]; then
